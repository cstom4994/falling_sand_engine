...
ttf_Font *ttf_new(const void *data, int len) {
    ttf_Font *self = (ttf_Font *)calloc(1, sizeof(*self));
    if (!self) {
        goto fail;
    }
    /* Copy font data */
    ...
    /* Init font */
    if (!stbtt_InitFont(&self->font, (unsigned char *)self->fontData, 0)) {
        goto fail;
    }
    /* Init font size */
    ttf_ptsize(self, 14);
...
void *ttf_render(ttf_Font *self, const char *str, int *w, int *h) {
    ...
    *h = ttf_height(self);
    ...
    while (*p) {
        ...
        int x0, y0, x1, y1;
        stbtt_GetCodepointBitmapBoxSubpixel(&self->font, c, self->scale, self->scale, xfract, 0, &x0, &y0, &x1, &y1);
        ...
        if (y < 0) y = 0;
        /* Render char */
        unsigned char *output = (unsigned char *)pixels + x + (y * *w);
        stbtt_MakeCodepointBitmapSubpixel(&self->font, output, *w - x, *h - y, *w, self->scale, self->scale, xfract, 0, c);
        /* Next */
