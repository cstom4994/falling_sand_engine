...
#ifndef ME_ENTT_REFL_HPP
#define ME_ENTT_REFL_HPP
...
#define ComponentTypeListAdd(T)                                                                                                       \
    ...
    constexpr auto ComponentTypeList_##T##_Size = decltype(numComponentTypes(ComponentTypeCounter<maxNumComponentTypes>()))::num + 1; \
    static_assert(ComponentTypeList_##T##_Size < maxNumComponentTypes);                                                               \
    ComponentTypeCounter<ComponentTypeList_##T##_Size> numComponentTypes(ComponentTypeCounter<ComponentTypeList_##T##_Size>);         \
    template <>                                                                                                                       \
...
#define UseComponentTypes() \
    static void forEachComponentType(auto &&f) { ComponentTypeList<decltype(numComponentTypes(ComponentTypeCounter<maxNumComponentTypes>()))::num>::each(std::forward<decltype(f)>(f)); }

//
// Props
//

constexpr uint32_t props_hash(std::string_view str) {
...
struct PropAttribs {
    ...
    uint32_t nameHash = props_hash(name);
...
[[maybe_unused]] inline static PropCounter<0> numProps(PropCounter<0>);
...
struct PropTagWrapper {
    struct Tag {
        inline static PropAttribs attribs = T::getPropAttribs(PropIndex<N>{});
    };
};
template <typename T, int N>
...
#define PropNamed(nameStr, type, name_, ...)                                                                                                                           \
    ...
    inline static PropCounter<decltype(numProps(PropCounter<maxNumProps>()))::num + 1> numProps(PropCounter<decltype(numProps(PropCounter<maxNumProps>()))::num + 1>); \
    static std::type_identity<PROP_PARENS_1(PROP_PARENS_3 type)> propType(name_##_Index);                                                                              \
    static constexpr PropAttribs getPropAttribs(name_##_Index) { return {.name = #name_, __VA_ARGS__}; };                                                              \
...
struct CountFields<Aggregate, std::index_sequence<Indices...>,
                   std::void_t<decltype(Aggregate{{(static_cast<void>(Indices), std::declval<CanConvertToAnything>())}..., {std::declval<CanConvertToAnything>()}})>>
