...
template <class T>
struct IsTriviallyMoveAssignableReference<T &> : std::is_trivially_move_assignable<T>::type {};
...
public:
    static constexpr bool kValue = (std::is_trivially_copyable<ExtentsRemoved>::value || !kIsCopyOrMoveConstructible) &&
                                   (std::is_trivially_copy_assignable<ExtentsRemoved>::value || !kIsCopyOrMoveAssignable) && (kIsCopyOrMoveConstructible || kIsCopyOrMoveAssignable) &&
...
struct OnlyLiteralZero {
    ...
    template <typename T, typename = typename std::enable_if<std::is_same<T, std::nullptr_t>::value || (std::is_integral<T>::value && !std::is_same<T, int>::value)>::type,
...
template <typename Compare, typename K, typename LK, phmap::enable_if_t<std::is_same<bool, phmap::invoke_result_t<Compare, const K &, const LK &>>::value, int> = 0>
constexpr phmap::weak_ordering do_three_way_comparison(const Compare &compare, const K &x, const LK &y) {
    return compare(x, y) ? phmap::weak_ordering::less : compare(y, x) ? phmap::weak_ordering::greater : phmap::weak_ordering::equivalent;
}
...
struct StringBtreeDefaultLess {
    ...
    phmap::weak_ordering operator()(std::string_view lhs, std::string_view rhs) const { return compare_internal::compare_result_as_ordering(lhs.compare(rhs)); }
#else
    phmap::weak_ordering operator()(std::string lhs, std::string rhs) const { return compare_internal::compare_result_as_ordering(lhs.compare(rhs)); }
#endif
};
...
#else
    phmap::weak_ordering operator()(std::string lhs, std::string rhs) const { return compare_internal::compare_result_as_ordering(rhs.compare(lhs)); }
#endif
};
...
struct common_params {
    ...
    using node_count_type = phmap::conditional_t<(kNodeSlotSpace / sizeof(slot_type) > (std::numeric_limits<uint8_t>::max)()), uint16_t, uint8_t>;  // NOLINT
...
public:
    ...
    using use_linear_search =
            std::integral_constant<bool, std::is_arithmetic<key_type>::value && (std::is_same<phmap::Less<key_type>, key_compare>::value || std::is_same<std::less<key_type>, key_compare>::value ||
...
private:
    ...
    constexpr static size_type NodeTargetValues(const int begin, const int end) {
        return begin == end ? begin : SizeWithNValues((begin + end) / 2 + 1) > params_type::kTargetNodeSize ? NodeTargetValues(begin, (begin + end) / 2) : NodeTargetValues((begin + end) / 2 + 1, end);
    }
...
public:
    ...
    btree(const btree &x);
    btree(btree &&x) noexcept : root_(std::move(x.root_)), rightmost_(phmap::exchange(x.rightmost_, EmptyNode())), size_(phmap::exchange(x.size_, 0)) { x.mutable_root() = EmptyNode(); }
...
private:
    ...
    void deallocate(const size_type sz, node_type *node) { phmap::priv::Deallocate<node_type::Alignment()>(mutable_allocator(), node, (size_t)sz); }
...
constexpr bool btree<P>::static_assert_validation() {
    ...
    static_assert(std::is_same<compare_result_type, bool>::value || std::is_convertible<compare_result_type, phmap::weak_ordering>::value,
...
template <typename P>
btree<P>::btree(const key_compare &comp, const allocator_type &alloc) : root_(comp, alloc, EmptyNode()), rightmost_(EmptyNode()), size_(0) {}
...
public:
    ...
    iterator insert(const_iterator hint, node_type &&node) {
        ...
        std::pair<iterator, bool> res = this->tree_.insert_hint_unique(iterator(hint), params_type::key(CommonAccess::GetSlot(node)), CommonAccess::GetSlot(node));
    ...
    template <typename T, typename phmap::enable_if_t<phmap::conjunction<std::is_same<value_type, typename T::value_type>, std::is_same<allocator_type, typename T::allocator_type>,
...
public:
    ...
    iterator try_emplace(const_iterator hint, const key_type &k, Args &&...args) {
        return this->tree_.insert_hint_unique(iterator(hint), k, std::piecewise_construct, std::forward_as_tuple(k), std::forward_as_tuple(std::forward<Args>(args)...)).first;
    }
    template <typename... Args>
    iterator try_emplace(const_iterator hint, key_type &&k, Args &&...args) {
        ...
        const key_type &key_ref = k;
        return this->tree_.insert_hint_unique(iterator(hint), key_ref, std::piecewise_construct, std::forward_as_tuple(std::move(k)), std::forward_as_tuple(std::forward<Args>(args)...)).first;
    }
...
public:
    ...
    // Range constructor.
    template <class InputIterator>
    btree_multiset_container(InputIterator b, InputIterator e, const key_compare &comp = key_compare(), const allocator_type &alloc = allocator_type()) : super_type(comp, alloc) {
        insert(b, e);
    }
...
class btree_set : public priv::btree_set_container<priv::btree<priv::set_params<Key, Compare, Alloc, /*TargetNodeSize=*/256, /*Multi=*/false>>> {
...
// ----------------------------------------------------------------------
template <typename Key, typename Compare, typename Alloc>
class btree_multiset : public priv::btree_multiset_container<priv::btree<priv::set_params<Key, Compare, Alloc, /*TargetNodeSize=*/256, /*Multi=*/true>>> {
...
class btree_map : public priv::btree_map_container<priv::btree<priv::map_params<Key, Value, Compare, Alloc, /*TargetNodeSize=*/256, /*Multi=*/false>>> {
...
// ----------------------------------------------------------------------
template <typename Key, typename Value, typename Compare, typename Alloc>
class btree_multimap : public priv::btree_multimap_container<priv::btree<priv::map_params<Key, Value, Compare, Alloc, /*TargetNodeSize=*/256, /*Multi=*/true>>> {
