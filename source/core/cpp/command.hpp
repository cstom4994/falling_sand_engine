...
#ifndef _METADOT_COMMAND_HPP_
#define _METADOT_COMMAND_HPP_
...
inline constexpr char s_ErrMsgReserved[] =
        "Reserved chars '\\, [, ], \"' must be escaped "
        "with \\";
}  // namespace
struct Reserved {
    ...
    static inline bool IsEscaping(std::string &input, size_t pos) { return pos < input.size() - 1 && IsEscapeChar(input[pos]) && IsReservedChar(input[pos + 1]); }
...
#define ARG_PARSE_GENERAL_SPEC(TYPE, TYPE_NAME, FUNCTION)                                                                                    \
    ARG_PARSE_BASE_SPEC(TYPE) {                                                                                                              \
        ...
        try {                                                                                                                                \
            ...
            throw Exception(std::string("Argument too large for ") + TYPE_NAME, input.m_String.substr(range.first, range.second));           \
        } catch (const std::invalid_argument &) {                                                                                            \
            throw Exception(std::string("Missing or invalid ") + TYPE_NAME + " argument", input.m_String.substr(range.first, range.second)); \
        }                                                                                                                                    \
    }
ARG_PARSE_BASE_SPEC(String) {
    ...
    if (input.m_String[range.first] != '"')
        ...
        while (true) {
            ...
            while (range.second != std::string::npos && Reserved::IsEscaped(input.m_String, range.second)) range.second = input.m_String.find('"', range.second + 1);
...
ARG_PARSE_BASE_SPEC(bool) {
    ...
    if (range.second - range.first == 4 && input.m_String[range.first] == 't') {
        for (size_t i = range.first + 1; i < range.second; ++i)
            if ((input.m_String[i] = char(std::tolower(input.m_String[i]))) != s_true[i - range.first]) throw Exception(s_err_msg + std::string(", expected true"), ARG_PARSE_SUBSTR(range));
        m_Value = true;
    } else if (range.second - range.first == 5 && input.m_String[range.first] == 'f') {
        for (size_t i = range.first + 1; i < range.second; ++i)
            if ((input.m_String[i] = char(std::tolower(input.m_String[i]))) != s_false[i - range.first]) throw Exception(s_err_msg + std::string(", expected false"), ARG_PARSE_SUBSTR(range));
        m_Value = false;
    } else
...
ARG_PARSE_BASE_SPEC(char) {
    ...
    if (len > 2 || len <= 0)
        ...
        if (!Reserved::IsEscaping(input.m_String, range.first)) throw Exception("Too many chars were given", ARG_PARSE_SUBSTR(range));
...
ArgumentParser<std::vector<T>>::ArgumentParser(String &input, size_t &start) {
    ...
    if (input.m_String[range.first] != '[') throw Exception("Invalid vector argument missing opening [", ARG_PARSE_SUBSTR(range));
    input.m_String[range.first] = ' ';
    while (true) {
        range = input.NextPoi(range.first);
        if (range.first == input.End())
            return;
...
public:
    ...
    explicit Arg(const String &name) : m_Arg(name) { static_assert(is_supported_type_v<ValueType>, "ValueType 'T' is not supported, see 'Supported types' for more help"); }
    Arg<T> &Parse(String &input, size_t &start) {
        size_t index = start;
        if (input.NextPoi(index).first == input.End()) throw Exception("Not enough arguments were given", input.m_String);
        m_Arg.m_Value = ArgumentParser<ValueType>(input, start).m_Value;
        return *this;
    }
    std::string Info() { return std::string(" [") + m_Arg.m_Name.m_String + ":" + m_Arg.m_TypeName.m_String + "]"; }
    ArgData<ValueType> m_Arg;
};
template <>
struct Arg<NULL_ARGUMENT> {
    Arg<NULL_ARGUMENT> &Parse(String &input, size_t &start) {
        if (input.NextPoi(start).first != input.End()) throw Exception("Too many arguments were given", input.m_String);
        return *this;
    }
};
static const char endl = '\n';
enum ItemType { COMMAND = 0, LOG, WARNING, ERROR, INFO, NONE };
struct Item {
...
public:
    Command(String name, String description, Fn function, Args... args)
        : m_Name(std::move(name)), m_Description(std::move(description)), m_Function(function), m_Arguments(args..., Arg<NULL_ARGUMENT>()) {}
    ...
    [[nodiscard]] std::string Help() final { return m_Name.m_String + DisplayArguments(std::make_index_sequence<sizeof...(Args)>{}) + "\n\t\t- " + m_Description.m_String + "\n\n"; }
...
public:
    Command(String name, String description, Fn function) : m_Name(std::move(name)), m_Description(std::move(description)), m_Function(function), m_Arguments(Arg<NULL_ARGUMENT>()) {}
    Item operator()(String &input) final {
        size_t start = 0;
        try {
    ...
    [[nodiscard]] std::string Help() final { return m_Name.m_String + "\n\t\t- " + m_Description.m_String + "\n\n"; }
...
METADOT_INLINE AutoComplete::AutoComplete(const AutoComplete &tree) : m_Size(tree.m_Size), m_Count(tree.m_Count) { DeepClone(tree.m_Root, m_Root); }
...
METADOT_INLINE void AutoComplete::Remove(const std::string &word) { RemoveAux(m_Root, word.c_str()); }
METADOT_INLINE void AutoComplete::Suggestions(const char *prefix, std::vector<std::string> &ac_options) {
    ACNode *ptr = m_Root;
    auto temp = prefix;
    while (ptr) {
        if (*prefix < ptr->m_Data) {
...
METADOT_INLINE std::string AutoComplete::Suggestions(const std::string &prefix, r_sVector &ac_options) {
    std::string temp = prefix;
    Suggestions(temp, ac_options, true);
    return temp;
}
METADOT_INLINE void AutoComplete::Suggestions(std::string &prefix, r_sVector ac_options, bool partial_complete) {
    ACNode *ptr = m_Root;
...
METADOT_INLINE std::unique_ptr<AutoComplete::sVector> AutoComplete::Suggestions(const char *prefix) {
    auto temp = std::make_unique<sVector>();
    Suggestions(prefix, *temp);
    return temp;
}
METADOT_INLINE void AutoComplete::SuggestionsAux(AutoComplete::ACNode *root, r_sVector ac_options, std::string buffer) {
    if (!root) return;
...
METADOT_INLINE void AutoComplete::DeepClone(AutoComplete::ACNode *src, AutoComplete::ACNode *&dest) {
    if (!src) return;
    dest = new ACNode(*src);
...
METADOT_INLINE CommandHistory::CommandHistory(unsigned int maxRecord) : m_Record(0), m_MaxRecord(maxRecord), m_History(maxRecord) {}
METADOT_INLINE void CommandHistory::PushBack(const std::string &line) { m_History[m_Record++ % m_MaxRecord] = line; }
METADOT_INLINE unsigned int CommandHistory::GetNewIndex() const { return (m_Record - 1) % m_MaxRecord; }
METADOT_INLINE const std::string &CommandHistory::GetNew() { return m_History[(m_Record - 1) % m_MaxRecord]; }
...
METADOT_INLINE const std::string &CommandHistory::operator[](size_t index) { return m_History[index]; }
...
METADOT_INLINE size_t CommandHistory::Size() { return m_Record < m_MaxRecord ? m_Record : m_MaxRecord; }
...
public:
    ...
    void RegisterCommand(const String &name, const String &description, Fn function, Args... args) {
        ...
        if (m_Commands.find(name.m_String) != m_Commands.end())
            throw Exception("ERROR: Command already exists");
        else if (range.first == name.End()) {
            Log(ERROR) << "Empty command name given" << endl;
            return;
        }
        std::string command_name = name.m_String.substr(range.first, range.second - range.first);
        if (name.NextPoi(name_index).first != name.End()) throw Exception("ERROR: Whitespace separated command names are forbidden");
        ...
        auto help = [this, command_name]() { Log(LOG) << m_Commands[command_name]->Help() << endl; };
        m_Commands["help " + command_name] = std::make_unique<Command<decltype(help)>>("help " + command_name, "Displays help info about command " + command_name, help);
    }
    ...
    void RegisterVariable(const String &name, T &var, Arg<Types>... args) {
        ...
        m_Commands["set " + var_name] = std::make_unique<Command<decltype(setter), Arg<Types>...>>("set " + var_name, "Sets the variable " + var_name, setter, args...);
    }
    template <typename T, typename... Types>
    void RegisterVariable(const String &name, T &var, void (*setter)(T &, Types...)) {
        auto var_name = RegisterVariableAux(name, var);
        auto setter_l = [&var, setter](Types... args) { setter(var, args...); };
        m_Commands["set " + var_name] = std::make_unique<Command<decltype(setter_l), Arg<Types>...>>("set " + var_name, "Sets the variable " + var_name, setter_l, Arg<Types>("")...);
    }
...
protected:
    ...
    std::string RegisterVariableAux(const String &name, T &var) {
        ...
        size_t name_index = 0;
        auto range = name.NextPoi(name_index);
        if (name.NextPoi(name_index).first != name.End()) throw Exception("ERROR: Whitespace separated variable names are forbidden");
...
METADOT_INLINE System::System() {
    RegisterCommand(s_Help.data(), "指令信息", [this]() {
        ...
        for (const auto &tuple : Commands()) {
            if (tuple.first.size() >= 5 && (tuple.first[3] == ' ' || tuple.first[4] == ' ')) continue;
...
METADOT_INLINE void System::RunCommand(const std::string &line) {
    ...
    Log(ItemType::COMMAND) << line << endl;
...
METADOT_INLINE std::unordered_map<std::string, std::unique_ptr<CommandBase>> &System::Commands() { return m_Commands; }
METADOT_INLINE void System::ParseCommandLine(const String &line) {
    ...
    if (is_cmd_help) {
        ...
        if (range.first != line.End()) command_name += " " + line.m_String.substr(range.first, range.second - range.first);
