...
static void callhook(lua_State *L, lua_Debug *ar) {
    ...
    if (!ar->event) {
        /* entering a function */
        lprofP_callhookIN(S, (char *)ar->name, (char *)ar->source, ar->linedefined, currentline);
...
static int profiler_init(lua_State *L) {
    ...
    outfile = NULL;
    if (lua_gettop(L) >= 1) outfile = luaL_checkstring(L, 1);
...
static const luaL_Reg prof_funcs[] = {{"pause", profiler_pause}, {"resume", profiler_resume}, {"start", profiler_init}, {"stop", profiler_stop}, {NULL, NULL}};

int metadot_bind_profiler(lua_State *L) {
    /* luaL_openlib(L, "profiler", prof_funcs, 0); */

    metadot_load(L, prof_funcs, "profiler");
