...
static uint32_t validateIndexValue(MeoVM* vm, uint32_t count, double value, const char* argName) {
    ...
    vm->fiber->error = meoStringFormat(vm, "$ out of bounds.", argName);
    return UINT32_MAX;
}
...
bool validateKey(MeoVM* vm, Value arg) {
    if (meoMapIsValidKey(arg)) return true;
