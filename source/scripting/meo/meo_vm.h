...
struct MeoVM {
    ...
    Obj* tempRoots[MEO_MAX_TEMP_ROOTS];
    ...
    // slots by calling meoEnsureSlots(), a stack is created and this is
    // initialized.
    Value* apiStack;

    MeoConfiguration config;
...
// Creates a new [MeoHandle] for [value].
...
static inline void meoCallFunction(MeoVM* vm, ObjFiber* fiber, ObjClosure* closure, int numArgs) {
    // Grow the call frame array if needed.
    if (fiber->numFrames + 1 > fiber->frameCapacity) {
        int max = fiber->frameCapacity * 2;
        fiber->frames = (CallFrame*)meoReallocate(vm, fiber->frames, sizeof(CallFrame) * fiber->frameCapacity, sizeof(CallFrame) * max);
    ...
    meoEnsureStack(vm, fiber, needed);

    meoAppendCallFrame(vm, fiber, closure, fiber->stackTop - numArgs);
}
...
//
// Defined here instead of in meo_value.h because it's critical that this be
// inlined. That means it must be defined in the header, but the meo_value.h
// header doesn't have a full definitely of MeoVM yet.
static inline ObjClass* meoGetClassInline(MeoVM* vm, Value value) {
...
// letter).
static inline bool meoIsLocalName(const char* name) { return name[0] >= 'a' && name[0] <= 'z'; }

static inline bool meoIsFalsyValue(Value value) { return IS_FALSE(value) || IS_NULL(value); }
