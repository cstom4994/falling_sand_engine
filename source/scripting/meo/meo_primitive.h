#ifndef meo_primitive_h
#define meo_primitive_h

#include "meo_vm.h"

// Binds a primitive method named [name] (in Meo) implemented using C function
// [fn] to `ObjClass` [cls].
#define PRIMITIVE(cls, name, function)                                               \
    do {                                                                             \
        int symbol = meoSymbolTableEnsure(vm, &vm->methodNames, name, strlen(name)); \
        Method method;                                                               \
        method.type = METHOD_PRIMITIVE;                                              \
        method.as.primitive = prim_##function;                                       \
        meoBindMethod(vm, cls, symbol, method);                                      \
    } while (false)
...
#define DEF_PRIMITIVE(name) static bool prim_##name(MeoVM* vm, Value* args)
...
// `UINT32_MAX`.
uint32_t validateIndex(MeoVM* vm, Value arg, uint32_t count, const char* argName);

// Validates that the given [arg] is a String. Returns true if it is. If not,
// reports an error and returns false.
bool validateString(MeoVM* vm, Value arg, const char* argName);
...
// range is decreasing.
uint32_t calculateRange(MeoVM* vm, ObjRange* range, uint32_t* length, int* step);
