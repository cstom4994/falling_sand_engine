...
#define PRIMITIVE(cls, name, function)                                         \
    do                                                                         \
    {                                                                          \
      int symbol = meoSymbolTableEnsure(vm,                                   \
...
#define DEF_PRIMITIVE(name)                                                    \
    static bool prim_##name(MeoVM* vm, Value* args)
...
// `UINT32_MAX`.
uint32_t validateIndex(MeoVM* vm, Value arg, uint32_t count,
                       const char* argName);
...
// range is decreasing.
uint32_t calculateRange(MeoVM* vm, ObjRange* range, uint32_t* length,
                        int* step);
