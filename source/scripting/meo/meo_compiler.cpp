...
#if MEO_DEBUG_DUMP_COMPILED_CODE
#include "meo_debug.h"
#endif
...
static void copyMethodAttributes(Compiler* compiler, bool isForeign, bool isStatic, const char* fullSignature, int32_t length);
...
static void printError(Parser* parser, int line, const char* label, const char* format, va_list args) {
    ...
    // Only report errors if there is a MeoErrorFn to handle them.
    ...
    parser->vm->config.errorFn(parser->vm, MEO_ERROR_COMPILE, module_name, line, message);
}
...
static int addConstant(Compiler* compiler, Value constant) {
    ...
    if (compiler->constants != NULL) {
        Value existing = meoMapGet(compiler->constants, constant);
        if (IS_NUM(existing)) return (int)AS_NUM(existing);
    }

    // It's a new constant.
    if (compiler->fn->constants.count < MAX_CONSTANTS) {
        if (IS_OBJ(constant)) meoPushRoot(compiler->parser->vm, AS_OBJ(constant));
        meoValueBufferWrite(compiler->parser->vm, &compiler->fn->constants, constant);
        if (IS_OBJ(constant)) meoPopRoot(compiler->parser->vm);

        if (compiler->constants == NULL) {
            compiler->constants = meoNewMap(compiler->parser->vm);
        }
        meoMapSet(compiler->parser->vm, compiler->constants, constant, NUM_VAL(compiler->fn->constants.count - 1));
    } else {
...
static void initCompiler(Compiler* compiler, Parser* parser, Compiler* parent, bool isMethod) {
    ...
    compiler->attributes = meoNewMap(parser->vm);
    compiler->fn = meoNewFunction(parser->vm, parser->module, compiler->numLocals);
}
...
static void twoCharToken(Parser* parser, char c, TokenType two, TokenType one) { makeToken(parser, matchChar(parser, c) ? two : one); }
...
static void readName(Parser* parser, TokenType type, char firstChar) {
    ByteBuffer string;
    meoByteBufferInit(&string);
    meoByteBufferWrite(parser->vm, &string, firstChar);

    while (isName(peekChar(parser)) || isDigit(peekChar(parser))) {
        char c = nextChar(parser);
        meoByteBufferWrite(parser->vm, &string, c);
    }
    ...
    parser->next.value = meoNewStringLength(parser->vm, (char*)string.data, string.count);

    meoByteBufferClear(parser->vm, &string);
...
static void readUnicodeEscape(Parser* parser, ByteBuffer* string, int length) {
    ...
    int numBytes = meoUtf8EncodeNumBytes(value);
    if (numBytes != 0) {
        meoByteBufferFill(parser->vm, string, 0, numBytes);
        meoUtf8Encode(value, string->data + string->count - numBytes);
    }
}
...
static void readRawString(Parser* parser) {
    ...
    parser->next.value = meoNewStringLength(parser->vm, ((char*)string.data) + offset, count);
    ...
    for (;;) {
        ...
        if (c == '\\') {
            switch (nextChar(parser)) {
                case '"':
                    meoByteBufferWrite(parser->vm, &string, '"');
                    break;
                case '\\':
                    meoByteBufferWrite(parser->vm, &string, '\\');
                    break;
                case '%':
                    ...
                    break;
                case '0':
                    meoByteBufferWrite(parser->vm, &string, '\0');
                    break;
                case 'a':
                    meoByteBufferWrite(parser->vm, &string, '\a');
                    break;
                case 'b':
                    meoByteBufferWrite(parser->vm, &string, '\b');
                    break;
                case 'e':
                    meoByteBufferWrite(parser->vm, &string, '\33');
                    break;
                case 'f':
                    meoByteBufferWrite(parser->vm, &string, '\f');
                    break;
                case 'n':
                    meoByteBufferWrite(parser->vm, &string, '\n');
                    break;
                case 'r':
                    meoByteBufferWrite(parser->vm, &string, '\r');
                    break;
                case 't':
                    meoByteBufferWrite(parser->vm, &string, '\t');
                    break;
                ...
                case 'U':
                    ...
                    break;
                case 'v':
                    meoByteBufferWrite(parser->vm, &string, '\v');
                    break;
                case 'x':
                    meoByteBufferWrite(parser->vm, &string, (uint8_t)readHexEscape(parser, 2, "byte"));
                    break;
            ...
            default:
                if (isName(c)) {
                    ...
                    if (c >= 32 && c <= 126) {
                        ...
                        // meaningful code units in Meo, the lexer works on raw bytes,
...
static int emitByte(Compiler* compiler, int byte) {
    meoByteBufferWrite(compiler->parser->vm, &compiler->fn->code, (uint8_t)byte);

    // Assume the instruction is associated with the most recently consumed token.
    meoIntBufferWrite(compiler->parser->vm, &compiler->fn->debug->sourceLines, compiler->parser->previous.line);
...
static int declareVariable(Compiler* compiler, Token* token) {
    ...
    if (compiler->scopeDepth == -1) {
        int line = -1;
        int symbol = meoDefineVariable(compiler->parser->vm, compiler->parser->module, token->start, token->length, NULL_VAL, &line);
...
static int resolveLocal(Compiler* compiler, const char* name, int length) {
    ...
    for (int i = compiler->numLocals - 1; i >= 0; i--) {
        if (compiler->locals[i].length == length && memcmp(name, compiler->locals[i].name, length) == 0) {
            return i;
        }
    }
...
static Variable resolveName(Compiler* compiler, const char* name, int length) {
    ...
    variable.index = meoSymbolTableFind(&compiler->parser->module->variableNames, name, length);
    return variable;
}
...
static ObjFn* endCompiler(Compiler* compiler, const char* debugName, int debugNameLength) {
    ...
    meoFunctionBindName(compiler->parser->vm, compiler->fn, debugName, debugNameLength);
...
#if MEO_DEBUG_DUMP_COMPILED_CODE
    meoDumpCode(compiler->parser->vm, compiler->fn);
#endif
...
static int methodSymbol(Compiler* compiler, const char* name, int length) { return meoSymbolTableEnsure(compiler->parser->vm, &compiler->parser->vm->methodNames, name, length); }

// Appends characters to [name] (and updates [length]) for [numParams] "_"
// surrounded by [leftBracket] and [rightBracket].
static void signatureParameterList(char name[MAX_METHOD_SIGNATURE], int* length, int numParams, char leftBracket, char rightBracket) {
...
static void loadCoreVariable(Compiler* compiler, const char* name) {
    int symbol = meoSymbolTableFind(&compiler->parser->module->variableNames, name, strlen(name));
...
static void boolean(Compiler* compiler, bool canAssign) { emitOp(compiler, compiler->parser->previous.type == TOKEN_FALSE ? CODE_FALSE : CODE_TRUE); }
...
static void field(Compiler* compiler, bool canAssign) {
    ...
    if (enclosingClass == NULL) {
        ...
        field = meoSymbolTableEnsure(compiler->parser->vm, &enclosingClass->fields, compiler->parser->previous.start, compiler->parser->previous.length);
...
static void name(Compiler* compiler, bool canAssign) {
    ...
    // on this.
    if (meoIsLocalName(token->start) && getEnclosingClass(compiler) != NULL) {
        loadThis(compiler);
    ...
    variable.scope = SCOPE_MODULE;
    variable.index = meoSymbolTableFind(&compiler->parser->module->variableNames, token->start, token->length);
    if (variable.index == -1) {
        // Implicitly define a module-level variable in
        // the hopes that we get a real definition later.
        variable.index = meoDeclareVariable(compiler->parser->vm, compiler->parser->module, token->start, token->length, token->line);
...
static void null(Compiler* compiler, bool canAssign) { emitOp(compiler, CODE_NULL); }

// A number or string literal.
static void literal(Compiler* compiler, bool canAssign) { emitConstant(compiler, compiler->parser->previous.value); }
...
// on the stack.
void expression(Compiler* compiler) { parsePrecedence(compiler, PREC_LOWEST); }
...
static void forStatement(Compiler* compiler) {
    ...
    // actually get names that aren't valid Meo identfiers, but that's the basic
    // idea.
    //
    ...
    if (compiler->numLocals + 2 > MAX_LOCALS) {
        error(compiler, "Cannot declare more than %d variables in one scope. (Not enough space for for-loops internal variables)", MAX_LOCALS);
        return;
    }
...
//
// Construction is a two-stage process in Meo that involves two separate
...
static int declareMethod(Compiler* compiler, Signature* signature, const char* name, int length) {
    ...
    for (int i = 0; i < methods->count; i++) {
        if (methods->data[i] == symbol) {
            ...
            error(compiler, "Class %s already defines a %smethod '%s'.", &compiler->enclosingClass->name->value, staticPrefix, name);
            break;
        }
    }

    meoIntBufferWrite(compiler->parser->vm, methods, symbol);
    return symbol;
}
...
static bool method(Compiler* compiler, Variable classVariable) {
    ...
    if (isForeign) {
        ...
        emitConstant(compiler, meoNewStringLength(compiler->parser->vm, fullSignature, length));
...
static void classDefinition(Compiler* compiler, bool isForeign) {
    ...
    Value classNameString = meoNewStringLength(compiler->parser->vm, compiler->parser->previous.start, compiler->parser->previous.length);
    ...
    classInfo.classAttributes = compiler->attributes->count > 0 ? meoNewMap(compiler->parser->vm) : NULL;
    ...
    // bytecode will be adjusted by [meoBindMethod] to take inherited fields
    // into account.
    meoSymbolTableInit(&classInfo.fields);

    // Set up symbol buffers to track duplicate static and instance methods.
    meoIntBufferInit(&classInfo.methods);
    meoIntBufferInit(&classInfo.staticMethods);
    ...
    meoSymbolTableClear(compiler->parser->vm, &classInfo.fields);
    meoIntBufferClear(compiler->parser->vm, &classInfo.methods);
    meoIntBufferClear(compiler->parser->vm, &classInfo.staticMethods);
    compiler->enclosingClass = NULL;
    popScope(compiler);
}
...
static void import(Compiler* compiler) {
    ...
    do {
        ...
        int sourceVariableConstant = addConstant(compiler, meoNewStringLength(compiler->parser->vm, sourceVariableToken.start, sourceVariableToken.length));
...
ObjFn* meoCompile(MeoVM* vm, ObjModule* module, const char* source, bool isExpression, bool printErrors) {
...
void meoBindMethodCode(ObjClass* classObj, ObjFn* fn) {
    int ip = 0;
    for (;;) {
        ...
        switch (instruction) {
            ...
            case CODE_CLOSURE: {
                ...
                meoBindMethodCode(classObj, AS_FN(fn->constants.data[constant]));
                break;
            }
...
void meoMarkCompiler(MeoVM* vm, Compiler* compiler) {
    meoGrayValue(vm, compiler->parser->current.value);
    meoGrayValue(vm, compiler->parser->previous.value);
    meoGrayValue(vm, compiler->parser->next.value);

    // Walk up the parent chain to mark the outer compilers too. The VM only
    // tracks the innermost one.
    do {
        meoGrayObj(vm, (Obj*)compiler->fn);
        meoGrayObj(vm, (Obj*)compiler->constants);
        meoGrayObj(vm, (Obj*)compiler->attributes);

        if (compiler->enclosingClass != NULL) {
            meoBlackenSymbolTable(vm, &compiler->enclosingClass->fields);

            if (compiler->enclosingClass->methodAttributes != NULL) {
                meoGrayObj(vm, (Obj*)compiler->enclosingClass->methodAttributes);
            }
            if (compiler->enclosingClass->classAttributes != NULL) {
                meoGrayObj(vm, (Obj*)compiler->enclosingClass->classAttributes);
            }
        }
...
static void disallowAttributes(Compiler* compiler) {
    if (compiler->numAttributes > 0) {
        ...
        meoMapClear(compiler->parser->vm, compiler->attributes);
        compiler->numAttributes = 0;
    }
}
...
static void addToAttributeGroup(Compiler* compiler, Value group, Value key, Value value) {
    MeoVM* vm = compiler->parser->vm;

    if (IS_OBJ(group)) meoPushRoot(vm, AS_OBJ(group));
    if (IS_OBJ(key)) meoPushRoot(vm, AS_OBJ(key));
    if (IS_OBJ(value)) meoPushRoot(vm, AS_OBJ(value));

    Value groupMapValue = meoMapGet(compiler->attributes, group);
    if (IS_UNDEFINED(groupMapValue)) {
        groupMapValue = OBJ_VAL(meoNewMap(vm));
        meoMapSet(vm, compiler->attributes, group, groupMapValue);
    }
    ...
    Value keyItemsValue = meoMapGet(groupMap, key);
    if (IS_UNDEFINED(keyItemsValue)) {
        keyItemsValue = OBJ_VAL(meoNewList(vm, 0));
        meoMapSet(vm, groupMap, key, keyItemsValue);
    }

    // keyItems.add(value)
    ObjList* keyItems = AS_LIST(keyItemsValue);
    meoValueBufferWrite(vm, &keyItems->elements, value);

    if (IS_OBJ(group)) meoPopRoot(vm);
    if (IS_OBJ(key)) meoPopRoot(vm);
...
static void copyAttributes(Compiler* compiler, ObjMap* into) {
    ...
    for (uint32_t attrIdx = 0; attrIdx < compiler->attributes->capacity; attrIdx++) {
        ...
        meoMapSet(vm, into, attrEntry->key, attrEntry->value);
    }

    meoMapClear(vm, compiler->attributes);
}
...
static void copyMethodAttributes(Compiler* compiler, bool isForeign, bool isStatic, const char* fullSignature, int32_t length) {
    ...
    ObjMap* methodAttr = meoNewMap(vm);
    meoPushRoot(vm, (Obj*)methodAttr);
    ...
    if (compiler->enclosingClass->methodAttributes == NULL) {
        compiler->enclosingClass->methodAttributes = meoNewMap(vm);
    }

    // Store the method attributes in the class map
    Value key = meoNewStringLength(vm, fullSignatureWithPrefix, fullLength);
    meoMapSet(vm, compiler->enclosingClass->methodAttributes, key, OBJ_VAL(methodAttr));
