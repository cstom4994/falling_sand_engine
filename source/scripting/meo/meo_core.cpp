...
static const char* core_source = R"meo(
class Bool {}
class Fiber {}
class Fn {}
class Null {}
...
DEF_PRIMITIVE(class_attributes) { RETURN_VAL(AS_CLASS(args[0])->attributes); }
...
DEF_PRIMITIVE(fiber_new) {
    ...
    RETURN_OBJ(meoNewFiber(vm, closure));
}
...
static bool runFiber(MeoVM* vm, ObjFiber* fiber, Value* args, bool isCall, bool hasValue, const char* verb) {

    if (meoHasError(fiber)) {
    ...
    if (fiber->numFrames == 1 && fiber->frames[0].ip == fiber->frames[0].closure->fn->code.data) {
...
DEF_PRIMITIVE(fiber_call) { return runFiber(vm, AS_FIBER(args[0]), args, true, false, "call"); }

DEF_PRIMITIVE(fiber_call1) { return runFiber(vm, AS_FIBER(args[0]), args, true, true, "call"); }

DEF_PRIMITIVE(fiber_current) { RETURN_OBJ(vm->fiber); }

DEF_PRIMITIVE(fiber_error) { RETURN_VAL(AS_FIBER(args[0])->error); }

DEF_PRIMITIVE(fiber_isDone) {
    ObjFiber* runFiber = AS_FIBER(args[0]);
    RETURN_BOOL(runFiber->numFrames == 0 || meoHasError(runFiber));
}
...
DEF_PRIMITIVE(fiber_transfer) { return runFiber(vm, AS_FIBER(args[0]), args, false, false, "transfer to"); }

DEF_PRIMITIVE(fiber_transfer1) { return runFiber(vm, AS_FIBER(args[0]), args, false, true, "transfer to"); }
...
DEF_PRIMITIVE(fiber_try) {
    ...
    if (!meoHasError(vm->fiber)) vm->fiber->state = FIBER_TRY;
    return false;
}
...
DEF_PRIMITIVE(fn_arity) { RETURN_NUM(AS_CLOSURE(args[0])->fn->arity); }

static void call_fn(MeoVM* vm, Value* args, int numArgs) {
    // +1 to include the function itself.
    meoCallFunction(vm, vm->fiber, AS_CLOSURE(args[0]), numArgs + 1);
}
...
DEF_PRIMITIVE(fn_toString) { RETURN_VAL(CONST_STRING(vm, "<fn>")); }
...
DEF_PRIMITIVE(list_filled) {
    ...
    ObjList* list = meoNewList(vm, size);
...
DEF_PRIMITIVE(list_new) { RETURN_OBJ(meoNewList(vm, 0)); }

DEF_PRIMITIVE(list_add) {
    meoValueBufferWrite(vm, &AS_LIST(args[0])->elements, args[1]);
    RETURN_VAL(args[1]);
}
...
DEF_PRIMITIVE(list_clear) {
    meoValueBufferClear(vm, &AS_LIST(args[0])->elements);
    RETURN_NULL;
}

DEF_PRIMITIVE(list_count) { RETURN_NUM(AS_LIST(args[0])->elements.count); }
...
DEF_PRIMITIVE(list_insert) {
    ...
    meoListInsert(vm, list, args[2], index);
    RETURN_VAL(args[2]);
}
...
DEF_PRIMITIVE(list_removeAt) {
    ...
    RETURN_VAL(meoListRemoveAt(vm, list, index));
}

DEF_PRIMITIVE(list_removeValue) {
    ObjList* list = AS_LIST(args[0]);
    int index = meoListIndexOf(vm, list, args[1]);
...
DEF_PRIMITIVE(list_indexOf) {
    ObjList* list = AS_LIST(args[0]);
    RETURN_NUM(meoListIndexOf(vm, list, args[1]));
}
...
DEF_PRIMITIVE(list_subscript) {
    ...
    ObjList* result = meoNewList(vm, count);
...
DEF_PRIMITIVE(map_new) { RETURN_OBJ(meoNewMap(vm)); }

DEF_PRIMITIVE(map_subscript) {
    if (!validateKey(vm, args[1])) return false;

    ObjMap* map = AS_MAP(args[0]);
    Value value = meoMapGet(map, args[1]);
...
DEF_PRIMITIVE(map_subscriptSetter) {
    ...
    meoMapSet(vm, AS_MAP(args[0]), args[1], args[2]);
    RETURN_VAL(args[2]);
}
...
DEF_PRIMITIVE(map_containsKey) {
    ...
    RETURN_BOOL(!IS_UNDEFINED(meoMapGet(AS_MAP(args[0]), args[1])));
}

DEF_PRIMITIVE(map_count) { RETURN_NUM(AS_MAP(args[0])->count); }
...
DEF_PRIMITIVE(map_remove) {
    ...
    RETURN_VAL(meoMapRemoveKey(vm, AS_MAP(args[0]), args[1]));
}
...
#define DEF_NUM_CONSTANT(name, value) \
    DEF_PRIMITIVE(num_##name) { RETURN_NUM(value); }

DEF_NUM_CONSTANT(infinity, INFINITY)
DEF_NUM_CONSTANT(nan, MEO_DOUBLE_NAN)
...
#define DEF_NUM_FN(name, fn) \
    DEF_PRIMITIVE(num_##name) { RETURN_NUM(fn(AS_NUM(args[0]))); }
...
DEF_PRIMITIVE(num_dotDot) {
    ...
    RETURN_VAL(meoNewRange(vm, from, to, true));
}
...
DEF_PRIMITIVE(num_dotDotDot) {
    ...
    double from = AS_NUM(args[0]);
    double to = AS_NUM(args[1]);
    RETURN_VAL(meoNewRange(vm, from, to, false));
}
...
DEF_PRIMITIVE(num_isInfinity) { RETURN_BOOL(isinf(AS_NUM(args[0]))); }
...
DEF_PRIMITIVE(num_isNan) { RETURN_BOOL(isnan(AS_NUM(args[0]))); }
...
DEF_PRIMITIVE(num_toString) { RETURN_VAL(meoNumToString(vm, AS_NUM(args[0]))); }
...
DEF_PRIMITIVE(object_same) { RETURN_BOOL(meoValuesEqual(args[1], args[2])); }

DEF_PRIMITIVE(object_not) { RETURN_VAL(FALSE_VAL); }

DEF_PRIMITIVE(object_eqeq) { RETURN_BOOL(meoValuesEqual(args[0], args[1])); }

DEF_PRIMITIVE(object_bangeq) { RETURN_BOOL(!meoValuesEqual(args[0], args[1])); }
...
DEF_PRIMITIVE(object_is) {
    ...
    ObjClass* classObj = meoGetClass(vm, args[0]);
...
DEF_PRIMITIVE(object_toString) {
    ...
    RETURN_VAL(meoStringFormat(vm, "instance of @", name));
}

DEF_PRIMITIVE(object_type) { RETURN_OBJ(meoGetClass(vm, args[0])); }

DEF_PRIMITIVE(range_from) { RETURN_NUM(AS_RANGE(args[0])->from); }

DEF_PRIMITIVE(range_to) { RETURN_NUM(AS_RANGE(args[0])->to); }
...
DEF_PRIMITIVE(range_isInclusive) { RETURN_BOOL(AS_RANGE(args[0])->isInclusive); }
...
DEF_PRIMITIVE(range_toString) {
    ...
    Value from = meoNumToString(vm, range->from);
    meoPushRoot(vm, AS_OBJ(from));

    Value to = meoNumToString(vm, range->to);
    meoPushRoot(vm, AS_OBJ(to));

    Value result = meoStringFormat(vm, "@$@", from, range->isInclusive ? ".." : "...", to);
...
DEF_PRIMITIVE(string_fromCodePoint) {
    ...
    RETURN_VAL(meoStringFromCodePoint(vm, codePoint));
}
...
DEF_PRIMITIVE(string_fromByte) {
    ...
    RETURN_VAL(meoStringFromByte(vm, (uint8_t)byte));
}
...
DEF_PRIMITIVE(string_byteCount) { RETURN_NUM(AS_STRING(args[0])->length); }
...
DEF_PRIMITIVE(string_codePointAt) {
    ...
    RETURN_NUM(meoUtf8Decode((uint8_t*)string->value + index, string->length - index));
}
...
DEF_PRIMITIVE(string_contains) {
    ...
    RETURN_BOOL(meoStringFind(string, search, 0) != UINT32_MAX);
}
...
DEF_PRIMITIVE(string_endsWith) {
    ...
    RETURN_BOOL(memcmp(string->value + string->length - search->length, search->value, search->length) == 0);
}
...
DEF_PRIMITIVE(string_indexOf1) {
    ...
    uint32_t index = meoStringFind(string, search, 0);
...
DEF_PRIMITIVE(string_indexOf2) {
    ...
    uint32_t index = meoStringFind(string, search, start);
    RETURN_NUM(index == UINT32_MAX ? -1 : (int)index);
}
...
DEF_PRIMITIVE(string_iteratorValue) {
    ...
    RETURN_VAL(meoStringCodePointAt(vm, string, index));
}
...
DEF_PRIMITIVE(string_plus) {
    ...
    RETURN_VAL(meoStringFormat(vm, "@@", args[0], args[1]));
}
...
DEF_PRIMITIVE(string_subscript) {
    ...
    RETURN_VAL(meoNewStringFromRange(vm, string, start, count, step));
}

DEF_PRIMITIVE(string_toString) { RETURN_VAL(args[0]); }

DEF_PRIMITIVE(system_clock) { RETURN_NUM((double)clock() / CLOCKS_PER_SEC); }

DEF_PRIMITIVE(system_gc) {
    meoCollectGarbage(vm);
    RETURN_NULL;
}
...
static ObjClass* defineClass(MeoVM* vm, ObjModule* module, const char* name) {
    ObjString* nameString = AS_STRING(meoNewString(vm, name));
    meoPushRoot(vm, (Obj*)nameString);

    ObjClass* classObj = meoNewSingleClass(vm, 0, nameString);

    meoDefineVariable(vm, module, name, nameString->length, OBJ_VAL(classObj), NULL);
...
void meoInitializeCore(MeoVM* vm) {
    ObjModule* coreModule = meoNewModule(vm, NULL);
    meoPushRoot(vm, (Obj*)coreModule);

    // The core module's key is null in the module map.
    meoMapSet(vm, vm->modules, NULL_VAL, OBJ_VAL(coreModule));
    meoPopRoot(vm);  // coreModule.
    ...
    meoBindSuperclass(vm, vm->classClass, vm->objectClass);
    ...
    // collected.
    meoBindSuperclass(vm, objectMetaclass, vm->classClass);
    ...
    meoInterpret(vm, NULL, core_source);

    vm->boolClass = AS_CLASS(meoFindVariable(vm, coreModule, "Bool"));
    PRIMITIVE(vm->boolClass, "toString", bool_toString);
    PRIMITIVE(vm->boolClass, "!", bool_not);

    vm->fiberClass = AS_CLASS(meoFindVariable(vm, coreModule, "Fiber"));
    ...
    vm->fnClass = AS_CLASS(meoFindVariable(vm, coreModule, "Fn"));
    ...
    vm->nullClass = AS_CLASS(meoFindVariable(vm, coreModule, "Null"));
    PRIMITIVE(vm->nullClass, "!", null_not);
    PRIMITIVE(vm->nullClass, "toString", null_toString);

    vm->numClass = AS_CLASS(meoFindVariable(vm, coreModule, "Num"));
    ...
    vm->stringClass = AS_CLASS(meoFindVariable(vm, coreModule, "String"));
    ...
    vm->listClass = AS_CLASS(meoFindVariable(vm, coreModule, "List"));
    ...
    vm->mapClass = AS_CLASS(meoFindVariable(vm, coreModule, "Map"));
    ...
    vm->rangeClass = AS_CLASS(meoFindVariable(vm, coreModule, "Range"));
    ...
    ObjClass* systemClass = AS_CLASS(meoFindVariable(vm, coreModule, "System"));
