...
ObjClass* meoNewClass(MeoVM* vm, ObjClass* superclass, int numFields,
                       ObjString* name)
{
...
void meoBindMethod(MeoVM* vm, ObjClass* classObj, int symbol, Method method)
{
  ...
  if (symbol >= classObj->methods.count)
  {
    ...
    meoMethodBufferFill(vm, &classObj->methods, noMethod,
...
ObjFiber* meoNewFiber(MeoVM* vm, ObjClosure* closure)
{
  ...
  int stackCapacity = closure == NULL
      ? 1
      : meoPowerOf2Ceil(closure->fn->maxSlots + 1);
...
void meoEnsureStack(MeoVM* vm, ObjFiber* fiber, int needed)
{
  ...
  fiber->stack = (Value*)meoReallocate(vm, fiber->stack,
...
Value meoListRemoveAt(MeoVM* vm, ObjList* list, uint32_t index)
{
  ...
  {
    list->elements.data = (Value*)meoReallocate(vm, list->elements.data,
...
Value meoNewString(MeoVM* vm, const char* text)
{
  return meoNewStringLength(vm, text, strlen(text));
}
...
Value meoNewStringFromRange(MeoVM* vm, ObjString* source, int start,
...
Value meoStringCodePointAt(MeoVM* vm, ObjString* string, uint32_t index)
{
  ...
  int codePoint = meoUtf8Decode((uint8_t*)string->value + index,
                                 string->length - index);
  if (codePoint == -1)
  {
...
ObjClass* meoGetClass(MeoVM* vm, Value value)
{
  return meoGetClassInline(vm, value);
}
...
bool meoValuesEqual(Value a, Value b)
{
  ...
  switch (aObj->type)
  {
    ...
    case OBJ_STRING:
    {
      ...
      meoStringEqualsCString(aString, bString->value, bString->length);
    }
