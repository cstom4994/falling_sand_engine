...
static void initObj(MeoVM* vm, Obj* obj, ObjType type, ObjClass* classObj) {
    obj->type = type;
    obj->isDark = false;
...
ObjClass* meoNewSingleClass(MeoVM* vm, int numFields, ObjString* name) {
    ...
    meoPushRoot(vm, (Obj*)classObj);
    meoMethodBufferInit(&classObj->methods);
    meoPopRoot(vm);

    return classObj;
}

void meoBindSuperclass(MeoVM* vm, ObjClass* subclass, ObjClass* superclass) {
    ...
    for (int i = 0; i < superclass->methods.count; i++) {
        meoBindMethod(vm, subclass, i, superclass->methods.data[i]);
    }
}

ObjClass* meoNewClass(MeoVM* vm, ObjClass* superclass, int numFields, ObjString* name) {
    // Create the metaclass.
    Value metaclassName = meoStringFormat(vm, "@ metaclass", OBJ_VAL(name));
    meoPushRoot(vm, AS_OBJ(metaclassName));

    ObjClass* metaclass = meoNewSingleClass(vm, 0, AS_STRING(metaclassName));
    metaclass->obj.classObj = vm->classClass;

    meoPopRoot(vm);

    // Make sure the metaclass isn't collected when we allocate the class.
    meoPushRoot(vm, (Obj*)metaclass);

    // Metaclasses always inherit Class and do not parallel the non-metaclass
    // hierarchy.
    meoBindSuperclass(vm, metaclass, vm->classClass);

    ObjClass* classObj = meoNewSingleClass(vm, numFields, name);
    ...
    meoBindSuperclass(vm, classObj, superclass);
...
void meoBindMethod(MeoVM* vm, ObjClass* classObj, int symbol, Method method) {
    // Make sure the buffer is big enough to contain the symbol's index.
    if (symbol >= classObj->methods.count) {
        Method noMethod;
        noMethod.type = METHOD_NONE;
        meoMethodBufferFill(vm, &classObj->methods, noMethod, symbol - classObj->methods.count + 1);
    }

    classObj->methods.data[symbol] = method;
}

ObjClosure* meoNewClosure(MeoVM* vm, ObjFn* fn) {
...
ObjFiber* meoNewFiber(MeoVM* vm, ObjClosure* closure) {
    ...
    int stackCapacity = closure == NULL ? 1 : meoPowerOf2Ceil(closure->fn->maxSlots + 1);
    ...
    if (closure != NULL) {
        ...
        meoAppendCallFrame(vm, fiber, closure, fiber->stack);
...
void meoEnsureStack(MeoVM* vm, ObjFiber* fiber, int needed) {
    if (fiber->stackCapacity >= needed) return;

    int capacity = meoPowerOf2Ceil(needed);

    Value* oldStack = fiber->stack;
    fiber->stack = (Value*)meoReallocate(vm, fiber->stack, sizeof(Value) * fiber->stackCapacity, sizeof(Value) * capacity);
...
ObjForeign* meoNewForeign(MeoVM* vm, ObjClass* classObj, size_t size) {
...
ObjFn* meoNewFunction(MeoVM* vm, ObjModule* module, int maxSlots) {
    FnDebug* debug = ALLOCATE(vm, FnDebug);
    debug->name = NULL;
    meoIntBufferInit(&debug->sourceLines);

    ObjFn* fn = ALLOCATE(vm, ObjFn);
    initObj(vm, &fn->obj, OBJ_FN, vm->fnClass);

    meoValueBufferInit(&fn->constants);
    meoByteBufferInit(&fn->code);
    fn->module = module;
...
void meoFunctionBindName(MeoVM* vm, ObjFn* fn, const char* name, int length) {
    fn->debug->name = ALLOCATE_ARRAY(vm, char, length + 1);
    memcpy(fn->debug->name, name, length);
    fn->debug->name[length] = '\0';
}

Value meoNewInstance(MeoVM* vm, ObjClass* classObj) {
...
ObjList* meoNewList(MeoVM* vm, uint32_t numElements) {
...
void meoListInsert(MeoVM* vm, ObjList* list, Value value, uint32_t index) {
    if (IS_OBJ(value)) meoPushRoot(vm, AS_OBJ(value));

    // Add a slot at the end of the list.
    meoValueBufferWrite(vm, &list->elements, NULL_VAL);
...
int meoListIndexOf(MeoVM* vm, ObjList* list, Value value) {
    int count = list->elements.count;
    for (int i = 0; i < count; i++) {
        Value item = list->elements.data[i];
        if (meoValuesEqual(item, value)) {
            return i;
        }
    }
    return -1;
...
Value meoListRemoveAt(MeoVM* vm, ObjList* list, uint32_t index) {
    Value removed = list->elements.data[index];

    if (IS_OBJ(removed)) meoPushRoot(vm, AS_OBJ(removed));
    ...
    if (list->elements.capacity / GROW_FACTOR >= list->elements.count) {
        list->elements.data = (Value*)meoReallocate(vm, list->elements.data, sizeof(Value) * list->elements.capacity, sizeof(Value) * (list->elements.capacity / GROW_FACTOR));
        list->elements.capacity /= GROW_FACTOR;
    }

    if (IS_OBJ(removed)) meoPopRoot(vm);

    list->elements.count--;
    return removed;
}

ObjMap* meoNewMap(MeoVM* vm) {
...
static inline uint32_t hashNumber(double num) {
    ...
    return hashBits(meoDoubleToBits(num));
}
...
static bool findEntry(MapEntry* entries, uint32_t capacity, Value key, MapEntry** result) {
    ...
    do {
        ...
        } else if (meoValuesEqual(entry->key, key)) {
            // We found the key.
            *result = entry;
            return true;
        }
...
static void resizeMap(MeoVM* vm, ObjMap* map, uint32_t capacity) {
...
Value meoMapGet(ObjMap* map, Value key) {
    MapEntry* entry;
...
void meoMapSet(MeoVM* vm, ObjMap* map, Value key, Value value) {
...
void meoMapClear(MeoVM* vm, ObjMap* map) {
    DEALLOCATE(vm, map->entries);
    map->entries = NULL;
    map->capacity = 0;
    map->count = 0;
...
Value meoMapRemoveKey(MeoVM* vm, ObjMap* map, Value key) {
    MapEntry* entry;
...
ObjModule* meoNewModule(MeoVM* vm, ObjString* name) {
    ...
    meoSymbolTableInit(&module->variableNames);
    meoValueBufferInit(&module->variables);
...
Value meoNewRange(MeoVM* vm, double from, double to, bool isInclusive) {
...
// hash.
static ObjString* allocateString(MeoVM* vm, size_t length) {
...
Value meoNewString(MeoVM* vm, const char* text) { return meoNewStringLength(vm, text, strlen(text)); }

Value meoNewStringLength(MeoVM* vm, const char* text, size_t length) {
...
Value meoNewStringFromRange(MeoVM* vm, ObjString* source, int start, uint32_t count, int step) {
    uint8_t* from = (uint8_t*)source->value;
    int length = 0;
    for (uint32_t i = 0; i < count; i++) {
        length += meoUtf8DecodeNumBytes(from[start + i * step]);
    }
    ...
    for (uint32_t i = 0; i < count; i++) {
        ...
        int codePoint = meoUtf8Decode(from + index, source->length - index);

        if (codePoint != -1) {
            to += meoUtf8Encode(codePoint, to);
        }
    }
...
Value meoNumToString(MeoVM* vm, double value) {
    ...
    return meoNewStringLength(vm, buffer, length);
}

Value meoStringFromCodePoint(MeoVM* vm, int value) {
    int length = meoUtf8EncodeNumBytes(value);
    ASSERT(length != 0, "Value out of range.");

    ObjString* string = allocateString(vm, length);

    meoUtf8Encode(value, (uint8_t*)string->value);
    hashString(string);

    return OBJ_VAL(string);
}

Value meoStringFromByte(MeoVM* vm, uint8_t value) {
    int length = 1;
    ObjString* string = allocateString(vm, length);
...
Value meoStringFormat(MeoVM* vm, const char* format, ...) {
    va_list argList;
...
Value meoStringCodePointAt(MeoVM* vm, ObjString* string, uint32_t index) {
    ASSERT(index < string->length, "Index out of bounds.");

    int codePoint = meoUtf8Decode((uint8_t*)string->value + index, string->length - index);
    if (codePoint == -1) {
        // If it isn't a valid UTF-8 sequence, treat it as a single raw byte.
        char bytes[2];
        bytes[0] = string->value[index];
        bytes[1] = '\0';
        return meoNewStringLength(vm, bytes, 1);
    }

    return meoStringFromCodePoint(vm, codePoint);
}

// Uses the Boyer-Moore-Horspool string matching algorithm.
uint32_t meoStringFind(ObjString* haystack, ObjString* needle, uint32_t start) {
...
ObjUpvalue* meoNewUpvalue(MeoVM* vm, Value* value) {
...
void meoGrayObj(MeoVM* vm, Obj* obj) {
    if (obj == NULL) return;
...
void meoGrayValue(MeoVM* vm, Value value) {
    if (!IS_OBJ(value)) return;
    meoGrayObj(vm, AS_OBJ(value));
}

void meoGrayBuffer(MeoVM* vm, ValueBuffer* buffer) {
    for (int i = 0; i < buffer->count; i++) {
        meoGrayValue(vm, buffer->data[i]);
    }
}

static void blackenClass(MeoVM* vm, ObjClass* classObj) {
    // The metaclass.
    meoGrayObj(vm, (Obj*)classObj->obj.classObj);

    // The superclass.
    meoGrayObj(vm, (Obj*)classObj->superclass);

    // Method function objects.
    for (int i = 0; i < classObj->methods.count; i++) {
        if (classObj->methods.data[i].type == METHOD_BLOCK) {
            meoGrayObj(vm, (Obj*)classObj->methods.data[i].as.closure);
        }
    }

    meoGrayObj(vm, (Obj*)classObj->name);

    if (!IS_NULL(classObj->attributes)) meoGrayObj(vm, AS_OBJ(classObj->attributes));
...
static void blackenClosure(MeoVM* vm, ObjClosure* closure) {
    // Mark the function.
    meoGrayObj(vm, (Obj*)closure->fn);

    // Mark the upvalues.
    for (int i = 0; i < closure->fn->numUpvalues; i++) {
        meoGrayObj(vm, (Obj*)closure->upvalues[i]);
    }
...
static void blackenFiber(MeoVM* vm, ObjFiber* fiber) {
    // Stack functions.
    for (int i = 0; i < fiber->numFrames; i++) {
        meoGrayObj(vm, (Obj*)fiber->frames[i].closure);
    }

    // Stack variables.
    for (Value* slot = fiber->stack; slot < fiber->stackTop; slot++) {
        meoGrayValue(vm, *slot);
    }

    // Open upvalues.
    ObjUpvalue* upvalue = fiber->openUpvalues;
    while (upvalue != NULL) {
        meoGrayObj(vm, (Obj*)upvalue);
        upvalue = upvalue->next;
    }

    // The caller.
    meoGrayObj(vm, (Obj*)fiber->caller);
    meoGrayValue(vm, fiber->error);
...
static void blackenFn(MeoVM* vm, ObjFn* fn) {
    // Mark the constants.
    meoGrayBuffer(vm, &fn->constants);
...
static void blackenForeign(MeoVM* vm, ObjForeign* foreign) {
...
static void blackenInstance(MeoVM* vm, ObjInstance* instance) {
    meoGrayObj(vm, (Obj*)instance->obj.classObj);

    // Mark the fields.
    for (int i = 0; i < instance->obj.classObj->numFields; i++) {
        meoGrayValue(vm, instance->fields[i]);
    }
...
static void blackenList(MeoVM* vm, ObjList* list) {
    // Mark the elements.
    meoGrayBuffer(vm, &list->elements);
...
static void blackenMap(MeoVM* vm, ObjMap* map) {
    // Mark the entries.
    for (uint32_t i = 0; i < map->capacity; i++) {
        MapEntry* entry = &map->entries[i];
        if (IS_UNDEFINED(entry->key)) continue;

        meoGrayValue(vm, entry->key);
        meoGrayValue(vm, entry->value);
    }
...
static void blackenModule(MeoVM* vm, ObjModule* module) {
    // Top-level variables.
    for (int i = 0; i < module->variables.count; i++) {
        meoGrayValue(vm, module->variables.data[i]);
    }

    meoBlackenSymbolTable(vm, &module->variableNames);

    meoGrayObj(vm, (Obj*)module->name);

    // Keep track of how much memory is still in use.
    vm->bytesAllocated += sizeof(ObjModule);
}

static void blackenRange(MeoVM* vm, ObjRange* range) {
    // Keep track of how much memory is still in use.
    vm->bytesAllocated += sizeof(ObjRange);
}

static void blackenString(MeoVM* vm, ObjString* string) {
    // Keep track of how much memory is still in use.
    vm->bytesAllocated += sizeof(ObjString) + string->length + 1;
}

static void blackenUpvalue(MeoVM* vm, ObjUpvalue* upvalue) {
    // Mark the closed-over object (in case it is closed).
    meoGrayValue(vm, upvalue->closed);

    // Keep track of how much memory is still in use.
    vm->bytesAllocated += sizeof(ObjUpvalue);
}

static void blackenObject(MeoVM* vm, Obj* obj) {
#if MEO_DEBUG_TRACE_MEMORY
    printf("mark ");
    meoDumpValue(OBJ_VAL(obj));
    printf(" @ %p\n", obj);
#endif
...
void meoBlackenObjects(MeoVM* vm) {
...
void meoFreeObj(MeoVM* vm, Obj* obj) {
#if MEO_DEBUG_TRACE_MEMORY
    printf("free ");
    meoDumpValue(OBJ_VAL(obj));
    printf(" @ %p\n", obj);
    ...
    switch (obj->type) {
        case OBJ_CLASS:
            meoMethodBufferClear(vm, &((ObjClass*)obj)->methods);
            break;
        ...
        case OBJ_FN: {
            ObjFn* fn = (ObjFn*)obj;
            meoValueBufferClear(vm, &fn->constants);
            meoByteBufferClear(vm, &fn->code);
            meoIntBufferClear(vm, &fn->debug->sourceLines);
        ...
        case OBJ_FOREIGN:
            meoFinalizeForeign(vm, (ObjForeign*)obj);
            break;

        case OBJ_LIST:
            meoValueBufferClear(vm, &((ObjList*)obj)->elements);
            break;
        ...
        case OBJ_MODULE:
            meoSymbolTableClear(vm, &((ObjModule*)obj)->variableNames);
            meoValueBufferClear(vm, &((ObjModule*)obj)->variables);
            break;
...
ObjClass* meoGetClass(MeoVM* vm, Value value) { return meoGetClassInline(vm, value); }

bool meoValuesEqual(Value a, Value b) {
    if (meoValuesSame(a, b)) return true;
    ...
    switch (aObj->type) {
        ...
        case OBJ_STRING: {
            ...
            return aString->hash == bString->hash && meoStringEqualsCString(aString, bString->value, bString->length);
        }
