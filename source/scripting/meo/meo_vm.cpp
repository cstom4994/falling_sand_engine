...
#include "meo_core.h"
#include "meo_debug.h"
#include "meo_primitive.h"

#if MEO_OPT_META || MEO_OPT_RANDOM
#include "meo_opt.h"
#endif

#if MEO_DEBUG_TRACE_MEMORY || MEO_DEBUG_TRACE_GC
#include <stdio.h>
#include <time.h>
#endif
...
void meoInitConfiguration(MeoConfiguration* config) {
...
MeoVM* meoNewVM(MeoConfiguration* config) {
    MeoReallocateFn reallocate = defaultReallocate;
    void* userData = NULL;
    if (config != NULL) {
    ...
    MeoVM* vm = (MeoVM*)reallocate(NULL, sizeof(*vm), userData);
    memset(vm, 0, sizeof(MeoVM));

    // Copy the configuration if given one.
    if (config != NULL) {
        memcpy(&vm->config, config, sizeof(MeoConfiguration));

        // We choose to set this after copying,
        // rather than modifying the user config pointer
        vm->config.reallocateFn = reallocate;
    } else {
        meoInitConfiguration(&vm->config);
    }
    ...
    meoSymbolTableInit(&vm->methodNames);

    vm->modules = meoNewMap(vm);
    meoInitializeCore(vm);
    return vm;
}
...
void meoFreeVM(MeoVM* vm) {
    ...
    meoSymbolTableClear(vm, &vm->methodNames);
...
void meoCollectGarbage(MeoVM* vm) {
    ...
    meoGrayObj(vm, (Obj*)vm->modules);

    // Temporary roots.
    for (int i = 0; i < vm->numTempRoots; i++) {
        meoGrayObj(vm, vm->tempRoots[i]);
    }

    // The current fiber.
    meoGrayObj(vm, (Obj*)vm->fiber);

    // The handles.
    for (MeoHandle* handle = vm->handles; handle != NULL; handle = handle->next) {
        meoGrayValue(vm, handle->value);
    }

    // Any object the compiler is using (if there is one).
    if (vm->compiler != NULL) meoMarkCompiler(vm, vm->compiler);

    // Method names.
    meoBlackenSymbolTable(vm, &vm->methodNames);

    // Now that we have grayed the roots, do a depth-first search over all of the
    // reachable objects.
    meoBlackenObjects(vm);
    ...
    printf("GC %lu before, %lu after (%lu collected), next at %lu. Took %.3fms.\n", (unsigned long)before, (unsigned long)vm->bytesAllocated, (unsigned long)(before - vm->bytesAllocated),
           (unsigned long)vm->nextGC, elapsed * 1000.0);
#endif
}

void* meoReallocate(MeoVM* vm, void* memory, size_t oldSize, size_t newSize) {
#if MEO_DEBUG_TRACE_MEMORY
    // Explicit cast because size_t has different sizes on 32-bit and 64-bit and
    // we need a consistent type for the format string.
...
#if MEO_DEBUG_GC_STRESS
    // Since collecting calls this function to free things, make sure we don't
    // recurse.
    if (newSize > 0) meoCollectGarbage(vm);
#else
    if (newSize > 0 && vm->bytesAllocated > vm->nextGC) meoCollectGarbage(vm);
#endif
...
static ObjUpvalue* captureUpvalue(MeoVM* vm, ObjFiber* fiber, Value* local) {
    // If there are no open upvalues at all, we must need a new one.
    if (fiber->openUpvalues == NULL) {
        fiber->openUpvalues = meoNewUpvalue(vm, local);
    ...
    ObjUpvalue* createdUpvalue = meoNewUpvalue(vm, local);
    if (prevUpvalue == NULL) {
...
static MeoForeignMethodFn findForeignMethod(MeoVM* vm, const char* moduleName, const char* className, bool isStatic, const char* signature) {
    MeoForeignMethodFn method = NULL;
    ...
    if (method == NULL) {
        ...
        if (strcmp(moduleName, "meta") == 0) {
            method = meoMetaBindForeignMethod(vm, className, isStatic, signature);
        }
#endif
#if MEO_OPT_RANDOM
        if (strcmp(moduleName, "random") == 0) {
            method = meoRandomBindForeignMethod(vm, className, isStatic, signature);
        }
#endif
    }
...
// method's signature and Meo methods where [methodValue] is a function.
//
// Aborts the current fiber if the method is a foreign method that could not be
// found.
static void bindMethod(MeoVM* vm, int methodType, int symbol, ObjModule* module, ObjClass* classObj, Value methodValue) {
    ...
    if (IS_STRING(methodValue)) {
        ...
        if (method.as.foreign == NULL) {
            vm->fiber->error = meoStringFormat(vm, "Could not find foreign method '@' for class $ in module '$'.", methodValue, classObj->name->value, module->name->value);
            return;
        }
    } else {
        ...
        meoBindMethodCode(classObj, method.as.closure->fn);
    }

    meoBindMethod(vm, classObj, symbol, method);
}

static void callForeign(MeoVM* vm, ObjFiber* fiber, MeoForeignMethodFn foreign, int numArgs) {
...
static void runtimeError(MeoVM* vm) {
    ASSERT(meoHasError(vm->fiber), "Should only call this after an error.");
...
static void methodNotFound(MeoVM* vm, ObjClass* classObj, int symbol) {
    vm->fiber->error = meoStringFormat(vm, "@ does not implement '$'.", OBJ_VAL(classObj->name), vm->methodNames.data[symbol]->value);
}

// Looks up the previously loaded module with [name].
//
// Returns `NULL` if no module with that name has been loaded.
static ObjModule* getModule(MeoVM* vm, Value name) {
    Value moduleValue = meoMapGet(vm->modules, name);
    return !IS_UNDEFINED(moduleValue) ? AS_MODULE(moduleValue) : NULL;
}

static ObjClosure* compileInModule(MeoVM* vm, Value name, const char* source, bool isExpression, bool printErrors) {
    // See if the module has already been loaded.
    ObjModule* module = getModule(vm, name);
    if (module == NULL) {
        module = meoNewModule(vm, AS_STRING(name));

        // It's possible for the meoMapSet below to resize the modules map,
        ...
        meoPushRoot(vm, (Obj*)module);

        // Store it in the VM's module registry so we don't load the same module
        // multiple times.
        meoMapSet(vm, vm->modules, name, OBJ_VAL(module));
        ...
        for (int i = 0; i < coreModule->variables.count; i++) {
            meoDefineVariable(vm, module, coreModule->variableNames.data[i]->value, coreModule->variableNames.data[i]->length, coreModule->variables.data[i], NULL);
        }
    }

    ObjFn* fn = meoCompile(vm, module, source, isExpression, printErrors);
    if (fn == NULL) {
    ...
    meoPushRoot(vm, (Obj*)fn);
    ObjClosure* closure = meoNewClosure(vm, fn);
    meoPopRoot(vm);  // fn.
...
// error message.
static Value validateSuperclass(MeoVM* vm, Value name, Value superclassValue, int numFields) {
    // Make sure the superclass is a class.
    if (!IS_CLASS(superclassValue)) {
        return meoStringFormat(vm, "Class '@' cannot inherit from a non-class object.", name);
    }
    ...
    if (superclass == vm->classClass || superclass == vm->fiberClass || superclass == vm->fnClass ||  // Includes OBJ_CLOSURE.
        superclass == vm->listClass || superclass == vm->mapClass || superclass == vm->rangeClass || superclass == vm->stringClass || superclass == vm->boolClass || superclass == vm->nullClass ||
        superclass == vm->numClass) {
        return meoStringFormat(vm, "Class '@' cannot inherit from built-in class '@'.", name, OBJ_VAL(superclass->name));
    }

    if (superclass->numFields == -1) {
        return meoStringFormat(vm, "Class '@' cannot inherit from foreign class '@'.", name, OBJ_VAL(superclass->name));
    }

    if (numFields == -1 && superclass->numFields > 0) {
        return meoStringFormat(vm, "Foreign class '@' may not inherit from a class with fields.", name);
    }

    if (superclass->numFields + numFields > MAX_FIELDS) {
        return meoStringFormat(vm,
...
static void bindForeignClass(MeoVM* vm, ObjClass* classObj, ObjModule* module) {
    ...
    if (methods.allocate == NULL && methods.finalize == NULL) {
        ...
        if (strcmp(module->name->value, "random") == 0) {
            methods = meoRandomBindForeignClass(vm, module->name->value, classObj->name->value);
        }
#endif
    }
    ...
    int symbol = meoSymbolTableEnsure(vm, &vm->methodNames, "<allocate>", 10);
    ...
    // symbol itself is always in the symbol table.
    symbol = meoSymbolTableEnsure(vm, &vm->methodNames, "<finalize>", 10);
    if (methods.finalize != NULL) {
        method.as.foreign = (MeoForeignMethodFn)methods.finalize;
        meoBindMethod(vm, classObj, symbol, method);
    }
}
...
static void createClass(MeoVM* vm, int numFields, ObjModule* module) {
    ...
    if (meoHasError(vm->fiber)) return;

    ObjClass* classObj = meoNewClass(vm, AS_CLASS(superclass), numFields, AS_STRING(name));
    vm->fiber->stackTop[-1] = OBJ_VAL(classObj);

    if (numFields == -1) bindForeignClass(vm, classObj, module);
}

static void createForeign(MeoVM* vm, ObjFiber* fiber, Value* stack) {
    ObjClass* classObj = AS_CLASS(stack[0]);
    ASSERT(classObj->numFields == -1, "Class must be a foreign class.");

    // TODO: Don't look up every time.
    int symbol = meoSymbolTableFind(&vm->methodNames, "<allocate>", 10);
...
void meoFinalizeForeign(MeoVM* vm, ObjForeign* foreign) {
    // TODO: Don't look up every time.
    int symbol = meoSymbolTableFind(&vm->methodNames, "<finalize>", 10);
    ...
    MeoFinalizerFn finalizer = (MeoFinalizerFn)method->as.foreign;
    finalizer(foreign->data);
}

// Let the host resolve an imported module name if it wants to.
static Value resolveModule(MeoVM* vm, Value name) {
    ...
    if (resolved == NULL) {
        vm->fiber->error = meoStringFormat(vm, "Could not resolve module '@' imported from '@'.", name, OBJ_VAL(importer));
        return NULL_VAL;
    }
    ...
    // Copy the string into a Meo String object.
    name = meoNewString(vm, resolved);
    DEALLOCATE(vm, (char*)resolved);
    return name;
}

static Value importModule(MeoVM* vm, Value name) {
    name = resolveModule(vm, name);

    // If the module is already loaded, we don't need to do anything.
    Value existing = meoMapGet(vm->modules, name);
    if (!IS_UNDEFINED(existing)) return existing;

    meoPushRoot(vm, AS_OBJ(name));

    MeoLoadModuleResult result = {0};
...
#if MEO_OPT_META
        if (strcmp(nameString->value, "meta") == 0) result.source = meoMetaSource();
#endif
#if MEO_OPT_RANDOM
        if (strcmp(nameString->value, "random") == 0) result.source = meoRandomSource();
#endif
    }

    if (result.source == NULL) {
        vm->fiber->error = meoStringFormat(vm, "Could not load module '@'.", name);
        meoPopRoot(vm);  // name.
        return NULL_VAL;
    }
    ...
    if (moduleClosure == NULL) {
        vm->fiber->error = meoStringFormat(vm, "Could not compile module '@'.", name);
        meoPopRoot(vm);  // name.
        return NULL_VAL;
    }
...
static Value getModuleVariable(MeoVM* vm, ObjModule* module, Value variableName) {
    ObjString* variable = AS_STRING(variableName);
    uint32_t variableEntry = meoSymbolTableFind(&module->variableNames, variable->value, variable->length);
    ...
    vm->fiber->error = meoStringFormat(vm, "Could not find a variable named '@' in module '@'.", variableName, OBJ_VAL(module->name));
    return NULL_VAL;
}

inline static bool checkArity(MeoVM* vm, Value value, int numArgs) {
...
static MeoInterpretResult runInterpreter(MeoVM* vm, ObjFiber* fiber) {
...
#define RUNTIME_ERROR()                                         \
    do {                                                        \
        STORE_FRAME();                                          \
        runtimeError(vm);                                       \
        if (vm->fiber == NULL) return MEO_RESULT_RUNTIME_ERROR; \
        fiber = vm->fiber;                                      \
        LOAD_FRAME();                                           \
        DISPATCH();                                             \
    } while (false)

#if MEO_DEBUG_TRACE_INSTRUCTIONS
// Prints the stack and instruction before each instruction is executed.
#define DEBUG_TRACE_INSTRUCTIONS()                             \
    do {                                                       \
        meoDumpStack(fiber);                                   \
        meoDumpInstruction(vm, fn, (int)(ip - fn->code.data)); \
    } while (false)
#else
#define DEBUG_TRACE_INSTRUCTIONS() \
    do {                           \
...
#if MEO_COMPUTED_GOTO

    static void* dispatchTable[] = {
#define OPCODE(name, _) &&code_##name,
#include "meo_opcodes.h"
#undef OPCODE
    };
...
#define DISPATCH() goto loop
    ...
    INTERPRET_LOOP {
        CASE_CODE(LOAD_LOCAL_0)
            ...
            : CASE_CODE(LOAD_LOCAL_4) : CASE_CODE(LOAD_LOCAL_5) : CASE_CODE(LOAD_LOCAL_6) : CASE_CODE(LOAD_LOCAL_7) : CASE_CODE(LOAD_LOCAL_8) : PUSH(stackStart[instruction - CODE_LOAD_LOCAL_0]);
        DISPATCH();
...

        {
            ...
            classObj = meoGetClassInline(vm, args[0]);
            goto completeCall;
            ...
            switch (method->type) {
                case METHOD_PRIMITIVE:
                    if (method->as.primitive(vm, args)) {
                        ...
                        fiber = vm->fiber;
                        if (fiber == NULL) return MEO_RESULT_SUCCESS;
                        if (meoHasError(fiber)) RUNTIME_ERROR();
                        LOAD_FRAME();
                    }
                    break;
                ...
                case METHOD_BLOCK:
                    STORE_FRAME();
                    meoCallFunction(vm, fiber, (ObjClosure*)method->as.closure, numArgs);
                    LOAD_FRAME();
                    break;
        ...
        CASE_CODE(JUMP_IF) : {
            ...
            if (meoIsFalsyValue(condition)) ip += offset;
            DISPATCH();
        }
        ...
        stackStart[0] = meoNewInstance(vm, AS_CLASS(stackStart[0]));
        DISPATCH();
        ...
        CASE_CODE(CLOSURE) : {
            ...
            ObjClosure* closure = meoNewClosure(vm, function);
        ...
        CASE_CODE(IMPORT_MODULE) : {
            ...
            if (IS_CLOSURE(PEEK())) {
                ...
                meoCallFunction(vm, fiber, closure, 1);
                LOAD_FRAME();
            } else {
    ...
    // or a runtime error.
    UNREACHABLE();
    return MEO_RESULT_RUNTIME_ERROR;
...
MeoHandle* meoMakeCallHandle(MeoVM* vm, const char* signature) {
    ...
    int method = meoSymbolTableEnsure(vm, &vm->methodNames, signature, signatureLength);

    // Create a little stub function that assumes the arguments are on the stack
    // and calls the method.
    ObjFn* fn = meoNewFunction(vm, NULL, numParams + 1);

    // Wrap the function in a closure and then in a handle. Do this here so it
    // doesn't get collected as we fill it in.
    MeoHandle* value = meoMakeHandle(vm, OBJ_VAL(fn));
    value->value = OBJ_VAL(meoNewClosure(vm, fn));

    meoByteBufferWrite(vm, &fn->code, (uint8_t)(CODE_CALL_0 + numParams));
    meoByteBufferWrite(vm, &fn->code, (method >> 8) & 0xff);
    meoByteBufferWrite(vm, &fn->code, method & 0xff);
    meoByteBufferWrite(vm, &fn->code, CODE_RETURN);
    meoByteBufferWrite(vm, &fn->code, CODE_END);
    meoIntBufferFill(vm, &fn->debug->sourceLines, 0, 5);
    meoFunctionBindName(vm, fn, signature, signatureLength);
...
MeoInterpretResult meoCall(MeoVM* vm, MeoHandle* method) {
    ...
    // Clear the API stack. Now that meoCall() has control, we no longer need
    ...
    // can call foreign methods from within calls to meoCall().
    vm->apiStack = NULL;
    ...
    meoCallFunction(vm, vm->fiber, closure, 0);
    MeoInterpretResult result = runInterpreter(vm, vm->fiber);
...
MeoHandle* meoMakeHandle(MeoVM* vm, Value value) {
    if (IS_OBJ(value)) meoPushRoot(vm, AS_OBJ(value));

    // Make a handle for it.
    MeoHandle* handle = ALLOCATE(vm, MeoHandle);
    handle->value = value;

    if (IS_OBJ(value)) meoPopRoot(vm);
...
MeoInterpretResult meoInterpret(MeoVM* vm, const char* module, const char* source) {
    ObjClosure* closure = meoCompileSource(vm, module, source, false, true);
    if (closure == NULL) return MEO_RESULT_COMPILE_ERROR;

    meoPushRoot(vm, (Obj*)closure);
    ObjFiber* fiber = meoNewFiber(vm, closure);
    meoPopRoot(vm);  // closure.
    vm->apiStack = NULL;

    return runInterpreter(vm, fiber);
}

ObjClosure* meoCompileSource(MeoVM* vm, const char* module, const char* source, bool isExpression, bool printErrors) {
    Value nameValue = NULL_VAL;
    if (module != NULL) {
        nameValue = meoNewString(vm, module);
        meoPushRoot(vm, AS_OBJ(nameValue));
    }

    ObjClosure* closure = compileInModule(vm, nameValue, source, isExpression, printErrors);

    if (module != NULL) meoPopRoot(vm);  // nameValue.
    return closure;
}

Value meoGetModuleVariable(MeoVM* vm, Value moduleName, Value variableName) {
    ObjModule* module = getModule(vm, moduleName);
    if (module == NULL) {
        vm->fiber->error = meoStringFormat(vm, "Module '@' is not loaded.", moduleName);
        return NULL_VAL;
    }
...
Value meoFindVariable(MeoVM* vm, ObjModule* module, const char* name) {
    int symbol = meoSymbolTableFind(&module->variableNames, name, strlen(name));
    return module->variables.data[symbol];
}

int meoDeclareVariable(MeoVM* vm, ObjModule* module, const char* name, size_t length, int line) {
    ...
    // right line.
    meoValueBufferWrite(vm, &module->variables, NUM_VAL(line));
    return meoSymbolTableAdd(vm, &module->variableNames, name, length);
}

int meoDefineVariable(MeoVM* vm, ObjModule* module, const char* name, size_t length, Value value, int* line) {
    if (module->variables.count == MAX_MODULE_VARS) return -2;

    if (IS_OBJ(value)) meoPushRoot(vm, AS_OBJ(value));

    // See if the variable is already explicitly or implicitly declared.
    int symbol = meoSymbolTableFind(&module->variableNames, name, length);

    if (symbol == -1) {
        // Brand new variable.
        symbol = meoSymbolTableAdd(vm, &module->variableNames, name, length);
        meoValueBufferWrite(vm, &module->variables, value);
        ...
        if (meoIsLocalName(name)) symbol = -3;
    } else {
...
// TODO: Inline?
void meoPushRoot(MeoVM* vm, Obj* obj) {
    ASSERT(obj != NULL, "Can't root NULL.");
    ASSERT(vm->numTempRoots < MEO_MAX_TEMP_ROOTS, "Too many temporary roots.");

    vm->tempRoots[vm->numTempRoots++] = obj;
}

void meoPopRoot(MeoVM* vm) {
...
void meoEnsureSlots(MeoVM* vm, int numSlots) {
    ...
    meoEnsureStack(vm, vm->fiber, needed);

    vm->fiber->stackTop = vm->apiStack + numSlots;
}

// Ensures that [slot] is a valid index into the API's stack of slots.
static void validateApiSlot(MeoVM* vm, int slot) {
    ASSERT(slot >= 0, "Slot cannot be negative.");
    ASSERT(slot < meoGetSlotCount(vm), "Not that many slots.");
}

// Gets the type of the object in [slot].
MeoType meoGetSlotType(MeoVM* vm, int slot) {
    validateApiSlot(vm, slot);
    if (IS_BOOL(vm->apiStack[slot])) return MEO_TYPE_BOOL;
    if (IS_NUM(vm->apiStack[slot])) return MEO_TYPE_NUM;
    if (IS_FOREIGN(vm->apiStack[slot])) return MEO_TYPE_FOREIGN;
    if (IS_LIST(vm->apiStack[slot])) return MEO_TYPE_LIST;
    if (IS_MAP(vm->apiStack[slot])) return MEO_TYPE_MAP;
    if (IS_NULL(vm->apiStack[slot])) return MEO_TYPE_NULL;
    if (IS_STRING(vm->apiStack[slot])) return MEO_TYPE_STRING;

    return MEO_TYPE_UNKNOWN;
}
...
MeoHandle* meoGetSlotHandle(MeoVM* vm, int slot) {
    validateApiSlot(vm, slot);
    return meoMakeHandle(vm, vm->apiStack[slot]);
}

// Stores [value] in [slot] in the foreign call stack.
static void setSlot(MeoVM* vm, int slot, Value value) {
    validateApiSlot(vm, slot);
    vm->apiStack[slot] = value;
}

void meoSetSlotBool(MeoVM* vm, int slot, bool value) { setSlot(vm, slot, BOOL_VAL(value)); }

void meoSetSlotBytes(MeoVM* vm, int slot, const char* bytes, size_t length) {
    ASSERT(bytes != NULL, "Byte array cannot be NULL.");
    setSlot(vm, slot, meoNewStringLength(vm, bytes, length));
}

void meoSetSlotDouble(MeoVM* vm, int slot, double value) { setSlot(vm, slot, NUM_VAL(value)); }
...
void* meoSetSlotNewForeign(MeoVM* vm, int slot, int classSlot, size_t size) {
    ...
    ObjForeign* foreign = meoNewForeign(vm, classObj, size);
    vm->apiStack[slot] = OBJ_VAL(foreign);

    return (void*)foreign->data;
}

void meoSetSlotNewList(MeoVM* vm, int slot) { setSlot(vm, slot, OBJ_VAL(meoNewList(vm, 0))); }

void meoSetSlotNewMap(MeoVM* vm, int slot) { setSlot(vm, slot, OBJ_VAL(meoNewMap(vm))); }

void meoSetSlotNull(MeoVM* vm, int slot) { setSlot(vm, slot, NULL_VAL); }

void meoSetSlotString(MeoVM* vm, int slot, const char* text) {
    ASSERT(text != NULL, "String cannot be NULL.");

    setSlot(vm, slot, meoNewString(vm, text));
}
...
void meoGetListElement(MeoVM* vm, int listSlot, int index, int elementSlot) {
    ...
    uint32_t usedIndex = meoValidateIndex(elements.count, index);
...
void meoSetListElement(MeoVM* vm, int listSlot, int index, int elementSlot) {
    ...
    uint32_t usedIndex = meoValidateIndex(list->elements.count, index);
    ASSERT(usedIndex != UINT32_MAX, "Index out of bounds.");
...
void meoInsertInList(MeoVM* vm, int listSlot, int index, int elementSlot) {
    ...
    // We don't use meoValidateIndex here because insert allows 1 past the end.
    if (index < 0) index = list->elements.count + 1 + index;

    ASSERT(index <= list->elements.count, "Index out of bounds.");

    meoListInsert(vm, list, vm->apiStack[elementSlot], index);
}
...
bool meoGetMapContainsKey(MeoVM* vm, int mapSlot, int keySlot) {
    ...
    ASSERT(meoMapIsValidKey(key), "Key must be a value type");
    if (!validateKey(vm, key)) return false;

    ObjMap* map = AS_MAP(vm->apiStack[mapSlot]);
    Value value = meoMapGet(map, key);
...
void meoGetMapValue(MeoVM* vm, int mapSlot, int keySlot, int valueSlot) {
    ...
    ObjMap* map = AS_MAP(vm->apiStack[mapSlot]);
    Value value = meoMapGet(map, vm->apiStack[keySlot]);
...
void meoRemoveMapValue(MeoVM* vm, int mapSlot, int keySlot, int removedValueSlot) {
    ...
    ObjMap* map = AS_MAP(vm->apiStack[mapSlot]);
    Value removed = meoMapRemoveKey(vm, map, key);
...
void meoGetVariable(MeoVM* vm, const char* module, const char* name, int slot) {
    ...
    Value moduleName = meoStringFormat(vm, "$", module);
    meoPushRoot(vm, AS_OBJ(moduleName));

    ObjModule* moduleObj = getModule(vm, moduleName);
    ASSERT(moduleObj != NULL, "Could not find module.");

    meoPopRoot(vm);  // moduleName.

    int variableSlot = meoSymbolTableFind(&moduleObj->variableNames, name, strlen(name));
...
bool meoHasVariable(MeoVM* vm, const char* module, const char* name) {
    ...
    // We don't use meoHasModule since we want to use the module object.
    ObjModule* moduleObj = getModule(vm, moduleName);
    ASSERT(moduleObj != NULL, "Could not find module.");
...
void* meoGetUserData(MeoVM* vm) { return vm->config.userData; }

void meoSetUserData(MeoVM* vm, void* userData) { vm->config.userData = userData; }
