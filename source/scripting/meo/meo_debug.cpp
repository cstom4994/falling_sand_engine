...
void meoDebugPrintStackTrace(MeoVM* vm) {
    ...
    if (IS_STRING(fiber->error)) {
        vm->config.errorFn(vm, MEO_ERROR_RUNTIME, NULL, -1, AS_CSTRING(fiber->error));
    } else {
        // TODO: Print something a little useful here. Maybe the name of the error's
        // class?
        vm->config.errorFn(vm, MEO_ERROR_RUNTIME, NULL, -1, "[error object]");
    }
    ...
    for (int i = fiber->numFrames - 1; i >= 0; i--) {
        ...
        // detail of what part of the core module is written in C and what is Meo.
        if (fn->module->name == NULL) continue;

        // -1 because IP has advanced past the instruction that it just executed.
        int line = fn->debug->sourceLines.data[frame->ip - fn->code.data - 1];
        vm->config.errorFn(vm, MEO_ERROR_STACK_TRACE, fn->module->name->value, line, fn->debug->name);
    }
}
...
static int dumpInstruction(MeoVM* vm, ObjFn* fn, int i, int* lastLine) {
    int start = i;
...
#define BYTE_INSTRUCTION(name)                \
    ...
    switch (code) {
        case CODE_CONSTANT: {
            ...
            meoDumpValue(fn->constants.data[constant]);
            printf("'\n");
            break;
        }
        ...
        case CODE_LOAD_MODULE_VAR: {
            int slot = READ_SHORT();
            printf("%-16s %5d '%s'\n", "LOAD_MODULE_VAR", slot, fn->module->variableNames.data[slot]->value);
            break;
        }

        case CODE_STORE_MODULE_VAR: {
            int slot = READ_SHORT();
            printf("%-16s %5d '%s'\n", "STORE_MODULE_VAR", slot, fn->module->variableNames.data[slot]->value);
            break;
        }
        ...
        case CODE_IMPORT_MODULE: {
            ...
            meoDumpValue(fn->constants.data[name]);
            printf("'\n");
            break;
        }
        ...
        case CODE_IMPORT_VARIABLE: {
            ...
            meoDumpValue(fn->constants.data[variable]);
            printf("'\n");
            break;
        }
...
int meoDumpInstruction(MeoVM* vm, ObjFn* fn, int i) { return dumpInstruction(vm, fn, i, NULL); }

void meoDumpCode(MeoVM* vm, ObjFn* fn) {
    printf("%s: %s\n", fn->module->name == NULL ? "<core>" : fn->module->name->value, fn->debug->name);
