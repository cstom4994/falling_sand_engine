#ifndef MEOPP_H_INCLUDED
#define MEOPP_H_INCLUDED
...
using LoadModuleFn = std::function<MeoLoadModuleResult(const char*)>;
using WriteFn = std::function<void(const char*)>;
using ReallocateFn = std::function<void*(void*, std::size_t)>;
using ErrorFn = std::function<void(MeoErrorType, const char*, int, const char*)>;
...
/*
 * The interface for getting the object pointer. The actual C++ object may lie within the Meo
...
/*
 * This wraps a class object by value. The lifetimes of these objects are managed in Meo.
 */
template <typename T>
...
public:
    ...
    static void setInSlot(MeoVM* vm, int slot, Args... arg) {
        meoEnsureSlots(vm, slot + 1);
        meoGetVariable(vm, getMeoModuleString<T>(), getMeoClassString<T>(), slot);
        ForeignObjectValue<T>* val = new (meoSetSlotNewForeign(vm, slot, slot, sizeof(ForeignObjectValue<T>))) ForeignObjectValue<T>();
...
public:
    ...
    static void setInSlot(MeoVM* vm, int slot, T* obj) {
        meoEnsureSlots(vm, slot + 1);
        meoGetVariable(vm, getMeoModuleString<T>(), getMeoClassString<T>(), slot);
        void* bytes = meoSetSlotNewForeign(vm, slot, slot, sizeof(ForeignObjectPtr<T>));
...
// given a Meo method signature, this returns a unique value
inline std::size_t hashMethodSignature(const char* module, const char* className, bool isStatic, const char* signature) {
...
template <typename T>
struct MeoSlotAPI {
    static T get(MeoVM* vm, int slot) {
        ForeignObject* obj = static_cast<ForeignObject*>(meoGetSlotForeign(vm, slot));
        return *static_cast<T*>(obj->objectPtr());
    }

    static void set(MeoVM* vm, int slot, T t) { ForeignObjectValue<T>::setInSlot(vm, slot, t); }
};
...
struct MeoSlotAPI<T&> {
    ...
    static void set(MeoVM* vm, int slot, T& t) { ForeignObjectPtr<T>::setInSlot(vm, slot, &t); }
};

template <typename T>
struct MeoSlotAPI<const T&> {
    static const T& get(MeoVM* vm, int slot) {
        ForeignObject* obj = static_cast<ForeignObject*>(meoGetSlotForeign(vm, slot));
        return *static_cast<T*>(obj->objectPtr());
    }

    static void set(MeoVM* vm, int slot, const T& t) { ForeignObjectPtr<T>::setInSlot(vm, 0, const_cast<T*>(&t)); }
};
...
struct MeoSlotAPI<const T*> {
    ...
    static void set(MeoVM* vm, int slot, const T* t) { ForeignObjectPtr<T>::setInSlot(vm, slot, const_cast<T*>(t)); }
};

template <>
struct MeoSlotAPI<float> {
    static float get(MeoVM* vm, int slot) { return float(meoGetSlotDouble(vm, slot)); }

    static void set(MeoVM* vm, int slot, float val) { meoSetSlotDouble(vm, slot, double(val)); }
};

template <>
struct MeoSlotAPI<double> {
    static double get(MeoVM* vm, int slot) { return meoGetSlotDouble(vm, slot); }

    static void set(MeoVM* vm, int slot, double val) { meoSetSlotDouble(vm, slot, val); }
};

template <>
struct MeoSlotAPI<int> {
    static int get(MeoVM* vm, int slot) { return int(meoGetSlotDouble(vm, slot)); }

    static void set(MeoVM* vm, int slot, int val) { meoSetSlotDouble(vm, slot, double(val)); }
};

template <>
struct MeoSlotAPI<unsigned> {
    static unsigned get(MeoVM* vm, int slot) { return unsigned(meoGetSlotDouble(vm, slot)); }

    static void set(MeoVM* vm, int slot, unsigned val) { meoSetSlotDouble(vm, slot, double(val)); }
};

template <>
struct MeoSlotAPI<bool> {
    static bool get(MeoVM* vm, int slot) { return meoGetSlotBool(vm, slot); }

    static void set(MeoVM* vm, int slot, bool val) { meoSetSlotBool(vm, slot, val); }
};

template <>
struct MeoSlotAPI<const char*> {
    static const char* get(MeoVM* vm, int slot) { return meoGetSlotString(vm, slot); }

    static void set(MeoVM* vm, int slot, const char* val) { meoSetSlotString(vm, slot, val); }
};

template <>
struct MeoSlotAPI<std::string> {
    static std::string get(MeoVM* vm, int slot) { return std::string(meoGetSlotString(vm, slot)); }

    static void set(MeoVM* vm, int slot, const std::string& str) { meoSetSlotString(vm, slot, str.c_str()); }
};

template <>
struct MeoSlotAPI<const std::string&> {
    static const char* get(MeoVM* vm, int slot) { return meoGetSlotString(vm, slot); }
...
// a helper for passing arguments to Meo
// explained here:
// http://stackoverflow.com/questions/17339789/how-to-call-a-function-on-all-variadic-template-args
template <typename... Args, std::size_t... index>
void passArgumentsToMeo(MeoVM* vm, const std::tuple<Args...>& tuple, std::index_sequence<index...>) {
    using Traits = ParameterPackTraits<Args...>;
    ExpandType{0, (MeoSlotAPI<typename Traits::template ParameterType<index>>::set(vm, index + 1, std::get<index>(tuple)), 0)...};
}

template <typename Function, std::size_t... index>
decltype(auto) invokeHelper(MeoVM* vm, Function&& f, std::index_sequence<index...>) {
    using Traits = FunctionTraits<std::remove_reference_t<decltype(f)>>;
    return f(MeoSlotAPI<typename Traits::template ArgumentType<index>>::get(vm, index + 1)...);
}

template <typename Function>
decltype(auto) invokeWithMeoArguments(MeoVM* vm, Function&& f) {
...
decltype(auto) invokeHelper(MeoVM* vm, R (C::*f)(Args...), std::index_sequence<index...>) {
    using Traits = FunctionTraits<decltype(f)>;
    ForeignObject* objWrapper = static_cast<ForeignObject*>(meoGetSlotForeign(vm, 0));
    C* obj = static_cast<C*>(objWrapper->objectPtr());
    return (obj->*f)(MeoSlotAPI<typename Traits::template ArgumentType<index>>::get(vm, index + 1)...);
}

// const variant
template <typename R, typename C, typename... Args, std::size_t... index>
decltype(auto) invokeHelper(MeoVM* vm, R (C::*f)(Args...) const, std::index_sequence<index...>) {
    using Traits = FunctionTraits<decltype(f)>;
    ForeignObject* objWrapper = static_cast<ForeignObject*>(meoGetSlotForeign(vm, 0));
...
decltype(auto) invokeWithMeoArguments(MeoVM* vm, R (C::*f)(Args...)) {
...
// const variant
template <typename R, typename C, typename... Args>
decltype(auto) invokeWithMeoArguments(MeoVM* vm, R (C::*f)(Args...) const) {
    constexpr auto Arity = FunctionTraits<decltype(f)>::Arity;
    return invokeHelper(vm, f, std::make_index_sequence<Arity>{});
}

// invokes plain invokeWithMeoArguments if true
// invokes invokeWithMeoArguments within MeoReturn if false
// to be used with std::is_void as the predicate
template <bool predicate>
struct InvokeWithoutReturningIf {

    template <typename Function>
    static void invoke(MeoVM* vm, Function&& f) {
        invokeWithMeoArguments(vm, std::forward<Function>(f));
    }

    template <typename R, typename C, typename... Args>
    static void invoke(MeoVM* vm, R (C::*f)(Args...)) {
        invokeWithMeoArguments(vm, std::forward<R (C::*)(Args...)>(f));
    }
};
...
struct InvokeWithoutReturningIf<false> {
    ...
    static void invoke(MeoVM* vm, Function&& f) {
        ...
        MeoSlotAPI<ReturnType>::set(vm, 0, invokeWithMeoArguments(vm, std::forward<Function>(f)));
    }

    template <typename R, typename C, typename... Args>
    static void invoke(MeoVM* vm, R (C::*f)(Args...)) {
        MeoSlotAPI<R>::set(vm, 0, invokeWithMeoArguments(vm, f));
    }

    template <typename R, typename C, typename... Args>
    static void invoke(MeoVM* vm, R (C::*f)(Args...) const) {
        MeoSlotAPI<R>::set(vm, 0, invokeWithMeoArguments(vm, f));
    }
};
...
struct ForeignMethodWrapper<R (*)(Args...), f> {
    ...
    static void call(MeoVM* vm) { InvokeWithoutReturningIf<std::is_void<R>::value>::invoke(vm, f); }
};
...
struct ForeignMethodWrapper<R (C::*)(Args...), m> {
    ...
    static void call(MeoVM* vm) { InvokeWithoutReturningIf<std::is_void<R>::value>::invoke(vm, m); }
};
...
struct SetFieldInSlot {
    template <typename T>
    static void set(MeoVM* vm, int slot, T& obj) {
...
struct SetFieldInSlot<false> {
    template <typename T>
    static void set(MeoVM* vm, int slot, T value) {
        MeoSlotAPI<T>::set(vm, slot, value);
    }
};

template <typename T, typename U, U T::*Field>
void propertyGetter(MeoVM* vm) {
    ForeignObject* objWrapper = static_cast<ForeignObject*>(meoGetSlotForeign(vm, 0));
...
template <typename T, typename U, U T::*Field>
void propertySetter(MeoVM* vm) {
    ForeignObject* objWrapper = static_cast<ForeignObject*>(meoGetSlotForeign(vm, 0));
    T* obj = static_cast<T*>(objWrapper->objectPtr());
    obj->*Field = MeoSlotAPI<U>::get(vm, 1);
}
...
// given a Meo class signature, this returns a unique value
...
void construct(MeoVM* vm, void* memory, std::index_sequence<index...>) {
    using Traits = ParameterPackTraits<Args...>;
    ForeignObjectValue<T>* obj = new (memory) ForeignObjectValue<T>{};
    constexpr std::size_t arity = sizeof...(Args);
    meoEnsureSlots(vm, arity);
    new (obj->objectPtr()) T{MeoSlotAPI<typename Traits::template ParameterType<index>>::get(vm, index + 1)...};
}

template <typename T, typename... Args>
void allocate(MeoVM* vm) {
    void* memory = meoSetSlotNewForeign(vm, 0, 0, sizeof(ForeignObjectValue<T>));
...
void registerFunction(MeoVM* vm, const std::string& mod, const std::string& clss, bool isStatic, std::string sig, MeoForeignMethodFn function);
void registerClass(MeoVM* vm, const std::string& mod, std::string clss, MeoForeignClassMethods methods);
...
// This class can hold any one of the values corresponding to the MeoType
// enum defined in meo.h
class Value {
public:
    Value() = default;
    ~Value();
...
class Method {
public:
    Method(VM* vm, MeoHandle* variable, MeoHandle* method);
    Method() = default;
...
private:
    ...
    mutable MeoHandle* method_{nullptr};
    mutable MeoHandle* variable_{nullptr};
};
...
public:
    ...
    ClassContext& bindCFunction(bool isStatic, std::string signature, MeoForeignMethodFn function);
...
public:
    ...
    RegisteredClassContext& bindCFunction(bool isStatic, std::string signature, MeoForeignMethodFn function);
};

class ModuleContext {
public:
    ModuleContext() = delete;
    ModuleContext(MeoVM* vm, std::string mod) : vm_(vm), name_(mod) {}
...
enum class Result { Success, CompileError, RuntimeError };
...
public:
    ...
    inline MeoVM* ptr() { return vm_; }

    inline operator MeoVM*() const { return vm_; }
...
template <typename T>
Value::Value(T* t) : type_{MEO_TYPE_FOREIGN}, string_{nullptr} {
    std::memcpy(storage_, &t, sizeof(T*));
}

template <>
inline float Value::as<float>() const {
    assert(type_ == MEO_TYPE_NUM);
...
Value Method::operator()(Args... args) const {
    ...
    meoEnsureSlots(vm_->ptr(), Arity + 1u);
    meoSetSlotHandle(vm_->ptr(), 0, variable_);

    std::tuple<Args...> tuple = std::make_tuple(args...);
    detail::passArgumentsToMeo(vm_->ptr(), tuple, std::make_index_sequence<Arity>{});

    auto result = meoCall(vm_->ptr(), method_);

    if (result == MEO_RESULT_SUCCESS) {
        MeoType type = meoGetSlotType(vm_->ptr(), 0);

        switch (type) {
            case MEO_TYPE_BOOL:
                return Value(meoGetSlotBool(vm_->ptr(), 0));
            case MEO_TYPE_NUM:
                return Value(meoGetSlotDouble(vm_->ptr(), 0));
            case MEO_TYPE_STRING:
                return Value(meoGetSlotString(vm_->ptr(), 0));
            case MEO_TYPE_FOREIGN:
                return Value(meoGetSlotForeign(vm_->ptr(), 0));
            default:
                assert("Invalid Meo type");
                break;
        }
    }
...
RegisteredClassContext<T> ModuleContext::bindClass(std::string className) {
    MeoForeignClassMethods wrapper{&detail::allocate<T, Args...>, &detail::finalize<T>};
...
ClassContext& ClassContext::bindFunction(bool isStatic, std::string s) {
    detail::registerFunction(module_.vm_, module_.name_, class_, isStatic, s, detail::ForeignMethodWrapper<decltype(f), f>::call);
    return *this;
}
...
template <typename T>
RegisteredClassContext<T>& RegisteredClassContext<T>::bindCFunction(bool isStatic, std::string s, MeoForeignMethodFn function) {
    detail::registerFunction(module_.vm_, module_.name_, class_, isStatic, s, function);
    return *this;
}

template <typename T>
T* getSlotForeign(MeoVM* vm, int slot) {
    detail::ForeignObject* obj = static_cast<detail::ForeignObject*>(meoGetSlotForeign(vm, slot));
    return static_cast<T*>(obj->objectPtr());
}

template <typename T>
void setSlotForeignValue(MeoVM* vm, int slot, const T& obj) {
    detail::ForeignObjectValue<T>::setInSlot(vm, slot, obj);
}

template <typename T>
void setSlotForeignPtr(MeoVM* vm, int slot, T* obj) {
    detail::ForeignObjectPtr<T>::setInSlot(vm, slot, obj);
}

}  // namespace MetaEngine::Meo

#endif  // MEOPP_H_INCLUDED
