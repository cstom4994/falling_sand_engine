...
void meoSymbolTableInit(SymbolTable* symbols) { meoStringBufferInit(symbols); }

void meoSymbolTableClear(MeoVM* vm, SymbolTable* symbols) { meoStringBufferClear(vm, symbols); }

int meoSymbolTableAdd(MeoVM* vm, SymbolTable* symbols, const char* name, size_t length) {
    ObjString* symbol = AS_STRING(meoNewStringLength(vm, name, length));

    meoPushRoot(vm, &symbol->obj);
    meoStringBufferWrite(vm, symbols, symbol);
    meoPopRoot(vm);
...
int meoSymbolTableEnsure(MeoVM* vm, SymbolTable* symbols, const char* name, size_t length) {
    ...
    int existing = meoSymbolTableFind(symbols, name, length);
    if (existing != -1) return existing;

    // New symbol, so add it.
    return meoSymbolTableAdd(vm, symbols, name, length);
}
...
int meoSymbolTableFind(const SymbolTable* symbols, const char* name, size_t length) {
    ...
    for (int i = 0; i < symbols->count; i++) {
        if (meoStringEqualsCString(symbols->data[i], name, length)) return i;
    }
...
void meoBlackenSymbolTable(MeoVM* vm, SymbolTable* symbolTable) {
    for (int i = 0; i < symbolTable->count; i++) {
        meoGrayObj(vm, &symbolTable->data[i]->obj);
    }
