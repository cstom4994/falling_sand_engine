...
#define IS_CONST(tok)                                                                              \
    (IS_LITERAL(tok, "const") || IS_LITERAL(tok, "__const") || IS_LITERAL(tok, "__const__"))
...
#define IS_INLINE(tok)                                                                             \
    (IS_LITERAL(tok, "inline") || IS_LITERAL(tok, "__inline") || IS_LITERAL(tok, "__inline__"))
...
static int next_token(lua_State *L, struct parser *P, struct token *tok) {
    ...
    /* UTF8 BOM */
    if (s[0] == '\xEF' && s[1] == '\xBB' && s[2] == '\xBF') { s += 3; }
    ...
    for (;;) {
        ...
        if (*s == '/' && *(s + 1) == '/') {
            ...
            s = strchr(s, '\n');
            if (!s) { luaL_error(L, "non-terminated comment"); }
...
static void require_token_line(lua_State *L, struct parser *P, struct token *tok, const char *file,
                               int line) {
    if (!next_token(L, P, tok)) { luaL_error(L, "unexpected end on line %s:%d", file, line); }
}
...
static void calculate_member_position(lua_State *L, struct parser *P, struct ctype *ct,
                                      struct ctype *mt, int *pbit_offset, int *pbitfield_type) {
    ...
    if (ct->type == UNION_TYPE) {
        ...
        *pbitfield_type = mt->align_mask;
        ...
        int bits_used =
                (ct->base_size - ALIGN_DOWN(ct->base_size, mt->align_mask)) * CHAR_BIT + bit_offset;
        ...
        /* unnamed bitfields don't update the struct alignment */
        if (!mt->has_member_name) { mt->align_mask = 0; }
#else
#error
#endif
        ...
        if (*pbitfield_type >= 0) { ct->base_size = ALIGN_UP(ct->base_size, *pbitfield_type); }
    ...
    if (mt->align_mask > (int) ct->align_mask) { ct->align_mask = mt->align_mask; }

    if (mt->has_bitfield || mt->is_bitfield) { ct->has_bitfield = 1; }
...
static int add_member(lua_State *L, int ct_usr, int mname, int mbr_usr, const struct ctype *mt,
                      int *midx) {
...
static int parse_struct(lua_State *L, struct parser *P, int tmp_usr, const struct ctype *ct) {
    ...
    for (;;) {
        ...
        if (tok.type == TOK_CLOSE_CURLY) {
            ...
            return luaL_error(L, "can't have members after a variable sized member on line %d",
                              P->line);
        } else {
            put_back(P);
        }
...
static int calculate_struct_offsets(lua_State *L, struct parser *P, int ct_usr, struct ctype *ct,
                                    int tmp_usr) {
    int i;
    int midx = 1;
    ...
    if (ct->base_size == 0) { ct->base_size = 1; }
...
static int parse_record(lua_State *L, struct parser *P, struct ctype *ct) {
    ...
    if (tok.type == TOK_TOKEN) {
        ...
        if (lua_isnil(L, -1)) {
            ...
            if (prevt->type != ct->type) {
                ...
                luaL_error(L, "type '%s' previously declared as '%s'", lua_tostring(L, -2),
                           lua_tostring(L, -1));
            }
         ...
         */
        if (!next_token(L, P, &tok)) { return 0; }
    ...
    if (ct->is_defined) { return luaL_error(L, "redefinition in line %d", P->line); }
...
#define SHORT_TYPE(u)                                                                              \
    (sizeof(short) == sizeof(int64_t)   ? u "int64_t"                                              \
     : sizeof(short) == sizeof(int32_t) ? u "int32_t"                                              \
                                        : u "int16_t")
        if (flags & UNSIGNED) {
...
#define INT_TYPE(u)                                                                                \
    (sizeof(int) == sizeof(int64_t)   ? u "int64_t"                                                \
     : sizeof(int) == sizeof(int32_t) ? u "int32_t"                                                \
                                      : u "int16_t")
        if (flags & UNSIGNED) {
...
static int parse_attribute(lua_State *L, struct parser *P, struct token *tok, struct ctype *ct,
    ...
    if (tok->type != TOK_TOKEN) {
        ...
        check_token(L, P, TOK_OPEN_PAREN, NULL, "unexpected token after __asm__ on line %d",
                    P->line);
        *asmname = *P;

        require_token(L, P, tok);
        while (tok->type == TOK_STRING) { require_token(L, P, tok); }
        ...
        check_token(L, P, TOK_OPEN_PAREN, NULL,
                    "expected parenthesis after __attribute__ or __declspec on line %d", P->line);
            ...
            if (tok->type == TOK_OPEN_PAREN) {
                ...
                switch (tok->type) {
                    ...
                    case TOK_OPEN_PAREN:
                        ...
                        check_token(L, P, TOK_CLOSE_PAREN, NULL, "expected align(#) on line %d",
                                    P->line);
                        break;
...
int parse_type(lua_State *L, struct parser *P, struct ctype *ct) {
    ...
    for (;;) {
        ...
        } else if (IS_VOLATILE(tok) || IS_RESTRICT(tok) || IS_LITERAL(tok, "static") ||
                   IS_INLINE(tok)) {
            /* ignored for now */
            require_token(L, P, &tok);
...
static void append_type_name(luaL_Buffer *B, int usr, const struct ctype *ct, enum name_type type) {
    ...
    if (type == FRONT || type == BOTH) {
        ...
        if (ct->is_unsigned) { luaL_addstring(B, "unsigned "); }
        ...
        for (i = 0; i < ct->pointers - ct->is_array; i++) {
            luaL_addchar(B, '*');
            if (ct->const_mask & (1 << (ct->pointers - i - 1))) { luaL_addstring(B, " const"); }
        }
    }

    if (type == BOTH || type == BACK) {
        if (ct->is_reference) { luaL_addstring(B, " &"); }
...
static void parse_function_arguments(lua_State *L, struct parser *P, int ct_usr, struct ctype *ct) {
    ...
    for (;;) {
        ...
        if (tok.type == TOK_CLOSE_PAREN) { break; }
        ...
        if (tok.type == TOK_VA_ARG) {
            ...
            if (at.type == VOID_TYPE && at.pointers == 0) {
                if (args) { luaL_error(L, "can't have argument of type void on line %d", P->line); }

                check_token(L, P, TOK_CLOSE_PAREN, "", "unexpected void in function on line %d",
                            P->line);
                lua_pop(L, 2);
                break;
            }
...
static struct ctype *parse_argument2(lua_State *L, struct parser *P, int ct_usr, struct ctype *ct,
 ...
 */
static struct ctype *parse_function(lua_State *L, struct parser *P, int ct_usr, struct ctype *ct,
                                    struct token *name, struct parser *asmname) {
    ...
    if (name->type == TOK_NIL) {
        for (;;) {
            ...
            if (tok.type == TOK_STAR) {
                ...
                if (ct->type == FUNCTION_TYPE) {
                    ...
                    luaL_error(L,
                               "maximum number of pointer derefs reached - use a struct to break "
                               "up the pointers on line %d",
                               P->line);
                } else {
                    ct->pointers++;
                    ct->const_mask <<= 1;
        ...
        check_token(L, P, TOK_CLOSE_PAREN, NULL, "unexpected token in function on line %d",
                    P->line);
     ...
     */
    if (lua_gettop(L) == ct_usr + 1) { lua_replace(L, ct_usr); }
    ...
    for (;;) {
        if (!next_token(L, P, &tok)) {
            ...
            if (ct->pointers == POINTER_MAX) {
                luaL_error(L,
                           "maximum number of pointer derefs reached - use a struct to break up "
                           "the pointers on line %d",
                           P->line);
            }
            ...
            /* array */
            if (ct->pointers == POINTER_MAX) {
                luaL_error(L, "maximum number of pointer derefs reached - use a struct to break up "
                              "the pointers");
            }
            ct->is_array = 1;
            ct->pointers++;
            ...
            if (tok.type == TOK_QUESTION) {
                ...
                check_token(L, P, TOK_CLOSE_SQUARE, "", "invalid character in array on line %d",
                            P->line);
...
static void find_canonical_usr(lua_State *L, int ct_usr, const struct ctype *ct) {
    ...
    if (ct->type != FUNCTION_PTR_TYPE && ct->type != FUNCTION_TYPE) { return; }
...
void parse_argument(lua_State *L, struct parser *P, int ct_usr, struct ctype *ct,
    ...
    if (lua_gettop(L) == top) { lua_pushvalue(L, ct_usr); }
...
static void push_strings(lua_State *L, struct parser *P) {
    ...
    for (;;) {
        ...
        while (p < e) {
            if (*p == '\\') {
                if (++p == e) { luaL_error(L, "parse error in string"); }
                switch (*p) {
                    case '\\':
                        *(t++) = '\\';
                        p++;
                    ...
                    default: {
                        ...
                        if (!is_digit(*p)) { luaL_error(L, "parse error in string"); }
                        u = *p - '0';
                        p++;
...
static void parse_constant_assignemnt(lua_State *L, struct parser *P, const struct ctype *type,
                                      const struct token *name) {
    int64_t val = calculate_constant(L, P);

    check_token(L, P, TOK_SEMICOLON, "", "expected ; after constant definition on line %d",
                P->line);
    ...
    switch (type->type) {
        ...
        case INT16_TYPE:
        case INT32_TYPE:
            if (type->is_unsigned) lua_pushinteger(L, (unsigned int) val);
            else
        ...
        default:
            luaL_error(L,
                       "expected a valid 8-, 16-, or 32-bit signed or unsigned integer type after "
                       "'static const' on line %d",
                       P->line);
    }
...
static int parse_root(lua_State *L, struct parser *P) {
    ...
    while (next_token(L, P, &tok)) {
        ...
        if (tok.type == TOK_SEMICOLON) {
            ...
            check_token(L, P, TOK_TOKEN, "pragma", "unexpected pre processor directive on line %d",
                        P->line);
            check_token(L, P, TOK_TOKEN, "pack", "unexpected pre processor directive on line %d",
                        P->line);
            ...
            if (tok.type == TOK_NUMBER) {
                if (tok.integer != 1 && tok.integer != 2 && tok.integer != 4 && tok.integer != 8 &&
                    tok.integer != 16) {
                    luaL_error(L, "pack directive with invalid pack size on line %d", P->line);
                }

                P->align_mask = (unsigned) (tok.integer - 1);
                check_token(L, P, TOK_CLOSE_PAREN, "", "invalid pack directive on line %d",
                            P->line);
                ...
                if (parse_root(L, P) != PRAGMA_POP) {
                    luaL_error(L,
                               "reached end of string without a pragma pop to match the push on "
                               "line %d",
                               line);
                }
...
static int try_cast(lua_State *L) {
    ...
    require_token(L, P, &tok);
    if (tok.type != TOK_CLOSE_PAREN || name.size) { return luaL_error(L, "invalid cast"); }
...
static int64_t calculate_constant2(lua_State *L, struct parser *P, struct token *tok) {
    if (tok->type == TOK_LOGICAL_NOT) {
        ...
        require_token(L, P, tok);
        if (tok->type != TOK_OPEN_PAREN) { luaL_error(L, "invalid sizeof at line %d", P->line); }
        ...
        require_token(L, P, tok);
        if (tok->type != TOK_CLOSE_PAREN) { luaL_error(L, "invalid sizeof at line %d", P->line); }
...
int64_t calculate_constant(lua_State *L, struct parser *P) {
    ...
    if (tok.type != TOK_NIL) { put_back(P); }
