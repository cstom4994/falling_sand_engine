...
local assert, unpack, setmetatable = assert, unpack or table.unpack,
                                     setmetatable
local _s = string
...
local action_names = {
    ...
    "REL_LG", "REL_PC", -- action arg (1 byte) or int arg, 1 buffer pos (link):
    "IMM_LG", "IMM_PC",
    -- action arg (1 byte) or int arg, 1 buffer pos (offset):
    "LABEL_LG", "LABEL_PC", -- action arg (1 byte), 1 buffer pos (offset):
    "ALIGN", -- action args (2 bytes), no buffer pos.
    "EXTERN", -- action arg (1 byte), no buffer pos.
    "ESC", -- no action arg, no buffer pos.
    "MARK", -- action arg (1 byte), no buffer pos, terminal action:
    "SECTION", -- no args, no buffer pos, terminal action:
    "STOP"
}
...
local map_opsizenum = {b = 1, w = 2, d = 4, q = 8, o = 16, t = 10}
...
function _M.revdef(s) return gsub(s, "@%w+", map_reg_rev) end
...
local function immexpr(expr)
    ...
    if prefix == "=>" then return "iJ", sub(expr, 3) end
    -- ->name (global label reference)
    if prefix == "->" then return "iJ", map_global[sub(expr, 3)] end
    ...
    if extname then return "iJ", map_extern[extname] end
...
local function rtexpr(expr)
    ...
    if tp then
        ...
        if not map_reg_valid_base[reg] then
            werror(
                "bad base register override `" .. (map_reg_rev[reg] or reg) ..
                    "'")
        end
        return reg, rnum, tp
    end
                ...
                if tailr == "" then
                    ...
                    if t.opsize == "f" then
                        ...
                        if reg == "@w4" or (x64 and reg == "@d4") then
                            wwarn("bad idea, try again with `" ..
                                      (x64 and "rsp'" or "esp'"))
                        end
                        t.mode = t.reg == 0 and "rmR" or
                                     (reg == "@b1" and "rmC" or "rm")
                    end
...
local map_op = {
    ...
    push_1 = (x64 and "rq:n50r|rw:50r|mq:nFF6m|mw:FF6m" or "rdw:50r|mdw:FF6m") ..
        "|S.:6AS|ib:n6Ai|i.:68i",
...
for name, n in pairs {
    ...
    sar = 7,
    sal = 4
} do map_op[name .. "_2"] = format("m1:D1%Xm|mC1qdwb:D3%Xm|mi:C1%XmU", n, n, n) end
...
for name, n in pairs {
    ...
    if n == 2 or n == 3 then
        map_op[fn .. "_2"] = format("Fff:D8%02XR|Fx2d:D8%XM|Fx2q:nDC%XM", nc, n,
                                    n)
    else
        map_op[fn .. "_2"] = format(
        ...
        elseif c == "m" or c == "M" then -- Encode ModRM/SIB.
            ...
            elseif c == "O" then
                ...
                elseif c == "J" then
                    ...
                    werror("bad char `" .. c .. "' in pattern `" .. pat ..
                               "' for `" .. op .. "'")
                end
            end
        end
...
local function templatehelp(template, nparams)
    ...
    for tm in gmatch(template, "[^%|]+") do
        ...
        s = s ..
                gsub(sub(tm, 2, nparams), ".",
                     function(c) return ", " .. map_modename[c] end)
    ...
    for tm in gmatch(template, "[^%|]+") do
        ...
        if matchtm(tm, args) then
            ...
            if prefix == "/" then -- Match both operand sizes.
                if args[1].opsize == sub(szm, 2, 2) and args[2].opsize ==
                    sub(szm, 3, 3) then
