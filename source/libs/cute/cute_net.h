...
typedef struct cn_crypto_key_t {
    uint8_t key[32];
} cn_crypto_key_t;
typedef struct cn_crypto_sign_public_t {
    uint8_t key[32];
} cn_crypto_sign_public_t;
typedef struct cn_crypto_sign_secret_t {
    uint8_t key[64];
} cn_crypto_sign_secret_t;
typedef struct cn_crypto_signature_t {
    uint8_t bytes[64];
 ...
 */
cn_result_t cn_generate_connect_token(uint64_t application_id,                           // A unique number to identify your game, can be whatever value you like.
...
cn_client_t* cn_client_create(uint16_t port,                             // Port for opening a UDP socket.
...
CN_INLINE cn_result_t cn_error_failure(const char* details) {
    cn_result_t result;
    result.code = CN_ERROR_FAILURE;
    result.details = details;
    return result;
}
CN_INLINE cn_result_t cn_error_success(void) {
    cn_result_t result;
    result.code = CN_ERROR_SUCCESS;
    result.details = NULL;
    return result;
}
#define CN_RETURN_IF_ERROR(x)                   \
    do {                                        \
        cn_result_t result = (x);               \
...
int hydro_secretbox_decrypt(void* m_, const uint8_t* c, size_t clen, uint64_t msg_id, const char ctx[hydro_secretbox_CONTEXTBYTES],
                            const uint8_t key[hydro_secretbox_KEYBYTES]) _hydro_attr_warn_unused_result_;
...
int hydro_secretbox_probe_verify(const uint8_t probe[hydro_secretbox_PROBEBYTES], const uint8_t* c, size_t c_len, const char ctx[hydro_secretbox_CONTEXTBYTES],
                                 const uint8_t key[hydro_secretbox_KEYBYTES]) _hydro_attr_warn_unused_result_;
...
int hydro_sign_verify(const uint8_t csig[hydro_sign_BYTES], const void* m_, size_t mlen, const char ctx[hydro_sign_CONTEXTBYTES],
...
int hydro_kx_kk_2(hydro_kx_session_keypair* kp, uint8_t packet2[hydro_kx_KK_PACKET2BYTES], const uint8_t packet1[hydro_kx_KK_PACKET1BYTES], const uint8_t peer_static_pk[hydro_kx_PUBLICKEYBYTES],
...
int hydro_kx_xx_2(hydro_kx_state* state, uint8_t packet2[hydro_kx_XX_PACKET2BYTES], const uint8_t packet1[hydro_kx_XX_PACKET1BYTES], const uint8_t psk[hydro_kx_PSKBYTES],
                  const hydro_kx_keypair* static_kp);

int hydro_kx_xx_3(hydro_kx_state* state, hydro_kx_session_keypair* kp, uint8_t packet3[hydro_kx_XX_PACKET3BYTES], uint8_t peer_static_pk[hydro_kx_PUBLICKEYBYTES],
                  const uint8_t packet2[hydro_kx_XX_PACKET2BYTES], const uint8_t psk[hydro_kx_PSKBYTES], const hydro_kx_keypair* static_kp);

int hydro_kx_xx_4(hydro_kx_state* state, hydro_kx_session_keypair* kp, uint8_t peer_static_pk[hydro_kx_PUBLICKEYBYTES], const uint8_t packet3[hydro_kx_XX_PACKET3BYTES],
...
int hydro_kx_nk_1(hydro_kx_state* state, uint8_t packet1[hydro_kx_NK_PACKET1BYTES], const uint8_t psk[hydro_kx_PSKBYTES], const uint8_t peer_static_pk[hydro_kx_PUBLICKEYBYTES]);

int hydro_kx_nk_2(hydro_kx_session_keypair* kp, uint8_t packet2[hydro_kx_NK_PACKET2BYTES], const uint8_t packet1[hydro_kx_NK_PACKET1BYTES], const uint8_t psk[hydro_kx_PSKBYTES],
                  const hydro_kx_keypair* static_kp);

int hydro_kx_nk_3(hydro_kx_state* state, hydro_kx_session_keypair* kp, const uint8_t packet2[hydro_kx_NK_PACKET2BYTES]);
...
int hydro_pwhash_deterministic(uint8_t* h, size_t h_len, const char* passwd, size_t passwd_len, const char ctx[hydro_pwhash_CONTEXTBYTES], const uint8_t master_key[hydro_pwhash_MASTERKEYBYTES],
                               uint64_t opslimit, size_t memlimit, uint8_t threads);

int hydro_pwhash_create(uint8_t stored[hydro_pwhash_STOREDBYTES], const char* passwd, size_t passwd_len, const uint8_t master_key[hydro_pwhash_MASTERKEYBYTES], uint64_t opslimit, size_t memlimit,
                        uint8_t threads);

int hydro_pwhash_verify(const uint8_t stored[hydro_pwhash_STOREDBYTES], const char* passwd, size_t passwd_len, const uint8_t master_key[hydro_pwhash_MASTERKEYBYTES], uint64_t opslimit_max,
                        size_t memlimit_max, uint8_t threads_max);

int hydro_pwhash_derive_static_key(uint8_t* static_key, size_t static_key_len, const uint8_t stored[hydro_pwhash_STOREDBYTES], const char* passwd, size_t passwd_len,
                                   const char ctx[hydro_pwhash_CONTEXTBYTES], const uint8_t master_key[hydro_pwhash_MASTERKEYBYTES], uint64_t opslimit_max, size_t memlimit_max, uint8_t threads_max);
...
static inline void mem_cpy(void* __restrict__ dst_, const void* __restrict__ src_, size_t n) {
...
static inline void mem_xor(void* __restrict__ dst_, const void* __restrict__ src_, size_t n) {
    unsigned char* dst = (unsigned char*)dst_;
    const unsigned char* src = (const unsigned char*)src_;
    size_t i;
...
static inline void mem_xor2(void* __restrict__ dst_, const void* __restrict__ src1_, const void* __restrict__ src2_, size_t n) {
    unsigned char* dst = (unsigned char*)dst_;
...
static inline void gimli_pad_u8(uint8_t buf[gimli_BLOCKBYTES], size_t pos, uint8_t domain) {
    buf[pos] ^= (domain << 1) | 1;
    buf[gimli_RATE - 1] ^= 0x80;
}

static inline void hydro_mem_ct_zero_u32(uint32_t* dst_, size_t n) {
...
static inline uint32_t hydro_mem_ct_cmp_u32(const uint32_t* b1_, const uint32_t* b2, size_t n) _hydro_attr_warn_unused_result_;
...
static int hydro_hash_init_with_tweak(hydro_hash_state* state, const char ctx[hydro_hash_CONTEXTBYTES], uint64_t tweak, const uint8_t key[hydro_hash_KEYBYTES]);
...
static int hydro_x25519_scalarmult(uint8_t out[hydro_x25519_BYTES], const uint8_t scalar[hydro_x25519_SECRETKEYBYTES], const uint8_t x1[hydro_x25519_PUBLICKEYBYTES],
...
static inline void hydro_x25519_scalarmult_base_uniform(uint8_t pk[hydro_x25519_PUBLICKEYBYTES], const uint8_t sk[hydro_x25519_SECRETKEYBYTES]);
...
static inline __m128i shift(__m128i x, int bits) { return _mm_slli_epi32(x, bits); }

static inline __m128i rotate(__m128i x, int bits) { return _mm_slli_epi32(x, bits) | _mm_srli_epi32(x, 32 - bits); }

#ifdef __SSSE3__
static inline __m128i rotate24(__m128i x) { return _mm_shuffle_epi8(x, _mm_set_epi8(12, 15, 14, 13, 8, 11, 10, 9, 4, 7, 6, 5, 0, 3, 2, 1)); }
#else
...
static const uint32_t coeffs[24] _hydro_attr_aligned_(16) = {
        0x9e377904, 0, 0, 0, 0x9e377908, 0, 0, 0, 0x9e37790c, 0, 0, 0, 0x9e377910, 0, 0, 0, 0x9e377914, 0, 0, 0, 0x9e377918, 0, 0, 0,
};

static void gimli_core(uint32_t state[gimli_BLOCKBYTES / 4]) {
...
static bool hydro_random_rbit(uint16_t x) {
    uint8_t x8;
...
static int hydro_random_init(void) {
    if (!RtlGenRandom((PVOID)hydro_random_context.state, (ULONG)sizeof hydro_random_context.state)) {
        return -1;
    }
    hydro_random_context.counter = ~LOAD64_LE(hydro_random_context.state);
...
static ssize_t hydro_random_safe_read(const int fd, void* const buf_, size_t len) {
...
static int hydro_random_init(void) {
    ...
    if (mbedtls_entropy_func(&entropy, (uint8_t*)&hydro_random_context.counter, sizeof hydro_random_context.counter) != 0) {
        return -1;
    }
    ...
    do {
        ...
        const uint8_t currentChunkSize = (dataLeftToConsume > MBEDTLS_ENTROPY_BLOCK_SIZE) ? MBEDTLS_ENTROPY_BLOCK_SIZE : dataLeftToConsume;

        // Forces mbedTLS to fetch fresh entropy, then get some to feed libhydrogen.
        if (mbedtls_entropy_gather(&entropy) != 0 || mbedtls_entropy_func(&entropy, &hydro_random_context.state[pos], currentChunkSize) != 0) {
            return -1;
        }
...
int hydro_hash_hash(uint8_t* out, size_t out_len, const void* in_, size_t in_len, const char ctx[hydro_hash_CONTEXTBYTES], const uint8_t key[hydro_hash_KEYBYTES]) {
    ...
    if (hydro_hash_init(&st, ctx, key) != 0 || hydro_hash_update(&st, in, in_len) != 0 || hydro_hash_final(&st, out, out_len) != 0) {
        return -1;
    }
    return 0;
}

void hydro_hash_keygen(uint8_t key[hydro_hash_KEYBYTES]) { hydro_random_buf(key, hydro_hash_KEYBYTES); }
...
void hydro_kdf_keygen(uint8_t key[hydro_kdf_KEYBYTES]) { hydro_random_buf(key, hydro_kdf_KEYBYTES); }

#define hydro_secretbox_IVBYTES 20
#define hydro_secretbox_SIVBYTES 20
#define hydro_secretbox_MACBYTES 16

void hydro_secretbox_keygen(uint8_t key[hydro_secretbox_KEYBYTES]) { hydro_random_buf(key, hydro_secretbox_KEYBYTES); }

static void hydro_secretbox_xor_enc(uint8_t buf[gimli_BLOCKBYTES], uint8_t* out, const uint8_t* in, size_t inlen) {
    size_t i;
    size_t leftover;
...
static void hydro_secretbox_xor_dec(uint8_t buf[gimli_BLOCKBYTES], uint8_t* out, const uint8_t* in, size_t inlen) {
    size_t i;
    size_t leftover;
...
static void hydro_secretbox_setup(uint8_t buf[gimli_BLOCKBYTES], uint64_t msg_id, const char ctx[hydro_secretbox_CONTEXTBYTES], const uint8_t key[hydro_secretbox_KEYBYTES],
...
static void hydro_secretbox_final(uint8_t* buf, const uint8_t key[hydro_secretbox_KEYBYTES], uint8_t tag) {
...
static int hydro_secretbox_encrypt_iv(uint8_t* c, const void* m_, size_t mlen, uint64_t msg_id, const char ctx[hydro_secretbox_CONTEXTBYTES], const uint8_t key[hydro_secretbox_KEYBYTES],
...
void hydro_secretbox_probe_create(uint8_t probe[hydro_secretbox_PROBEBYTES], const uint8_t* c, size_t c_len, const char ctx[hydro_secretbox_CONTEXTBYTES],
...
static const hydro_x25519_limb_t hydro_x25519_MONTGOMERY_FACTOR = (hydro_x25519_limb_t)0xd2b51da312547e1bull;

static const hydro_x25519_scalar_t hydro_x25519_sc_p = {hydro_x25519_LIMB(0x5812631a5cf5d3ed), hydro_x25519_LIMB(0x14def9dea2f79cd6), hydro_x25519_LIMB(0x0000000000000000),
                                                        hydro_x25519_LIMB(0x1000000000000000)};

static const hydro_x25519_scalar_t hydro_x25519_sc_r2 = {hydro_x25519_LIMB(0xa40611e3449c0f01), hydro_x25519_LIMB(0xd00e1ba768859347), hydro_x25519_LIMB(0xceec73d217f5be65),
...
static inline hydro_x25519_limb_t hydro_x25519_umaal(hydro_x25519_limb_t* carry, hydro_x25519_limb_t acc, hydro_x25519_limb_t mand, hydro_x25519_limb_t mier) {
...
static inline hydro_x25519_limb_t hydro_x25519_adc(hydro_x25519_limb_t* carry, hydro_x25519_limb_t acc, hydro_x25519_limb_t mand) {
...
static inline hydro_x25519_limb_t hydro_x25519_adc0(hydro_x25519_limb_t* carry, hydro_x25519_limb_t acc) {
...
static void hydro_x25519_propagate(hydro_x25519_fe x, hydro_x25519_limb_t over) {
...
static void hydro_x25519_add(hydro_x25519_fe out, const hydro_x25519_fe a, const hydro_x25519_fe b) {
...
static void hydro_x25519_sub(hydro_x25519_fe out, const hydro_x25519_fe a, const hydro_x25519_fe b) {
...
static void hydro_x25519_swapin(hydro_x25519_limb_t* x, const uint8_t* in) {
    int i;
...
static void hydro_x25519_swapout(uint8_t* out, hydro_x25519_limb_t* x) {
    int i;
...
static void hydro_x25519_mul(hydro_x25519_fe out, const hydro_x25519_fe a, const hydro_x25519_fe b, int nb) {
...
static void hydro_x25519_sqr(hydro_x25519_fe out, const hydro_x25519_fe a) { hydro_x25519_mul(out, a, a, hydro_x25519_NLIMBS); }

static void hydro_x25519_mul1(hydro_x25519_fe out, const hydro_x25519_fe a) { hydro_x25519_mul(out, a, out, hydro_x25519_NLIMBS); }

static void hydro_x25519_sqr1(hydro_x25519_fe a) { hydro_x25519_mul1(a, a); }

static void hydro_x25519_condswap(hydro_x25519_limb_t a[2 * hydro_x25519_NLIMBS], hydro_x25519_limb_t b[2 * hydro_x25519_NLIMBS], hydro_x25519_limb_t doswap) {
    int i;
...
static int hydro_x25519_canon(hydro_x25519_fe x) {
...
static void hydro_x25519_ladder_part1(hydro_x25519_fe xs[5]) {
...
static void hydro_x25519_ladder_part2(hydro_x25519_fe xs[5], const hydro_x25519_fe x1) {
    hydro_x25519_limb_t *x2 = xs[0], *z2 = xs[1], *x3 = xs[2], *z3 = xs[3], *t1 = xs[4];
...
static void hydro_x25519_core(hydro_x25519_fe xs[5], const uint8_t scalar[hydro_x25519_BYTES], const uint8_t* x1, bool clamp) {
...
static int hydro_x25519_scalarmult(uint8_t out[hydro_x25519_BYTES], const uint8_t scalar[hydro_x25519_SECRETKEYBYTES], const uint8_t x1[hydro_x25519_PUBLICKEYBYTES], bool clamp) {
...
static inline int hydro_x25519_scalarmult_base(uint8_t pk[hydro_x25519_PUBLICKEYBYTES], const uint8_t sk[hydro_x25519_SECRETKEYBYTES]) {
...
static void hydro_x25519_sc_montmul(hydro_x25519_scalar_t out, const hydro_x25519_scalar_t a, const hydro_x25519_scalar_t b) {
    hydro_x25519_limb_t hic = 0;
    int i, j;

    for (i = 0; i < hydro_x25519_NLIMBS; i++) {
        hydro_x25519_limb_t carry = 0, carry2 = 0, mand = a[i], mand2 = hydro_x25519_MONTGOMERY_FACTOR;
...
static void hydro_kx_aead_init(uint8_t aead_state[gimli_BLOCKBYTES], uint8_t k[hydro_kx_AEAD_KEYBYTES], hydro_kx_state* state) {
...
static void hydro_kx_aead_final(uint8_t* aead_state, const uint8_t key[hydro_kx_AEAD_KEYBYTES]) {
...
static void hydro_kx_aead_xor_enc(uint8_t aead_state[gimli_BLOCKBYTES], uint8_t* out, const uint8_t* in, size_t inlen) {
    size_t i;
    size_t leftover;
...
static void hydro_kx_aead_xor_dec(uint8_t aead_state[gimli_BLOCKBYTES], uint8_t* out, const uint8_t* in, size_t inlen) {
    size_t i;
    size_t leftover;
...
static void hydro_kx_aead_encrypt(hydro_kx_state* state, uint8_t* c, const uint8_t* m, size_t mlen) {
...
static int hydro_kx_aead_decrypt(hydro_kx_state* state, uint8_t* m, const uint8_t* c, size_t clen) _hydro_attr_warn_unused_result_;

static int hydro_kx_aead_decrypt(hydro_kx_state* state, uint8_t* m, const uint8_t* c, size_t clen) {
...
static void hydro_kx_init_state(hydro_kx_state* state, const char* name) {
    mem_zero(state, sizeof *state);
...
static void hydro_kx_final(hydro_kx_state* state, uint8_t session_k1[hydro_kx_SESSIONKEYBYTES], uint8_t session_k2[hydro_kx_SESSIONKEYBYTES]) {
...
static int hydro_kx_dh(hydro_kx_state* state, const uint8_t sk[hydro_x25519_SECRETKEYBYTES], const uint8_t pk[hydro_x25519_PUBLICKEYBYTES]) {
...
static void hydro_kx_eph_keygen(hydro_kx_state* state, hydro_kx_keypair* kp) {
    hydro_kx_keygen(kp);
...
int hydro_kx_n_2(hydro_kx_session_keypair* kp, const uint8_t packet1[hydro_kx_N_PACKET1BYTES], const uint8_t psk[hydro_kx_PSKBYTES], const hydro_kx_keypair* static_kp) {
    ...
    if (hydro_kx_dh(&state, static_kp->sk, peer_eph_pk) != 0 || hydro_kx_aead_decrypt(&state, NULL, packet1_mac, hydro_kx_AEAD_MACBYTES) != 0) {
        return -1;
    }
...
int hydro_kx_kk_1(hydro_kx_state* state, uint8_t packet1[hydro_kx_KK_PACKET1BYTES], const uint8_t peer_static_pk[hydro_kx_PUBLICKEYBYTES], const hydro_kx_keypair* static_kp) {
    ...
    hydro_kx_eph_keygen(state, &state->eph_kp);
    if (hydro_kx_dh(state, state->eph_kp.sk, peer_static_pk) != 0 || hydro_kx_dh(state, static_kp->sk, peer_static_pk) != 0) {
        return -1;
    }
    hydro_kx_aead_encrypt(state, packet1_mac, NULL, 0);
    memcpy(packet1_eph_pk, state->eph_kp.pk, sizeof state->eph_kp.pk);
...
int hydro_kx_kk_2(hydro_kx_session_keypair* kp, uint8_t packet2[hydro_kx_KK_PACKET2BYTES], const uint8_t packet1[hydro_kx_KK_PACKET1BYTES], const uint8_t peer_static_pk[hydro_kx_PUBLICKEYBYTES],
    ...
    if (hydro_kx_dh(&state, static_kp->sk, peer_eph_pk) != 0 || hydro_kx_dh(&state, static_kp->sk, peer_static_pk) != 0 ||
        hydro_kx_aead_decrypt(&state, NULL, packet1_mac, hydro_kx_AEAD_MACBYTES) != 0) {
        return -1;
    }

    hydro_kx_eph_keygen(&state, &state.eph_kp);
    if (hydro_kx_dh(&state, state.eph_kp.sk, peer_eph_pk) != 0 || hydro_kx_dh(&state, state.eph_kp.sk, peer_static_pk) != 0) {
        return -1;
    }
...
int hydro_kx_kk_3(hydro_kx_state* state, hydro_kx_session_keypair* kp, const uint8_t packet2[hydro_kx_KK_PACKET2BYTES], const hydro_kx_keypair* static_kp) {
    ...
    hydro_hash_update(&state->h_st, peer_eph_pk, hydro_kx_PUBLICKEYBYTES);
    if (hydro_kx_dh(state, state->eph_kp.sk, peer_eph_pk) != 0 || hydro_kx_dh(state, static_kp->sk, peer_eph_pk) != 0) {
        return -1;
    }
...
int hydro_kx_xx_2(hydro_kx_state* state, uint8_t packet2[hydro_kx_XX_PACKET2BYTES], const uint8_t packet1[hydro_kx_XX_PACKET1BYTES], const uint8_t psk[hydro_kx_PSKBYTES],
    ...
    uint8_t* packet2_mac = &packet2[hydro_kx_PUBLICKEYBYTES + hydro_kx_PUBLICKEYBYTES + hydro_kx_AEAD_MACBYTES];
...
int hydro_kx_xx_3(hydro_kx_state* state, hydro_kx_session_keypair* kp, uint8_t packet3[hydro_kx_XX_PACKET3BYTES], uint8_t peer_static_pk[hydro_kx_PUBLICKEYBYTES],
    ...
    const uint8_t* packet2_mac = &packet2[hydro_kx_PUBLICKEYBYTES + hydro_kx_PUBLICKEYBYTES + hydro_kx_AEAD_MACBYTES];
    ...
    hydro_hash_update(&state->h_st, peer_eph_pk, hydro_kx_PUBLICKEYBYTES);
    if (hydro_kx_dh(state, state->eph_kp.sk, peer_eph_pk) != 0 || hydro_kx_aead_decrypt(state, peer_static_pk, peer_enc_static_pk, hydro_kx_PUBLICKEYBYTES + hydro_kx_AEAD_MACBYTES) != 0 ||
        hydro_kx_dh(state, state->eph_kp.sk, peer_static_pk) != 0 || hydro_kx_aead_decrypt(state, NULL, packet2_mac, hydro_kx_AEAD_MACBYTES) != 0) {
        return -1;
    }
...
int hydro_kx_xx_4(hydro_kx_state* state, hydro_kx_session_keypair* kp, uint8_t peer_static_pk[hydro_kx_PUBLICKEYBYTES], const uint8_t packet3[hydro_kx_XX_PACKET3BYTES],
    ...
    if (hydro_kx_aead_decrypt(state, peer_static_pk, peer_enc_static_pk, hydro_kx_PUBLICKEYBYTES + hydro_kx_AEAD_MACBYTES) != 0 || hydro_kx_dh(state, state->eph_kp.sk, peer_static_pk) != 0) {
        return -1;
    }
    hydro_hash_update(&state->h_st, psk, hydro_kx_PSKBYTES);
...
int hydro_kx_nk_3(hydro_kx_state* state, hydro_kx_session_keypair* kp, const uint8_t packet2[hydro_kx_NK_PACKET2BYTES]) {
    ...
    hydro_hash_update(&state->h_st, peer_eph_pk, hydro_x25519_PUBLICKEYBYTES);
    if (hydro_kx_dh(state, state->eph_kp.sk, peer_eph_pk) != 0 || hydro_kx_aead_decrypt(state, NULL, packet2_mac, hydro_kx_AEAD_MACBYTES) != 0) {
        return -1;
    }
    hydro_kx_final(state, kp->rx, kp->tx);
...
#define hydro_pwhash_PARAMSBYTES (hydro_pwhash_HASH_ALGBYTES + hydro_pwhash_THREADSBYTES + hydro_pwhash_OPSLIMITBYTES + hydro_pwhash_MEMLIMITBYTES + hydro_pwhash_SALTBYTES + hydro_pwhash_HASHBYTES)
#define hydro_pwhash_ENC_ALG 0x01
#define hydro_pwhash_HASH_ALG 0x01
#define hydro_pwhash_CONTEXT "hydro_pw"

static int _hydro_pwhash_hash(uint8_t out[hydro_random_SEEDBYTES], size_t h_len, const uint8_t salt[hydro_pwhash_SALTBYTES], const char* passwd, size_t passwd_len,
                              const char ctx[hydro_pwhash_CONTEXTBYTES], const uint8_t master_key[hydro_pwhash_MASTERKEYBYTES], uint64_t opslimit, size_t memlimit, uint8_t threads) {
    _hydro_attr_aligned_(16) uint8_t state[gimli_BLOCKBYTES];
    hydro_hash_state h_st;
    uint8_t tmp64_u8[8];
    uint64_t i;
...
void hydro_pwhash_keygen(uint8_t master_key[hydro_pwhash_MASTERKEYBYTES]) { hydro_random_buf(master_key, hydro_pwhash_MASTERKEYBYTES); }
...
int hydro_pwhash_deterministic(uint8_t* h, size_t h_len, const char* passwd, size_t passwd_len, const char ctx[hydro_pwhash_CONTEXTBYTES], const uint8_t master_key[hydro_pwhash_MASTERKEYBYTES],
    ...
    (void)memlimit;
    if (_hydro_pwhash_hash(seed, h_len, zero, passwd, passwd_len, ctx, master_key, opslimit, memlimit, threads) != 0) {
        return -1;
    }
...
int hydro_pwhash_create(uint8_t stored[hydro_pwhash_STOREDBYTES], const char* passwd, size_t passwd_len, const uint8_t master_key[hydro_pwhash_MASTERKEYBYTES], uint64_t opslimit, size_t memlimit,
    ...
    COMPILER_ASSERT(hydro_pwhash_STOREDBYTES >= hydro_pwhash_ENC_ALGBYTES + hydro_secretbox_HEADERBYTES + hydro_pwhash_PARAMSBYTES);
    (void)memlimit;
    ...
    *threads_u8 = threads;
    ...
    COMPILER_ASSERT(sizeof zero >= hydro_pwhash_MASTERKEYBYTES);
    if (_hydro_pwhash_hash(h, hydro_pwhash_HASHBYTES, salt, passwd, passwd_len, hydro_pwhash_CONTEXT, zero, opslimit, memlimit, threads) != 0) {
        return -1;
    }
    COMPILER_ASSERT(hydro_pwhash_MASTERKEYBYTES == hydro_secretbox_KEYBYTES);

    return hydro_secretbox_encrypt(secretbox, hash_alg, hydro_pwhash_PARAMSBYTES, (uint64_t)*enc_alg, hydro_pwhash_CONTEXT, master_key);
}

static int _hydro_pwhash_verify(uint8_t computed_h[hydro_pwhash_HASHBYTES], const uint8_t stored[hydro_pwhash_STOREDBYTES], const char* passwd, size_t passwd_len,
                                const uint8_t master_key[hydro_pwhash_MASTERKEYBYTES], uint64_t opslimit_max, size_t memlimit_max, uint8_t threads_max) {
    ...
    if (hydro_secretbox_decrypt(params, secretbox, hydro_secretbox_HEADERBYTES + hydro_pwhash_PARAMSBYTES, (uint64_t)*enc_alg, hydro_pwhash_CONTEXT, master_key) != 0) {
        return -1;
    }
    if (*hash_alg != hydro_pwhash_HASH_ALG || (opslimit = LOAD64_LE(opslimit_u8)) > opslimit_max || (memlimit = (size_t)LOAD64_LE(memlimit_u8)) > memlimit_max ||
        (threads = *threads_u8) > threads_max) {
        return -1;
    }
    if (_hydro_pwhash_hash(computed_h, hydro_pwhash_HASHBYTES, salt, passwd, passwd_len, hydro_pwhash_CONTEXT, zero, opslimit, memlimit, threads) == 0 &&
...
int hydro_pwhash_verify(const uint8_t stored[hydro_pwhash_STOREDBYTES], const char* passwd, size_t passwd_len, const uint8_t master_key[hydro_pwhash_MASTERKEYBYTES], uint64_t opslimit_max,
    ...
    ret = _hydro_pwhash_verify(computed_h, stored, passwd, passwd_len, master_key, opslimit_max, memlimit_max, threads_max);
...
int hydro_pwhash_derive_static_key(uint8_t* static_key, size_t static_key_len, const uint8_t stored[hydro_pwhash_STOREDBYTES], const char* passwd, size_t passwd_len,
    ...
    if (_hydro_pwhash_verify(computed_h, stored, passwd, passwd_len, master_key, opslimit_max, memlimit_max, threads_max) != 0) {
        hydro_memzero(computed_h, sizeof computed_h);
        return -1;
    }
...
int hydro_pwhash_reencrypt(uint8_t stored[hydro_pwhash_STOREDBYTES], const uint8_t master_key[hydro_pwhash_MASTERKEYBYTES], const uint8_t new_master_key[hydro_pwhash_MASTERKEYBYTES]) {
    ...
    if (hydro_secretbox_decrypt(secretbox, secretbox, hydro_secretbox_HEADERBYTES + hydro_pwhash_PARAMSBYTES, (uint64_t)*enc_alg, hydro_pwhash_CONTEXT, master_key) != 0) {
        return -1;
    }
    memmove(params, secretbox, hydro_pwhash_PARAMSBYTES);
    return hydro_secretbox_encrypt(secretbox, params, hydro_pwhash_PARAMSBYTES, (uint64_t)*enc_alg, hydro_pwhash_CONTEXT, new_master_key);
}
...
int hydro_pwhash_upgrade(uint8_t stored[hydro_pwhash_STOREDBYTES], const uint8_t master_key[hydro_pwhash_MASTERKEYBYTES], uint64_t opslimit, size_t memlimit, uint8_t threads) {
    ...
    *threads_u8 = threads;
    ...
    return hydro_secretbox_encrypt(secretbox, params, hydro_pwhash_PARAMSBYTES, (uint64_t)*enc_alg, hydro_pwhash_CONTEXT, master_key);
}
...
static void hydro_sign_p2(uint8_t sig[hydro_x25519_BYTES], const uint8_t challenge[hydro_sign_CHALLENGEBYTES], const uint8_t eph_sk[hydro_x25519_BYTES], const uint8_t sk[hydro_x25519_BYTES]) {
...
static void hydro_sign_challenge(uint8_t challenge[hydro_sign_CHALLENGEBYTES], const uint8_t nonce[hydro_sign_NONCEBYTES], const uint8_t pk[hydro_sign_PUBLICKEYBYTES],
...
static int hydro_sign_prehash(uint8_t csig[hydro_sign_BYTES], const uint8_t prehash[hydro_sign_PREHASHBYTES], const uint8_t sk[hydro_sign_SECRETKEYBYTES]) {
    hydro_hash_state st;
...
static int hydro_sign_verify_core(hydro_x25519_fe xs[5], const hydro_x25519_limb_t* other1, const uint8_t other2[hydro_x25519_BYTES]) {
...
static int hydro_sign_verify_p2(const uint8_t sig[hydro_x25519_BYTES], const uint8_t challenge[hydro_sign_CHALLENGEBYTES], const uint8_t nonce[hydro_sign_NONCEBYTES],
...
static int hydro_sign_verify_challenge(const uint8_t csig[hydro_sign_BYTES], const uint8_t challenge[hydro_sign_CHALLENGEBYTES], const uint8_t pk[hydro_sign_PUBLICKEYBYTES]) {
...
void hydro_sign_keygen(hydro_sign_keypair* kp) {
    ...
    COMPILER_ASSERT(hydro_sign_SECRETKEYBYTES == hydro_x25519_SECRETKEYBYTES + hydro_x25519_PUBLICKEYBYTES);
...
int hydro_sign_init(hydro_sign_state* state, const char ctx[hydro_sign_CONTEXTBYTES]) { return hydro_hash_init(&state->hash_st, ctx, NULL); }

int hydro_sign_update(hydro_sign_state* state, const void* m_, size_t mlen) { return hydro_hash_update(&state->hash_st, m_, mlen); }
...
int hydro_sign_create(uint8_t csig[hydro_sign_BYTES], const void* m_, size_t mlen, const char ctx[hydro_sign_CONTEXTBYTES], const uint8_t sk[hydro_sign_SECRETKEYBYTES]) {
    ...
    if (hydro_sign_init(&st, ctx) != 0 || hydro_sign_update(&st, m_, mlen) != 0 || hydro_sign_final_create(&st, csig, sk) != 0) {
        return -1;
    }
    return 0;
}
...
int hydro_sign_verify(const uint8_t csig[hydro_sign_BYTES], const void* m_, size_t mlen, const char ctx[hydro_sign_CONTEXTBYTES], const uint8_t pk[hydro_sign_PUBLICKEYBYTES]) {
    ...
    if (hydro_sign_init(&st, ctx) != 0 || hydro_sign_update(&st, m_, mlen) != 0 || hydro_sign_final_verify(&st, csig, pk) != 0) {
        return -1;
    }
    return 0;
}
...
void cn_write_key(uint8_t** p, const cn_crypto_key_t* key) { cn_write_bytes(p, (const uint8_t*)key, sizeof(*key)); }
...
CN_INLINE void cn_list_init(cn_list_t* list) { cn_list_init_node(&list->nodes); }
...
CN_INLINE int cn_list_empty(cn_list_t* list) { return list->nodes.next == list->nodes.prev && list->nodes.next == &list->nodes; }

CN_INLINE cn_list_node_t* cn_list_begin(cn_list_t* list) { return list->nodes.next; }

CN_INLINE cn_list_node_t* cn_list_end(cn_list_t* list) { return &list->nodes; }

CN_INLINE cn_list_node_t* cn_list_front(cn_list_t* list) { return list->nodes.next; }

CN_INLINE cn_list_node_t* cn_list_back(cn_list_t* list) { return list->nodes.prev; }
...
void cn_endpoint_to_string(cn_endpoint_t endpoint, char* buffer, int buffer_size) {
    ...
    if (endpoint.type == CN_ADDRESS_TYPE_IPV6) {
        if (endpoint.port == 0) {
            ...
            if (endpoint.type == CN_ADDRESS_TYPE_IPV4) {
        if (endpoint.port != 0) {
            CN_SNPRINTF(buffer, CN_ENDPOINT_STRING_MAX_LENGTH, "%d.%d.%d.%d:%d", endpoint.u.ipv4[0], endpoint.u.ipv4[1], endpoint.u.ipv4[2], endpoint.u.ipv4[3], endpoint.port);
        } else {
            CN_SNPRINTF(buffer, CN_ENDPOINT_STRING_MAX_LENGTH, "%d.%d.%d.%d", endpoint.u.ipv4[0], endpoint.u.ipv4[1], endpoint.u.ipv4[2], endpoint.u.ipv4[3]);
        }
    } else {
...
int cn_endpoint_equals(cn_endpoint_t a, cn_endpoint_t b) {
    ...
    if (a.type == CN_ADDRESS_TYPE_IPV4) {
        for (int i = 0; i < 4; ++i)
            if (a.u.ipv4[i] != b.u.ipv4[i]) return 0;
    }
#ifndef CUTE_NET_NO_IPV6
    else if (a.type == CN_ADDRESS_TYPE_IPV6) {
        for (int i = 0; i < 8; ++i)
            if (a.u.ipv6[i] != b.u.ipv6[i]) return 0;
    }
#endif
    else {
        return 0;
...
int cn_socket_init2(cn_socket_t* the_socket, const char* address_and_port, int send_buffer_size, int receive_buffer_size) {
...
#endif
    {
        ...
        CN_MEMSET(&socket_endpoint, 0, sizeof(socket_endpoint));
        socket_endpoint.sin_family = AF_INET;
        socket_endpoint.sin_addr.s_addr = (((uint32_t)endpoint.u.ipv4[0])) | (((uint32_t)endpoint.u.ipv4[1]) << 8) | (((uint32_t)endpoint.u.ipv4[2]) << 16) | (((uint32_t)endpoint.u.ipv4[3]) << 24);
...
int cn_socket_send_internal(cn_socket_t* socket, cn_endpoint_t send_to, const void* data, int byte_count) {
    ...
    if (endpoint.type == CN_ADDRESS_TYPE_IPV6) {
        ...
        socket_address.sin_addr.s_addr = (((uint32_t)endpoint.u.ipv4[0])) | (((uint32_t)endpoint.u.ipv4[1]) << 8) | (((uint32_t)endpoint.u.ipv4[2]) << 16) | (((uint32_t)endpoint.u.ipv4[3]) << 24);
...
void cn_crypto_encrypt(const cn_crypto_key_t* key, uint8_t* data, int data_size, uint64_t msg_id) { hydro_secretbox_encrypt(data, data, (uint64_t)data_size, msg_id, CN_CRYPTO_CONTEXT, key->key); }
...
cn_result_t cn_generate_connect_token(uint64_t application_id, uint64_t creation_timestamp, const cn_crypto_key_t* client_to_server_key, const cn_crypto_key_t* server_to_client_key,
                                      ...
                                      const cn_crypto_sign_secret_t* shared_secret_key, uint8_t* token_ptr_out) {
    ...
    for (int i = 0; i < zeroes; ++i) cn_write_uint8(p, 0);
...
cn_result_t cn_protocol_read_connect_token_packet_public_section(uint8_t* buffer, uint64_t application_id, uint64_t current_time, cn_protocol_packet_connect_token_t* packet) {
    ...
    for (int i = 0; i < count; ++i) packet->endpoints[i] = cn_read_endpoint(&buffer);
...
void* cn_protocol_packet_open(uint8_t* buffer, int size, const cn_crypto_key_t* key, cn_protocol_packet_allocator_t* pa, cn_protocol_replay_buffer_t* replay_buffer, uint64_t* sequence_ptr) {
    ...
    switch (type) {
        ...
        case CN_PROTOCOL_PACKET_TYPE_CHALLENGE_RESPONSE:
            ...
            break;
        case CN_PROTOCOL_PACKET_TYPE_PAYLOAD:
            CN_CHECK((size - 73 < 1) | (size - 73 > 1255));
            if (ret) return NULL;
            break;
    }
...
cn_result_t cn_protocol_server_decrypt_connect_token_packet(uint8_t* packet_buffer, const cn_crypto_sign_public_t* pk, const cn_crypto_sign_secret_t* sk, uint64_t application_id,
                                                            uint64_t current_time, cn_protocol_connect_token_decrypted_t* token) {
    // Read public section.
    cn_protocol_packet_connect_token_t packet;
...
static int s_keys_equal(const cn_hashtable_t* table, const void* a, const void* b) { return !CN_MEMCMP(a, b, table->key_size); }
    ...
    while (base_count > 0) {
        ...
        if (slot_hash) {
            ...
            if (slot_base == base_slot) {
                ...
                if (slot_hash == hash && s_keys_equal(table, found_key, key)) return slot;
            }
        }
...
void* cn_hashtable_insert(cn_hashtable_t* table, const void* key, const void* item) {
    ...
    slot = first_free;
    while (table->slots[slot].key_hash) slot = (slot + 1) % table->slot_capacity;
...
int cn_hashtable_count(const cn_hashtable_t* table) { return table->count; }

void* cn_hashtable_items(const cn_hashtable_t* table) { return table->items_data; }

void* cn_hashtable_keys(const cn_hashtable_t* table) { return table->items_key; }
...
void cn_protocol_encryption_map_cleanup(cn_protocol_encryption_map_t* map) { cn_hashtable_cleanup(&map->table); }

void cn_protocol_encryption_map_clear(cn_protocol_encryption_map_t* map) { cn_hashtable_clear(&map->table); }

int cn_protocol_encryption_map_count(cn_protocol_encryption_map_t* map) { return cn_hashtable_count(&map->table); }

void cn_protocol_encryption_map_insert(cn_protocol_encryption_map_t* map, cn_endpoint_t endpoint, const cn_protocol_encryption_state_t* state) { cn_hashtable_insert(&map->table, &endpoint, state); }
...
void cn_protocol_encryption_map_remove(cn_protocol_encryption_map_t* map, cn_endpoint_t endpoint) { cn_hashtable_remove(&map->table, &endpoint); }

cn_endpoint_t* cn_protocol_encryption_map_get_endpoints(cn_protocol_encryption_map_t* map) { return (cn_endpoint_t*)cn_hashtable_keys(&map->table); }

cn_protocol_encryption_state_t* cn_protocol_encryption_map_get_states(cn_protocol_encryption_map_t* map) { return (cn_protocol_encryption_state_t*)cn_hashtable_items(&map->table); }
...
static CN_INLINE const char* s_protocol_client_state_str(cn_protocol_client_state_t state) {
    switch (state) {
        case CN_PROTOCOL_CLIENT_STATE_CONNECT_TOKEN_EXPIRED:
            return "CONNECT_TOKEN_EXPIRED";
        case CN_PROTOCOL_CLIENT_STATE_INVALID_CONNECT_TOKEN:
            return "INVALID_CONNECT_TOKEN";
        case CN_PROTOCOL_CLIENT_STATE_CONNECTION_TIMED_OUT:
            return "CONNECTION_TIMED_OUT";
        case CN_PROTOCOL_CLIENT_STATE_CHALLENGED_RESPONSE_TIMED_OUT:
            return "CHALLENGED_RESPONSE_TIMED_OUT";
        case CN_PROTOCOL_CLIENT_STATE_CONNECTION_REQUEST_TIMED_OUT:
            return "CONNECTION_REQUEST_TIMED_OUT";
        case CN_PROTOCOL_CLIENT_STATE_CONNECTION_DENIED:
            return "CONNECTION_DENIED";
        case CN_PROTOCOL_CLIENT_STATE_DISCONNECTED:
            return "DISCONNECTED";
...
cn_result_t cn_protocol_client_connect(cn_protocol_client_t* client, const uint8_t* connect_token) {
    uint8_t* connect_token_packet = cn_protocol_client_read_connect_token_from_web_service((uint8_t*)connect_token, client->application_id, client->current_time, &client->connect_token);
...
static CN_INLINE cn_endpoint_t s_protocol_server_endpoint(cn_protocol_client_t* client) { return client->connect_token.endpoints[client->server_endpoint_index]; }
...
cn_protocol_client_state_t cn_protocol_client_get_state(cn_protocol_client_t* client) { return client->state; }

uint64_t cn_protocol_client_get_id(cn_protocol_client_t* client) { return client->client_id; }

uint32_t cn_protocol_client_get_max_clients(cn_protocol_client_t* client) { return client->max_clients; }

cn_endpoint_t cn_protocol_client_get_server_address(cn_protocol_client_t* client) { return s_protocol_server_endpoint(client); }

uint16_t cn_protocol_client_get_port(cn_protocol_client_t* client) { return client->socket.endpoint.port; }
...
static CN_INLINE uint32_t s_table_index(cn_handle_t handle) { return (uint32_t)((handle & 0xFFFFFFFF00000000ULL) >> 32); }
...
bool cn_protocol_server_pop_event(cn_protocol_server_t* server, cn_protocol_server_event_t* event) { return s_protocol_server_event_pull(server, event) ? false : true; }
...
bool cn_protocol_server_running(cn_protocol_server_t* server) { return server->running; }
...
int cn_protocol_server_client_count(cn_protocol_server_t* server) { return server->client_count; }
...
bool cn_protocol_server_is_client_connected(cn_protocol_server_t* server, int client_index) { return server->client_is_connected[client_index]; }
...
static CN_INLINE int s_sequence_greater_than(uint16_t a, uint16_t b) { return ((a > b) && (a - b <= 32768)) | ((a < b) && (b - a > 32768)); }

static CN_INLINE int s_sequence_less_than(uint16_t a, uint16_t b) { return s_sequence_greater_than(b, a); }

static CN_INLINE int s_sequence_is_stale(cn_sequence_buffer_t* buffer, uint16_t sequence) { return s_sequence_less_than(sequence, buffer->sequence - ((uint16_t)buffer->capacity)); }
...
int cn_sequence_buffer_is_empty(cn_sequence_buffer_t* sequence_buffer, uint16_t sequence) { return sequence_buffer->entry_sequence[sequence % sequence_buffer->capacity] == CN_SEQUENCE_BUFFER_EMPTY; }
...
static void s_packet_assembly_cleanup(cn_packet_assembly_t* assembly) { cn_sequence_buffer_cleanup(&assembly->fragments_received, s_fragment_reassembly_entry_cleanup); }
...
uint16_t cn_ack_system_get_sequence(cn_ack_system_t* ack_system) { return ack_system->sequence; }
    ...
    for (int i = 0; i < 32; ++i) {
        ...
        if (bit_was_set) {
            ...
            if (sent_packet && !sent_packet->acked) {
                ...
                if (ack_system->rtt == 0.0f && rtt > 0.0f)
                    ack_system->rtt = rtt;
                else
...
uint16_t* cn_ack_system_get_acks(cn_ack_system_t* ack_system) { return ack_system->acks; }

int cn_ack_system_get_acks_count(cn_ack_system_t* ack_system) { return ack_system->acks_count; }

void cn_ack_system_clear_acks(cn_ack_system_t* ack_system) { ack_system->acks_count = 0; }
...
double cn_ack_system_rtt(cn_ack_system_t* ack_system) { return ack_system->rtt; }

double cn_ack_system_packet_loss(cn_ack_system_t* ack_system) { return ack_system->packet_loss; }

double cn_ack_system_bandwidth_outgoing_kbps(cn_ack_system_t* ack_system) { return ack_system->outgoing_bandwidth_kbps; }

double cn_ack_system_bandwidth_incoming_kbps(cn_ack_system_t* ack_system) { return ack_system->incoming_bandwidth_kbps; }

uint64_t cn_ack_system_get_counter(cn_ack_system_t* ack_system, cn_ack_system_counter_t counter) { return ack_system->counters[counter]; }
...
static cn_result_t s_transport_send_fragments(cn_transport_t* transport) {
    ...
    while (fragments_space_available_send) {
        ...
        for (int i = 0; i < fragment_count_to_send; ++i) {
            ...
            int header_size = s_transport_write_header(fragment->data, this_fragment_size + CN_TRANSPORT_HEADER_SIZE, 1, item->fragment_sequence, item->fragment_count, fragment_header_index,
                                                       (uint16_t)this_fragment_size);
...
cn_result_t s_transport_send_reliably(cn_transport_t* transport, const void* data, int size) {
    ...
    if (final_fragment_size > 0)
        fragment_count++;
    else
        final_fragment_size = fragment_size;
...
void cn_transport_free_packet(cn_transport_t* transport, void* data) { CN_FREE(data, transport->mem_ctx); }
...
int cn_transport_unacked_fragment_count(cn_transport_t* transport) { return transport->fragments_count; }
...
cn_result_t cn_client_connect(cn_client_t* client, const uint8_t* connect_token) { return cn_protocol_client_connect(client->p_client, connect_token); }

void cn_client_disconnect(cn_client_t* client) { cn_protocol_client_disconnect(client->p_client); }
...
void cn_client_free_packet(cn_client_t* client, void* packet) { cn_transport_free_packet(client->transport, packet); }
...
cn_client_state_t cn_client_state_get(const cn_client_t* client) { return (cn_client_state_t)cn_protocol_client_get_state(client->p_client); }

const char* cn_client_state_string(cn_client_state_t state) {
    switch (state) {
        case CN_CLIENT_STATE_CONNECT_TOKEN_EXPIRED:
            return "CN_CLIENT_STATE_CONNECT_TOKEN_EXPIRED";
        case CN_CLIENT_STATE_INVALID_CONNECT_TOKEN:
            return "CN_CLIENT_STATE_INVALID_CONNECT_TOKEN";
        case CN_CLIENT_STATE_CONNECTION_TIMED_OUT:
            return "CN_CLIENT_STATE_CONNECTION_TIMED_OUT";
        case CN_CLIENT_STATE_CHALLENGE_RESPONSE_TIMED_OUT:
            return "CN_CLIENT_STATE_CHALLENGE_RESPONSE_TIMED_OUT";
        case CN_CLIENT_STATE_CONNECTION_REQUEST_TIMED_OUT:
            return "CN_CLIENT_STATE_CONNECTION_REQUEST_TIMED_OUT";
        case CN_CLIENT_STATE_CONNECTION_DENIED:
            return "CN_CLIENT_STATE_CONNECTION_DENIED";
        case CN_CLIENT_STATE_DISCONNECTED:
            return "CN_CLIENT_STATE_DISCONNECTED";
        case CN_CLIENT_STATE_SENDING_CONNECTION_REQUEST:
            return "CN_CLIENT_STATE_SENDING_CONNECTION_REQUEST";
        case CN_CLIENT_STATE_SENDING_CHALLENGE_RESPONSE:
            return "CN_CLIENT_STATE_SENDING_CHALLENGE_RESPONSE";
        case CN_CLIENT_STATE_CONNECTED:
...
float cn_client_get_packet_loss_estimate(cn_client_t* client) { return (float)client->transport->ack_system->packet_loss; }

float cn_client_get_rtt_estimate(cn_client_t* client) { return (float)client->transport->ack_system->rtt; }

float cn_client_get_incoming_kbps_estimate(cn_client_t* client) { return (float)client->transport->ack_system->incoming_bandwidth_kbps; }

float cn_client_get_outgoing_kbps_estimate(cn_client_t* client) { return (float)client->transport->ack_system->outgoing_bandwidth_kbps; }
...
static CN_INLINE int s_server_event_pull(cn_server_t* server, cn_server_event_t* event) { return cn_circular_buffer_pull(&server->event_queue, event, sizeof(cn_server_event_t)); }
...
bool cn_server_pop_event(cn_server_t* server, cn_server_event_t* event) { return s_server_event_pull(server, event) ? false : true; }
...
bool cn_server_is_client_connected(cn_server_t* server, int client_index) { return cn_protocol_server_is_client_connected(server->p_server, client_index); }
...
#define CN_TEST_PRINT_FILE_LINE(s)                                                                                            \
    do {                                                                                                                      \
        CN_FPRINTF(CN_TEST_IO_STREAM, "Extra info:   %s\n\tLine number:  %d\n\tFile:         %s\n\t", s, __LINE__, __FILE__); \
    } while (0)
#define CN_TEST_ASSERT(x)                                    \
    do {                                                     \
        if (!(x)) {                                          \
            CN_TEST_PRINT_FILE_LINE("Assertion was false."); \
            return -1;                                       \
        }                                                    \
    } while (0)
#define CN_TEST_CHECK(x)                                          \
    do {                                                          \
        if (x) {                                                  \
            CN_TEST_PRINT_FILE_LINE("Return code failed check."); \
            return -1;                                            \
        }                                                         \
    } while (0)
#define CN_TEST_CHECK_POINTER(x)                              \
    do {                                                      \
        if (!(x)) {                                           \
            CN_TEST_PRINT_FILE_LINE("Pointer failed check."); \
            return -1;                                        \
        }                                                     \
    } while (0)

#define CN_TEST_CASE(function, description) \
    int function();                         \
    cn_test_t cn_test_##function = {#function, description, function}
...
int cn_test_generate_connect_token() {
    ...
    const char* endpoints[] = {"[::1]:5000", "[::1]:5001", "[::1]:5002"};
    ...
    CN_TEST_CHECK(cn_is_error(cn_generate_connect_token(application_id, current_timestamp, &client_to_server_key, &server_to_client_key, expiration_timestamp, handshake_timeout, 3, endpoints,
                                                        client_id, user_data, &sk, token_buffer)));
...
int cn_client_server() {
    ...
    CN_TEST_CHECK(cn_is_error(cn_generate_connect_token(application_id, current_timestamp, &client_to_server_key, &server_to_client_key, expiration_timestamp, handshake_timeout,
                                                        sizeof(endpoints) / sizeof(endpoints[0]), endpoints, client_id, user_data, &sk, connect_token)));
...
int cn_protocol_connect_a_few_clients() {
    ...
    while (iters++ < 100) {
        ...
        if (cn_protocol_client_get_state(client0) == CN_PROTOCOL_CLIENT_STATE_CONNECTED && cn_protocol_client_get_state(client1) == CN_PROTOCOL_CLIENT_STATE_CONNECTED &&
            cn_protocol_client_get_state(client2) == CN_PROTOCOL_CLIENT_STATE_CONNECTED)
            break;
    }
    CN_TEST_ASSERT(cn_protocol_server_running(server));
    CN_TEST_ASSERT(iters < 100);
...
int cn_protocol_multiple_connections_and_payloads() {
    ...
    int client_count = 2;
    while (iters++ < 100) {
        for (int i = 0; i < client_count; ++i) cn_protocol_client_update(clients[i], dt, 0);

        cn_protocol_server_update(server, dt, 0);

        for (int i = 0; i < client_count; ++i)
            if (cn_protocol_client_get_state(clients[i]) <= 0) break;
