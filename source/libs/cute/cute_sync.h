...
struct cute_semaphore_t {
    void* id;
    cute_atomic_int_t count;
};
...
int cute_atomic_add(cute_atomic_int_t* atomic, int addend) { return SDL_AtomicAdd((SDL_atomic_t*)atomic, addend); }

int cute_atomic_set(cute_atomic_int_t* atomic, int value) { return SDL_AtomicSet((SDL_atomic_t*)atomic, value); }

int cute_atomic_get(cute_atomic_int_t* atomic) { return SDL_AtomicGet((SDL_atomic_t*)atomic); }

int cute_atomic_cas(cute_atomic_int_t* atomic, int expected, int value) { return SDL_AtomicCAS((SDL_atomic_t*)atomic, expected, value); }

void* cute_atomic_ptr_set(void** atomic, void* value) { return SDL_AtomicSetPtr(atomic, value); }

void* cute_atomic_ptr_get(void** atomic) { return SDL_AtomicGetPtr(atomic); }

int cute_atomic_ptr_cas(void** atomic, void* expected, void* value) { return SDL_AtomicCASPtr(atomic, expected, value); }

#elif defined(CUTE_SYNC_WINDOWS)

int cute_atomic_add(cute_atomic_int_t* atomic, int addend) { return (int)_InterlockedExchangeAdd(&atomic->i, (LONG)addend); }

int cute_atomic_set(cute_atomic_int_t* atomic, int value) { return (int)_InterlockedExchange(&atomic->i, value); }

int cute_atomic_get(cute_atomic_int_t* atomic) { return (int)_InterlockedCompareExchange(&atomic->i, 0, 0); }

int cute_atomic_cas(cute_atomic_int_t* atomic, int expected, int value) { return (int)_InterlockedCompareExchange(&atomic->i, value, expected) == value; }

void* cute_atomic_ptr_set(void** atomic, void* value) { return _InterlockedExchangePointer(atomic, value); }

void* cute_atomic_ptr_get(void** atomic) { return _InterlockedCompareExchangePointer(atomic, NULL, NULL); }

int cute_atomic_ptr_cas(void** atomic, void* expected, void* value) { return _InterlockedCompareExchangePointer(atomic, value, expected) == value; }
...
int cute_atomic_add(cute_atomic_int_t* atomic, int addend) { return (int)__sync_fetch_and_add(&atomic->i, addend); }
...
int cute_atomic_get(cute_atomic_int_t* atomic) { return (int)__sync_fetch_and_add(&atomic->i, 0); }

int cute_atomic_cas(cute_atomic_int_t* atomic, int expected, int value) { return (int)__sync_val_compare_and_swap(&atomic->i, expected, value) == value; }
...
void* cute_atomic_ptr_get(void** atomic) { return __sync_fetch_and_add(atomic, NULL); }

int cute_atomic_ptr_cas(void** atomic, void* expected, void* value) { return __sync_val_compare_and_swap(atomic, expected, value) == value; }
...
int cute_lock(cute_mutex_t* mutex) { return !SDL_LockMutex((SDL_mutex*)mutex->align); }

int cute_unlock(cute_mutex_t* mutex) { return !SDL_UnlockMutex((SDL_mutex*)mutex->align); }

int cute_trylock(cute_mutex_t* mutex) { return !SDL_TryLockMutex((SDL_mutex*)mutex->align); }

void cute_mutex_destroy(cute_mutex_t* mutex) { SDL_DestroyMutex((SDL_mutex*)mutex->align); }
...
int cute_cv_wake_all(cute_cv_t* cv) { return !SDL_CondBroadcast((SDL_cond*)cv->align); }

int cute_cv_wake_one(cute_cv_t* cv) { return !SDL_CondSignal((SDL_cond*)cv->align); }

int cute_cv_wait(cute_cv_t* cv, cute_mutex_t* mutex) { return !SDL_CondWait((SDL_cond*)cv, (SDL_mutex*)mutex->align); }

void cute_cv_destroy(cute_cv_t* cv) { SDL_DestroyCond((SDL_cond*)cv->align); }
...
int cute_semaphore_post(cute_semaphore_t* semaphore) { return !SDL_SemPost((SDL_sem*)semaphore->id); }

int cute_semaphore_try(cute_semaphore_t* semaphore) { return !SDL_SemTryWait((SDL_sem*)semaphore->id); }

int cute_semaphore_wait(cute_semaphore_t* semaphore) { return !SDL_SemWait((SDL_sem*)semaphore->id); }

int cute_semaphore_value(cute_semaphore_t* semaphore) { return SDL_SemValue((SDL_sem*)semaphore->id); }

void cute_semaphore_destroy(cute_semaphore_t* semaphore) { SDL_DestroySemaphore((SDL_sem*)semaphore->id); }

cute_thread_t* cute_thread_create(cute_thread_fn func, const char* name, void* udata) { return (cute_thread_t*)SDL_CreateThread(func, name, udata); }

void cute_thread_detach(cute_thread_t* thread) { SDL_DetachThread((SDL_Thread*)thread); }

cute_thread_id_t cute_thread_get_id(cute_thread_t* thread) { return SDL_GetThreadID((SDL_Thread*)thread); }

cute_thread_id_t cute_thread_id() { return SDL_ThreadID(); }
...
int cute_core_count() { return SDL_GetCPUCount(); }

int cute_cacheline_size() { return SDL_GetCPUCacheLineSize(); }

int cute_ram_size() { return SDL_GetSystemRAM(); }
...
int cute_trylock(cute_mutex_t* mutex) { return !TryEnterCriticalSection((CRITICAL_SECTION*)mutex); }

void cute_mutex_destroy(cute_mutex_t* mutex) { DeleteCriticalSection((CRITICAL_SECTION*)mutex); }
...
int cute_cv_wait(cute_cv_t* cv, cute_mutex_t* mutex) { return !!SleepConditionVariableCS((CONDITION_VARIABLE*)cv, (CRITICAL_SECTION*)mutex, INFINITE); }
...
int cute_semaphore_try(cute_semaphore_t* semaphore) { return s_wait(semaphore, 0); }

int cute_semaphore_wait(cute_semaphore_t* semaphore) { return s_wait(semaphore, INFINITE); }

int cute_semaphore_value(cute_semaphore_t* semaphore) { return cute_atomic_get(&semaphore->count); }

void cute_semaphore_destroy(cute_semaphore_t* semaphore) { CloseHandle((HANDLE)semaphore->id); }
...
void cute_thread_detach(cute_thread_t* thread) { CloseHandle((HANDLE)thread); }

cute_thread_id_t cute_thread_get_id(cute_thread_t* thread) { return GetThreadId((HANDLE)thread); }

cute_thread_id_t cute_thread_id() { return GetCurrentThreadId(); }
...
int cute_trylock(cute_mutex_t* mutex) { return !pthread_mutex_trylock((pthread_mutex_t*)mutex); }

void cute_mutex_destroy(cute_mutex_t* mutex) { pthread_mutex_destroy((pthread_mutex_t*)mutex); }
...
int cute_cv_wait(cute_cv_t* cv, cute_mutex_t* mutex) { return !pthread_cond_wait((pthread_cond_t*)cv, (pthread_mutex_t*)mutex); }

void cute_cv_destroy(cute_cv_t* cv) { pthread_cond_destroy((pthread_cond_t*)cv); }
...
int cute_semaphore_post(cute_semaphore_t* semaphore) { return !sem_post((sem_t*)semaphore->id); }

int cute_semaphore_try(cute_semaphore_t* semaphore) { return !sem_trywait((sem_t*)semaphore->id); }

int cute_semaphore_wait(cute_semaphore_t* semaphore) { return !sem_wait((sem_t*)semaphore->id); }
...
void cute_thread_detach(cute_thread_t* thread) { pthread_detach((pthread_t)thread); }

cute_thread_id_t cute_thread_get_id(cute_thread_t* thread) { return (cute_thread_id_t)thread; }

cute_thread_id_t cute_thread_id() { return (cute_thread_id_t)pthread_self(); }
...
int cute_core_count() { return (int)sysconf(_SC_NPROCESSORS_ONLN); }
...
int cute_ram_size() { return (int)(sysconf(_SC_PHYS_PAGES) * sysconf(_SC_PAGESIZE) / (1024 * 1024)); }
