...
ResponseHandle **ResponseHandle::constructUserdata(lua_State *L) {
    ...
    luaL_getmetatable(L, "MetaEngine.Net.ResponseHandle");
...
std::string ResponseHandle::readAll() { return stream->str(); }

static ResponseHandle *checkUData(lua_State *L) {
    void *ud = luaL_checkudata(L, 1, "MetaEngine.Net.ResponseHandle");
...
static const luaL_Reg luaBinding[] = {{"readAll", luaReadAll}, {nullptr, nullptr}};

void ResponseHandle::initMetatable(lua_State *L) {
    luaL_newmetatable(L, "MetaEngine.Net.ResponseHandle");
