...
public:
    ...
    CColorFloat(float r = 0, float g = 0, float b = 0, float a = 1.f) : r(r), g(g), b(b), a(a), multiplied_with_alpha(false) { InitMasks(); }

    CColorFloat(const CColorFloat &other) : r(other.r), g(other.g), b(other.b), a(other.a), multiplied_with_alpha(false) { InitMasks(); }

    CColorFloat(const uint32 clor) : multiplied_with_alpha(false) {
        InitMasks();
        Set32(clor);
    }
    ...
    //-------------------------------------------------------------------------
    // Operators
    CColorFloat operator+(const CColorFloat &other) const { return CColorFloat(r + other.r, g + other.g, b + other.b, a + other.a); }
    ...
    CColorFloat operator*(float num) const { return CColorFloat(r * num, g * num, b * num, a * num); }
    ...
    bool FloatCompare(float v1, float v2, float e = 1.f / 512.f) const { return (fabs(v1 - v2) < e); }

    bool operator==(const CColorFloat &other) const {
        if (FloatCompare(r, other.r) && FloatCompare(g, other.g) && FloatCompare(b, other.b) && FloatCompare(a, other.a)) return true;
        return false;
    }

    bool operator!=(const CColorFloat &other) const { return !operator==(other); }
...
public:
    ...
    CColorUint8(uint8 r = 0, uint8 g = 0, uint8 b = 0, uint8 a = 255) : r(r), g(g), b(b), a(a) { InitMasks(); }

    CColorUint8(const CColorUint8 &other) : r(other.r), g(other.g), b(other.b), a(other.a) { InitMasks(); }
    ...
    //-------------------------------------------------------------------------
    // Operators
    CColorUint8 operator+(const CColorUint8 &other) const { return CColorUint8(r + other.r, g + other.g, b + other.b, a + other.a); }
    ...
    CColorUint8 operator*(float num) const { return CColorUint8((uint8)(r * num), (uint8)(g * num), (uint8)(b * num), (uint8)(a * num)); }

    CColorUint8 operator/(float num) const { return CColorUint8((uint8)((float)r / num), (uint8)((float)g / num), (uint8)((float)b / num), (uint8)((float)a / num)); }
    ...
    bool operator==(const CColorUint8 &other) const { return (r == other.r && g == other.g && b == other.b && a == other.a); }

    bool operator!=(const CColorUint8 &other) const { return !operator==(other); }
