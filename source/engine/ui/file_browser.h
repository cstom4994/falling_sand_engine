...
inline void ImGui::FileBrowser::Display() {
    ...
    if (openFlag_ && (flags_ & ImGuiFileBrowserFlags_NoModal)) {
        if (posIsSet_) SetNextWindowPos(ImVec2(static_cast<float>(posX_), static_cast<float>(posY_)));
        SetNextWindowSize(ImVec2(static_cast<float>(width_), static_cast<float>(height_)));
    } else {
        if (posIsSet_) SetNextWindowPos(ImVec2(static_cast<float>(posX_), static_cast<float>(posY_)), ImGuiCond_FirstUseEver);
    ...
    if (!(flags_ & ImGuiFileBrowserFlags_SelectDirectory) && (flags_ & ImGuiFileBrowserFlags_EnterNewFilename)) reserveHeight += GetFrameHeightWithSpacing();
    {
    ...
    bool shouldExit = Button("cancel") || closeFlag_ || ((flags_ & ImGuiFileBrowserFlags_CloseOnEsc) && IsWindowFocused(ImGuiFocusedFlags_RootAndChildWindows) && IsKeyPressed(ImGuiKey_Escape));
    if (shouldExit) {
        CloseCurrentPopup();
    }
