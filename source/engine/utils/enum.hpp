// Metadot enum module is enhanced based on ME::meta::doenum modification
// Metadot code Copyright(c) 2022-2023, KaoruXun All rights reserved.
// ME::meta::doenum code by Daniil Goncharov licensed under the MIT License
// https://github.com/Neargye/ME::meta::doenum
...
#if defined(ME_ENUM_NO_ASSERT)
#define ME_ENUM_ASSERT(...) static_cast<void>(0)
#else
#include <cassert>
#define ME_ENUM_ASSERT(...) assert((__VA_ARGS__))
#endif
...
// Checks ME::meta::doenum compiler compatibility.
#if defined(__clang__) && __clang_major__ >= 5 || defined(__GNUC__) && __GNUC__ >= 9 || defined(_MSC_VER) && _MSC_VER >= 1910 || defined(__RESHARPER__)
#undef ME_ENUM_SUPPORTED
#define ME_ENUM_SUPPORTED 1
#endif

// Checks ME::meta::doenum compiler aliases compatibility.
#if defined(__clang__) && __clang_major__ >= 5 || defined(__GNUC__) && __GNUC__ >= 9 || defined(_MSC_VER) && _MSC_VER >= 1920
#undef ME_ENUM_SUPPORTED_ALIASES
#define ME_ENUM_SUPPORTED_ALIASES 1
#endif

// Enum value must be greater or equals than ME_ENUM_RANGE_MIN. By default ME_ENUM_RANGE_MIN = -128.
// If need another min range for all enum types by default, redefine the macro ME_ENUM_RANGE_MIN.
#if !defined(ME_ENUM_RANGE_MIN)
#define ME_ENUM_RANGE_MIN -128
#endif

// Enum value must be less or equals than ME_ENUM_RANGE_MAX. By default ME_ENUM_RANGE_MAX = 128.
// If need another max range for all enum types by default, redefine the macro ME_ENUM_RANGE_MAX.
#if !defined(ME_ENUM_RANGE_MAX)
#define ME_ENUM_RANGE_MAX 127
#endif

// Improve ReSharper C++ intellisense performance with builtins, avoiding unnecessary template instantiations.
#if defined(__RESHARPER__)
#undef ME_ENUM_GET_ENUM_NAME_BUILTIN
#undef ME_ENUM_GET_TYPE_NAME_BUILTIN
#if __RESHARPER__ >= 20230100
#define ME_ENUM_GET_ENUM_NAME_BUILTIN(V) __rscpp_enumerator_name(V)
#define ME_ENUM_GET_TYPE_NAME_BUILTIN(T) __rscpp_type_name<T>()
#else
#define ME_ENUM_GET_ENUM_NAME_BUILTIN(V) nullptr
#define ME_ENUM_GET_TYPE_NAME_BUILTIN(T) nullptr
#endif
#endif

namespace ME::meta::doenum {
...
static_assert(std::is_same_v<string_view::value_type, string::value_type>, "ME::meta::doenum::customize requires same string_view::value_type and string::value_type");
static_assert(
        [] {
            if constexpr (std::is_same_v<char_type, wchar_t>) {
                ...
                static_assert(std::size(c) == std::size(wc), "ME::meta::doenum::customize identifier characters are multichars in wchar_t.");
        ...
        "ME::meta::doenum::customize wchar_t is not compatible with ASCII.");

namespace customize {

// Enum value must be in range [ME_ENUM_RANGE_MIN, ME_ENUM_RANGE_MAX]. By default ME_ENUM_RANGE_MIN = -128, ME_ENUM_RANGE_MAX = 128.
// If need another range for all enum types by default, redefine the macro ME_ENUM_RANGE_MIN and ME_ENUM_RANGE_MAX.
// If need another range for specific enum type, add specialization enum_range for necessary enum type.
template <typename E>
struct enum_range {
    static_assert(std::is_enum_v<E>, "ME::meta::doenum::customize::enum_range requires enum type.");
    static constexpr int min = ME_ENUM_RANGE_MIN;
    static constexpr int max = ME_ENUM_RANGE_MAX;
    static_assert(max > min, "ME::meta::doenum::customize::enum_range requires max > min.");
};

static_assert(ME_ENUM_RANGE_MAX > ME_ENUM_RANGE_MIN, "ME_ENUM_RANGE_MAX must be greater than ME_ENUM_RANGE_MIN.");
static_assert((ME_ENUM_RANGE_MAX - ME_ENUM_RANGE_MIN) < (std::numeric_limits<std::uint16_t>::max)(), "ME_ENUM_RANGE must be less than UINT16_MAX.");
...
public:
    ...
    constexpr customize_t(detail::customize_tag tag) : std::pair<detail::customize_tag, string_view>{tag, string_view{}} { ME_ENUM_ASSERT(tag != detail::customize_tag::custom_tag); }
};
...
template <typename T>
struct supported
#if defined(ME_ENUM_SUPPORTED) && ME_ENUM_SUPPORTED || defined(ME_ENUM_NO_CHECK_SUPPORT)
    : std::true_type {
};
#else
    : std::false_type {
...
template <typename T, typename = void>
struct range_min : std::integral_constant<int, ME_ENUM_RANGE_MIN> {};
...
template <typename T, typename = void>
struct range_max : std::integral_constant<int, ME_ENUM_RANGE_MAX> {};
...
class static_str {
public:
    constexpr explicit static_str(str_view str) noexcept : static_str{str.str_, std::make_integer_sequence<std::uint16_t, N>{}} { ME_ENUM_ASSERT(str.size_ == N); }

    constexpr explicit static_str(string_view str) noexcept : static_str{str.data(), std::make_integer_sequence<std::uint16_t, N>{}} { ME_ENUM_ASSERT(str.size() == N); }
...
constexpr bool is_nothrow_invocable() {
    return is_default_predicate<BinaryPredicate>() || std::is_nothrow_invocable_r_v<bool, BinaryPredicate, char_type, char_type>;
}
...
constexpr bool cmp_less(L lhs, R rhs) noexcept {
    static_assert(std::is_integral_v<L> && std::is_integral_v<R>, "ME::meta::doenum::detail::cmp_less requires integral type.");
...
constexpr I log2(I value) noexcept {
    static_assert(std::is_integral_v<I>, "ME::meta::doenum::detail::log2 requires integral type.");

    if constexpr (std::is_same_v<I, bool>) {  // bool special case
        return ME_ENUM_ASSERT(false), value;
    } else {
        auto ret = I{0};
...
#define ME_ENUM_ARRAY_CONSTEXPR 1
#else
...
constexpr auto n() noexcept {
    static_assert(is_enum_v<E>, "ME::meta::doenum::detail::n requires enum type.");

    if constexpr (supported<E>::value) {
#if defined(ME_ENUM_GET_TYPE_NAME_BUILTIN)
        constexpr auto name_ptr = ME_ENUM_GET_TYPE_NAME_BUILTIN(E);
...
constexpr auto type_name() noexcept {
    ...
    static_assert(std::is_same_v<std::decay_t<decltype(custom)>, customize::customize_t>, "ME::meta::doenum::customize requires customize_t type.");
    if constexpr (custom.first == customize::detail::customize_tag::custom_tag) {
        constexpr auto name = custom.second;
        static_assert(!name.empty(), "ME::meta::doenum::customize requires not empty string.");
        ...
        static_assert(always_false_v<E>, "ME::meta::doenum::customize invalid.");
    }
}
...
template <auto V>
constexpr auto n() noexcept {
    static_assert(is_enum_v<decltype(V)>, "ME::meta::doenum::detail::n requires enum type.");

    if constexpr (supported<decltype(V)>::value) {
#if defined(ME_ENUM_GET_ENUM_NAME_BUILTIN)
        constexpr auto name_ptr = ME_ENUM_GET_ENUM_NAME_BUILTIN(V);
...
#define ME_ENUM_VS_2017_WORKAROUND 1
#endif

#if defined(ME_ENUM_VS_2017_WORKAROUND)
...
constexpr U ualue(std::size_t i) noexcept {
    if constexpr (std::is_same_v<U, bool>) {  // bool special case
        static_assert(O == 0, "ME::meta::doenum::detail::ualue requires valid offset.");
...
#define ME_ENUM_FOR_EACH_256(T)                                                                                                                                                                        \
    ...
    T(5)                                                                                                                                                                                               \
    T(6)                                                                                                                                                                                               \
    T(7) T(8) T(9) T(10) T(11) T(12) T(13) T(14) T(15) T(16) T(17) T(18) T(19) T(20) T(21) T(22) T(23) T(24) T(25) T(26) T(27) T(28) T(29) T(30) T(31) T(32) T(33) T(34) T(35) T(36) T(37) T(38) T(39) \
            T(40) T(41) T(42) T(43) T(44) T(45) T(46) T(47) T(48) T(49) T(50) T(51) T(52) T(53) T(54) T(55) T(56) T(57) T(58) T(59) T(60) T(61) T(62) T(63) T(64) T(65) T(66) T(67) T(68) T(69) T(70)  \
                    T(71) T(72) T(73) T(74) T(75) T(76) T(77) T(78) T(79) T(80) T(81) T(82) T(83) T(84) T(85) T(86) T(87) T(88) T(89) T(90) T(91) T(92) T(93) T(94) T(95) T(96) T(97) T(98) T(99)      \
                            T(100) T(101) T(102) T(103) T(104) T(105) T(106) T(107) T(108) T(109) T(110) T(111) T(112) T(113) T(114) T(115) T(116) T(117) T(118) T(119) T(120) T(121) T(122) T(123)    \
                                    T(124) T(125) T(126) T(127) T(128) T(129) T(130) T(131) T(132) T(133) T(134) T(135) T(136) T(137) T(138) T(139) T(140) T(141) T(142) T(143) T(144) T(145) T(146)   \
                                            T(147) T(148) T(149) T(150) T(151) T(152) T(153) T(154) T(155) T(156) T(157) T(158) T(159) T(160) T(161) T(162) T(163) T(164) T(165) T(166) T(167) T(168)  \
                                                    T(169) T(170) T(171) T(172) T(173) T(174) T(175) T(176) T(177) T(178) T(179) T(180) T(181) T(182) T(183) T(184) T(185) T(186) T(187) T(188) T(189) \
                                                            T(190) T(191) T(192) T(193) T(194) T(195) T(196) T(197) T(198) T(199) T(200) T(201) T(202) T(203) T(204) T(205) T(206) T(207) T(208)       \
                                                                    T(209) T(210) T(211) T(212) T(213) T(214) T(215) T(216) T(217) T(218) T(219) T(220) T(221) T(222) T(223) T(224) T(225) T(226)      \
                                                                            T(227) T(228) T(229) T(230) T(231) T(232) T(233) T(234) T(235) T(236) T(237) T(238) T(239) T(240) T(241) T(242) T(243)     \
                                                                                    T(244) T(245) T(246) T(247) T(248) T(249) T(250) T(251) T(252) T(253) T(254) T(255)
...
#define ME_ENUM_V(O)                                            \
    ...
    ME_ENUM_FOR_EACH_256(ME_ENUM_V);
...
#if defined(ME_ENUM_ARRAY_CONSTEXPR)
...
constexpr auto values() noexcept {
    ...
    static_assert(range_size > 0, "ME::meta::doenum::enum_range requires valid size.");
    static_assert(range_size < (std::numeric_limits<std::uint16_t>::max)(), "ME::meta::doenum::enum_range requires valid size.");
...
#if defined(ME_ENUM_AUTO_IS_FLAGS)
...
constexpr U values_ors() noexcept {
    static_assert(S == enum_subtype::flags, "ME::meta::doenum::detail::values_ors requires valid subtype.");
...
struct enable_if_enum<true, R> {
    using type = R;
    static_assert(supported<R>::value, "ME::meta::doenum unsupported compiler (https://github.com/Neargye/ME::meta::doenum#compiler-compatibility).");
};
...
#if defined(ME_ENUM_ENABLE_HASH) || defined(ME_ENUM_ENABLE_HASH_SWITCH)
...
struct constexpr_hash_t<Value, std::enable_if_t<std::is_same_v<Value, string_view>>> {
    static constexpr std::uint32_t crc_table[256]{
            0x00000000L, 0x77073096L, 0xee0e612cL, 0x990951baL, 0x076dc419L, 0x706af48fL, 0xe963a535L, 0x9e6495a3L, 0x0edb8832L, 0x79dcb8a4L, 0xe0d5e91eL, 0x97d2d988L, 0x09b64c2bL, 0x7eb17cbdL,
            0xe7b82d07L, 0x90bf1d91L, 0x1db71064L, 0x6ab020f2L, 0xf3b97148L, 0x84be41deL, 0x1adad47dL, 0x6ddde4ebL, 0xf4d4b551L, 0x83d385c7L, 0x136c9856L, 0x646ba8c0L, 0xfd62f97aL, 0x8a65c9ecL,
            0x14015c4fL, 0x63066cd9L, 0xfa0f3d63L, 0x8d080df5L, 0x3b6e20c8L, 0x4c69105eL, 0xd56041e4L, 0xa2677172L, 0x3c03e4d1L, 0x4b04d447L, 0xd20d85fdL, 0xa50ab56bL, 0x35b5a8faL, 0x42b2986cL,
            0xdbbbc9d6L, 0xacbcf940L, 0x32d86ce3L, 0x45df5c75L, 0xdcd60dcfL, 0xabd13d59L, 0x26d930acL, 0x51de003aL, 0xc8d75180L, 0xbfd06116L, 0x21b4f4b5L, 0x56b3c423L, 0xcfba9599L, 0xb8bda50fL,
            0x2802b89eL, 0x5f058808L, 0xc60cd9b2L, 0xb10be924L, 0x2f6f7c87L, 0x58684c11L, 0xc1611dabL, 0xb6662d3dL, 0x76dc4190L, 0x01db7106L, 0x98d220bcL, 0xefd5102aL, 0x71b18589L, 0x06b6b51fL,
            0x9fbfe4a5L, 0xe8b8d433L, 0x7807c9a2L, 0x0f00f934L, 0x9609a88eL, 0xe10e9818L, 0x7f6a0dbbL, 0x086d3d2dL, 0x91646c97L, 0xe6635c01L, 0x6b6b51f4L, 0x1c6c6162L, 0x856530d8L, 0xf262004eL,
            0x6c0695edL, 0x1b01a57bL, 0x8208f4c1L, 0xf50fc457L, 0x65b0d9c6L, 0x12b7e950L, 0x8bbeb8eaL, 0xfcb9887cL, 0x62dd1ddfL, 0x15da2d49L, 0x8cd37cf3L, 0xfbd44c65L, 0x4db26158L, 0x3ab551ceL,
            0xa3bc0074L, 0xd4bb30e2L, 0x4adfa541L, 0x3dd895d7L, 0xa4d1c46dL, 0xd3d6f4fbL, 0x4369e96aL, 0x346ed9fcL, 0xad678846L, 0xda60b8d0L, 0x44042d73L, 0x33031de5L, 0xaa0a4c5fL, 0xdd0d7cc9L,
            0x5005713cL, 0x270241aaL, 0xbe0b1010L, 0xc90c2086L, 0x5768b525L, 0x206f85b3L, 0xb966d409L, 0xce61e49fL, 0x5edef90eL, 0x29d9c998L, 0xb0d09822L, 0xc7d7a8b4L, 0x59b33d17L, 0x2eb40d81L,
            0xb7bd5c3bL, 0xc0ba6cadL, 0xedb88320L, 0x9abfb3b6L, 0x03b6e20cL, 0x74b1d29aL, 0xead54739L, 0x9dd277afL, 0x04db2615L, 0x73dc1683L, 0xe3630b12L, 0x94643b84L, 0x0d6d6a3eL, 0x7a6a5aa8L,
            0xe40ecf0bL, 0x9309ff9dL, 0x0a00ae27L, 0x7d079eb1L, 0xf00f9344L, 0x8708a3d2L, 0x1e01f268L, 0x6906c2feL, 0xf762575dL, 0x806567cbL, 0x196c3671L, 0x6e6b06e7L, 0xfed41b76L, 0x89d32be0L,
            0x10da7a5aL, 0x67dd4accL, 0xf9b9df6fL, 0x8ebeeff9L, 0x17b7be43L, 0x60b08ed5L, 0xd6d6a3e8L, 0xa1d1937eL, 0x38d8c2c4L, 0x4fdff252L, 0xd1bb67f1L, 0xa6bc5767L, 0x3fb506ddL, 0x48b2364bL,
            0xd80d2bdaL, 0xaf0a1b4cL, 0x36034af6L, 0x41047a60L, 0xdf60efc3L, 0xa867df55L, 0x316e8eefL, 0x4669be79L, 0xcb61b38cL, 0xbc66831aL, 0x256fd2a0L, 0x5268e236L, 0xcc0c7795L, 0xbb0b4703L,
            0x220216b9L, 0x5505262fL, 0xc5ba3bbeL, 0xb2bd0b28L, 0x2bb45a92L, 0x5cb36a04L, 0xc2d7ffa7L, 0xb5d0cf31L, 0x2cd99e8bL, 0x5bdeae1dL, 0x9b64c2b0L, 0xec63f226L, 0x756aa39cL, 0x026d930aL,
            0x9c0906a9L, 0xeb0e363fL, 0x72076785L, 0x05005713L, 0x95bf4a82L, 0xe2b87a14L, 0x7bb12baeL, 0x0cb61b38L, 0x92d28e9bL, 0xe5d5be0dL, 0x7cdcefb7L, 0x0bdbdf21L, 0x86d3d2d4L, 0xf1d4e242L,
            0x68ddb3f8L, 0x1fda836eL, 0x81be16cdL, 0xf6b9265bL, 0x6fb077e1L, 0x18b74777L, 0x88085ae6L, 0xff0f6a70L, 0x66063bcaL, 0x11010b5cL, 0x8f659effL, 0xf862ae69L, 0x616bffd3L, 0x166ccf45L,
            0xa00ae278L, 0xd70dd2eeL, 0x4e048354L, 0x3903b3c2L, 0xa7672661L, 0xd06016f7L, 0x4969474dL, 0x3e6e77dbL, 0xaed16a4aL, 0xd9d65adcL, 0x40df0b66L, 0x37d83bf0L, 0xa9bcae53L, 0xdebb9ec5L,
            0x47b2cf7fL, 0x30b5ffe9L, 0xbdbdf21cL, 0xcabac28aL, 0x53b39330L, 0x24b4a3a6L, 0xbad03605L, 0xcdd70693L, 0x54de5729L, 0x23d967bfL, 0xb3667a2eL, 0xc4614ab8L, 0x5d681b02L, 0x2a6f2b94L,
        ...
        if constexpr ((val) + Page < size) {                                                                                            \
            ...
            if constexpr (CallValue == case_call_t::index) {                                                                            \
                if constexpr (std::is_invocable_r_v<result_t, Lambda, std::integral_constant<std::size_t, val + Page>>) {               \
                    ...
                    ME_ENUM_ASSERT(false && "ME::meta::doenum::detail::constexpr_switch wrong result type.");                           \
                }                                                                                                                       \
...
template <auto* GlobValues, case_call_t CallValue, std::size_t Page = 0, typename Hash = constexpr_hash_t<typename std::decay_t<decltype(*GlobValues)>::value_type>,
          typename BinaryPredicate = std::equal_to<>, typename Lambda, typename ResultGetterType>
constexpr decltype(auto) constexpr_switch(Lambda&& lambda, typename std::decay_t<decltype(*GlobValues)>::value_type searched, ResultGetterType&& def, BinaryPredicate&& pred = {}) {
    ...
    static_assert(has_duplicate<GlobValues, hash_t>(), "ME::meta::doenum::detail::constexpr_switch duplicated hash found, please report it: https://github.com/Neargye/ME::meta::doenum/issues.");
    constexpr std::array values = *GlobValues;
    constexpr std::size_t size = values.size();
    constexpr std::array cases = calculate_cases<GlobValues, hash_t>(Page);

    switch (hash_v<hash_t>(searched)) {
        ME_ENUM_FOR_EACH_256(ME_ENUM_CASE)
        default:
...
// Checks is ME::meta::doenum supported compiler.
...
// Provides the member constant value which is equal to true, if T is an [Unscoped enumeration](https://en.cppreference.com/w/cpp/language/enum#Unscoped_enumeration) type. Otherwise, value is equal to
// false.
template <typename T>
...
// Provides the member constant value which is equal to true, if T is an [Scoped enumeration](https://en.cppreference.com/w/cpp/language/enum#Scoped_enumerations) type. Otherwise, value is equal to
// false.
template <typename T>
...
[[nodiscard]] constexpr auto enum_type_name() noexcept -> detail::enable_if_t<E, string_view> {
    ...
    static_assert(!name.empty(), "ME::meta::doenum::enum_type_name enum type does not have a name.");
...
[[nodiscard]] constexpr auto enum_value(std::size_t index) noexcept -> detail::enable_if_t<E, std::decay_t<E>> {
    ...
    if constexpr (detail::is_sparse_v<D, S>) {
        return ME_ENUM_ASSERT(index < detail::count_v<D, S>), detail::values_v<D, S>[index];
    } else {
        constexpr auto min = (S == detail::enum_subtype::flags) ? detail::log2(detail::min_v<D, S>) : detail::min_v<D, S>;

        return ME_ENUM_ASSERT(index < detail::count_v<D, S>), detail::value<D, min, S>(index);
    }
}
...
[[nodiscard]] constexpr auto enum_value() noexcept -> detail::enable_if_t<E, std::decay_t<E>> {
    using D = std::decay_t<E>;
    static_assert(I < detail::count_v<D, S>, "ME::meta::doenum::enum_value out of range.");
...
#if defined(ME_ENUM_ENABLE_HASH)
        return detail::constexpr_switch<&detail::values_v<D, S>, detail::case_call_t::index>([](std::size_t i) { return optional<std::size_t>{i}; }, value,
...
[[nodiscard]] constexpr auto enum_index() noexcept -> detail::enable_if_t<decltype(V), std::size_t> {
    ...
    static_assert(index, "ME::meta::doenum::enum_index enum value does not have a index.");
...
[[nodiscard]] constexpr auto enum_name() noexcept -> detail::enable_if_t<decltype(V), string_view> {
    ...
    static_assert(!name.empty(), "ME::meta::doenum::enum_name enum value does not have a name.");
...
// Allows you to write ME::meta::doenum::enum_cast<foo>("bar", ME::meta::doenum::case_insensitive);
...
#if defined(ME_ENUM_ENABLE_HASH)
            return detail::constexpr_switch<&detail::values_v<D, S>, detail::case_call_t::value>([](D v) { return optional<D>{v}; }, static_cast<D>(value),
...
[[nodiscard]] constexpr auto enum_cast(string_view value, [[maybe_unused]] BinaryPredicate p = {}) noexcept(detail::is_nothrow_invocable<BinaryPredicate>())
    ...
    if constexpr (detail::count_v<D, S> == 0) {
        ...
        return detail::constexpr_switch<&detail::names_v<D, S>, detail::case_call_t::index>([](std::size_t i) { return optional<D>{detail::values_v<D, S>[i]}; }, value,
                                                                                            detail::default_result_type_lambda<optional<D>>,
...
#undef ME_ENUM_GET_ENUM_NAME_BUILTIN
#undef ME_ENUM_GET_TYPE_NAME_BUILTIN
#undef ME_ENUM_VS_2017_WORKAROUND
#undef ME_ENUM_ARRAY_CONSTEXPR
#undef ME_ENUM_FOR_EACH_256
