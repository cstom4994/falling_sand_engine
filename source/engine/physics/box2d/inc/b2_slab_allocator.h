...
private:
    ...
    public:
        ...
        Slab(uint32 numberOfItems) : m_numberOfItems(numberOfItems) {
        ...
        T* GetFirstItem() const { return (T*)((uint8*)(this + 1)); }
...
public:
    ...
    b2SlabAllocator(const uint32 itemsPerSlab) : m_itemsPerSlab(itemsPerSlab) {}

    /// Free all allocated slabs.
    ~b2SlabAllocator() { FreeAllSlabs(); }
    ...
    void SetItemsPerSlab(uint32 itemsPerSlab) { m_itemsPerSlab = itemsPerSlab; }

    // Get the size of the next allocated slab.
    uint32 GetItemsPerSlab() const { return m_itemsPerSlab; }

    /// Allocate a item from the slab.
    T* Allocate() {
        // Allocate a slab if needed here.
        if (m_freeList.GetFreeList()->GetFreeList().IsEmpty() && !AllocateSlab()) return NULL;
        return m_freeList.Allocate();
    }

    /// Free an item from the slab.
    void Free(T* object) { m_freeList.Free(object); }
    ...
    /// Free all slabs.
    void FreeAllSlabs() {
        const b2TypedIntrusiveListNode<b2TrackedBlock>& slabList = m_slabs.GetList();
    ...
    void FreeEmptySlabs() {
        const b2IntrusiveListNode& freeItemList = m_freeList.GetFreeList()->GetFreeList();
        const b2IntrusiveListNode* freeItemListTerminator = freeItemList.GetTerminator();
        const b2TypedIntrusiveListNode<b2TrackedBlock>& slabList = m_slabs.GetList();
        const b2TypedIntrusiveListNode<b2TrackedBlock>* slabListTerminator = slabList.GetTerminator();
        ...
        while (block != slabListTerminator) {
            ...
            uint8 freeItems = 0;
            bool empty = false;
            for (b2IntrusiveListNode* itemNode = freeItemList.GetNext(); itemNode != freeItemListTerminator; itemNode = itemNode->GetNext()) {
                const uint8* itemNodeAddress = (uint8*)itemNode;
                if (itemNodeAddress >= slabItemStart && itemNodeAddress <= slabItemEnd) {
                    ++freeItems;
    ...
    const b2TypedFreeList<T>& GetFreeList() const { return m_freeList; }
...
private:
    ...
    Slab* BlockGetSlab(void* memory) { return (Slab*)memory; }

    /// Get a pointer to the first item in the array of items referenced by a
    /// Slab.
    T* SlabGetFirstItem(Slab* slab) { return (T*)(slab + 1); }
