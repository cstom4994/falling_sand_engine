...
struct MetaGenerator::Impl : CPP14ParserBaseVisitor {
    ...
    struct Declarator {
        AutoField::Mode mode{AutoField::Mode::Variable};
        std::vector<AutoAttr> attrs;
    ...
    std::string GetTextPro(antlr4::ParserRuleContext* ctx) const { return {code.begin() + ctx->getStart()->getStartIndex(), code.begin() + ctx->getStop()->getStopIndex() + 1}; }
    ...
    antlrcpp::Any visitAttributeSpecifierSeq(CPP14Parser::AttributeSpecifierSeqContext* ctx) override final {
        ...
        for (auto* ctxAttributeSpecifier : ctx->attributeSpecifier()) {
            std::vector<AutoAttr> partialAttrs = std::any_cast<std::vector<AutoAttr>>(visit(ctxAttributeSpecifier));
    ...
    antlrcpp::Any visitAttributeSpecifier(CPP14Parser::AttributeSpecifierContext* ctx) override final {
        ...
        if (ctx->attributeList()) {
            for (auto* ctxAttr : ctx->attributeList()->attribute()) {
                AutoAttr attr = std::any_cast<AutoAttr>(visit(ctxAttr));
    ...
    antlrcpp::Any visitAttribute(CPP14Parser::AttributeContext* ctx) override final {
        AutoAttr attr;
        if (ctx->attributeNamespace()) attr.ns = ctx->attributeNamespace()->getText();
        attr.name = ctx->Identifier()->getText();
        if (ctx->attributeArgumentClause() && ctx->attributeArgumentClause()->balancedTokenSeq()) attr.value = ctx->attributeArgumentClause()->balancedTokenSeq()->getText();
        return attr;
    }
    ...
    antlrcpp::Any visitTypeParameter(CPP14Parser::TypeParameterContext* ctx) override final {
        ...
        if (ctx->Typename_())
            ...
            if (ctx->templateparameterList()) {
                std::vector<Parameter> tparams = std::any_cast<std::vector<Parameter>>(visit(ctx->templateparameterList()));
    ...
    antlrcpp::Any visitPointerAbstractDeclarator(CPP14Parser::PointerAbstractDeclaratorContext* ctx) override final {
        ...
        for (auto* ctxPointerOperator : ctx->pointerOperator()) declarator.pointerOperators.push_back(ctxPointerOperator->getText());
        return declarator;
    }
    ...
    antlrcpp::Any visitAbstractPackDeclarator(CPP14Parser::AbstractPackDeclaratorContext* ctx) override final {
        ...
        for (auto* ctxPointerOpeartor : ctx->pointerOperator()) declarator.pointerOperators.push_back(ctxPointerOpeartor->getText());
        return declarator;
    }
    ...
    antlrcpp::Any visitParameterDeclaration(CPP14Parser::ParameterDeclarationContext* ctx) override final {
        Parameter parameter;
        std::size_t start = ctx->proDeclSpecifierSeq()->preDeclSpecifierSeq() ? ctx->proDeclSpecifierSeq()->preDeclSpecifierSeq()->getStart()->getStartIndex()
                                                                              : ctx->proDeclSpecifierSeq()->proSimpleTypeSpecifier()->getStart()->getStartIndex();
        std::size_t stop = ctx->proDeclSpecifierSeq()->postDeclSpecifierSeq() ? ctx->proDeclSpecifierSeq()->postDeclSpecifierSeq()->getStop()->getStopIndex()
        ...
        if (ctx->declarator()) {
            ...
            for (const auto& pointerOperator : declarator.pointerOperators) parameter.type += pointerOperator;
        ...
        if (ctx->initializerClause()) parameter.initializer = GetTextPro(ctx->initializerClause());
    ...
    antlrcpp::Any visitEnumSpecifier(CPP14Parser::EnumSpecifierContext* ctx) override final {
        ...
        if (ctx->enumHead()->nestedNameSpecifier()) typeMeta.name = GetTextPro(ctx->enumHead()->nestedNameSpecifier());

        typeMeta.name += ctx->enumHead()->Identifier()->getText();

        if (ctx->enumHead()->attributeSpecifierSeq()) typeMeta.attrs = std::any_cast<std::vector<AutoAttr>>(visit(ctx->enumHead()->attributeSpecifierSeq()));

        if (ctx->enumeratorList()) typeMeta.fields = std::any_cast<std::vector<AutoField>>(visit(ctx->enumeratorList()));
    ...
    antlrcpp::Any visitEnumeratorList(CPP14Parser::EnumeratorListContext* ctx) override final {
        std::vector<AutoField> fields;
        for (auto* ctxEnumeratorDefinition : ctx->enumeratorDefinition()) {
            AutoField field = std::any_cast<AutoField>(visit(ctxEnumeratorDefinition));
    ...
    antlrcpp::Any visitEnumeratorDefinition(CPP14Parser::EnumeratorDefinitionContext* ctx) override final {
        AutoField field;
        field.mode = AutoField::Mode::Value;
        field.accessSpecifier = AccessSpecifier::PUBLIC;
        field.name = ctx->enumerator()->getText();
        if (ctx->attributeSpecifierSeq()) field.attrs = std::any_cast<std::vector<AutoAttr>>(visit(ctx->attributeSpecifierSeq()));
        return field;
    }

    antlrcpp::Any visitTemplateDeclaration(CPP14Parser::TemplateDeclarationContext* ctx) override final {
        templateParameters = std::any_cast<std::vector<Parameter>>(visit(ctx->templateparameterList()));
        ...
        // C++ 20
        if (rst.type() == typeid(std::vector<AutoField>)) {
            for (auto& field : std::any_cast<std::vector<AutoField>>(rst)) field.isTemplate = true;
        } else if (rst.type() == typeid(AutoField)) {
            AutoField field = std::any_cast<AutoField>(rst);
            field.isTemplate = true;
            rst = std::vector<AutoField>{field};
        }
    ...
    antlrcpp::Any visitTemplateparameterList(CPP14Parser::TemplateparameterListContext* ctx) override final {
        ...
        for (auto* ctxTemplateParameter : ctx->templateParameter()) {
            Parameter ele = std::any_cast<Parameter>(visit(ctxTemplateParameter));
    ...
    antlrcpp::Any visitClassSpecifier(CPP14Parser::ClassSpecifierContext* ctx) override final {
        ...
        if (!accessSpecifiers.empty() && accessSpecifiers.back() != AccessSpecifier::PUBLIC) return {};
        ...
        if (ctx->classHead()->attributeSpecifierSeq()) typeMeta.attrs = std::any_cast<std::vector<AutoAttr>>(visit(ctx->classHead()->attributeSpecifierSeq()));
        ...
        if (ctx->classHead()->baseClause()) typeMeta.bases = std::any_cast<std::vector<AutoBase>>(visit(ctx->classHead()->baseClause()));

        if (ctx->memberSpecification()) {
            for (auto* child : ctx->memberSpecification()->children) {
                auto rst = visit(child);
                if (rst.type() == typeid(std::vector<AutoField>)) {
                    for (auto& field : std::any_cast<std::vector<AutoField>>(rst)) typeMeta.fields.push_back(std::move(field));
                }
            }
        }
    ...
    antlrcpp::Any visitNamespaceDefinition(CPP14Parser::NamespaceDefinitionContext* ctx) override final {
        ...
        if (ctx->Identifier())
            ...
            std::vector<std::string> curNamespaces = std::any_cast<std::vector<std::string>>(visitOriginalNamespaceName(ctx->originalNamespaceName()));
    ...
    antlrcpp::Any visitNestedNameSpecifier(CPP14Parser::NestedNameSpecifierContext* ctx) override final {
        if (ctx->nestedNameSpecifier()) {
            std::vector<std::string> curNamespaces = std::any_cast<std::vector<std::string>>(visitNestedNameSpecifier(ctx->nestedNameSpecifier()));
    ...
    antlrcpp::Any visitProDeclSpecifierSeq(CPP14Parser::ProDeclSpecifierSeqContext* ctx) override final {
        ...
        if (ctx->attributeSpecifierSeq()) {
            std::vector<AutoAttr> attrs = std::any_cast<std::vector<AutoAttr>>(visit(ctx->attributeSpecifierSeq()));
    ...
    antlrcpp::Any visitMemberDeclaration(CPP14Parser::MemberDeclarationContext* ctx) override final {
        if (ctx->emptyDeclaration() || ctx->aliasDeclaration() || ctx->staticAssertDeclaration() || ctx->usingDeclaration()) return {};
        ...
        if (ctx->functionDefinition()) {
            AutoField field = std::any_cast<AutoField>(visit(ctx->functionDefinition()));
            return std::vector<AutoField>{field};
        }

        std::vector<AutoAttr> commonAttrs;
        std::vector<DeclSpecifier> declSpecifiers;
        ...
        if (ctx->memberDeclaratorList()) {
            ...
            for (auto& declarator : declarators) {
                AutoField field = std::move(declarator).GenerateField();
                if (!accessSpecifiers.empty()) field.accessSpecifier = accessSpecifiers.back();
                field.declSpecifiers = declSpecifiers;
                if (field.IsStaticConstexprVariable()) field.mode = AutoField::Mode::Value;
    ...
    antlrcpp::Any visitMemberDeclaratorList(CPP14Parser::MemberDeclaratorListContext* ctx) override final {
        ...
        for (auto* ctxMemberDeclarator : ctx->memberDeclarator()) {
            Declarator declarator = std::any_cast<Declarator>(visitMemberDeclarator(ctxMemberDeclarator));
    ...
    antlrcpp::Any visitMemberDeclarator(CPP14Parser::MemberDeclaratorContext* ctx) override final {
        ...
        if (ctx->declarator())
            ...
            if (ctx->attributeSpecifierSeq()) declarator.attrs = std::any_cast<std::vector<AutoAttr>>(visit(ctx->attributeSpecifierSeq()));
        } else
            assert(false);

        if (ctx->braceOrEqualInitializer()) declarator.initializer = std::any_cast<std::string>(visit(ctx->braceOrEqualInitializer()));
    ...
    antlrcpp::Any visitNoPointerDeclarator(CPP14Parser::NoPointerDeclaratorContext* ctx) override final {
        Declarator declarator = ctx->noPointerDeclarator() ? std::any_cast<Declarator>(visit(ctx->noPointerDeclarator())) : Declarator{};
        ...
        if (ctx->parametersAndQualifiers()) {
            declarator.mode = AutoField::Mode::Function;
            if (ctx->parametersAndQualifiers()->attributeSpecifierSeq()) {
                std::vector<AutoAttr> attrs = std::any_cast<std::vector<AutoAttr>>(visit(ctx->parametersAndQualifiers()->attributeSpecifierSeq()));
                for (auto& attr : attrs) declarator.attrs.push_back(std::move(attr));
            }
            if (auto* ctxClause = ctx->parametersAndQualifiers()->parameterDeclarationClause()) {
                for (auto* ctxParameterDeclaration : ctxClause->parameterDeclarationList()->parameterDeclaration()) {
                    Parameter parameter = std::any_cast<Parameter>(visit(ctxParameterDeclaration));
                    declarator.parameters.push_back(std::move(parameter));
                }
            }
            if (auto* ctxCVSeq = ctx->parametersAndQualifiers()->cvqualifierseq()) {
                for (auto* ctxCV : ctxCVSeq->cvQualifier()) declarator.qualifiers.push_back(ctxCV->getText());
            }
            if (auto* ctxRef = ctx->parametersAndQualifiers()->refqualifier()) declarator.qualifiers.push_back(ctxRef->getText());

            if (ctx->parametersAndQualifiers()->exceptionSpecification()) declarator.qualifiers.push_back(ctx->parametersAndQualifiers()->exceptionSpecification()->getText());
        }
    ...
    antlrcpp::Any visitSimpleDeclaration(CPP14Parser::SimpleDeclarationContext* ctx) override final {
        ...
        if (ctx->proDeclSpecifierSeq()) {
            DeclSpecifierSeq seq = std::any_cast<DeclSpecifierSeq>(visit(ctx->proDeclSpecifierSeq()));
            declSpecifiers = std::move(seq.declSpecifiers);
            commonAttrs = std::move(seq.attrs);
        }
        if (ctx->attributeSpecifierSeq()) {
    ...
    antlrcpp::Any visitInitDeclaratorList(CPP14Parser::InitDeclaratorListContext* ctx) override final {
        ...
        for (auto* ctxInitDeclarator : ctx->initDeclarator()) {
            Declarator declarator = std::any_cast<Declarator>(visit(ctxInitDeclarator));
            declarators.push_back(std::move(declarator));
        }
        return declarators;
    }
    ...
    antlrcpp::Any visitFunctionDefinition(CPP14Parser::FunctionDefinitionContext* ctx) override final {
        ...
        AutoField field = std::move(declarator).GenerateField();
        if (!accessSpecifiers.empty()) field.accessSpecifier = accessSpecifiers.back();
        field.mode = AutoField::Mode::Function;
        if (ctx->attributeSpecifierSeq()) field.attrs = std::any_cast<std::vector<AutoAttr>>(visit(ctx->attributeSpecifierSeq()));
        if (ctx->proDeclSpecifierSeq()) {
            DeclSpecifierSeq seq = std::any_cast<DeclSpecifierSeq>(visit(ctx->proDeclSpecifierSeq()));
...
MetaGenerator::MetaGenerator() : pImpl{std::make_unique<MetaGenerator::Impl>()} {}
