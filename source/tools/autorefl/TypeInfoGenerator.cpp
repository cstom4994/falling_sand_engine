...
string TypeInfoGenerator::Generate(const vector<TypeMeta>& typeMetas) {
    ...
    ss << "// This file is generated by MetaDot game engine" << endl << endl << "#pragma once" << endl << endl << "#include \"core/cpp/static_relfection.hpp\"" << endl << endl;
    ...
    for (const auto& typeMeta : typeMetas) {
        ...
        ss << "template<" << typeMeta.GenerateTemplateList() << ">" << endl << "struct MetaEngine::StaticRefl::TypeInfo<" << fullname << "> :" << endl;
        ...
        if (!typeMeta.attrs.empty()) {
            ...
            for (const auto& attr : typeMeta.attrs) {
                auto name = attr.GenerateName(attr.ns.empty() ? config.nonNamespaceNameWithoutQuotation : !config.namespaceNameWithQuotation);
                ss << indent << indent << "Attr {" << name;
                if (!attr.value.empty()) ss << ", " << attr.GenerateValue(config.isAttrValueToFunction);
                ss << "}," << endl;
            }
            ss << indent;
        }
        ...
        if (typeMeta.HaveAnyOutputField()) {
            ...
            for (const auto& field : typeMeta.fields) {
                if (field.isTemplate || field.accessSpecifier != AccessSpecifier::PUBLIC || field.IsFriendFunction() || field.IsDeletedFunction()) continue;
                ...
                ss << attr.GenerateName(attr.ns.empty() ? config.nonNamespaceNameWithoutQuotation : !config.namespaceNameWithQuotation) << ", ";

                // [value]
                switch (field.mode) {
                    case AutoField::Mode::Variable: {
                        ss << "&" << tname << "::" << field.name;
                        break;
                    }
                    case AutoField::Mode::Function: {
                        if (field.name == typeMeta.name) {
                            // constructor
                    ...
                    case AutoField::Mode::Value: {
                        ss << tname << "::" << field.name;
                        break;
                    }
                }
                // attributes
                const bool hasInitializerAttr = config.isInitializerAsAttr && field.mode != AutoField::Mode::Value && !field.initializer.empty();
                const bool hasDefaultFunctionsAttr = config.generateDefaultFunctions && field.mode == AutoField::Mode::Function && field.GetDefaultParameterNum() > 0;
                if (!field.attrs.empty() || hasInitializerAttr || hasDefaultFunctionsAttr) {
                    ...
                    if (hasInitializerAttr) {
                        ...
                        auto name = attr.GenerateName(attr.ns.empty() ? config.nonNamespaceNameWithoutQuotation : !config.namespaceNameWithQuotation);
                        ss << indent << indent << indent << "Attr {" << name << ", " << attr.GenerateValue(field.GenerateSimpleFieldType()) << "}," << endl;
                    }
                    // [default functions]
                    if (hasDefaultFunctionsAttr) {
                        ...
                        auto name = attr.GenerateName(attr.ns.empty() ? config.nonNamespaceNameWithoutQuotation : !config.namespaceNameWithQuotation);
                        ss << indent << indent << indent << "Attr {" << name << ", std::tuple {" << endl;
                        ...
                        for (std::size_t i = 1; i <= defaultParameterNum; i++) {
                            ...
                            if (isConstructor) {
                                ss << indent << indent << indent << indent << "WrapConstructor<" << tname << "(" << field.GenerateParamTypeList(num) << ")>()";
                                ...
                                ss << indent << indent << indent << indent << "[](" << ftname << " this_" << (num > 0 ? ", " : "") << field.GenerateNamedParameterList(num) << "){ return "
                                   << (isPointer ? "this_->" : ("std::forward<" + ftname + ">(this_).")) << field.name << "(" << field.GenerateForwardArgumentList(num) << "); }";
                            } else {  // static
                                auto qualifiers = field.GenerateQualifiers();
                                bool isPointer = qualifiers.find('&') == std::string::npos;
                                if (isPointer) qualifiers += "*";
                                ...
                                auto ftname = tname + " " + qualifiers;
                                ss << indent << indent << indent << indent << "[](" << field.GenerateNamedParameterList(num) << "){ return " << tname << "::" << field.name << "("
