...
#include "NuklearImpl.h"

#ifndef NK_ASSERT
#define NK_ASSERT(condition)                                                                                                     \
    do {                                                                                                                         \
        if (!(condition)) { TraceLog(LOG_WARNING, "NUKLEAR: Failed assert \"%s\" (%s:%i)", #condition, "nuklear.h", __LINE__); } \
    } while (0)
#endif// NK_ASSERT
...
extern "C"
{
    ...
    void nk_raylib_input_keyboard(struct nk_context *ctx) {
    ...
    void nk_raylib_input_mouse(struct nk_context *ctx) {
    ...
    struct
            Rectangle
            RectangleFromNuklear(struct nk_rect rect)
    {
        Rectangle output;
        output.x = rect.x;
        output.y = rect.y;
    ...
    struct
            nk_rect
            RectangleToNuklear(Rectangle rect)
    {
    ...
    struct Texture TextureFromNuklear(struct nk_image img)
    {
        Texture tex;
    ...
    void CleanupNuklearImage(struct nk_image img) {
