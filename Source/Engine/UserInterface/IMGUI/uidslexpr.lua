...
local titleize = function(t)
    return string.gsub(t, '%w+', function(w)
...
local loadDSL = function(uidsl)
    local uidslsetname = 'UIDSL'
    --[[
  ...
  or a branch (group or list)
    ...
    local defineField = function(t, ui)
        return function(name)
            assert(not string.find(name, '[^%w_]'),
                fmt("name should contain letters, digits and underscores only, got \"%s\".", name))
    ...
    local safeenv = {
        ...
        group = function(name)
            return enter(name, 'group', true)
        end,
        endgroup = leave,
        struct = function(name)
            return enter(name, 'struct')
        end,
        endstruct = leave,
        list = function(name)
            return enter(name, 'list')
        end,
        endlist = leave
    }
    ...
    local function _genImGuiInspector(rootvar, container, branch, indent, arrayidx)
        ...
        for _, v in pairs(branch.fields) do
            ...
            if arrayidx then
                emitf('std::string label_with_id_%s = %q"["+std::to_string(%s)+"]##%s";', v.name, label, arrayidx,
                    v.name)
            ...
            elseif v.ui == 'menu' and v.meta.items and #v.meta.items > 0 then
                ...
                emitf('  if (%s_values[current_item_%s]==(%s)) break;', v.name, v.name, thisvar)
                emitf('if (ImGui::Combo(%s, &current_item_%s, %s_labels, %d)) {', label, v.name, v.name, #labels)
            ...
            elseif numericformat[v.type] then
                ...
                if ui == 'slider' or v.meta and (not v.meta.ui and v.meta.min and v.meta.max) then
                    emitf('if(ImGui::Slider%s(%s, %s%s))', ctl, label, addr, minmax)
                elseif ui == 'drag' then
            ...
            elseif v.type == 'color' then
                ...
                if meta.hdr then
                    table.insert(flags, 'HDR')
                end
                table.insert(flags, meta.hsv and 'DisplayHSV' or 'DisplayRGB')
                table.insert(flags, (meta.float or meta.hdr) and 'Float' or 'Uint8')
                if meta.wheel then
                    table.insert(flags, 'PickerHueWheel')
                end
                local flagstring = table.concat(map(function(t)
                    return 'ImGuiColorEditFlags_' .. t
                end, flags), ' | ')
                if flagstring == '' then
                    flagstring = '0'
                end
    ...
    local f, msg = load(uidsl, "uidsl", 't', safeenv)
    if not f then
        print(msg)
        return nil, msg
    end
