...
class ImGuiDSL;
class UIDSLfSet;
using ParmPtr = std::shared_ptr<ImGuiDSL>;
using ConstParmPtr = std::shared_ptr<const ImGuiDSL>;
using ParmSetPtr = std::shared_ptr<UIDSLfSet>;
...
public:
    ImGuiDSL(UIDSLfSet *root) : root_(root) {}
    ImGuiDSL(ImGuiDSL &&) = default;
    ~ImGuiDSL() = default;
    ImGuiDSL(ImGuiDSL const &that)
        : root_(that.root_), ui_type_(that.ui_type_), value_(that.value_), expected_value_type_(that.expected_value_type_), default_(that.default_), name_(that.name_), path_(that.path_), label_(that.label_), meta_(that.meta_), menu_values_(that.menu_values_), menu_items_(that.menu_items_), menu_labels_(that.menu_labels_) {
        fields_.reserve(that.fields_.size());
        for (auto f: that.fields_)
            fields_.push_back(std::make_shared<ImGuiDSL>(*f));
        listValues_.reserve(that.listValues_.size());
        for (auto v: that.listValues_)
            listValues_.push_back(std::make_shared<ImGuiDSL>(*v));
    }
    ...
    void resizeList(size_t cnt) {
        if (auto oldsize = listValues_.size(); oldsize < cnt) {
            for (auto i = oldsize; i < cnt; ++i) {
                auto newItem = std::make_shared<ImGuiDSL>(root_);
                ...
                for (auto f: fields_) {
                    auto newField = std::make_shared<ImGuiDSL>(*f);
...
protected:
    friend class UIDSLfSet;
    // setup function should only be called by UIDSLfSet
    ...
    void addField(ParmPtr child) {
        ...
        if (!listValues_.empty()) {
            for (auto item: listValues_) {
                item->fields_.push_back(std::make_shared<ImGuiDSL>(*child));
            }
        }
    }
...
protected:
    ...
    static lua_State *defaultLuaRuntime();// shared lua runtime for uidsl parsing and `disablewhen` expression evaluation
    ...
    friend class ImGuiDSL;

public:
    /// enable lua to call UIDSLfSet functions
    ///
    /// example:
    /// ```lua
    /// local UIDSLfSet = require('UIDSLfSet')
    /// local parms = UIDSLfSet.new()
    /// parms:loadDSL(...)
    ...
    bool transferTo(UIDSLfSet &that);// convert this into that, try best to respect other's existing type & format
    ...
    ConstParmPtr const get(string const &key) const {
        return std::const_pointer_cast<const ImGuiDSL>(root_->getField(key));
    }
