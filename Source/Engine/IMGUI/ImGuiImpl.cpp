...
static const char *Mtd_ImGuiGetClipText(void *) {
    return GetClipboardText();
}

static void Mtd_ImGuiSetClipText(void *, const char *text) {
    SetClipboardText(text);
}

static void Mtd_ImGuiNewFrame() {
...
static void Mtd_ImGuiTriangleVert(ImDrawVert &idx_vert) {
    Color *c;
...
static void Mtd_ImGuiRenderTriangles(unsigned int count, int indexStart, const ImVector<ImDrawIdx> &indexBuffer, const ImVector<ImDrawVert> &vertBuffer, void *texturePtr) {
    if (count < 3)
        return;
    ...
    for (unsigned int i = 0; i <= (count - 3); i += 3) {
        ...
        Mtd_ImGuiTriangleVert(vertexA);
        Mtd_ImGuiTriangleVert(vertexB);
        Mtd_ImGuiTriangleVert(vertexC);
    }
    rlEnd();
}
...
static void rlRenderData(ImDrawData *data) {
    ...
    for (int l = 0; l < data->CmdListsCount; ++l) {
        ...
        for (const auto &cmd: commandList->CmdBuffer) {
            ...
            Mtd_ImGuiRenderTriangles(cmd.ElemCount, cmd.IdxOffset, commandList->IdxBuffer, commandList->VtxBuffer, cmd.TextureId);
            rlDrawRenderBatchActive();
        }
    }
...
void Mtd_ImGuiEndInitImGui() {
    ...
    ImGuiIO &io = ImGui::GetIO();
    io.BackendPlatformName = "imgui_impl_metadot";
    ...
    io.SetClipboardTextFn = Mtd_ImGuiSetClipText;
    io.GetClipboardTextFn = Mtd_ImGuiGetClipText;

    io.ClipboardUserData = nullptr;


    Mtd_ImGuiReloadFonts();
}

void Mtd_ImGuiSetup(bool dark) {
    Mtd_ImGuiBeginInitImGui();
