...
namespace MetaEngine::ECS {
    ...
    public:
        ...
        flat_set() = default;
        explicit flat_set(container_type other) : container_type(std::move(other)) {}
    ...
    public:
        ...
        public:
            explicit value_compare(key_compare kc) : keyComp(kc) {}
        ...
        std::pair<iterator, bool> try_emplace(const Key &key, Args &&...args) {
            auto lower =
                    std::lower_bound(begin(), end(), key, [&](const value_type &val, const Key &key) {
            ...
            return {container_type::emplace(lower, std::piecewise_construct, std::forward_as_tuple(key),
