...
public:
    ...
    constexpr str_view(const char *a, int &i) noexcept
        : m_pStr(a), m_Size{static_cast<uint32_t>([](const char *a, int &i) constexpr {
...
#define property_vtable()                                                           \
    ...
    struct setup_entry : property::settings::user_entry
    {
        ...
        constexpr operator table_action_entry(void) const noexcept {
            if (m_FunctionDynamicFlags) return {m_FunctionDynamicFlags, m_FunctionTypeGetSet, m_FunctionLists, m_Offset};
    ...
    namespace details {
            ...
            if constexpr (is_specialized_v<std::vector, T>) {
                switch (Cmd) {
                    ...
                    case lists_cmd::READ_NEXT:
                        if (++InOut == Var.size()) InOut = lists_iterator_ends_v;
                        break;
                    default:
                        assert(false);
                }
        ...
        constexpr auto HandleListsPropertyVar(const char (&pName)[N], std::size_t Offset) noexcept {
            ...
            if constexpr (is_specialized_v<std::unique_ptr, val> || is_specialized_v<std::shared_ptr, val> || std::is_pointer_v<val>) return property::setup_entry(pName, details::SystemListTableGetSetForPointers<var>, Offset, SystemLists<var>);
