...
#pragma region LuaA
    ...
    void luaA_open(lua_State *L) {
        ...
        luaA_conversion_type(L, luaA_type_add(L, "bool", sizeof(bool)), luaA_push_bool,
                             luaA_to_bool);
        luaA_conversion_type(L, luaA_type_add(L, "_Bool", sizeof(bool)), luaA_push_bool,
                             luaA_to_bool);
        ...
        luaA_conversion_push(L, void *, luaA_push_void);// sizeof(void)
    }
...
*/
    ...
    int luaA_struct_push_member_offset_type(lua_State *L, luaA_Type type, size_t offset,
        ...
        if (!lua_isnil(L, -1)) {
            ...
            if (!lua_isnil(L, -1)) {
                ...
                lua_pop(L, 4);
                return luaA_push_type(L, stype, (addOffset(offset, c_in), c_in));
            }

            lua_pop(L, 3);
            lua_pushfstring(
                    L,
                    "luaA_struct_push_member: Member offset '%d' not registered for struct '%s'!",
    ...
    int luaA_struct_push_member_name_type(lua_State *L, luaA_Type type, const char *member,
        ...
        if (!lua_isnil(L, -1)) {
            ...
            lua_pop(L, 3);
            lua_pushfstring(
                    L, "luaA_struct_push_member: Member name '%s' not registered for struct '%s'!",
    ...
    void luaA_struct_to_member_offset_type(lua_State *L, luaA_Type type, size_t offset, void *c_out,
        ...
        if (!lua_isnil(L, -1)) {
            ...
            if (!lua_isnil(L, -1)) {
                ...
                luaA_Type stype = lua_tointeger(L, -1);
                lua_pop(L, 4);
                luaA_to_type(L, stype, (addOffset(offset, c_out), c_out), index);
                return;
            }

            lua_pop(L, 3);
            lua_pushfstring(
                    L, "luaA_struct_to_member: Member offset '%d' not registered for struct '%s'!",
                    offset, luaA_typename(L, type));
            lua_error(L);
        }
    ...
    void luaA_struct_to_member_name_type(lua_State *L, luaA_Type type, const char *member,
        ...
        if (!lua_isnil(L, -1)) {
            ...
            lua_pop(L, 3);
            lua_pushfstring(
                    L, "luaA_struct_to_member: Member name '%s' not registered for struct '%s'!",
                    member, luaA_typename(L, type));
            lua_error(L);
        }
    ...
    luaA_Type luaA_struct_typeof_member_offset_type(lua_State *L, luaA_Type type, size_t offset) {
        ...
        if (!lua_isnil(L, -1)) {
            ...
            lua_pushfstring(
                    L,
                    "luaA_struct_typeof_member: Member offset '%d' not registered for struct '%s'!",
                    offset, luaA_typename(L, type));
            lua_error(L);
        }
    ...
    luaA_Type luaA_struct_typeof_member_name_type(lua_State *L, luaA_Type type,
        ...
        if (!lua_isnil(L, -1)) {
            ...
            lua_pushfstring(
                    L,
                    "luaA_struct_typeof_member: Member name '%s' not registered for struct '%s'!",
                    member, luaA_typename(L, type));
            lua_error(L);
        }
    ...
    void luaA_struct_member_type(lua_State *L, luaA_Type type, const char *member, luaA_Type mtype,
        ...
        lua_pop(L, 2);
        lua_pushfstring(L, "luaA_struct_member: Struct '%s' not registered!",
                        luaA_typename(L, type));
        lua_error(L);
    }
...
*/
    ...
    bool luaA_enum_has_value_type(lua_State *L, luaA_Type type, const void *value) {
        ...
        lua_pop(L, 2);
        lua_pushfstring(L, "luaA_enum_has_value: Enum '%s' not registered!",
                        luaA_typename(L, type));
        lua_error(L);
        return false;
    }
...
*/
    ...
    static int luaA_call_entry(lua_State *L) {
        ...
        void *ret_data = addOffsetR(ret_ptr, ret_stack);
        // void *arg_data = arg_stack + arg_ptr;
        void *arg_data = addOffsetR(arg_ptr, arg_stack);
        ...
        for (int i = 0; i < arg_num; i++) {
            ...
            addOffset(luaA_typesize(L, arg_type), arg_pos);
        }
