...
namespace BaseEngine {
    ...
    namespace {
        ...
        bool RectBooleanPrivate(const TRect &first, TRect &second, TRect &third) {
            ...
            if (third.y + third.h > first.y + first.h) {
                ...
                if (used == 1) {
                    tmp = TRect(third.x, first.y + first.h, third.w,
                                (third.y + third.h) - (first.y + first.h));
                } else {
                    second = TRect(third.x, first.y + first.h, third.w,
                                   (third.y + third.h) - (first.y + first.h));
                }
            ...
            if (third.x + third.w >
                first.x + first.w)// this means that most of the right wont ever hit the target
            {
                switch (used) {
                    case 2:
                        return false;
                    ...
                    case 1:
                        tmp = TRect(first.x + first.w, third.y,
                                    (third.x + third.w) - (first.x + first.w), third.h);
                        break;

                    case 0:
                        second = TRect(first.x + first.w, third.y,
                                       (third.x + third.w) - (first.x + first.w), third.h);
                        break;
    ...
    bool RectHit(const TRect &first, const TRect &second) {
        ...
        return ((first.x > second.x - first.w && first.x < second.x + second.w) &&
                (first.y > second.y - first.h && first.y < second.y + second.h));
    }

    template<class TRect>
    bool RectIsInside(const TRect &inside, const TRect &thiss) {
        return (inside.x >= thiss.x && inside.y >= thiss.y &&
    ...
    bool RectIsInside(const TRect &inside, PType x, PType y) {
        return (inside.x <= x && inside.x + inside.w > x && inside.y <= y &&
                inside.y + inside.h > y);
    }
    ...
    void RectClampInside(TRect &inside, const TRect &borders) {
        ...
        if (inside.x + inside.w > borders.x + borders.w)
            inside.x = (borders.x + borders.w) - inside.w;
        if (inside.y + inside.h > borders.y + borders.h)
            inside.y = (borders.y + borders.h) - inside.h;
    }
	...
	*/
        ...
        if (active.y + active.h > first.y + first.h) {
            left_overs.push_back(TRect(active.x, first.y + first.h, active.w,
                                       (active.y + active.h) - (first.y + first.h)));
        ...
        if (active.x + active.w >
            first.x + first.w)// this means that most of the right wont ever hit the target
        {
            left_overs.push_back(TRect(first.x + first.w, active.y,
                                       (active.x + active.w) - (first.x + first.w), active.h));
	...
	*/
    ...
    void RectFindCrossPoints(const TRect &first, const TRect &second, std::vector<TPoint> &points) {
        ...
        // a1, a2 == b1, b3
        tmp_coord = RectFindCrossPointsLine(
                TPoint(first.x, first.y), TPoint(first.x + first.w, first.y),
                TPoint(second.x, second.y), TPoint(second.x, second.y + second.h));
        if (!tmp_coord.empty()) points.push_back(tmp_coord);

        // a1, a2 == b2, b4
        tmp_coord = RectFindCrossPointsLine(TPoint(first.x, first.y),
                                            TPoint(first.x + first.w, first.y),
                                            TPoint(second.x + second.w, second.y),
                                            TPoint(second.x + second.w, second.y + second.h));
        if (!tmp_coord.empty()) points.push_back(tmp_coord);

        // a3, a4 == b1, b3
        tmp_coord = RectFindCrossPointsLine(
                TPoint(first.x, first.y + first.h), TPoint(first.x + first.w, first.y + first.h),
                TPoint(second.x, second.y), TPoint(second.x, second.y + second.h));
        if (!tmp_coord.empty()) points.push_back(tmp_coord);

        // a3, a4 == b2, b4
        tmp_coord = RectFindCrossPointsLine(TPoint(first.x, first.y + first.h),
                                            TPoint(first.x + first.w, first.y + first.h),
                                            TPoint(second.x + second.w, second.y),
                                            TPoint(second.x + second.w, second.y + second.h));
        if (!tmp_coord.empty()) points.push_back(tmp_coord);
        ...
        // a1, a3 == b1, b2
        tmp_coord = RectFindCrossPointsLine(
                TPoint(first.x, first.y), TPoint(first.x, first.y + first.h),
                TPoint(second.x, second.y), TPoint(second.x + second.w, second.y));
        if (!tmp_coord.empty()) points.push_back(tmp_coord);

        // a1, a3 == b3, b4
        tmp_coord = RectFindCrossPointsLine(TPoint(first.x, first.y),
                                            TPoint(first.x, first.y + first.h),
                                            TPoint(second.x, second.y + second.h),
                                            TPoint(second.x + second.w, second.y + second.h));
        if (!tmp_coord.empty()) points.push_back(tmp_coord);

        // a2, a4 == b1, b2
        tmp_coord = RectFindCrossPointsLine(
                TPoint(first.x + first.w, first.y), TPoint(first.x + first.w, first.y + first.h),
                TPoint(second.x, second.y), TPoint(second.x + second.w, second.y));
        if (!tmp_coord.empty()) points.push_back(tmp_coord);

        // a2, a4 == b3, b4
        tmp_coord = RectFindCrossPointsLine(TPoint(first.x + first.w, first.y),
                                            TPoint(first.x + first.w, first.y + first.h),
                                            TPoint(second.x, second.y + second.h),
                                            TPoint(second.x + second.w, second.y + second.h));
        if (!tmp_coord.empty()) points.push_back(tmp_coord);
    ...
    TPoint RectFindCrossPointsLine(const TPoint &first_begin, const TPoint &first_end,
                                   const TPoint &second_begin, const TPoint &second_end) {
        TPoint return_value;

        // First we'll check if we hit each other or not
        // in the horizontal sense
        if (first_begin.x <= second_begin.x && first_end.x >= second_end.x &&
            second_begin.y <= first_begin.y && second_end.y >= first_end.y) {
...
namespace BaseEngine {
    namespace easing {
        ...
        float Elastic::impl_easeIn(float t, float b, float c, float d) {
            ...
            float postFix =
                    a *
                    powf(2, 10 * (t -= 1));// this is a fix, again, with post-increment operators
	...
	*/
        ...
        float PetriHacks::LittleBackAndForth::f(float x) {
            return ((pow(x + 0.17f, 1.3f) * sin(1.f / (x + 0.17f))) * 1.043f) + 0.035f;
        }

        float PetriHacks::DimishingShakeBack::f(float x) {
            return ((cos(x * 3.1415f) + 1) * 0.5f) * sin(x * 30.0f);
        }

        float PetriHacks::DimishingShake::f(float x) {
            return ((cos(x * 3.1415f) + 1) * 0.5f) * sin(x * 30.0f) + x;
        }
