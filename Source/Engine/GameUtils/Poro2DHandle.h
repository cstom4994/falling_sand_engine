...
namespace BaseEngine {
    ...
    bool CircleRectCollide(const VectorT &circle_p, float circle_r, const RectT &rect) {
        if (BaseEngine::RectHit(RectT(circle_p.x - circle_r, circle_p.y - circle_r, circle_r * 2.f,
                                      circle_r * 2.f),
                                rect) == false) {
            return false;
        }
        ...
        for (std::size_t i = 0; i < rect_p.size(); ++i) {
            dist = BaseEngine::math::DistanceFromLineSquared(rect_p[i], rect_p[(i + 1) % 4],
                                                             circle_p);
    ...
    VectorT CircleRectResolveByPushingCircle(const VectorT &circle_p, float circle_r,
        ...
        for (std::size_t i = 0; i < rect_p.size(); ++i) {
            point = BaseEngine::math::ClosestPointOnLineSegment(rect_p[i], rect_p[(i + 1) % 4],
                                                                circle_p);
...
namespace BaseEngine {
    ...
    bool CircleCircleCollide(const VectorT &p1, float r1, const VectorT &p2, float r2) {
        ...
        if (delta.LengthSquared() <= BaseEngine::math::Square(r1 + r2)) { return true; }

        return false;
    }

    template<class VectorT>
    VectorT CircleCircleResolveByPushingCircle(const VectorT &push_me_pos, float push_me_r,
                                               const VectorT &immovable_pos, float immovable_r) {
        const float extra_push = 0.1f;
