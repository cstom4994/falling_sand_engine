...
namespace dukpp {
    ...
    struct variadic_args
    {
        ...
        duk_value &operator[](va_t::size_type const pos) noexcept { return args[static_cast<size_t>(pos)]; }
...
namespace dukpp {
    ...
    std::vector<T> duk_value::as_array() const {
        ...
        for (duk_size_t i = 0; i < len; i++) {
            ...
            vec.push_back(types::DukType<typename types::Bare<T>::type>::template read<T>(mContext, elem_idx));
            duk_pop(mContext);
        }
    ...
    std::map<std::string, T> duk_value::as_map() const {
        ...
        while (duk_next(mContext, -1, 1)) {
            map[duk_safe_to_string(mContext, -2)] = types::DukType<typename types::Bare<T>::type>::template read<T>(mContext, -1);
            duk_pop_2(mContext);
        }
...
namespace dukpp::detail {
    ...
    get_stack_values_helper(duk_context *ctx, detail::index_tuple<Indexes...>) {
        return std::forward_as_tuple(types::DukType<typename types::Bare<Args>::type>::template read<typename types::ArgStorage<Args>::type>(ctx, Indexes)...);
    }
...
namespace dukpp {
    ...
    struct _class
    {
        ...
        _class &add_base_class() {
            static_assert(!std::is_pointer<BaseCls>::value && !std::is_pointer<Cls>::value && !std::is_const<BaseCls>::value && !std::is_const<Cls>::value, "Use bare class names.");
        ...
        _class &add_method(char const *name,
                           typename std::conditional<isConst, RetType (Cls::*)(Ts...) const, RetType (Cls::*)(Ts...)>::type method) {
        ...
        register_property(typename std::conditional<isConstGetter, RetT (Cls::*)() const,
            ...
            duk_uint_t flags = DUK_DEFPROP_HAVE_GETTER | DUK_DEFPROP_HAVE_SETTER | DUK_DEFPROP_HAVE_CONFIGURABLE// set not configurable (from JS)
