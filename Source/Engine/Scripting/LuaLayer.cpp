...
#define LUACON_WARN_PREF "[warn_]"
    ...
    struct LuaConsole
    {
        ...
        static char *Strdup(const char *str) {
            ...
            void *buf = METAENGINE_MALLOC(len);
            IM_ASSERT(buf);
            ...
            if (show_script_editor) {
                ...
                if (ImGui::Begin("Script Editor", &show_script_editor)) {
                    ...
                    if (ImGui::SmallButton("Clear")) {
                        ...
                        memset((void *) text, 0, IM_ARRAYSIZE(text));
                    }
                    ImGui::SameLine();
    ...
    static std::string nd_print(lua_State *L, const char *channel) {
        ...
        s_lua_layer->printToLuaConsole(L, (std::string(channel) + s).c_str());
        return s;
    }
    ...
    static int byteCodeWriterCallback(lua_State* L, const void* p, size_t sz, void* ud)
    {
        if (coroutinesByteCode)
            METAENGINE_FREE(coroutinesByteCode);
        coroutinesByteCode = (char*)METAENGINE_MALLOC(sz);
