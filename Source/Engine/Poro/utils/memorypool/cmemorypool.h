...
#elif defined(METADOT_PLATFORM_IPHONE)
#include <malloc/malloc.h>// mac os x
#else
...
namespace CEngine {
    ...
    public:
        CSimpleMemoryPool()
            : myMemoryBlock(NULL), myAllocatedMemoryBlock(NULL), myFreeMemoryBlock(NULL),
              myUnitSize(sizeof(T)), myBlockSize(number_of_units * (sizeof(T) + sizeof(Unit))),
            ...
            if (myMemoryBlock != NULL) {
                for (size_type i = 0; i < number_of_units; i++) {
                    Unit *cur_unit =
                            (Unit *) ((char *) myMemoryBlock + i * (myUnitSize + sizeof(Unit)));

                    cur_unit->prev = NULL;
                    cur_unit->next = myFreeMemoryBlock;

                    if (myFreeMemoryBlock != NULL) { myFreeMemoryBlock->prev = cur_unit; }
                    myFreeMemoryBlock = cur_unit;
                }
            }
        }

        ~CSimpleMemoryPool() { free(myMemoryBlock); }
        ...
        void *GetMem(size_type sizeo) {
            ...
            if (myFreeMemoryBlock) { myFreeMemoryBlock->prev = NULL; }

            cur_unit->next = myAllocatedMemoryBlock;

            if (myAllocatedMemoryBlock) { myAllocatedMemoryBlock->prev = cur_unit; }
        ...
        void Free(void *p) {
            if (HasPointer(p)) {
                ...
                if (myAllocatedMemoryBlock) { myAllocatedMemoryBlock->prev = NULL; }

                cur_unit->next = myFreeMemoryBlock;
                if (myFreeMemoryBlock) { myFreeMemoryBlock->prev = cur_unit; }

                myFreeMemoryBlock = cur_unit;
                if (myNumOfAllocatedObjects > 0) myNumOfAllocatedObjects--;
            } else {
                free(p);
                // delete p;
            }
        ...
        bool IsEmpty() const { return (myNumOfAllocatedObjects == 0); }

        bool IsFull() const { return (myFreeMemoryBlock == NULL); }
    ...
    public:
        ...
        CMemoryPoolForObjects() : myPools(), myFreePool(NULL), myCleanThePoolCount(0) {
            myPools.push_back(new PoolType);
            myFreePool = myPools[0];
        }

        ~CMemoryPoolForObjects() { Clear(); }

        void Clear() {
            for (unsigned int i = 0; i < myPools.size(); ++i) delete myPools[i];
            myPools.clear();
            myFreePool = NULL;
