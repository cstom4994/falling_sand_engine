...
#define SDL_METAENGINE_Render_CIRCLE_SEGMENT_ANGLE_FACTOR 0.625f


#define CALCULATE_CIRCLE_DT_AND_SEGMENTS(radius) \
	dt = SDL_METAENGINE_Render_CIRCLE_SEGMENT_ANGLE_FACTOR/sqrtf(radius);  /* s = rA, so dA = ds/r.  ds of 1.25*sqrt(radius) is good */ \
...
static float SetLineThickness(METAENGINE_Render_Renderer* renderer, float thickness)
{
	...
	#ifndef SDL_METAENGINE_Render_SKIP_LINE_WIDTH
...
static void Arc(METAENGINE_Render_Renderer* renderer, METAENGINE_Render_Target* target, float x, float y, float radius, float start_angle, float end_angle, SDL_Color color)
{
    ...
    dt = ((end_angle - start_angle)/360)*(SDL_METAENGINE_Render_CIRCLE_SEGMENT_ANGLE_FACTOR/sqrtf(outer_radius));  // s = rA, so dA = ds/r.  ds of 1.25*sqrt(radius) is good, use A in degrees.
...
static void ArcFilled(METAENGINE_Render_Renderer* renderer, METAENGINE_Render_Target* target, float x, float y, float radius, float start_angle, float end_angle, SDL_Color color)
{
    ...
    dt = ((end_angle - start_angle)/360)*(SDL_METAENGINE_Render_CIRCLE_SEGMENT_ANGLE_FACTOR/sqrtf(radius));  // s = rA, so dA = ds/r.  ds of 1.25*sqrt(radius) is good, use A in degrees.
...
static void SectorFilled(METAENGINE_Render_Renderer* renderer, METAENGINE_Render_Target* target, float x, float y, float inner_radius, float outer_radius, float start_angle, float end_angle, SDL_Color color)
{
    ...
    t = start_angle;
    dt = ((end_angle - start_angle)/360)*(SDL_METAENGINE_Render_CIRCLE_SEGMENT_ANGLE_FACTOR/sqrtf(outer_radius)) * DEG_PER_RAD;  // s = rA, so dA = ds/r.  ds of 1.25*sqrt(radius) is good, use A in degrees.
