...
#include <string>
    ...
    template <typename T> Runtime::Inspect::parameter Runtime::Inspect::parameter::make_parameter(std::string name) {
    ...
    template <typename ... T>
    Runtime::Inspect::function_cmd<Ret, FA...>::converter<T...>::converter(int n) {}

    template <typename Ret, typename ... FA>
    template <typename ... T>
    void Runtime::Inspect::function_cmd<Ret, FA...>::converter<T...>::for_each(std::function<void(const parameter&)> f) {
    }

    template <typename Ret, typename ... FA>
    template <typename ... T>
    template <typename ... A>
    std::string Runtime::Inspect::function_cmd<Ret, FA...>::converter<T...>::call(std::queue<std::string> s, std::function<Ret(FA...)> func, A ... args) const {
        try {
    ...
    Runtime::Inspect::function_cmd<Ret, FA...>::converter<Current, Next...>::converter(int n) : argN(n), next(n + 1) {}

    template <typename Ret, typename ... FA>
    template <typename Current, typename ... Next>
    void Runtime::Inspect::function_cmd<Ret, FA...>::converter<Current, Next...>::for_each(std::function<void(const parameter&)> f) {
    ...
    template <typename Current, typename ... Next>
    template <typename ... A>
    std::string Runtime::Inspect::function_cmd<Ret, FA...>::converter<Current, Next...>::call(std::queue<std::string> stack, std::function<Ret(FA...)> func, A ... args) const {
    ...
    template <typename Ret, typename ... FA>
    Runtime::Inspect::function_cmd<Ret, FA...>::function_cmd(std::string name, std::function<Ret(FA...)> func) : i_cmd(name), func(func), conv(1) {
        conv.for_each(
    ...
    template <typename Ret, typename ... FA>
    Runtime::Inspect::i_cmd::form Runtime::Inspect::function_cmd<Ret, FA...>::get_form() const {
        return i_cmd::function;
    }

    template <typename Ret, typename ... FA>
    std::string Runtime::Inspect::function_cmd<Ret, FA...>::get_return_type() const {
        return translator<Ret>::name();
    }

    template <typename Ret, typename ... FA>
    std::string Runtime::Inspect::function_cmd<Ret, FA...>::call(std::queue<std::string> q) const {
        if (q.size() != sizeof...(FA))
...
#include <limits>
    ...
    namespace {
        ...
        Runtime::Inspect::function_cmd<R, T...>* mk_function_cmd(std::string name, F<R(T...)> func)
        {
            return new Runtime::Inspect::function_cmd<R, T...>(name, func);
        }
    }

    template <typename R, typename ... T>
    void Runtime::Inspect::service::provide_command(std::string name, R(*funcPtr)(T...)) {
        std::function<R(T...)> func(funcPtr);
        commands[name] = new function_cmd<R, T...>(name, func);
    }

    template <typename C, typename R, typename ... T>
    void Runtime::Inspect::service::provide_command(std::string name, R(C::* funcPtr)(T...), C* object) {
    ...
    template <typename L>
    void Runtime::Inspect::service::provide_command(std::string name, L lambda) {
        auto func = to_function(lambda);
        commands[name] = mk_function_cmd(name, func);
    }

    template <typename T>
    void Runtime::Inspect::service::provide_value(std::string name, T& var) {
    ...
    template <typename T> Runtime::Inspect::variable_cmd<T>::variable_cmd(std::string name, T& var) : i_cmd(name), variable(var) {
    }

    template <typename T> Runtime::Inspect::i_cmd::form Runtime::Inspect::variable_cmd<T>::get_form() const {
        return i_cmd::variable;
    }

    template <typename T> std::string Runtime::Inspect::variable_cmd<T>::get_return_type() const {
        return translator<T>::name();
    }

    template <typename T> std::string Runtime::Inspect::variable_cmd<T>::call(std::queue<std::string> q) const {
        if (q.empty())
    ...
    template <typename T> Runtime::Inspect::variable_cmd<const T>::variable_cmd(std::string name, const T& var) : i_cmd(name), variable(var) {
    }

    template <typename T> Runtime::Inspect::i_cmd::form Runtime::Inspect::variable_cmd<const T>::get_form() const {
        return i_cmd::variable;
    }

    template <typename T> std::string Runtime::Inspect::variable_cmd<const T>::get_return_type() const {
        return translator<const T>::name();
    }

    template <typename T> std::string Runtime::Inspect::variable_cmd<const T>::call(std::queue<std::string> q) const {
        if (q.empty())
            return Runtime::Inspect::to_string<T>(variable);
