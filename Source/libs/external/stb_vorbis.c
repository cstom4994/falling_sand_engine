...
static void crc32_init(void) {
    ...
    uint32 s;
    for (i = 0; i < 256; i++) {
        for (s = (uint32)i << 24, j = 0; j < 8; ++j) s = (s << 1) ^ (s >= (1U << 31) ? CRC32_POLY : 0);
        crc_table[i] = s;
    }
}
...
static void compute_window(int n, float *window) {
    int n2 = n >> 1, i;
    for (i = 0; i < n2; ++i) window[i] = (float)sin(0.5 * M_PI * square((float)sin((i - 0 + 0.5) / n2 * 0.5 * M_PI)));
}
...
static float inverse_db_table[256] = {1.0649863e-07f, 1.1341951e-07f, 1.2079015e-07f, 1.2863978e-07f, 1.3699951e-07f, 1.4590251e-07f, 1.5538408e-07f, 1.6548181e-07f,
...
static int residue_decode(vorb *f, Codebook *book, float *target, int offset, int n, int rtype) {
    ...
    if (rtype == 0) {
        ...
        for (k = 0; k < step; ++k)
            if (!codebook_decode_step(f, book, target + offset + k, n - offset - k, step)) return FALSE;
    } else {
        for (k = 0; k < n;) {
...
void inverse_mdct_slow(float *buffer, int n, vorb *f, int blocktype) {
    ...
    for (i = 0; i < n; ++i) {
        float acc = 0;
        for (j = 0; j < n2; ++j) acc += x[j] * mcos[(2 * i + 1 + n2) * (2 * j + 1) & nmask];
        buffer[i] = acc;
    }
    free(x);
}
...
void dct_iv_slow(float *buffer, int n) {
    ...
    for (i = 0; i < n; ++i) {
        float acc = 0;
        for (j = 0; j < n; ++j) acc += x[j] * mcos[((2 * i + 1) * (2 * j + 1)) & nmask];
        buffer[i] = acc;
    }
}
...
static void inverse_mdct(float *buffer, int n, vorb *f, int blocktype) {
    ...
    for (; l < (ld - 3) >> 1; ++l) {
        ...
        int lim = 1 << (l + 1);
        int i;
        for (i = 0; i < lim; ++i) imdct_step3_inner_r_loop(n >> (l + 4), u, n2 - 1 - k0 * i, -k0_2, A, 1 << (l + 3));
    }
...
static int vorbis_decode_packet_rest(vorb *f, int *len, Mode *m, int left_start, int left_end, int right_start, int right_end, int *p_left) {
    ...
    for (i = 0; i < f->channels; ++i) {
        if (really_zero_channel[i]) {
            ...
            for (j = 0; j < n2; ++j) f->channel_buffers[i][j] *= f->floor_buffers[i][j];
        }
    }
#endif
    ...
    // INVERSE MDCT
    CHECK(f);
    for (i = 0; i < f->channels; ++i) inverse_mdct(f->channel_buffers[i], n, f, m->blockflag);
    CHECK(f);
...
static int vorbis_finish_frame(stb_vorbis *f, int len, int left, int right) {
    ...
    if (f->previous_length) {
        ...
        for (i = 0; i < f->channels; ++i) {
            for (j = 0; j < n; ++j) f->channel_buffers[i][left + j] = f->channel_buffers[i][left + j] * w[j] + f->previous_window[i][j] * w[n - 1 - j];
        }
    }
    ...
    for (i = 0; i < f->channels; ++i)
        for (j = 0; right + j < len; ++j) f->previous_window[i][j] = f->channel_buffers[i][right + j];
...
static int start_decoder(vorb *f) {
    ...
    for (i = 0; i < f->codebook_count; ++i) {
        ...
        if (c->sparse && total >= c->entries >> 2) {
            ...
            if (c->entries > (int)f->setup_temp_memory_required) f->setup_temp_memory_required = c->entries;
    ...
    for (i = 0; i < f->floor_count; ++i) {
        ...
        if (f->floor_types[i] == 0) {
            ...
            for (j = 0; j < g->number_of_books; ++j) g->book_list[j] = get_bits(f, 8);
            ...
            for (j = 0; j < g->values; ++j) g->sorted_order[j] = (uint8)p[j].y;
    ...
    if (integer_divide_table[1][1] == 0)
        for (i = 0; i < DIVTAB_NUMER; ++i)
            for (j = 1; j < DIVTAB_DENOM; ++j) integer_divide_table[i][j] = i / j;
#endif
    ...
    if (f->alloc.alloc_buffer) {
        ...
        if (f->setup_offset + sizeof(*f) + f->temp_memory_required > (unsigned)f->temp_offset) return error(f, VORBIS_outofmem);
    }
...
static void vorbis_deinit(stb_vorbis *p) {
    ...
    if (p->residue_config) {
        for (i = 0; i < p->residue_count; ++i) {
            ...
            if (r->classdata) {
                for (j = 0; j < p->codebooks[r->classbook].entries; ++j) setup_free(p, r->classdata[j]);
    ...
    if (p->mapping) {
        for (i = 0; i < p->mapping_count; ++i) setup_free(p, p->mapping[i].chan);
...
int stb_vorbis_seek_frame(stb_vorbis *f, unsigned int sample_number) {
    ...
    while (f->current_loc < sample_number) {
        ...
        if (!peek_decode_initial(f, &left_start, &left_end, &right_start, &right_end, &mode)) return error(f, VORBIS_seek_failed);
...
void stb_vorbis_seek_start(stb_vorbis *f) {
    if (IS_PUSH_MODE(f)) {
        error(f, VORBIS_invalid_api_mixing);
        return;
    }
...
float stb_vorbis_stream_length_in_seconds(stb_vorbis *f) { return stb_vorbis_stream_length_in_samples(f) / (float)f->sample_rate; }
...
static void convert_samples_short(int buf_c, short **buffer, int b_offset, int data_c, float **data, int d_offset, int samples) {
    ...
    if (buf_c != data_c && buf_c <= 2 && data_c <= 6) {
        ...
        for (i = 0; i < buf_c; ++i) compute_samples(channel_selector[buf_c][i], buffer[i] + b_offset, data_c, data, d_offset, samples);
    } else {
        int limit = buf_c < data_c ? buf_c : data_c;
        for (i = 0; i < limit; ++i) copy_samples(buffer[i] + b_offset, data[i] + d_offset, samples);
        for (; i < buf_c; ++i) memset(buffer[i] + b_offset, 0, sizeof(short) * samples);
    }
}
...
int stb_vorbis_get_samples_short_interleaved(stb_vorbis *f, int channels, short *buffer, int num_shorts) {
    ...
    while (n < len) {
        ...
        if (n + k >= len) k = len - n;
        if (k) convert_channels_short_interleaved(channels, buffer, f->channels, f->channel_buffers, f->channel_buffer_start, k);
        buffer += k * channels;
        n += k;
...
int stb_vorbis_get_samples_short(stb_vorbis *f, int channels, short **buffer, int len) {
    ...
    while (n < len) {
        int k = f->channel_buffer_end - f->channel_buffer_start;
        if (n + k >= len) k = len - n;
        if (k) convert_samples_short(channels, buffer, n, f->channels, f->channel_buffers, f->channel_buffer_start, k);
        n += k;
        f->channel_buffer_start += k;
        if (n == len) break;
        if (!stb_vorbis_get_frame_float(f, NULL, &outputs)) break;
...
int stb_vorbis_get_samples_float(stb_vorbis *f, int channels, float **buffer, int num_samples) {
    ...
    while (n < num_samples) {
        ...
        if (k) {
            for (i = 0; i < z; ++i) memcpy(buffer[i] + n, f->channel_buffers[i] + f->channel_buffer_start, sizeof(float) * k);
            for (; i < channels; ++i) memset(buffer[i] + n, 0, sizeof(float) * k);
        }
        n += k;
        f->channel_buffer_start += k;
...
/* Version history
    ...
    0.91 - conditional compiles to omit parts of the API and the infrastructure to support them: STB_VORBIS_NO_PULLDATA_API, STB_VORBIS_NO_PUSHDATA_API, STB_VORBIS_NO_STDIO,
   STB_VORBIS_NO_INTEGER_CONVERSION 0.90 - first public release
*/
