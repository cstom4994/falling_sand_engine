...
void output_ident(FILE *outputf, bool only_ident, struct keyval *parseid, struct keyval_list *IDENT) {
    ...
    for (ident_count = 0; ident_count < IDENT->count; ident_count++) {
        ...
        if (strcmp(ckv->key, "INHERIT") == 0) {
            fprintf(outputf, "\t\t\terror = css_stylesheet_style_inherit(result, %s);\n", parseid->val);
        } else {
            fprintf(outputf, "\t\t\terror = css__stylesheet_style_appendOPV(result, %s, %s);\n", parseid->val, ckv->val);
        }
...
void output_ident_list(FILE *outputf, struct keyval *parseid, struct keyval_list *kvlist) {
    ...
    fprintf(outputf,
            ...
            "\t\terror = css__stylesheet_style_append(result, %s);\n"
            "\t}\n\n",
            parseid->val, ckv->val, ikv->key, ckv->val, ikv->val);
}

void output_invalidcss(FILE *outputf) { fprintf(outputf, "{\n\t\terror = CSS_INVALID;\n\t}\n\n"); }
...
void output_wrap(FILE *outputf, struct keyval *parseid, struct keyval_list *WRAP) {
    ...
    fprintf(outputf, "	return %s(c, vector, ctx, result, %s);\n}\n", ckv->val, parseid->val);
}
...
int css_property_parser_gen(int argc, char **argv) {
    ...
    if (WRAP.count > 0) {
        ...
        if (IDENT.count > 0) output_ident(outputf, only_ident, base.item[0], &IDENT);

        if (URI.count > 0) output_uri(outputf, base.item[0], &URI);

        if (NUMBER.count > 0) output_number(outputf, base.item[0], &NUMBER);
