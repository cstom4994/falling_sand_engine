...
static lwc_error lwc__initialise(void)
{
    ...
    ctx = (lwc_context*)LWC_ALLOC(sizeof(lwc_context));
    ...
    ctx->buckets = (lwc_string**)LWC_ALLOC(sizeof(lwc_string*) * ctx->bucketcount);
...
static lwc_error lwc__intern(const char* s, size_t slen,
    lwc_string** ret,
    lwc_hasher hasher,
    lwc_strncmp compare,
    lwc_memcpy copy)
{
    ...
    *ret = str = (lwc_string*)LWC_ALLOC(sizeof(lwc_string) + slen + 1);
...
static lwc_hash lwc__calculate_hash(const char* str, size_t len)
{
...
static void* lwc__lcase_memcpy(void* _target, const void* _source, size_t n)
{
    char* target = _target;
