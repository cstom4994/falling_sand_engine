...
#define IS_PUNCT_OR_SPACE(x) (!(((x) >= 'A' && (x) <= 'Z') || ((x) >= 'a' && (x) <= 'z') || ((x) >= '0' && (x) <= '9')))
...
parserutils_charset_aliases_canon *parserutils__charset_alias_canonicalise(const char *alias, size_t len) {
    ...
    c = (parserutils_charset_aliases_alias *)bsearch(&s, &charset_aliases[0], charset_aliases_count, sizeof(parserutils_charset_aliases_alias), parserutils_charset_alias_match);
...
const char *parserutils_charset_mibenum_to_name(uint16_t mibenum) {
    ...
    for (i = 0; i < charset_aliases_canon_count; ++i) {
        ...
        if (c->mib_enum == mibenum) return c->name;
    }
 ...
 */
bool parserutils_charset_mibenum_is_unicode(uint16_t mibenum) { return MIBENUM_IS_UNICODE(mibenum); }
