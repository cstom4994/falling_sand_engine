...
#include "call_x64.h"
#include "dynasm/dasm_x86.h"
#elif defined __arm__ || defined __arm || defined __ARM__ || defined __ARM || defined ARM ||       \
        defined _ARM_ || defined ARMV4I || defined _M_ARM
#include "call_arm.h"
...
void free_code(struct jit *jit, lua_State *L, cfunction func) {
    ...
    for (i = 0; i < jit->pagenum; i++) {
        ...
        if ((uint8_t *) h < (uint8_t *) p || (uint8_t *) p + p->size <= (uint8_t *) h) { continue; }
        ...
        FreePage(p, p->size);
        memmove(&jit->pages[i], &jit->pages[i + 1],
                (jit->pagenum - (i + 1)) * sizeof(jit->pages[0]));
        jit->pagenum--;
        return;
    }
