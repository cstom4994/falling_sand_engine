...
int path_is_slash(char c) { return (c == '/') | (c == '\\'); }
...
int path_pop_ext(const char* path, char* out, char* ext) {
    ...
    if (last_period != NULL && last_period > last_slash) {
        if (ext != NULL) CUTE_PATH_STRNCPY(ext, last_period + 1, CUTE_PATH_MAX_EXT);
    } else {
        last_period = p;
    }
...
int path_pop(const char* path, char* out, char* pop) {
    ...
    while (!path_is_slash(*--path) && pop_len != total_len) ++pop_len;
...
#define CUTE_PATH_EXPECT(X)                                                                            \
    do {                                                                                               \
        if (!(X)) printf("Failed cute_path.h unit test at line %d of file %s.\n", __LINE__, __FILE__); \
    } while (0)
