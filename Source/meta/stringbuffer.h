...
public:
    ...
    BasicStringBuffer(const std::string& str) : StreamType((std::streambuf*)this), data(str.begin(), str.end()), src(nullptr), num(), inputInitialized(false) {}
    BasicStringBuffer(const std::string& str, std::istream& source) : StreamType((std::streambuf*)this), data(str.begin(), str.end()), src(&source), num(), inputInitialized(false) {}

    virtual ~BasicStringBuffer() {}

    /// Append text as-is using the += operator
    inline void operator+=(const char& c) { data.push_back(c); }
    template <size_t N>
    inline void operator+=(const char (&str)[N]) {
        data.insert(data.end(), &str[0], &str[N - 1]);
    }
    inline void operator+=(const char* str) { data.insert(data.end(), str, str + strlen(str) * sizeof(char)); }
    inline void operator+=(const std::string& str) { data.insert(data.end(), str.begin(), str.end()); }
    ...
    template <size_t N>
    inline BasicStringBuffer& operator<<(const char (&str)[N]) {
        data.insert(data.end(), &str[0], &str[N - 1]);
        return *this;
    }
    ...
    inline BasicStringBuffer& appendNumber(const T& value) {
        if constexpr (std::is_same_v<bool, T>) {
            if (auto [p, ec] = std::to_chars((char*)num.data(), (char*)num.data() + num.size(), (int)value); ec == std::errc()) data.insert(data.end(), num.data(), p);
    ...
    template <typename T>
    inline BasicStringBuffer& operator<<(const T& t) {
        syncOutput();
        (std::ostream&)(*this) << t;
        return *this;
    }
    ...
    inline const char* c_str(bool addNulTerminator = true) {
        if (addNulTerminator) nullTerminate();

        return &(*this)[0];
    }

    inline std::string str() const { return std::string(data.begin(), data.end()); }

    inline const std::vector<char>& vec() const { return data; }
...
protected:
    ...
    inline bool syncInput() {
        ...
        } else if (char* start = &data[0]; gptr == nullptr || gptr == egptr || (start != eback || start + size > egptr))  // Changes are required
        {
            ((std::istream*)this)->clear();
    ...
    virtual std::streambuf::pos_type seekoff(std::streambuf::off_type off, std::ios_base::seekdir dir, std::ios_base::openmode which = std::ios_base::in | std::ios_base::out) {
        ...
        if (repositionRead || repositionWrite) {
            if (repositionWrite) syncOutput();

            if (repositionRead) syncInput();
            ...
            if ((gptr() != nullptr || !repositionRead) && (pptr() != nullptr || !repositionWrite)) {
                ...
                if (repositionRead) {
                    ...
                    if (newInputOffset + off < 0 || &data[0] + newInputOffset + off > &data[0] + data.size() * sizeof(char)) return std::streambuf::pos_type(std::streambuf::off_type(-1));
                }
                ...
                if (repositionWrite) {
                    ...
                    if (newOutputOffset + off < 0 || &data[0] + newOutputOffset + off > epptr()) return std::streambuf::pos_type(std::streambuf::off_type(-1));
                }

                if (repositionRead) setg(&data[0], &data[0] + newInputOffset + off, &data[0] + data.size() * sizeof(char));

                if (repositionWrite) setp(&data[0] + newOutputOffset + off, &data[0] + data.size() * sizeof(char));
...
public:
    ...
    BasicStringBuffer(const std::string& str) : std::istream((std::streambuf*)this), std::ostream((std::streambuf*)this), data(str.begin(), str.end()), src(nullptr), num(), inputInitialized(false) {}
    BasicStringBuffer(const std::string& str, std::istream& source)
        : std::istream((std::streambuf*)this), std::ostream((std::streambuf*)this), data(str.begin(), str.end()), src(&source), num(), inputInitialized(false) {}

    virtual ~BasicStringBuffer() {}

    inline void operator<<(std::streambuf* sb) { (std::ostream&)(*this) << sb; }
...
static std::ostream& operator<<(std::ostream& os, BasicStringBuffer<std::ostream>& sb) { return sb.size() > 0 ? os.write(&sb[0], sb.size()) : os; }

static std::ostream& operator<<(std::ostream& os, BasicStringBuffer<std::iostream>& sb) { return sb.size() > 0 ? os.write(&sb[0], sb.size()) : os; }
...
public:
    ...
    inline StringBuffer* operator->() const { return sb.get(); }
...
public:
    ...
    inline OStringBuffer* operator->() const { return sb.get(); }
...
public:
    ...
    inline IStringBuffer* operator->() const { return sb.get(); }
