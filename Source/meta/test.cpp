...
const std::unordered_map<std::string, A::TestEnum> A::TestEnumCache = {{"first", A::TestEnum::first}, {"second", A::TestEnum::second}};

const std::unordered_map<std::string, A::TestEnum> A::TestEnumCacheCustom = {{"firstCustom", A::TestEnum::first}, {"secondCustom", A::TestEnum::second}};

json::OutStreamType &operator<<(json::OutStreamType &os, const A::TestEnum &testEnum) {
    switch (testEnum) {
        case A::TestEnum::first:
            json::Put::String(os, "first");
            break;
        case A::TestEnum::second:
            json::Put::String(os, "second");
            break;
    }
    return os;
}
...
std::istream &operator>>(std::istream &is, A::TestEnum &testEnum) {
    ...
    if (found != A::TestEnumCache.end()) testEnum = found->second;
...
class ReflTestClass {
public:
    ReflTestClass() : f1(0.0f), f2(0.0f) {
        a1[0] = 0.0f;
        a1[1] = 0.0f;
    }
    ReflTestClass(float capacity, float currentLevel, float tickMarkOne, float tickMarkTwo) : f1(capacity), f2(currentLevel) {
        a1[0] = tickMarkOne;
        a1[1] = tickMarkTwo;
    }
...
class Wheel {
public:
    enum_t(Rim, short, {Regular = 0, Spinner = 1});
...
public:
    ...
    Car(Wheel frontLeft, Wheel frontRight, Wheel backLeft, Wheel backRight, const std::string &driver, const std::string &passenger, const std::vector<std::vector<std::vector<std::string>>> &testNest,
        const std::map<std::string, std::vector<std::vector<std::string>>> &testMapNest, const std::map<std::string, std::string> &occupantId,
        std::map<std::string, std::shared_ptr<CupHolder>> occupantCupHolderUsage, const std::vector<std::shared_ptr<CupHolder>> &cupHolders, const ReflTestClass &fuelTank, float milesPerGallon)
        : testNest(testNest),
          testMapNest(testMapNest),
          occupantId(occupantId),
          occupantCupHolderUsage(occupantCupHolderUsage),
    ...
    ReflTestClass fuelTank;
    float milesPerGallon;

    REFLECT(Car, wheels, occupants, testNest, testMapNest, occupantId, occupantCupHolderUsage, cupHolders, fuelTank, milesPerGallon)
};
...
public:
    ...
    REFLECT(MassiveObject, f001, f002, f003, f004, f005, f006, f007, f008, f009, f010, f011, f012, f013, f014, f015, f016, f017, f018, f019, f020, f021, f022, f023, f024, f025, f026, f027, f028, f029,
            f030, f031, f032, f033, f034, f035, f036, f037, f038, f039, f040, f041, f042, f043, f044, f045, f046, f047, f048, f049, f050, f051, f052, f053, f054, f055, f056, f057, f058, f059, f060,
            f061, f062, f063, f064, f065, f066, f067, f068, f069, f070, f071, f072, f073, f074, f075, f076, f077, f078, f079, f080, f081, f082, f083, f084, f085, f086, f087, f088, f089, f090, f091,
            f092, f093, f094, f095, f096, f097, f098, f099, f100, f101, f102, f103, f104, f105, f106, f107, f108, f109, f110, f111, f112, f113, f114, f115, f116, f117, f118, f119, f120, f121, f122,
            f123, f124, f125
...
Car outputExamples() {
    ...
    SubTest::Supers::ForEach(sub, [&](auto superInfo, auto &superObj) {
        ...
        superInfo.template forEach<json::Name>([&](auto &jsonName) { std::cout << "jsonName: " << jsonName.value << std::endl; });
        ...
        if constexpr (SuperInfo::template HasAnnotation<json::Name>) std::cout << superInfo.template getAnnotation<json::Name>().value << std::endl;

        using Super = typename decltype(superInfo)::Type;
        std::cout << decltype(superInfo)::Index << ": " << TypeToStr<Super>() << " {" << std::endl;
        Super::Class::ForEachField(superObj, [&](auto &field, auto &value) { std::cout << "  " << field.name << ": " << value << std::endl; });
        std::cout << "}" << std::endl;
    });
    ...
    std::vector<std::vector<std::vector<std::string>>> testNest = {{{"a", "b", "c"}, {"d", "e"}}, {{"f", "g", "h", "i"}, {"j"}}};
    std::map<std::string, std::vector<std::vector<std::string>>> testMapNest = {{"one", {{"a", "b", "c"}, {"d", "e"}}}, {"two", {{"f", "g"}, {"h"}}}};
    ...
    ReflTestClass fuelTank(15.0f, 14.6f, 1.0f, 7.5f);
    ...
    for (size_t i = 0; i < Wheel::Class::TotalFields; i++) std::cout << Wheel::Class::Fields[i].name << std::endl;

    for (size_t i = 0; i < Wheel::Class::TotalFields; i++) {
        Wheel::Class::FieldAt(frontLeft, i, [&](auto &field, auto &value) { std::cout << field.name << ": " << value << std::endl; });
    }

    Wheel::Class::ForEachField(backRight, [&](auto &field, auto &value) { std::cout << field.name << ": " << value << std::endl; });
    ...
    Car::Class::ForEachField(car, [&](auto &field, auto &value) {
        ...
        if constexpr (!ExtendedTypeSupport::is_iterable<Type>::value && !is_reflected<Type>::value)
            ...
            std::cout << "(carObject) " << field.name << ": "
                      << "[Skipped, this would be a good place for recursion!]" << std::endl;
            ...
            std::cout << "(carObjectIterable) " << field.name << ": "
                      << "[Skipped, this would be a good place for recursion!]" << std::endl;
    });

    ReflTestClass::Class::ForEachField(fuelTank, [&](auto &field, auto &value) {
...
enum class Status { Initial, Cached, Finished };
...
NOTE(State, Rest::Controller, Super<Point>(json::Name{"point"}), Super<Another>)
struct State : public Point, public Another {
    ...
    NOTE(status, json::Name{"code"}, json::EnumInt)
    static Status status;

    NOTE(update, Rest::Method::PATCH, Rest::Url{"/update/{value}/{otherValue}"}, Rest::PathParam<int>{"value"}, Rest::PathParam<long>{"otherValue"})
...
template <>
struct reflection::Proxy<ProxiedObjectMapping> : public ProxiedObjectMapping {
...
struct OwnedObjectModel {
    ...
    MAP_WITH(UnownedObjDao, (a, a), (b, b), (c, d), (d, e))
...
void objectMapperExamples() {
    ...
    for (auto &pair : objDao.d) std::cout << "objDao.d[]: { " << pair.first << ", " << pair.second << " }" << std::endl;
