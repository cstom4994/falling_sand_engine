...
struct vector_compatible_resize_traits {
    ...
    // If false, vector is a 100% compatible drop-in replacement for
...
public:
    ...
    explicit vector(const Allocator &alloc) noexcept : _data{0, alloc} {}

    explicit vector(size_type n, const Allocator &alloc = Allocator()) : vector(_allocate_uninitialized_tag{}, n, alloc) { _value_construct_range(begin(), end()); }

    vector(size_type n, const T &value, const Allocator &alloc = Allocator()) : vector(_allocate_uninitialized_tag{}, n, alloc) { _value_construct_range(begin(), end(), value); }

    template <typename InputIt, typename ItCat = typename std::iterator_traits<InputIt>::iterator_category>
    vector(InputIt b, InputIt e, const Allocator &alloc = Allocator()) : vector(b, e, alloc, ItCat{}) {}

    vector(std::initializer_list<T> lst, const Allocator &alloc = Allocator()) : vector(_allocate_uninitialized_tag{}, lst.size(), alloc) { _copy_construct_range(lst.begin(), lst.end(), begin()); }

    vector(const vector &other) : vector(_allocate_uninitialized_tag{}, other.size(), alloc_traits::select_on_container_copy_construction(other._allocator())) {
        _copy_construct_range(other.begin(), other.end(), begin());
    }

    template <typename OtherResizeTraits>
    vector(const vector<T, OtherResizeTraits, Allocator> &other) : vector(_allocate_uninitialized_tag{}, other.size(), alloc_traits::select_on_container_copy_construction(other._allocator())) {
        _copy_construct_range(other.begin(), other.end(), begin());
    }

    template <typename OtherResizeTraits>
    vector(const vector<T, OtherResizeTraits, Allocator> &other, const Allocator &alloc) : vector(_allocate_uninitialized_tag{}, other.size(), alloc) {
        _copy_construct_range(other.begin(), other.end(), begin());
    }

    vector(vector &&other) noexcept { _swap_with_allocator(std::move(other)); }

    template <typename OtherResizeTraits>
    vector(vector<T, OtherResizeTraits, Allocator> &&other) noexcept {
        _swap_with_allocator(std::move(other));
    }

    template <typename OtherResizeTraits>
    vector(vector<T, OtherResizeTraits, Allocator> &&other, const Allocator &alloc) noexcept : vector(alloc) {
        if constexpr (!alloc_traits::is_always_equal::value) {
            if (alloc != other._allocator()) {
                // Incompatible allocators.  Allocate new storage.
                auto replacement = vector(_allocate_uninitialized_tag{}, other.size(), alloc);
    ...
    ~vector() { _destroy(begin(), end()); }

    vector &operator=(const vector &other) { return _copy_assignment(other); }

    template <typename OtherResizeTraits>
    vector &operator=(const vector<T, OtherResizeTraits, Allocator> &other) {
        return _copy_assignment(other);
    }

    vector &operator=(vector &&other) noexcept(noexcept(alloc_traits::propagate_on_container_move_assignment::value || alloc_traits::is_always_equal::value)) {
        return _move_assignment(std::move(other));
    }

    template <typename OtherResizeTraits>
    vector &operator=(vector<T, OtherResizeTraits, Allocator> &&other) noexcept(noexcept(alloc_traits::propagate_on_container_move_assignment::value || alloc_traits::is_always_equal::value)) {
        return _move_assignment(std::move(other));
    }
    ...
    void assign(size_type count, const T &value) {
        if (count > capacity_full()) {
            _swap_without_allocator(vector(count, value, _allocator()));
        } else {
    ...
    reference at(size_type idx) {
        if (idx >= size()) {
            throw std::out_of_range("vector<T,ResizeTraits,Alloc>::at(" + std::to_string(idx) + ") out of range");
        }
        return (*this)[idx];
    }
    ...
    const_iterator cend() const noexcept { return _storage_begin() + _offset + size(); }
    ...
    void reserve(size_type front, size_type back) {
        if (front > capacity_front() || back > capacity_back()) {
            ...
            if (new_full_capacity > max_size()) {
                throw std::length_error("vector<T,ResizeTraits,Alloc>::reserve(" + std::to_string(front) + ", " + std::to_string(back) + ") exceeds max_size()");
            }
    ...
    // Resizes the vector, by adding or removing from the front.
    void resize_front(size_type count) { _resize_front(count); }

    void resize_front(size_type count, const T &value) { _resize_front(count, value); }

    // Resizes the vector, by adding or removing from the back.
    void resize_back(size_type count) { _resize_back(count); }

    void resize_back(size_type count, const T &value) { _resize_back(count, value); }
    ...
    template <typename OtherResizeTraits>
    void swap(vector<T, OtherResizeTraits, Allocator> &other) noexcept(noexcept(alloc_traits::propagate_on_container_swap::value || alloc_traits::is_always_equal::value)) {
        if constexpr (alloc_traits::propagate_on_container_swap::value) {
            ...
            if (_allocator() == other._allocator()) {
                ...
                auto new_this = vector(_allocate_uninitialized_tag{}, other.size(), _allocator());
                _nothrow_move_construct_range(other.begin(), other.end(), new_this.begin());

                auto new_other = vector(_allocate_uninitialized_tag{}, size(), other._allocator());
...
private:
    ...
    static constexpr auto _calls_default_constructor_directly = std::is_same_v<allocator_type, std::allocator<T>>;
    // Confirmation that allocator_traits will only directly call placement new(ptr)T(const T&)
    static constexpr auto _calls_copy_constructor_directly = std::is_same_v<allocator_type, std::allocator<T>>;
    // Confirmation that allocator_traits will only directly call ~T()
    static constexpr auto _calls_destructor_directly = std::is_same_v<allocator_type, std::allocator<T>>;
    ...
    struct Data : Allocator {
        ...
        Data() = default;
        Data(size_type size, const Allocator &alloc) : Allocator{alloc}, _storage{size ? std::allocator_traits<Allocator>::allocate(allocator(), size) : nullptr}, _allocated{size} {}
    ...
    vector(InputIt b, InputIt e, const Allocator &alloc, std::input_iterator_tag) : vector{} {
        for (; b != e; ++b) {
            push_back(*b);
        }
    }
    ...
    template <typename InputIt>
    vector(InputIt b, InputIt e, const Allocator &alloc, std::forward_iterator_tag) : vector(_allocate_uninitialized_tag{}, std::distance(b, e), alloc) {
    ...
    // Create an uninitialized empty vector, with specified storage params
    vector(_allocate_uninitialized_tag, size_type size, size_type allocated, size_type offset, const Allocator &alloc) : _size{size}, _offset{offset}, _data{allocated, alloc} {}

    // Create an uninitialized empty vector, with storage for expected size
    vector(_allocate_uninitialized_tag, size_type size, const Allocator &alloc) : vector(_allocate_uninitialized_tag{}, size, size, 0, alloc) {}

    // Create an uninitialized empty vector, with storage for expected reallocated size
    vector(_reallocate_uninitialized_tag, size_type size, const Allocator &alloc) : vector(_allocate_uninitialized_tag{}, size, _calc_reallocation(size), _calc_offset(size), alloc) {}

    static constexpr size_type _calc_reallocation(size_type size) { return size * _full_realloc::num / _full_realloc::den; }

    static constexpr size_type _calc_offset(size_type size) { return size * _front_realloc::num / _front_realloc::den; }
    ...
    template <typename OtherResizeTraits>
    vector &_copy_assignment(const vector<T, OtherResizeTraits, Allocator> &other) {
        if constexpr (alloc_traits::propagate_on_container_copy_assignment::value) {
            if constexpr (!alloc_traits::is_always_equal::value) {
                if (other._allocator() != _allocator() || other.size() > capacity_full()) {
                    _swap_with_allocator(vector(other, other._allocator()));
                    return *this;
                }
            }
        }
        if (other.size() > capacity_full()) {
            _swap_without_allocator(vector(other, _allocator()));
        } else {
    ...
    template <typename OtherResizeTraits>
    vector &_move_assignment(vector<T, OtherResizeTraits, Allocator> &&other) noexcept(noexcept(alloc_traits::propagate_on_container_move_assignment::value || alloc_traits::is_always_equal::value)) {
        if constexpr (!alloc_traits::is_always_equal::value) {
            if (_allocator() != other._allocator()) {
                if constexpr (alloc_traits::propagate_on_container_move_assignment::value) {
                    ...
                    if (other.size() > capacity_full()) {
                        _swap_without_allocator(vector(std::move(other), _allocator()));
                    } else {
    ...
    void _assign(It b, It e) {
        ...
        if (std::distance(b, e) > static_cast<difference_type>(capacity_full())) {
            _swap_without_allocator(vector(b, e, _allocator()));
        } else {
    ...
    iterator _insert(const_iterator it, It b, It e, std::input_iterator_tag) {
        // Input Iterators require a single-pass solution
        auto allocated = vector(b, e);
        _insert(it, allocated.begin(), allocated.end());
    }

    template <typename OtherResizeTraits>
    void _swap_with_allocator(vector<T, OtherResizeTraits, Allocator> &&other) noexcept {
        // Swap everything
    ...
    template <typename OtherResizeTraits>
    void _swap_without_allocator(vector<T, OtherResizeTraits, Allocator> &&other) noexcept {
    ...
    void _reallocate(size_type allocated, size_type offset) {
        auto replacement = vector(_allocate_uninitialized_tag{}, size(), allocated, offset, _allocator());
    ...
    iterator _insert_storage(const_iterator it, size_type count) {
        ...
        auto replacement = vector(_reallocate_uninitialized_tag{}, required_size, _allocator());
        auto index = std::distance(cbegin(), it);
    ...
    void _reassign_existing_storage(It b, It e) {
        ...
        if (size() == 0) {
            // Existing vector is empty.  Construct at the ideal location
            _copy_construct_range(b, e, ideal_begin);
        } else if (size_delta == 0) {
            // Existing vector is the same size.  Avoid any construction by copy-assigning everything
            std::copy(b, e, begin());
            return;
        } else if (size_delta < 0) {
    ...
    iterator _mutable_iterator(const_iterator i) { return begin() + std::distance(cbegin(), i); }
...
inline bool operator==(const vector<T, LResizeTraits, LAlloc> &lhs, const vector<T, RResizeTraits, RAlloc> &rhs) {
...
inline void swap(vector<T, ResizeTraits, Alloc> &lhs, vector<T, ResizeTraits, Alloc> &rhs) noexcept(noexcept(lhs.swap(rhs))) {
    lhs.swap(rhs);
}
...
vector(InputIt, InputIt, Alloc = Alloc()) -> vector<typename std::iterator_traits<InputIt>::value_type, fast_resize_traits, Alloc>;

}  // namespace MetaEngine

namespace std {
template <typename T, typename ResizeTraits, typename Alloc>
struct hash<MetaEngine::vector<T, ResizeTraits, Alloc>> {
    size_t operator()(const MetaEngine::vector<T, ResizeTraits, Alloc> &v) const {
        size_t hash = 0;
