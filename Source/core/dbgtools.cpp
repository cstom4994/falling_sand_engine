...
#define METADOT_DBG_CALLSTACK_UNIX
#endif
...
static uint16_t read_elf_type_from_self() {
    ...
    if (f) {
        ...
        if (read == 1) memcpy(&type, elf_header + 0x10, sizeof(type));
    }
...
static mmap_entry_t* parse_mmaps(const char* maps_file) {
    ...
    while (!feof(maps)) {
        ...
        sscanf(buf, "%lx-%lx %*s %lx", &next->range_start, &next->range_end, &next->file_offset);
...
static void* mmap_translate(mmap_entry_t* mmap, void* addr) {
    for (; mmap != NULL; mmap = mmap->next) {
        ...
        if (addr_i >= mmap->range_start && addr_i <= mmap->range_end) return (void*)(addr_i - mmap->range_start + mmap->file_offset);
    }
...
static FILE* run_addr2line(void** addresses, int num_addresses, char* tmp_buffer, size_t tmp_buf_len) {
    mmap_entry_t* list = is_using_pie() ? parse_mmaps("/proc/self/maps") : 0x0;
        ...
        if (addr2line != 0x0) {
            if (fgets(tmp_buffer, (int)tmp_buf_len, addr2line) != 0x0) {
                ...
                *line_start = '\0';
                ...
                if (tmp_buffer[0] != '?' && tmp_buffer[1] != '?') out_syms[i].file = alloc_string(&outbuf, tmp_buffer, strlen(tmp_buffer));
...
static int callstack_symbols_initialize() {
    ...
    if (!dbghelp.initialized) {
        ...
        if (dbghelp.SymInitialize && dbghelp.SymFromAddr && dbghelp.SymGetLineFromAddr64 && dbghelp.SymSetOptions) {
            ...
            if (GetEnvironmentVariable("DBGTOOLS_SYMBOL_DEBUG_OUTPUT", 0, 0) > 0) dbghelp.SymSetOptions(SYMOPT_DEBUG);
