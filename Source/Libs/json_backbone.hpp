...
namespace json_backbone {
    ...
    {
    };// Instantiated to make noexcept rules to compile with the tuple trick
    template<class T>
    ...
    namespace arithmetics {
        ...
        std::size_t constexpr find_first(std::array<I, N> const &values, I value,
                                         std::size_t current_index = 0) {
            return N <= current_index ? N : value == values.at(current_index) ? current_index
        ...
        std::size_t constexpr find_last(std::array<I, N> const &values, I value,
                                        std::size_t current_index = 0) {
            return N <= current_index ? N : value == values.at(N - current_index - 1) ? (N - current_index - 1)
        ...
        bool constexpr all_equals(std::array<I, N> const &values, I current_value,
                                  std::size_t current_index = 0) {
            return N <= current_index ? true : (current_value == values.at(current_index)) ? all_equals(values, current_value, current_index + 1)
                                                                                           : false;
        }
    };// namespace arithmetics
    ...
    public:
        ...
        template<class T, class Enabler = std::enable_if_t<!(target_type_list_t::template has_type<std::decay_t<T>>() || std::is_base_of<variant, std::decay_t<T>>::value), void>,
                 class Shim1 = void>
        variant &assign(T &&value) {
    ...
    bool operator==(variant<Value...> const &lhs, variant<Value...> const &rhs) {
        if (lhs.type_index() == rhs.type_index()) {
            static std::array<bool (*)(variant<Value...> const &, variant<Value...> const &),
                              sizeof...(Value)>
                    appliers = {helpers::equals<variant<Value...>, Value>...};
    ...
    bool operator<(variant<Value...> const &lhs, variant<Value...> const &rhs) {
        if (lhs.type_index() == rhs.type_index()) {
            static std::array<bool (*)(variant<Value...> const &, variant<Value...> const &),
                              sizeof...(Value)>
                    appliers = {helpers::less<variant<Value...>, Value>...};
            return appliers[lhs.type_index()](lhs, rhs);
        }
