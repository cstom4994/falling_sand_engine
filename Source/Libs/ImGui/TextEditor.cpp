...
TextEditor::TextEditor()
    : mLineSpacing(1.0f), mUndoIndex(0), mTabSize(4), mOverwrite(false), mReadOnly(false),
      ...
      mTextStart(20.0f), mLeftMargin(10), mColorRangeMin(0), mColorRangeMax(0),
      mSelectionMode(SelectionMode::Normal), mCheckComments(true), mLastClick(-1.0f) {
...
std::string TextEditor::GetText(const Coordinates &aStart, const Coordinates &aEnd) const {
    ...
    for (auto it = aStart; it <= aEnd; Advance(it)) {
        if (prevLineNo != it.mLine && it.mLine < (int) mLines.size()) result.push_back('\n');
...
void TextEditor::Advance(Coordinates &aCoordinates) const {
    if (aCoordinates.mLine < (int) mLines.size()) {
        ...
        if (aCoordinates.mColumn + 1 < (int) line.size()) ++aCoordinates.mColumn;
        else {
            ++aCoordinates.mLine;
...
TextEditor::Coordinates TextEditor::ScreenPosToCoordinates(const ImVec2 &aPosition) const {
	...
	*/
    ...
    float cumulatedStringWidth[2] = {
            0.0f,
            0.0f};//( [0] is the lastest, [1] is the previous. I use that trick to check where cursor is exactly (important for tabs)
    ...
    if (lineNo >= 0 && lineNo < (int) mLines.size()) {
        ...
        while (mTextStart + cumulatedStringWidth[0] < local.x &&
            ...
            cumulatedStringWidth[0] =
                    ImGui::GetFont()
                            ->CalcTextSizeA(ImGui::GetFontSize(), FLT_MAX, -1.0f,
                                            cumulatedString.c_str(), nullptr, nullptr)
                            .x;
...
TextEditor::Coordinates TextEditor::FindWordStart(const Coordinates &aFrom) const {
    ...
    if (at.mColumn >= (int) line.size()) return at;

    auto cstart = (PaletteIndex) line[at.mColumn].mColorIndex;
    while (at.mColumn > 0) {
        if (cstart != (PaletteIndex) line[at.mColumn - 1].mColorIndex) break;
        --at.mColumn;
    }
    return at;
}
...
TextEditor::Coordinates TextEditor::FindWordEnd(const Coordinates &aFrom) const {
    ...
    while (at.mColumn < (int) line.size()) {
        if (cstart != (PaletteIndex) line[at.mColumn].mColorIndex) break;
        ++at.mColumn;
    }
    return at;
}
...
bool TextEditor::IsOnWordBoundary(const Coordinates &aAt) const {
    ...
    if (aAt.mColumn >= (int) line.size()) return true;
...
std::string TextEditor::GetWordAt(const Coordinates &aCoords) const {
    ...
    for (auto it = start; it < end; Advance(it)) r.push_back(mLines[it.mLine][it.mColumn].mChar);
    ...
    if (ImGui::IsWindowFocused()) {
        ...
        if (!IsReadOnly() && ctrl && !shift && !alt &&
            ...
            for (int i = 0; i < io.InputQueueCharacters.Size; i++) {
                ...
                if (c != 0) {
                    if (isprint(c) || isspace(c)) { EnterCharacter((char) c, shift); }
                }
            }
    ...
    if (ImGui::IsWindowHovered()) {
        if (!shift && !alt) {
			...
			*/
                ...
                mState.mCursorPosition = mInteractiveStart = mInteractiveEnd =
                        SanitizeCoordinates(ScreenPosToCoordinates(ImGui::GetMousePos()));
                if (ctrl) mSelectionMode = SelectionMode::Word;
                else
                    mSelectionMode = SelectionMode::Normal;
                SetSelection(mInteractiveStart, mInteractiveEnd, mSelectionMode);
...
void TextEditor::Render() {
    ...
    if (!mLines.empty()) {
        ...
        while (lineNo <= lineMax) {
            ...
            longest = std::max(
                    mTextStart + TextDistanceToLineStart(Coordinates(lineNo, (int) line.size())),
                    longest);
            auto columnNo = 0;
            ...
            if (mState.mCursorPosition.mLine == lineNo) {
                ...
                if (focused) {
                    ...
                    if (elapsed > 400) {
                        ...
                        ImVec2 cend(textScreenPos.x + cx + (mOverwrite ? mCharAdvance.x : 1.0f),
                                    lineStartScreenPos.y + mCharAdvance.y);
                        drawList->AddRectFilled(cstart, cend, mPalette[(int) PaletteIndex::Cursor]);
                        if (elapsed > 800) timeStart = timeEnd;
                    }
                }
            }
            ...
            for (auto &glyph: line) {
                ...
                if (glyph.mChar == '\t')
                    bufferOffset.x = (1.0f * fontScale + std::floor((1.0f + bufferOffset.x)) /
...
void TextEditor::SetText(const std::string &aText) {
    ...
    for (auto chr: aText) {
        if (chr == '\r') {
            ...
            mLines.emplace_back(Line());
        else { mLines.back().emplace_back(Glyph(chr, PaletteIndex::Default)); }
    }
                ...
                if (aShift) {
                    if (line.empty() == false) {
                        if (line.front().mChar == '\t') {
                            ...
                            if (i == end.mLine && end.mColumn > 0) end.mColumn--;
                            modified = true;
                        }
                    } else {
                        for (int j = 0;
                             j < mTabSize && line.empty() == false && line.front().mChar == ' ';
                             j++) {
                            line.erase(line.begin());
                            if (i == end.mLine && end.mColumn > 0) end.mColumn--;
                            modified = true;
...
void TextEditor::SetSelection(const Coordinates &aStart, const Coordinates &aEnd,
    ...
    if (aStart > aEnd) std::swap(mState.mSelectionStart, mState.mSelectionEnd);
...
void TextEditor::MoveLeft(int aAmount, bool aSelect, bool aWordMode) {
    ...
    while (aAmount-- > 0) {
        if (mState.mCursorPosition.mColumn == 0) {
            ...
            if (aWordMode) mState.mCursorPosition = FindWordStart(mState.mCursorPosition);
        }
    }
        ...
        if (mState.mCursorPosition.mColumn >= (int) line.size()) {
            if (mState.mCursorPosition.mLine < (int) mLines.size() - 1) {
                mState.mCursorPosition.mLine = std::max(
                        0, std::min((int) mLines.size() - 1, mState.mCursorPosition.mLine + 1));
                mState.mCursorPosition.mColumn = 0;
            }
        } else {
            mState.mCursorPosition.mColumn =
                    std::max(0, std::min((int) line.size(), mState.mCursorPosition.mColumn + 1));
            if (aWordMode) mState.mCursorPosition = FindWordEnd(mState.mCursorPosition);
        }
    }
    ...
    if (HasSelection()) {
        ...
        if (mState.mCursorPosition.mColumn == 0) {
            ...
            u.mRemovedStart = u.mRemovedEnd =
                    Coordinates(pos.mLine - 1, (int) mLines[pos.mLine - 1].size());
            Advance(u.mRemovedEnd);
...
bool TextEditor::CanUndo() const { return mUndoIndex > 0; }

bool TextEditor::CanRedo() const { return mUndoIndex < (int) mUndoBuffer.size(); }
        ...
        for (auto currentCoord = Coordinates(0, 0); currentCoord < end; Advance(currentCoord)) {
            ...
            if (!line.empty()) {
                ...
                if (withinString) {
                    ...
                    if (c == '\"') {
                        if (currentCoord.mColumn + 1 < (int) line.size() &&
                            line[currentCoord.mColumn + 1].mChar == '\"') {
                    ...
                    if (c == '\"') {
                        ...
                        if (singleStartStr.size() > 0 &&
                            ...
                            withinSingleLineComment = true;
                                 ...
                                 currentCoord.mColumn + startStr.size() <= line.size() &&
...
float TextEditor::TextDistanceToLineStart(const Coordinates &aFrom) const {
    ...
    for (size_t it = 0u; it < line.size() && it < (unsigned) aFrom.mColumn; ++it) {
        if (line[it].mChar == '\t') {
            distance = (1.0f * fontScale +
                        std::floor((1.0f + distance)) / (float(mTabSize) * spaceSize)) *
                       (float(mTabSize) * spaceSize);
        } else {
            char tempCString[2];
...
static bool TokenizeCStyleNumber(const char *in_begin, const char *in_end, const char *&out_begin,
    ...
    if (p < in_end) {
        if (*p == '.') {
            ...
            while (p < in_end && ((*p >= '0' && *p <= '9') || (*p >= 'a' && *p <= 'f') ||
                                  (*p >= 'A' && *p <= 'F')))
                p++;
        } else if (*p == 'b' || *p == 'B') {
...
const TextEditor::LanguageDefinition &TextEditor::LanguageDefinition::C() {
    ...
    if (!inited) {
        ...
        langDef.mTokenize = [](const char *in_begin, const char *in_end, const char *&out_begin,
            ...
            while (in_begin < in_end && isblank(*in_begin)) in_begin++;
