...
#if defined(MY_CPU_AMD64) || defined(_M_ARM64) || defined(_M_IA64) || defined(__AARCH64EL__) || defined(__AARCH64EB__)
#define MY_CPU_64BIT
#endif
...
#if defined(MY_CPU_X86) || defined(_M_ARM) || defined(__ARMEL__) || defined(__THUMBEL__) || defined(__ARMEB__) || defined(__THUMBEB__)
#define MY_CPU_32BIT
#endif
...
#if defined(MY_CPU_X86_OR_AMD64) || defined(MY_CPU_ARM_LE) || defined(MY_CPU_IA64_LE) || defined(__LITTLE_ENDIAN__) || defined(__ARMEL__) || defined(__THUMBEL__) || defined(__AARCH64EL__) || defined(__MIPSEL__) || defined(__MIPSEL) || defined(_MIPSEL) || defined(__BFIN__) || (defined(__BYTE_ORDER__) && (__BYTE_ORDER__ == __ORDER_LITTLE_ENDIAN__))
#define MY_CPU_LE
#endif
...
static UInt32 MY_FAST_CALL CrcUpdateT4(UInt32 v, const void *data, size_t size, const UInt32 *table) {
    ...
    for (; size >= 4; size -= 4, p += 4) {
        v ^= *(const UInt32 *) p;
        v =
                table[0x300 + ((v) &0xFF)] ^ table[0x200 + ((v >> 8) & 0xFF)] ^ table[0x100 + ((v >> 16) & 0xFF)] ^ table[0x000 + ((v >> 24))];
    }
    for (; size > 0; size--, p++)
        v = CRC_UPDATE_BYTE_2(v, *p);
    return v;
...
static UInt32 MY_FAST_CALL CrcUpdateT8(UInt32 v, const void *data, size_t size, const UInt32 *table) {
    ...
    for (; size >= 8; size -= 8, p += 8) {
        ...
        v ^= *(const UInt32 *) p;
        v =
                table[0x700 + ((v) &0xFF)] ^ table[0x600 + ((v >> 8) & 0xFF)] ^ table[0x500 + ((v >> 16) & 0xFF)] ^ table[0x400 + ((v >> 24))];
        d = *((const UInt32 *) p + 1);
        v ^=
                table[0x300 + ((d) &0xFF)] ^ table[0x200 + ((d >> 8) & 0xFF)] ^ table[0x100 + ((d >> 16) & 0xFF)] ^ table[0x000 + ((d >> 24))];
    }
    for (; size > 0; size--, p++)
        v = CRC_UPDATE_BYTE_2(v, *p);
    return v;
...
static UInt32 MY_FAST_CALL CrcUpdateT1_BeT4(UInt32 v, const void *data, size_t size, const UInt32 *table) {
    ...
    for (; size >= 4; size -= 4, p += 4) {
        v ^= *(const UInt32 *) p;
        v =
                table[0x000 + ((v) &0xFF)] ^ table[0x100 + ((v >> 8) & 0xFF)] ^ table[0x200 + ((v >> 16) & 0xFF)] ^ table[0x300 + ((v >> 24))];
    }
    for (; size > 0; size--, p++)
        v = CRC_UPDATE_BYTE_2_BE(v, *p);
...
static UInt32 MY_FAST_CALL CrcUpdateT1_BeT8(UInt32 v, const void *data, size_t size, const UInt32 *table) {
    ...
    for (; size >= 8; size -= 8, p += 8) {
        ...
        v ^= *(const UInt32 *) p;
        v =
                table[0x400 + ((v) &0xFF)] ^ table[0x500 + ((v >> 8) & 0xFF)] ^ table[0x600 + ((v >> 16) & 0xFF)] ^ table[0x700 + ((v >> 24))];
        d = *((const UInt32 *) p + 1);
        v ^=
                table[0x000 + ((d) &0xFF)] ^ table[0x100 + ((d >> 8) & 0xFF)] ^ table[0x200 + ((d >> 16) & 0xFF)] ^ table[0x300 + ((d >> 24))];
    }
    for (; size > 0; size--, p++)
        v = CRC_UPDATE_BYTE_2_BE(v, *p);
    return CRC_UINT32_SWAP(v);
...
#define CHECK_CPUID_IS_SUPPORTED \
    if (CheckFlag(1 << 18) == 0 || CheckFlag(1 << 21) == 0) return False;
#else
#define CHECK_CPUID_IS_SUPPORTED
#endif
...
#define MY_ALLOC_ZE(T, p, size, alloc)  \
    {                                   \
        if ((size) == 0) p = NULL;      \
            ...
            MY_ALLOC(T, p, size, alloc) \
    }

#define MY_ALLOC_AND_CPY(to, size, from, alloc) \
    {                                           \
        MY_ALLOC(Byte, to, size, alloc);        \
        memcpy(to, from, size);                 \
    }
...
#define MY_ALLOC_ZE_AND_CPY(to, size, from, alloc)       \
    {                                                    \
        if ((size) == 0) p = NULL;                       \
        else { MY_ALLOC_AND_CPY(to, size, from, alloc) } \
    }
...
static SRes CheckSupportedFolder(const CSzFolder *f) {
    ...
    if (f->NumCoders == 2) {
        ...
        if (
                /* c->MethodID > (UInt32)0xFFFFFFFF || */
                c->NumStreams != 1 || f->NumPackStreams != 1 || f->PackStreams[0] != 0 || f->NumBonds != 1 || f->Bonds[0].InIndex != 1 || f->Bonds[0].OutIndex != 0)
            return SZ_ERROR_UNSUPPORTED;
    ...
    if (f->NumCoders == 4) {
        if (!IS_SUPPORTED_CODER(&f->Coders[1]) || !IS_SUPPORTED_CODER(&f->Coders[2]) || !IS_BCJ2(&f->Coders[3]))
            return SZ_ERROR_UNSUPPORTED;
        if (f->NumPackStreams != 4 || f->PackStreams[0] != 2 || f->PackStreams[1] != 6 || f->PackStreams[2] != 1 || f->PackStreams[3] != 0 || f->NumBonds != 3 || f->Bonds[0].InIndex != 5 || f->Bonds[0].OutIndex != 0 || f->Bonds[1].InIndex != 4 || f->Bonds[1].OutIndex != 1 || f->Bonds[2].InIndex != 3 || f->Bonds[2].OutIndex != 2)
            return SZ_ERROR_UNSUPPORTED;
        return SZ_OK;
    }
        ...
        if (IS_MAIN_METHOD((UInt32) coder->MethodID)) {
...

            {
...

                {
                    ...
                    if (p.dest != p.destLim || p.state != BCJ2_STREAM_MAIN)
                        return SZ_ERROR_DATA;
                }
            }
        }
...
static SRes SzAr_DecodeFolder(const CSzAr *p, UInt32 folderIndex,
    ...
    if (sd.Size != 0 || folder.UnpackStream != p->FoToMainUnpackSizeIndex[folderIndex] || outSize != SzAr_GetFolderUnpackSize(p, folderIndex))
        return SZ_ERROR_FAIL;
    {
        unsigned i;
...
#define TREE_DECODE(probs, limit, i)                      \
    {                                                     \
        i = 1;                                            \
        do { TREE_GET_BIT(probs, i); } while (i < limit); \
        i -= limit;                                       \
    }
...
#define TREE_DECODE_CHECK(probs, limit, i)                    \
    {                                                         \
        i = 1;                                                \
        do { GET_BIT_CHECK(probs + i, i) } while (i < limit); \
        i -= limit;                                           \
    }
