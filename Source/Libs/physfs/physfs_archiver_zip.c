...
static PHYSFS_ErrorCode zlib_error_code(int rc) {
    switch (rc) {
        case Z_OK:
            return PHYSFS_ERR_OK; /* not an error. */
        case Z_STREAM_END:
            return PHYSFS_ERR_OK; /* not an error. */
        case Z_ERRNO:
...
static int zip_parse_local(PHYSFS_Io *io, ZIPentry *entry) {
    ...
    BAIL_IF(ui32 && (ui32 != 0xFFFFFFFF) &&
                    (ui32 != entry->compressed_size),
            PHYSFS_ERR_CORRUPT, 0);

    BAIL_IF_ERRPASS(!readui32(io, &ui32), 0);
    BAIL_IF(ui32 && (ui32 != 0xFFFFFFFF) &&
                    (ui32 != entry->uncompressed_size),
            PHYSFS_ERR_CORRUPT, 0);
...
static ZIPentry *zip_load_entry(ZIPinfo *info, const int zip64,
    ...
    if (isdir)
        ...
        retval->resolved = (zip_has_symlink_attr(retval, external_attr)) ? ZIP_UNRESOLVED_SYMLINK : ZIP_UNRESOLVED_FILE;
    } /* else */
