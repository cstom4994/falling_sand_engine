...
PHYSFS_DECL const char *PHYSFS_getErrorByCode(PHYSFS_ErrorCode code) {
    switch (code) {
        ...
        case PHYSFS_ERR_SYMLINK_FORBIDDEN:
            return "symlinks are forbidden";
        case PHYSFS_ERR_NO_WRITE_DIR:
            return "write directory is not set";
        case PHYSFS_ERR_OPEN_FOR_READING:
            return "file open for reading";
...
PHYSFS_sint64 PHYSFS_tell(PHYSFS_File *handle) {
    ...
    const PHYSFS_sint64 retval = fh->forReading ? (pos - fh->buffill) + fh->bufpos : (pos + fh->buffill);
    return retval;
} /* PHYSFS_tell */
...
static PHYSFS_uint32 hashPathName(__PHYSFS_DirTree *dt, const char *name) {
    const PHYSFS_uint32 hashval = dt->case_sensitive ? __PHYSFS_hashString(name) : dt->only_usascii ? __PHYSFS_hashStringCaseFoldUSAscii(name)
                                                                                                    : __PHYSFS_hashStringCaseFold(name);
