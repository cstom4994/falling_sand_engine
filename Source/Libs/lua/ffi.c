...
static int type_error(lua_State *L, int idx, const char *to_type, int to_usr,
    ...
    if (to_usr) { to_usr = lua_absindex(L, to_usr); }
    ...
    switch (lua_type(L, idx)) {                                                                    \
        ...
        case LUA_TNUMBER:                                                                          \
            ...
            break;                                                                                 \
                                                                                                   \
        case LUA_TSTRING:                                                                          \
            if (!ALLOW_POINTERS) { type_error(L, idx, #TYPE, 0, NULL); }                           \
        ...
        case LUA_TFUNCTION:                                                                        \
            if (!ALLOW_POINTERS) { type_error(L, idx, #TYPE, 0, NULL); }                           \
            if (!get_cfunction_address(L, idx, &f)) { type_error(L, idx, #TYPE, 0, NULL); }        \
            ret = (TYPE) (intptr_t) f;                                                             \
            break;                                                                                 \
                                                                                                   \
        case LUA_TUSERDATA:                                                                        \
...
static uint64_t cast_uint64(lua_State *L, int idx, int is_cast) {
    TO_NUMBER(uint64_t, is_cast, lua_tointeger);
    return ret;
}
...
double check_double(lua_State *L, int idx) {
    TO_NUMBER(double, 0, lua_tonumber);
    return ret;
}
...
uintptr_t check_uintptr(lua_State *L, int idx) {
    TO_NUMBER(uintptr_t, 1, lua_tointeger);
    return ret;
}
...
static size_t unpack_vararg(lua_State *L, int i, char *to) {
    ...
    switch (lua_type(L, i)) {
        ...
        case LUA_TNUMBER:
            *(double *) to =
                    lua_tonumber(L, i);// TODO in Lua 5.3: lua_tointeger sometimes should be here
...
void unpack_varargs_stack(lua_State *L, int first, int last, char *to) {
    ...
    for (i = first; i <= last; i++) { to += unpack_vararg(L, i, to); }
}
...
static void *check_pointer(lua_State *L, int idx, struct ctype *ct) {
    ...
    switch (lua_type(L, idx)) {
        ...
        case LUA_TNUMBER:
            ...
            ct->pointers = 0;
            lua_pushnil(L);
            return (void *) (uintptr_t) lua_tonumber(
                    L, idx);// TODO in Lua 5.3: maybe change to lua_tointeger
...
static void set_struct(lua_State *L, int idx, void *to, int to_usr, const struct ctype *tt,
                       int check_pointers);
...
static int get_cfunction_address(lua_State *L, int idx, cfunction *addr) {
    ...
    if (!lua_getupvalue(L, idx, n - 1)) return 0;
...
static cfunction check_cfunction(lua_State *L, int idx, int to_usr, const struct ctype *tt,
                                 int check_pointers) {
    void *p;
    struct ctype ft;
    cfunction f;
    ...
    switch (lua_type(L, idx)) {
        case LUA_TFUNCTION:
            if (get_cfunction_address(L, idx, &f)) { return f; }
...
static void set_value(lua_State *L, int idx, void *to, int to_usr, const struct ctype *tt,
    ...
    if (tt->is_array) {
        ...
        *(uint64_t *) to = val | (*(uint64_t *) to & (hi_mask | low_mask));
        ...
        if ((uintptr_t) origto & (tt->base_size - 1)) { to = misalign.c; }
#endif
...
#ifndef ALLOW_MISALIGNED_ACCESS
        if ((uintptr_t) origto & (tt->base_size - 1)) { memcpy(origto, misalign.c, tt->base_size); }
#endif
    }
...
static void setmintop(lua_State *L, int idx) {
    if (lua_gettop(L) < idx) { lua_settop(L, idx); }
}
...
static int is_scalar(struct ctype *ct) {
    int type = ct->type;
    if (ct->pointers || ct->is_reference) { return !ct->is_array; }
...
static int should_pack(lua_State *L, int ct_usr, struct ctype *ct, int idx) {
    ...
    if (IS_COMPLEX(ct->type)) { return 0; }
...
static int do_new(lua_State *L, int is_cast) {
    ...
    if (!is_cast) { get_variable_array_size(L, 2, &ct); }
    ...
    if (scalar && cargs > 1) { return luaL_error(L, "too many initializers"); }
...
static int ffi_istype(lua_State *L) {
    ...
    if (ft.type == INVALID_TYPE) { goto fail; }

    if (!is_same_type(L, 3, 4, &tt, &ft)) { goto fail; }

    if (tt.pointers != ft.pointers) { goto fail; }

    if (tt.is_array != ft.is_array) { goto fail; }

    if (tt.is_array && tt.array_size != ft.array_size) { goto fail; }

    if (tt.calling_convention != ft.calling_convention) { goto fail; }
...
static int cdata_set(lua_State *L) {
    ...
    if (!ct.is_jitted) { luaL_error(L, "can't set the function for a non-lua callback"); }

    if (*p == NULL) { luaL_error(L, "can't set the function for a free'd callback"); }
...
int push_user_mt(lua_State *L, int ct_usr, const struct ctype *ct) {
    if (ct->type != STRUCT_TYPE && ct->type != UNION_TYPE && !IS_COMPLEX(ct->type)) { return 0; }
...
static ptrdiff_t lookup_cdata_index(lua_State *L, int idx, int ct_usr, struct ctype *ct) {
    ...
    switch (type) {
        ...
        case LUA_TUSERDATA:
            ...
            if (!ct->pointers || is_void_ptr(ct)) { return -1; }

            // unbox cdata
            if (type == LUA_TUSERDATA) {
                if (!cdata_tointeger(L, idx, &off)) { return -1; }
            } else {
...
static int cdata_newindex(lua_State *L) {
    ...
    if (off < 0) {
        if (!push_user_mt(L, -1, &tt)) { goto err; }
    ...
    if (tt.const_mask & 1) { return luaL_error(L, "can't set const data"); }
...
static int cdata_index(lua_State *L) {
    ...
    if (off < 0) {
        assert(lua_gettop(L) == 3);
        if (!push_user_mt(L, -1, &ct)) { goto err; }
        ...
        *pf = *(cfunction *) data;
        ...
        switch (ct.type) {
            case BOOL_TYPE:
                ...
                break;
            case INT8_TYPE:
                lua_pushinteger(L, ct.is_unsigned ? (lua_Integer) * (uint8_t *) data
                                                  : (lua_Integer) * (int8_t *) data);
                break;
            case INT16_TYPE:
                lua_pushinteger(L, ct.is_unsigned ? (lua_Integer) * (uint16_t *) data
                                                  : (lua_Integer) * (int16_t *) data);
                break;
            case ENUM_TYPE:
            case INT32_TYPE:
                lua_pushinteger(L, ct.is_unsigned ? (lua_Integer) * (uint32_t *) data
                                                  : (lua_Integer) * (int32_t *) data);
                break;
            case INT64_TYPE:
                to = push_cdata(L, -1, &ct);
...
static int call_user_binop(lua_State *L, const char *opfield, int lidx, int lusr,
                           const struct ctype *lt, int ridx, int rusr, const struct ctype *rt) {
...
err:
    ...
    push_type_name(L, -2, &lt);
    return luaL_error(L, "trying to compare incompatible types %s and %s", lua_tostring(L, -2),
                      lua_tostring(L, -1));
}
...
static void print_type(lua_State *L, const struct ctype *ct) {
    lua_pushfstring(L,
                    " sz %d %d %d align %d ptr %d %d %d type %s%s %d %d %d name %d call %d %d var "
                    "%d %d %d bit %d %d %d %d jit %d",
                    /* sz */
                    ...
                    /* type */
                    ct->is_unsigned ? "u" : "", etype_tostring(ct->type), ct->is_reference,
                    ct->is_defined, ct->is_null,
                    /* name */
                    ct->has_member_name,
                    /* call */
                    ct->calling_convention, ct->has_var_arg,
                    /* var */
                    ct->is_variable_array, ct->is_variable_struct, ct->variable_size_known,
                    /* bit */
...
static int ffi_string(lua_State *L) {
    ...
    if (is_void_ptr(&ct)) {
        ...
        if (lua_isuserdata(L, 2)) {
            ptrdiff_t val;
            if (!cdata_tointeger(L, 2, &val)) { type_error(L, 2, "int", 0, NULL); }
            sz = (size_t) val;
...
static int ffi_fill(lua_State *L) {
    ...
    if (!lua_isnoneornil(L, 3)) { val = (int) luaL_checkinteger(L, 3); }
...
static int ffi_load(lua_State *L) {
    ...
    *lib = LoadLibraryA(libname);
    ...
    if (!*lib) { return luaL_error(L, "could not load library %s", lua_tostring(L, 1)); }
...
static void *find_symbol(lua_State *L, int modidx, const char *asmname) {
    ...
    for (i = 0; i < num && sym == NULL; i++) {
        if (libs[i]) { sym = GetProcAddressA(libs[i], asmname); }
    }
...
static int cmodule_index(lua_State *L) {
    ...
    if (!sym) { return luaL_error(L, "failed to find function/global %s", asmname); }
    ...
    switch (ct.type) {
        ...
        case INT8_TYPE:
            lua_pushinteger(L, ct.is_unsigned ? (lua_Integer) * (uint8_t *) sym
                                              : (lua_Integer) * (int8_t *) sym);
            return 1;

        case INT16_TYPE:
            lua_pushinteger(L, ct.is_unsigned ? (lua_Integer) * (uint16_t *) sym
                                              : (lua_Integer) * (int16_t *) sym);
            return 1;

        case INT32_TYPE:
        case ENUM_TYPE:
            lua_pushinteger(L, ct.is_unsigned ? (lua_Integer) * (uint32_t *) sym
                                              : (lua_Integer) * (int32_t *) sym);
            return 1;
    }
...
static int cmodule_newindex(lua_State *L) {
    ...
    if (sym == NULL) { return luaL_error(L, "failed to find global %s", name); }
...
static int jit_gc(lua_State *L) {
    ...
    struct jit *jit = get_jit(L);
    dasm_free(jit);
    for (i = 0; i < jit->pagenum; i++) { FreePage(jit->pages[i], jit->pages[i]->size); }
    free(jit->pages);
    free(jit->globals);
    return 0;
}
...
static int do64(lua_State *L, int is_unsigned) {
    ...
    if (!is_unsigned && (high < 0 || low < 0)) { val = -val; }
...
static int ffi_i64(lua_State *L) { return do64(L, 0); }

static int ffi_u64(lua_State *L) { return do64(L, 1); }
...
static const luaL_Reg callback_mt[] = {{"__gc", &callback_free}, {NULL, NULL}};
...
static const luaL_Reg cmodule_mt[] = {{"__index", &cmodule_index},
                                      {"__newindex", &cmodule_newindex},
                                      {NULL, NULL}};

static const luaL_Reg jit_mt[] = {{"__gc", &jit_gc}, {NULL, NULL}};
...
static const luaL_Reg ffi_reg[] = {{"cdef", &ffi_cdef},       {"load", &ffi_load},
                                   ...
                                   {"istype", &ffi_istype},   {"errno", &ffi_errno},
                                   {"string", &ffi_string},   {"copy", &ffi_copy},
                                   {"fill", &ffi_fill},       {"abi", &ffi_abi},
                                   {"debug", &ffi_debug},     {"i64", &ffi_i64},
                                   {"u64", &ffi_u64},         {NULL, NULL}};

/* leaves the usr table on the stack */
static void push_builtin(lua_State *L, struct ctype *ct, const char *name, int type, int size,
                         int align, int is_unsigned) {
    memset(ct, 0, sizeof(*ct));
    ct->type = type;
    ct->base_size = size;
    ct->align_mask = align;
...
#undef STR2
    ...
    /* setup builtin types */
    {
        ...
        push_builtin(L, &ct, "complex float", COMPLEX_FLOAT_TYPE, sizeof(complex_float),
                     ALIGNOF(cf), 0);
        push_builtin(L, &ct, "complex double", COMPLEX_DOUBLE_TYPE, sizeof(complex_double),
                     ALIGNOF(cd), 0);
#if defined HAVE_LONG_DOUBLE && defined HAVE_COMPLEX
