...
struct ctype *push_ctype(lua_State *L, int ct_usr, const struct ctype *ct)
{
    ...
    *ret = *ct;
    ...
    if (!ct->is_defined && ct_usr && !lua_isnil(L, ct_usr)) { update_on_definition(L, ct_usr, -1); }
...
void *push_cdata(lua_State *L, int ct_usr, const struct ctype *ct) {
     ...
     */
    if (ct->has_bitfield) { sz = ALIGN_UP(sz, 7); }
...
void *check_cdata(lua_State *L, int idx, struct ctype *ct) {
    ...
    if (ct->type == INVALID_TYPE) { luaL_error(L, "expected cdata for arg #%d", idx); }
    return p;
}
