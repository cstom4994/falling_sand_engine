...
void dasm_free(Dst_DECL) {
    ...
    for (i = 0; i < D->maxsection; i++)
        if (D->sections[i].buf) DASM_M_FREE(Dst, D->sections[i].buf, D->sections[i].bsize);
...
#define CK(x, st)                                                                                  \
    do {                                                                                           \
        if (!(x)) return DASM_S_##st | (p - D->actionlist - 1);                                    \
    } while (0)
#else
#define CK(x, st) ((void) 0)
#endif
        ...
        while (b != endb) {
            ...
            while (1) {
                ...
                switch (action) {
                    ...
                    patchrel:
                        CK((n & 3) == 0 && (((n + 4) + ((ins & 2048) ? 0x00008000 : 0x02000000)) >>
                                            ((ins & 2048) ? 16 : 26)) == 0,
                           RANGE_REL);
