...
// Link to https://github.com/megumumpkin/imgui_lua_bindings
...
#define IM_VEC_4_ARG(name)                                                                         \
    ...
    const ImVec4 name((double) i_##name##_x, (double) i_##name##_y, (double) i_##name##_z,         \
                      (double) i_##name##_w);
...
#define OPTIONAL_NUMBER_ARG(name, otherwise)                                                       \
    ...
    if (arg <= max_args) { name = lua_tonumber(L, arg++); }
...
#define OPTIONAL_INT_ARG(name, otherwise)                                                          \
    int name = otherwise;                                                                          \
    if (arg <= max_args) { name = (int) lua_tonumber(L, arg++); }
...
#define OPTIONAL_UINT_ARG(name, otherwise)                                                         \
    ...
    if (arg <= max_args) { name = (unsigned int) luaL_checkinteger(L, arg++); }
...
#define OPTIONAL_BOOL_ARG(name, otherwise)                                                         \
    bool name = otherwise;                                                                         \
    if (arg <= max_args) { name = lua_toboolean(L, arg++); }
...
#define VOID_ARG(name)                                                                             \
    ...
    switch (type_##name) {                                                                         \
        ...
        case LUA_TSTRING: {                                                                        \
            size_t i_##name##_size;                                                                \
            o_##name##_str =                                                                       \
                    const_cast<char *>(luaL_checklstring(L, arg_##name, &(i_##name##_size)));      \
