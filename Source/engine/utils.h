...
#define ListForEach(cellPointer, list)                                                             \
    for (cellPointer = GetFirstCell(list); cellPointer != NULL;                                    \
         cellPointer = GetNextCell(cellPointer))
...
Matrix4x4 GetProjectionMatrix(float right, float left, float top, float bottom, float near,
                              float far);
