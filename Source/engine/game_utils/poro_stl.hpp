...
namespace BaseEngine {
    ...
    public:
        ...
        CArray2D()
            : myWidth(0), myHeight(0), mySize(0), myArraysLittleHelper(*this),
              myNullReference(_Ty()) {}

        CArray2D(int _width, int _height)
            : myWidth(_width), myHeight(_height), mySize(0), myArraysLittleHelper(*this),
              myNullReference(_Ty()) {
        ...
        CArray2D(const CArray2D<_Ty, _A> &other)
            : myWidth(other.myWidth), myHeight(other.myHeight), mySize(other.mySize),
              myArraysLittleHelper(*this), myDataArray(other.myDataArray), myNullReference(_Ty()) {}
        ...
        void Set(int _x, int _y, const CArray2D &_who) {
            ...
            for (y = 0; y <= myHeight; y++) {
                for (x = 0; x <= myWidth; x++) {
                    if (x >= _x && x <= _x + _who.GetWidth() && y >= _y &&
                        y <= _y + _who.GetHeight()) {
...
*/
    ...
    public:
        virtual ~CAutoList() {
            GetList().erase(
                    std::find(GetList().begin(), GetList().end(), reinterpret_cast<T *>(this)));
        ...
        static std::list<T *> &GetList() {
            if (myAutoListPointerList == NULL) { myAutoListPointerList = new std::list<T *>; }
...
namespace BaseEngine {
    ...
    public:
        ...
        // multiply stuff
        const CBitMask<T> &Multiply(const bitmask::PointXForm &xform) {
            return MultiplyImpl(xform);
        }
        const CBitMask<T> &Multiply(const bitmask::PointMatrix &matrix) {
            return MultiplyImpl(matrix);
        }
...
namespace BaseEngine {
    ...
    public:
		...
		*/
            if (i->second.empty()) { myMap.erase(i); }
        }
...
*/
    std::string StringReplace(const std::string &what, const std::string &with,
                              const std::string &in_here, int limit = -1);
...
namespace BaseEngine {
    ...
    void VectorClearPointers(std::vector<T *> &container) {
        for (std::size_t i = 0; i < container.size(); ++i) { delete container[i]; }
    ...
    template<>
    void VectorLoadFromTxtFile<std::string>(std::vector<std::string> &array,
                                            const std::string &filename);

    inline void VectorLoadFromTxtFile(std::vector<std::string> &array,
                                      const std::string &filename) {
        std::ifstream file(filename.c_str());
        std::string temp;
        while (std::getline(file, temp)) { array.push_back(temp); }
    ...
    void VectorSaveToTxtFile(std::vector<T> &array, const std::string &filename) {
        ...
        for (std::size_t i = 0; i < array.size(); ++i) { file << array[i] << std::endl; }
        file.close();
    }
