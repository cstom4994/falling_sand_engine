...
static void init_component_pool(struct entity_world *w, int index, int stride, int opt_size) {
...
static void entity_new_type(lua_State *L, int world_index, int cid, int stride, int opt_size) {
...
static int lnew_type(lua_State *L) {
...
static int lcount_memory(lua_State *L) {
...
static void init_buffers(struct component_pool *pool) {
    int cap = pool->cap;
    if (pool->stride <= 0) {
        // only id
...
static void move_buffers(struct component_pool *pool, void *buffer, entity_index_t *id) {
...
static void free_buffers(struct component_pool *c) {
    if (c->stride > 0) {
        free(c->buffer);
        c->buffer = NULL;
        c->id = NULL;
...
static void shrink_component_pool(lua_State *L, struct component_pool *c, int cid) {
    if (c->id == NULL) return;
...
static int lcollect_memory(lua_State *L) {
    struct entity_world *w = getW(L);
    int i;
    for (i = 0; i < MAX_COMPONENT; i++) {
...
static inline int add_component_id_(struct component_pool *pool, int cid, entity_index_t eid) {
    int cap = pool->cap;
    int index = pool->n;
    if (pool->n == 0) {
        if (pool->id == NULL) {
    ...
    if (index > 0 && (cmp = ENTITY_INDEX_CMP(pool->id[index - 1], eid)) >= 0) {
        ...
        if (pool->stride > 0) {
            memmove((uint8_t *)pool->buffer + (index + 1) * pool->stride, (uint8_t *)pool->buffer + index * pool->stride, (pool->n - index) * pool->stride);
        }
    }
    pool->id[index] = eid;
    ++pool->n;
...
int ecs_add_component_id_(struct entity_world *w, int cid, entity_index_t eid) {
...
entity_index_t ecs_new_entityid_(struct entity_world *w) {
    ...
    if (n < 0) return INVALID_ENTITY;
    return make_index_(n);
}

static int lnew_entity(lua_State *L) {
    struct entity_world *w = getW(L);
...
static int binary_search(entity_index_t *a, int from, int to, uint32_t v) {
    while (from < to) {
        int mid = (from + to) / 2;
...
static inline int search_after(struct component_pool *pool, uint32_t eid, int from_index) {
...
int ecs_lookup_component_(struct component_pool *pool, entity_index_t eindex, int guess_index) {
    ...
    if (guess_index < 0 || guess_index >= pool->n) return binary_search(pool->id, 0, pool->n, eid);
    entity_index_t *a = pool->id;
    entity_index_t lower_index = a[guess_index];
    uint32_t lower = index_(lower_index);
    if (eid <= lower) {
        if (eid == lower) return guess_index;
        return binary_search(a, 0, guess_index, eid);
    }
    return search_after(pool, eid, guess_index);
}

static inline int lookup_component_from(struct component_pool *pool, entity_index_t eindex, int from_index) {
    int n = pool->n;
    if (from_index >= n) return -1;
    uint32_t eid = index_(eindex);
    entity_index_t *a = pool->id;
    uint32_t from_id = index_(a[from_index]);
    if (eid <= from_id) {
        if (eid == from_id) return from_index;
        return -1;
    }
    return search_after(pool, eid, from_index);
}

static inline void move_tag(struct component_pool *pool, int from, int to, int delta) { pool->id[to] = DEC_ENTITY_INDEX(pool->id[from], delta); }

static inline void move_item(struct component_pool *pool, int from, int to, int delta) {
...
static void remove_entityid(struct entity_world *w, struct component_pool *removed) {
    int n = removed->n;
    if (n == 0) return;
    int i;
...
static int less_part(struct component_pool *c, entity_index_t vidx, int from) {
    if (from >= c->n) return c->n;
    entity_index_t *id = c->id;
    uint32_t v = index_(vidx);
    if (v <= index_(id[from])) return from;
    int begin = from;
    int end = c->n;
    while (begin < end) {
        int mid = (begin + end) / 2;
...
static void remove_all(lua_State *L, struct entity_world *w, struct component_pool *removed, int cid) {
    struct component_pool *pool = &w->c[cid];
    if (pool->n == 0) return;
...
static int ladd_component(lua_State *L) {
    struct entity_world *w = getW(L);
    uint32_t n = luaL_checkinteger(L, 2);
    if (n >= MAX_ENTITY) return luaL_error(L, "Invalid entity index %u", n);
    entity_index_t eid = make_index_(n);
    int cid = check_cid(L, w, 3);
    int index = ecs_add_component_id_(w, cid, eid);
    if (index < 0) return luaL_error(L, "Component %d exist", cid);
...
static int lupdate(lua_State *L) {
    ...
    if (removed->n > 0) {
        // mark removed
        for (i = 0; i < MAX_COMPONENT; i++) {
            if (i != removed_id) remove_all(L, w, removed, i);
        }
...
static int lclear_type(lua_State *L) {
    struct entity_world *w = getW(L);
...
static int add_sibling_index_(lua_State *L, struct entity_world *w, int cid, int index, int slibling_id) {
    entity_index_t eid;
    if (cid < 0) {
...
static int lcontext(lua_State *L) {
    struct entity_world *w = getW(L);
    ...
    static struct ecs_capi c_api = {
            entity_iter_, entity_clear_type_, entity_sibling_index_, entity_add_sibling_, entity_new_, entity_remove_, entity_enable_tag_, entity_disable_tag_, entity_get_lua_, entity_group_enable_,
    };
    ctx->api = &c_api;
    ...
    for (i = 1; i <= n; i++) {
        ...
        lua_pop(L, 1);
        int cid = ctx->cid[i];
        if (cid < 0 || cid >= MAX_COMPONENT) return luaL_error(L, "Invalid id (%d) at index %d", cid, i);
    }
    return 1;
}
...
static int ldeinit_world(lua_State *L) {
...
static const int sizeof_type[TYPE_COUNT] = {
        sizeof(int),      sizeof(float), sizeof(char), sizeof(int64_t),
...
static int check_type(lua_State *L) {
...
static void get_name(lua_State *L, char *name, int index) {
    size_t sz;
    const char *s = lua_tolstring(L, index, &sz);
    if (sz >= MAX_COMPONENT_NAME) luaL_error(L, "string '%s' is too long", s);
    memcpy(name, s, sz + 1);
}

static void get_field(lua_State *L, int i, struct group_field *f) {
...
static void write_value(lua_State *L, struct group_field *f, char *buffer) {
    int luat = lua_type(L, -1);
    char *ptr = buffer + f->offset;
    switch (f->type) {
        case TYPE_INT:
            if (!lua_isinteger(L, -1)) luaL_error(L, "Invalid .%s type %s (int)", f->key[0] ? f->key : "*", lua_typename(L, luat));
            *(int *)ptr = lua_tointeger(L, -1);
            break;
        case TYPE_FLOAT:
            if (luat != LUA_TNUMBER) luaL_error(L, "Invalid .%s type %s (float)", f->key[0] ? f->key : "*", lua_typename(L, luat));
            *(float *)ptr = lua_tonumber(L, -1);
            break;
        case TYPE_BOOL:
            if (luat != LUA_TBOOLEAN) luaL_error(L, "Invalid .%s type %s (bool)", f->key[0] ? f->key : "*", lua_typename(L, luat));
            *(unsigned char *)ptr = lua_toboolean(L, -1);
            break;
        case TYPE_INT64:
            if (!lua_isinteger(L, -1)) luaL_error(L, "Invalid .%s type %s (int64)", f->key[0] ? f->key : "*", lua_typename(L, luat));
        ...
        case TYPE_BYTE:
            ...
            break;
        case TYPE_DOUBLE:
            if (luat != LUA_TNUMBER) luaL_error(L, "Invalid .%s type %s (double)", f->key[0] ? f->key : "*", lua_typename(L, luat));
            *(double *)ptr = lua_tonumber(L, -1);
            break;
        case TYPE_USERDATA:
            if (luat != LUA_TLIGHTUSERDATA) luaL_error(L, "Invalid .%s type %s (pointer)", f->key[0] ? f->key : "*", lua_typename(L, luat));
...
static void write_value_check(lua_State *L, struct group_field *f, const char *buffer, const char *name) {
...
static inline void write_component(lua_State *L, int field_n, struct group_field *f, int index, char *buffer) {
    int i;
    for (i = 0; i < field_n; i++) {
...
static inline void write_component_check(lua_State *L, int field_n, struct group_field *f, int index, const char *buffer, const char *name) {
    int i;
    for (i = 0; i < field_n; i++) {
        lua_getfield(L, index, f[i].key);
...
static void read_value(lua_State *L, struct group_field *f, const char *buffer) {
...
static void read_component(lua_State *L, int field_n, struct group_field *f, int index, const char *buffer) {
    int i;
    for (i = 0; i < field_n; i++) {
...
static int get_len(lua_State *L, int index) {
    lua_len(L, index);
...
static inline int is_temporary(int attrib) {
    if (attrib & COMPONENT_FILTER) return 0;
    return (attrib & COMPONENT_IN) == 0 && (attrib & COMPONENT_OUT) == 0;
}

static int get_write_component(lua_State *L, int lua_index, const char *name, struct group_field *f, struct component_pool *c) {
...
static void write_component_object_check(lua_State *L, int n, struct group_field *f, const void *buffer, const char *name) {
    if (f->key[0] == 0) {
...
static int remove_tag(lua_State *L, int lua_index, const char *name) {
    int r = 0;
    switch (lua_getfield(L, lua_index, name)) {
        case LUA_TNIL:
            r = 1;
...
static int update_iter(lua_State *L, int world_index, int lua_index, struct group_iter *iter, int idx, int mainkey, int skip) {
...
static void update_last_index(lua_State *L, int world_index, int lua_index, struct group_iter *iter, int idx) {
...
static void check_update(lua_State *L, int world_index, int lua_index, struct group_iter *iter, int idx) {
    int mainkey = iter->k[0].id;
    int i;
    struct group_field *f = iter->f;
...
static void read_component_in_field(lua_State *L, int lua_index, const char *name, int n, struct group_field *f, void *buffer) {
    if (n == 0) {
        // It's tag
...
// -1 : end ; 0 : next ; 1 : succ
static int query_index(struct group_iter *iter, int skip, int mainkey, int idx, unsigned int index[MAX_COMPONENT]) {
...
static void read_iter(lua_State *L, int world_index, int obj_index, struct group_iter *iter, unsigned int index[MAX_COMPONENT]) {
    struct group_field *f = iter->f;
    int i;
    for (i = 0; i < iter->nkey; i++) {
        struct group_key *k = &iter->k[i];
...
static inline struct group_iter *submit_index(lua_State *L, int world_index, int iter_index, int i, int check) {
...
static int lsubmit(lua_State *L) {
    int world_index = 1;
    int iter_index = 2;
    ...
    if (i < 1) return luaL_error(L, "Invalid iterator index %d", i);
    lua_pop(L, 1);
    submit_index(L, world_index, iter_index, i - 1, 0);
    return 0;
}

static inline int leach_group_(lua_State *L, int check) {
    struct group_iter *iter = lua_touserdata(L, 1);
    if (lua_rawgeti(L, 2, 1) != LUA_TNUMBER) {
        return luaL_error(L, "Invalid group iterator");
    }
    int i = lua_tointeger(L, -1);
    if (i < 0) return luaL_error(L, "Invalid iterator index %d", i);
    lua_pop(L, 1);
...
static int leach_group_nocheck(lua_State *L) { return leach_group_(L, 0); }

static int leach_group_check(lua_State *L) { return leach_group_(L, 1); }
...
static void create_key_cache(lua_State *L, struct group_key *k, struct group_field *f) {
...
static inline int lpairs_group_(lua_State *L, int check) {
    struct group_iter *iter = lua_touserdata(L, 1);
    ...
    for (i = 0; i < iter->nkey; i++) {
        ...
        f += k->field_n;
        if (k->attrib & COMPONENT_OPTIONAL) ++opt;
    }
    if (opt) {
...
static inline int lpairs_group(lua_State *L) { return lpairs_group_(L, 0); }

static inline int lpairs_group_check(lua_State *L) { return lpairs_group_(L, 1); }

static int lfirst(lua_State *L) {
...
static int check_boolean(lua_State *L, const char *key) {
    int r = 0;
...
static int is_value(lua_State *L, struct group_field *f) {
...
static int get_key(struct entity_world *w, lua_State *L, struct group_key *key, struct group_field *f) {
...
static int need_input(struct group_iter *iter, struct group_key *key) {
    int i;
    if (!(key->attrib & COMPONENT_IN)) return 0;
    if (key->attrib & COMPONENT_OPTIONAL) {
        for (i = 0; i < iter->nkey; i++) {
            struct group_key *okey = &iter->k[i];
            if (key->id == okey->id && !(okey->attrib & COMPONENT_FILTER)) return 0;
        }
        return 1;
    }
    // must input
    ...
    for (i = 0; i < iter->nkey; i++) {
        struct group_key *okey = &iter->k[i];
        if (key->id == okey->id && !(okey->attrib & COMPONENT_FILTER) && !(okey->attrib & COMPONENT_OPTIONAL)) return 0;
    }
    return 1;
}

static struct group_iter *create_group_iter(lua_State *L, int nkey, int field_n) {
...
static int generate_diff(lua_State *L, struct group_iter *origin, struct group_iter *ext) {
    int field_n = 0;
    int n = 0;
    int i;
    ...
    if (origin->world != ext->world) luaL_error(L, "Different world");
...
static int merge_key(struct group_iter *origin, struct group_key *key) {
    ...
    if (key->attrib & COMPONENT_IN) {
        if (!(okey->attrib & COMPONENT_IN)) return 0;
    }
    if (key->attrib & COMPONENT_OUT) {
        if (!(okey->attrib & COMPONENT_OUT)) return 0;
    }
...
static int generate_merge(lua_State *L, struct group_iter *origin, struct group_iter *ext) {
    int field_n = 0;
    int n = 0;
    int i;
    int dirty = 0;
    ...
    for (i = 0; i < ext->nkey; i++) {
        ...
        if (m > 0) {
            *key = ext->k[i];
            ...
            assert(okey);
            if (is_temporary(ext->k[i].attrib) && !is_temporary(okey->attrib)) luaL_error(L, "Change temporary");
...
// diff : input iter
// merge : for submit
static int lmergeiter(lua_State *L) {
...
static int lgroupiter(lua_State *L) {
    struct entity_world *w = getW(L);
    luaL_checktype(L, 2, LUA_TTABLE);
    ...
    for (i = 0; i < nkey; i++) {
        ...
        lua_pop(L, 1);
        if (c->stride == STRIDE_LUA) {
            if (n != 0) return luaL_error(L, ".%s is object component, no fields needed", iter->k[i].name);
...
static int lcheck_iter(lua_State *L) {
...
static int lindex_entity(lua_State *L) {
    struct entity_world *w = getW(L);
    uint64_t eid = (uint64_t)luaL_checkinteger(L, 2);
    int index = entity_id_find(&w->eid, eid);
    if (index < 0) return luaL_error(L, "Missing entity %d", eid);
    lua_pushinteger(L, index);
    return 1;
}
...
static int lremove(lua_State *L) {
    ...
    if (lua_isinteger(L, 2)) {
        ...
        if (index < 0) return luaL_error(L, "No eid %d", lua_tointeger(L, 2));
...
static int lobject(lua_State *L) {
    ...
    if (c->stride == STRIDE_LUA) {
        ...
        if (lua_type(L, 2) != LUA_TBOOLEAN) return luaL_error(L, "%s is a tag, need boolean", iter->k[0].name);
...
static int ldumpid(lua_State *L) {
    struct entity_world *w = getW(L);
    int cid = luaL_checkinteger(L, 2);
    if (cid == ENTITYID_TAG) {
        int n = w->eid.n;
...
static int lfilter(lua_State *L) {
    ...
    for (i = 0; entity_iter_(w, mainkey, i); i++) {
        ...
        if (j == iter->nkey) entity_enable_tag_(w, mainkey, i, tagid);
    }
    return 0;
}

int laccess(lua_State *L) {
    struct entity_world *w = getW(L);
    uint64_t eid = (uint64_t)luaL_checkinteger(L, 2);
    int idx = entity_id_find(&w->eid, eid);
    if (idx < 0) return luaL_error(L, "eid %d not found", idx);
    struct group_iter *iter = lua_touserdata(L, 3);
    int value_index = 4;
    if (iter->nkey > 1) return luaL_error(L, "More than one key in pattern");
    if (iter->world != w) return luaL_error(L, "World mismatch");
...
// 2: groupid
// 3: eid
static int lgroup_add(lua_State *L) {
    struct entity_world *w = getW(L);
...
// 2: tagid
// 3...: groupids
static int lgroup_enable(lua_State *L) {
    struct entity_world *w = getW(L);
...
static int lgroup_get(lua_State *L) {
    struct entity_world *w = getW(L);
    int groupid = luaL_checkinteger(L, 2);
...
int metadot_bind_ecs_core(lua_State *L) {
    ...
    // metadot_load(L, l, engine_funcs_name_ecs);
...
static int ltestuserdata(lua_State *L) {
    struct ecs_context *ctx = lua_touserdata(L, 1);
...
static int lgetlua(lua_State *L) {
    struct ecs_context *ctx = lua_touserdata(L, 1);
...
static int lsiblinglua(lua_State *L) {
    struct ecs_context *ctx = lua_touserdata(L, 1);
    int index = luaL_checkinteger(L, 2);
...
int metadot_bind_ecs_test(lua_State *L) {
    luaL_checkversion(L);
    luaL_Reg l[] = {
            {"test", ltest}, {"sum", lsum}, {"testuserdata", ltestuserdata}, {"getlua", lgetlua}, {"siblinglua", lsiblinglua}, {NULL, NULL},
    };
    luaL_newlib(L, l);
    return 1;
}
