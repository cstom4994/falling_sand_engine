...
public:
    ...
    void RegisterCommand(const String &name, const String &description, Fn function, Args... args) {
        ...
        if (name.NextPoi(name_index).first != name.End()) throw CVar::Exception("ERROR: Whitespace separated command names are forbidden");
        ...
        auto help = [this, command_name]() { Log(LOG) << m_Commands[command_name]->Help() << CVar::endl; };
...
protected:
    ...
    std::string RegisterVariableAux(const String &name, T &var) {
        ...
        size_t name_index = 0;
        auto range = name.NextPoi(name_index);
        if (name.NextPoi(name_index).first != name.End()) throw CVar::Exception("ERROR: Whitespace separated variable names are forbidden");
