...
#define R_GET_PIXEL(surface, x, y)                                                           \
    *((U32 *) ((U8 *) surface->pixels + ((y) *surface->pitch) + ((x) * sizeof(U32))))
...
#define R_alloc(allocator, size)                                                                   \
    ((allocator).allocator_proc(&(allocator), R_current_source_location, R_allocator_mode_alloc,   \
                                (R_lit(R_allocator_args){0, (size), 0})))
#define R_free(allocator, ptr)                                                                     \
    ((allocator).allocator_proc(&(allocator), R_current_source_location, R_allocator_mode_free,    \
                                (R_lit(R_allocator_args){(ptr), 0, 0})))
#define R_realloc(allocator, ptr, new_size, old_size)                                              \
    ((allocator).allocator_proc(&(allocator), R_current_source_location, R_allocator_mode_realloc, \
                                (R_lit(R_allocator_args){(ptr), (new_size), (old_size)})))
...
typedef void *(R_allocator_proc) (struct R_allocator *this_allocator,
...
R_public void *R_libc_allocator_wrapper(R_allocator *this_allocator,
                                        R_source_location source_location, R_allocator_mode mode,
                                        R_allocator_args args);
...
#define R_read_file(io, filename, dst, dst_size)                                                   \
    ((io).read_file_proc((io).user_data, filename, dst, dst_size))
...
#pragma region error
#define R_make_recorded_error(error_type)                                                          \
    (R_lit(R_recorded_error){R_current_source_location, error_type})
...
#define R_for_str_split(iter, src, split_by)                                                       \
    ...
    for (; R_str_valid(R_macro_var(src_));                                                         \
         iter = R_str_pop_first_split(&R_macro_var(src_), R_macro_var(split_by_)))
#pragma endregion
...
R_public R_vec2 R_center_to_object(
        R_sizef center_this,
...
R_public void R_vec3_ortho_normalize(
        R_vec3 *v1,
...
R_public R_vec3
R_vec3_rotate_by_quaternion(R_vec3 v, R_quaternion q);// R_transform a vector by quaternion rotation
...
R_public R_vec3 R_vec3_barycenter(
        R_vec3 p, R_vec3 a, R_vec3 b,
...
R_public float R_mat_trace(
        R_mat mat);// Returns the trace of the matrix (sum of the values along the diagonal)
...
R_public R_mat R_mat_perspective(
        double fovy, double aspect, double near_val,
...
R_public R_quaternion
R_quaternion_lerp(R_quaternion q1, R_quaternion q2,
                  float amount);// Calculate linear interpolation between two quaternions
R_public R_quaternion
R_quaternion_nlerp(R_quaternion q1, R_quaternion q2,
                   float amount);// Calculate slerp-optimized interpolation between two quaternions
R_public R_quaternion R_quaternion_slerp(
        R_quaternion q1, R_quaternion q2,
        float amount);// Calculates spherical linear interpolation between two quaternions
R_public R_quaternion R_quaternion_from_vec3_to_vec3(
        R_vec3 from,
        R_vec3 to);// Calculate quaternion based on the rotation from one vector to another
R_public R_quaternion
R_quaternion_from_mat(R_mat mat);// Returns a quaternion for a given rotation matrix
R_public R_mat R_quaternion_to_mat(R_quaternion q);// Returns a matrix for a given quaternion
R_public R_quaternion R_quaternion_from_axis_angle(
        R_vec3 axis,
        float angle);// Returns rotation quaternion for an angle and axis. NOTE: angle must be provided in radians
R_public void R_quaternion_to_axis_angle(
        R_quaternion q, R_vec3 *outAxis,
        float *outAngle);// Returns the rotation angle and axis for a given quaternion
R_public R_quaternion R_quaternion_from_euler(
        float roll, float pitch, float yaw);// Returns he quaternion equivalent to Euler angles
R_public R_vec3 R_quaternion_to_euler(
        R_quaternion
                q);// Return the Euler angles equivalent to quaternion (roll, pitch, yaw). NOTE: Angles are returned in a R_vec3 struct in degrees
R_public R_quaternion R_quaternion_transform(
        R_quaternion q, R_mat mat);// R_transform a quaternion given a transformation matrix
...
R_public R_bool R_check_collision_circle_rec(
        R_vec2 center, float radius, R_rec rec);// Check collision between circle and rectangle
...
R_public R_rec
R_get_collision_rec(R_rec rec1, R_rec rec2);// Get collision rectangle for two rectangles collision
...
R_public R_bool R_check_collision_boxes(
        R_bounding_box box1, R_bounding_box box2);// Detect collision between two bounding boxes
R_public R_bool
R_check_collision_box_sphere(R_bounding_box box, R_vec3 center,
                             float radius);// Detect collision between box and sphere
R_public R_bool R_check_collision_ray_sphere(
        R_ray ray, R_vec3 center, float radius);// Detect collision between ray and sphere
R_public R_bool R_check_collision_ray_sphere_ex(
        R_ray ray, R_vec3 center, float radius,
        R_vec3 *collision_point);// Detect collision between ray and sphere, returns collision point
R_public R_bool
R_check_collision_ray_box(R_ray ray, R_bounding_box box);// Detect collision between ray and box

R_public R_ray_hit_info
R_collision_ray_triangle(R_ray ray, R_vec3 p1, R_vec3 p2,
                         R_vec3 p3);// Get collision info between ray and triangle
R_public R_ray_hit_info R_collision_ray_ground(
        R_ray ray,
