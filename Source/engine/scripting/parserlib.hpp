...
#define COUNTER_INC                                                                                \
    ...
    {                                                                                              \
        enum {                                                                                     \
            value = Counter<__LINE__ - 1>::value + 1                                               \
        };                                                                                         \
    }
    ...
    public:
        ...
        select_last_t<Args...> *getByPath() {
            int types[] = {id<Args>()...};
            return static_cast<select_last_t<Args...> *>(
                    getByTypeIds(std::begin(types), std::end(types)));
        }
    ...
    public:
        void add_members(std::initializer_list<ast_member *> members) {
            for (auto member: members) { m_members.push_back(member); }
        }
    ...
    public:
        ...
        void dup(const _ast_list &src) {
            for (ast_node *obj: src.m_objects) { push_back(obj); }
        }
    ...
    public:
        ...
        virtual void construct(ast_stack &st) override {
            ...
            if (Required && m_objects.empty()) { throw std::logic_error("Invalid AST stack."); }
        }
