        ...
        if (_info.node) {
            try {
                ...
                if (_config.profiling) {
                    auto start = std::chrono::high_resolution_clock::now();
                    transformBlock(block, out, config.implicitReturnRoot ? ExpUsage::Return : ExpUsage::Common, nullptr, true);
                    auto stop = std::chrono::high_resolution_clock::now();
                    std::chrono::duration<double> diff = stop - start;
    ...
    void transformClassDecl(ClassDecl_t *classDecl, str_list &out, ExpUsage usage, ExpList_t *expList = nullptr) {
        ...
        if (!parentVar.empty()) {
            ...
            _buf << "for "sv << key << ',' << val << " in pairs "sv << parentVar << ".__base"sv << '\n'
                 << '\t' << baseVar << '[' << key << "]="sv << val << " if "sv << baseVar << '[' << key << "]==nil and "sv << key << "\\match(\"^__\") and not ("sv << key << "==\"__index\" and "sv
                 << val << "=="sv << parentVar << ".__base)"sv;
            transformBlock(toAst<Block_t>(clearBuf(), x), temp, ExpUsage::Common);
        }
