...
namespace mu {
    ...
    std::unordered_set<std::string> LuaKeywords = {
            "and"s,      "break"s,  "do"s,   "else"s, "elseif"s, "end"s,  "false"s, "for"s,
            "function"s, "goto"s,   "if"s,   "in"s,   "local"s,  "nil"s,  "not"s,   "or"s,
            "repeat"s,   "return"s, "then"s, "true"s, "until"s,  "while"s};

    std::unordered_set<std::string> Keywords = {
            "and"s,      "break"s,  "do"s,      "else"s, "elseif"s, "end"s,    "false"s, "for"s,
            "function"s, "goto"s,   "if"s,      "in"s,   "local"s,  "nil"s,    "not"s,   "or"s,
            "repeat"s,   "return"s, "then"s,    "true"s, "until"s,  "while"s,  "as"s,    "class"s,
            "continue"s, "export"s, "extends"s, "from"s, "global"s, "import"s, "macro"s, "switch"s,
            "try"s,      "unless"s, "using"s,   "when"s, "with"s};
    ...
    MuParser::MuParser() {
        ...
        Num = ("0x" >>
               (+num_lit >> ('.' >> +num_lit >> -num_expo_hex | num_expo_hex | lj_num | true_()) |
                ('.' >> +num_lit >> -num_expo_hex))) |
              (+num_char >> ('.' >> +num_char >> -num_expo | num_expo | lj_num | true_())) |
              ('.' >> +num_char >> -num_expo);
        ...
        empty_block_error = pl::user(true_(), [](const item_t &item) {
            throw ParserError("must be followed by a statement or an indented block", *item.begin,
                              *item.end);
            return false;
        });
...
#define disable_do_chain_arg_table_block(patt)                                                     \
    (DisableDoChainArgTableBlock >>                                                                \
     ((patt) >> EnableDoChainArgTableBlock | EnableDoChainArgTableBlock >> Cut))
#define disable_arg_table_block(patt)                                                              \
    (DisableArgTableBlock >> ((patt) >> EnableArgTableBlock | EnableArgTableBlock >> Cut))
#define body_with(str)                                                                             \
    (Space >> (key(str) >> Space >> (InBlock | Statement) | InBlock | empty_block_error))
...
#define body (Space >> (InBlock | Statement | empty_block_error))
        ...
        LocalAttrib =
                (const_attrib >> Seperator >> local_const_item >> *(sym(',') >> local_const_item) |
                 close_attrib >> Seperator >> Space >> Variable >>
                         *(sym(',') >> Space >> Variable)) >>
                Assign;

        colon_import_name = sym('\\') >> Space >> Variable;
        ImportName = colon_import_name | Space >> Variable;
        ImportNameList = Seperator >> *SpaceBreak >> ImportName >>
                         *((+SpaceBreak | sym(',') >> *SpaceBreak) >> ImportName);
        ...
        import_literal_chain =
                Seperator >> import_literal_inner >> *(expr('.') >> import_literal_inner);
        ImportLiteral = sym('\'') >> import_literal_chain >> symx('\'') |
                        sym('"') >> import_literal_chain >> symx('"');

        macro_name_pair = Space >> MacroName >> Space >> symx(':') >> Space >> MacroName;
        import_all_macro = expr('$');
        ImportTabItem = variable_pair | normal_pair | sym(':') >> MacroName | macro_name_pair |
                        Space >> import_all_macro | meta_variable_pair | meta_normal_pair | Exp;
        ImportTabList = ImportTabItem >> *(sym(',') >> ImportTabItem);
        ImportTabLine = (PushIndent >> (ImportTabList >> PopIndent | PopIndent)) | Space;
        import_tab_lines = SpaceBreak >> ImportTabLine >>
                           *(-sym(',') >> SpaceBreak >> ImportTabLine) >> -sym(',');
        ImportTabLit = Seperator >> (sym('{') >> -ImportTabList >> -sym(',') >> -import_tab_lines >>
                                             White >> sym('}') |
                                     KeyValue >> *(sym(',') >> KeyValue));

        ImportAs = ImportLiteral >>
                   -(Space >> key("as") >> Space >> (ImportTabLit | Variable | import_all_macro));
        ...
        With = Space >> key("with") >> -existential_op >>
               disable_do_chain_arg_table_block(WithExp) >> body_with("do");
        SwitchCase = Space >> key("when") >> disable_chain(disable_arg_table_block(SwitchList)) >>
                     body_with("then");
        SwitchElse = Space >> key("else") >> body;

        SwitchBlock = *(Break >> *EmptyLine >> CheckIndent >> SwitchCase) >>
                      -(Break >> *EmptyLine >> CheckIndent >> SwitchElse);

        exp_not_tab = not_(simple_table | TableLit) >> Exp;

        SwitchList = Seperator >> (and_(simple_table | TableLit) >> Exp |
                                   exp_not_tab >> *(sym(',') >> exp_not_tab));
        Switch = Space >> key("switch") >> Exp >> Seperator >>
                 (SwitchCase >> Space >>
        ...
        IfElseIf = -(Break >> *EmptyLine >> CheckIndent) >> Space >> key("elseif") >> IfCond >>
                   body_with("then");
        ...
        Repeat = key("repeat") >> Body >> Break >> *EmptyLine >> CheckIndent >> Space >>
                 key("until") >> Exp;
        ...
        catch_block = Break >> *EmptyLine >> CheckIndent >> Space >> key("catch") >> Space >>
                      Variable >> InBlock;
        ...
        CompFor = key("for") >> Space >> Variable >> sym('=') >> Exp >> sym(',') >> Exp >>
                  -for_step_value;
        CompClause = Space >> (CompFor | CompForEach | key("when") >> Exp);

        Assign = sym('=') >> Seperator >>
                 (With | If | Switch | TableBlock | Exp >> *(Space >> set(",;") >> Exp));

        update_op = expr("..") | expr("//") | expr("or") | expr("and") | expr(">>") | expr("<<") |
                    expr("??") | set("+-*/%&|");
        ...
        unary_operator = expr('-') >> not_(set(">=") | space_one) | expr('#') |
                         expr('~') >> not_(expr('=') | space_one) | expr("not") >> not_(AlphaNum);
        ...
        BinaryOperator = (expr("or") >> not_(AlphaNum)) | (expr("and") >> not_(AlphaNum)) |
                         expr("<=") | expr(">=") | expr("~=") | expr("!=") | expr("==") |
                         expr("..") | expr("<<") | expr(">>") | expr("//") | set("+-*/%><|&~");
        ...
        chain_block =
                ...
                +SpaceBreak >> Advance >>
                ensure(chain_line >> *(+SpaceBreak >> chain_line), PopIndent);
        ChainValue = Space >> Seperator >> (Chain | Callable) >> -existential_op >>
                     -(InvokeArgs | chain_block) >> -table_appending_op;
        ...
        ChainItem = Invoke >> -existential_op | DotChainItem >> -existential_op | Slice |
        ...
        default_value = true_();
        Slice = symx('[') >> not_('[') >> (Exp | default_value) >> sym(',') >>
                (Exp | default_value) >> (sym(',') >> Exp | default_value) >> sym(']');

        Invoke = Seperator >> (FnArgs | SingleString | DoubleString | and_(expr('[')) >> LuaString |
                               and_(expr('{')) >> TableLit);

        SpreadExp = sym("...") >> Exp;

        TableValue = variable_pair_def | normal_pair_def | meta_variable_pair_def |
                     meta_normal_pair_def | SpreadExp | normal_def;

        table_lit_lines = SpaceBreak >> TableLitLine >>
                          *(-sym(',') >> SpaceBreak >> TableLitLine) >> -sym(',');

        TableLit = sym('{') >> Seperator >> -TableValueList >> -sym(',') >> -table_lit_lines >>
                   White >> sym('}');
        ...
        TableBlockIndent = sym('*') >> Seperator >>
                           disable_arg_table_block(KeyValueList >> -sym(',') >>
                                                   -(+SpaceBreak >> Advance >>
                                                     ensure(KeyValueList >> -sym(',') >>
                                                                    *(+SpaceBreak >> KeyValueLine),
                                                            PopIndent)));

        class_member_list = Seperator >> KeyValue >> *(sym(',') >> KeyValue);
        ClassLine = CheckIndent >> (class_member_list | Space >> Statement) >> -sym(',');
        ClassBlock = +SpaceBreak >> Advance >> Seperator >> ClassLine >>
                     *(+SpaceBreak >> ClassLine) >> PopIndent;
        ...
        Export = pl::user(key("export"),
                 ...
                 (pl::user(Space >> export_default >> Exp,
                           [](const item_t &item) {
                               State *st = reinterpret_cast<State *>(item.user_data);
                               if (st->exportDefault) {
                                   throw ParserError("export default has already been declared",
                                                     *item.begin, *item.end);
                               }
                               if (st->exportCount > 1) {
                                   throw ParserError("there are items already being exported",
                                                     *item.begin, *item.end);
                               }
                   ...
                   pl::user(true_(),
                            [](const item_t &item) {
                                ...
                                if (st->exportDefault && st->exportCount > 1) {
                                    throw ParserError("can not export any more items when 'export "
                                                      "default' is declared",
                                                      *item.begin, *item.end);
                                }
                                return true;
        ...
        normal_pair = (KeyName | sym('[') >> not_('[') >> Exp >> sym(']') | Space >> String) >>
                      symx(':') >> not_(':') >> (Exp | TableBlock | +SpaceBreak >> Exp);
        ...
        KeyValueLine = CheckIndent >> (KeyValueList >> -sym(',') | TableBlockIndent |
                                       Space >> expr('*') >> (SpreadExp | Exp | TableBlock));

        FnArgDef = (Variable | SelfName >> -existential_op) >> -(sym('=') >> Space >> Exp);

        FnArgDefList =
                Space >> Seperator >> ((FnArgDef >> *((sym(',') | Break) >> White >> FnArgDef) >>
        ...
        PipeBody = Seperator >> PipeOperator >> unary_exp >>
                   *(+SpaceBreak >> CheckIndent >> Space >> PipeOperator >> unary_exp);
        ...
        invoke_args_with_table =
                sym(',') >> (TableBlock | SpaceBreak >> Advance >> ArgBlock >> -arg_table_block) |
                arg_table_block;

        leading_spaces_error = pl::user(
                +space_one >> expr('(') >> Exp >> +(sym(',') >> Exp) >> sym(')'),
                [](const item_t &item) {
                    throw ParserError(
        ...
        InvokeArgs = not_(set("-~")) >> Seperator >>
                     ((Exp >> *(sym(',') >> Exp) >> -invoke_args_with_table) | arg_table_block |
                      leading_spaces_error);

        const_value = (expr("nil") | expr("true") | expr("false")) >> not_(AlphaNum);

        SimpleValue = Space >> (const_value | If | Switch | Try | With | ClassDecl | ForEach | For |
                                While | Do | unary_value | TblComprehension | TableLit |
                                Comprehension | FunLit | Num);
        ...
        mu_comment =
                check_indent >>
                (mu_multiline_comment >> *(set(" \t") | mu_multiline_comment) >> -mu_line_comment |
                 mu_line_comment) >>
                and_(Break);
        ...
        statement_sep = and_(*SpaceBreak >> CheckIndent >> Space >>
                             (set("($'\"") | expr("[[") | expr("[=")));
        Statement = Seperator >> -(mu_comment >> *(Break >> mu_comment) >> Break >> CheckIndent) >>
                    Space >>
                    (Import | While | Repeat | For | ForEach | Return | Local | Global | Export |
                     Macro | MacroInPlace | BreakLoop | Label | Goto | ShortTabAppending |
        ...
        empty_line_break =
                (check_indent >> (MultiLineComment >> Space | Comment) |
                 advance >> ensure(MultiLineComment >> Space | Comment, PopIndent) | plain_space) >>
                and_(Break);
        ...
        Line = CheckIndent >> Statement | empty_line_break |
    ...
    ParseInfo MuParser::parse(std::string_view codes, rule &r) {
        ParseInfo res;
        if (codes.substr(0, 3) == "\xEF\xBB\xBF"sv) { codes = codes.substr(3); }
        try {
    ...
    std::string MuParser::toString(ast_node *node) {
        return _converter.to_bytes(std::wstring(node->m_begin.m_it, node->m_end.m_it));
    }

    std::string MuParser::toString(input::iterator begin, input::iterator end) {
        return _converter.to_bytes(std::wstring(begin, end));
    }

    input MuParser::encode(std::string_view codes) {
    ...
    std::string ParseInfo::errorMessage(std::string_view msg, const input_range *loc) const {
        ...
        while (col < static_cast<int>(line.size()) && (line[col] == ' ' || line[col] == '\t')) {
            col++;
        }
