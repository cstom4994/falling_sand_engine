...
// Link to https://github.com/pigpigyyy/Muscript
...
#define SERROR(msg, node)                                                                          \
    throw std::logic_error(_info.errorMessage(                                                     \
            "[File] "s + __FILE__ + ",\n[Func] "s + __FUNCTION__ + ",\n[Line] "s +                 \
    ...
    public:
#ifndef MU_NO_MACRO
        MuCompilerImpl(lua_State *sharedState, const std::function<void(void *)> &luaOpen,
                       bool sameModule)
            : L(sharedState), _luaOpen(luaOpen) {
            BLOCK_START
            BREAK_IF(!sameModule);
            BREAK_IF(!L);
            _sameModule = true;
        ...
        CompileInfo compile(std::string_view codes, const MuConfig &config) {
            ...
            if (!config.options.empty()) { options = std::make_unique<Options>(config.options); }
            DEFER(clear());
            if (_info.node) {
                try {
                    ...
                    if (_info.exportMacro) {
                        for (auto stmt_: block->statements.objects()) {
                            ...
                            switch (stmt->content->getId()) {
                                    ...
                                    if (auto importNode = stmt->content.as<Import_t>()) {
                                        if (auto importAs = importNode->content.as<ImportAs_t>()) {
                                            if (importAs->target.is<import_all_macro_t>()) {
                                                break;
                                            } else if (auto tab = importAs->target
                                                                          .as<ImportTabLit_t>()) {
                                                bool macroImportingOnly = true;
                                                for (auto item: tab->items.objects()) {
                                                    if (!ast_is<MacroName_t, macro_name_pair_t,
                                ...
                                default:
                                    throw std::logic_error(_info.errorMessage(
                                            "macro exporting module only accepts macro definition, "
                    ...
                    if (_config.profiling) {
                        ...
                        transformBlock(block, out,
                                       config.implicitReturnRoot ? ExpUsage::Return
                                                                 : ExpUsage::Common,
                                       nullptr, true);
                        auto stop = std::chrono::high_resolution_clock::now();
                        std::chrono::duration<double> diff = stop - start;
                    ...
                    if (!gotos.empty()) {
                        for (const auto &gotoNode: gotos) {
                            ...
                            if (noLabel) {
                                throw std::logic_error(_info.errorMessage(
                                        "no visible label '"s + gotoNode.label + "' for <goto>"s,
                                        gotoNode.ptr->label));
                            }
                        }
                    }
                    if (config.lintGlobalVariable) {
                        ...
                        std::sort(globals->begin(), globals->end(),
                                  [](const GlobalVar &varA, const GlobalVar &varB) {
                    ...
                    if (L) {
                        int top = lua_gettop(L);
                        DEFER(lua_settop(L, top));
                        if (!options) { options = std::make_unique<Options>(); }
                        pushMu("options"sv);
                    ...
                    return {std::move(out.back()), Empty,     std::move(globals),
                            std::move(options),    parseTime, compileTime};
    ...
    private:
        ...
        bool isDefined(const std::string &name) const {
            ...
            if (allows) { checkShadowScopeOnly = allows->find(name) == allows->end(); }
            for (auto it = _scopes.rbegin(); it != _scopes.rend(); ++it) {
        ...
        void checkConst(const std::string &name, ast_node *x) const {
            if (isConst(name)) {
                throw std::logic_error(_info.errorMessage(
                        "attempt to assign to const variable '"s + name + '\'', x));
            }
        }
        ...
        void addGlobalVar(const std::string &name, ast_node *x) {
            if (isLocal(name))
                throw std::logic_error(
                        _info.errorMessage("can not declare a local variable to be global"sv, x));
            auto &scope = _scopes.back();
            if (!scope.globals) {
        ...
        std::string getUnusedLabel(std::string_view label) const {
            ...
            if (static_cast<int>(_labels.size()) <= scopeIndex ||
                _labels[scopeIndex] == std::nullopt) {
        ...
        const std::string nll(ast_node *node) const {
            if (_config.reserveLineNumber) {
                return " -- "s + std::to_string(node->m_begin.m_line + _config.lineOffset) +
                       _newLine;
            } else {
                return _newLine;
            }
        ...
        std::string join(const str_list &items) {
            if (items.empty()) return Empty;
                ...
                return items.front();
            for (const auto &item: items) { _joinBuf << item; }
        ...
        std::string join(const str_list &items, std::string_view sep) {
            ...
            for (auto it = begin; it != items.end(); ++it) { _joinBuf << sepStr << *it; }
            auto result = _joinBuf.str();
            _joinBuf.str("");
            _joinBuf.clear();
            return result;
        ...
        unary_exp_t *singleUnaryExpFrom(ast_node *item) const {
            ...
            switch (item->getId()) {
                ...
                case id<unary_exp_t>(): {
                    ...
                    if (unary->expos.size() == 1) { return unary; }
                    return nullptr;
                }
                default:
                    break;
        ...
        Value_t *singleValueFrom(ast_node *item) const {
            if (auto unary = singleUnaryExpFrom(item)) {
                if (unary->ops.empty()) { return static_cast<Value_t *>(unary->expos.back()); }
            }
            return nullptr;
        }
        ...
        ast_ptr<false, Exp_t> newExp(Value_t *left, BinaryOperator_t *op, Value_t *right,
                                     ast_node *x) {
        ...
        Statement_t *lastStatementFrom(const node_container &stmts) const {
            if (!stmts.empty()) {
                auto it = stmts.end();
                --it;
                while (!static_cast<Statement_t *>(*it)->content && it != stmts.begin()) { --it; }
        ...
        Exp_t *lastExpFromStatement(Statement_t *stmt) {
            ...
            switch (stmt->content->getId()) {
                ...
                case id<Export_t>(): {
                    ...
                    if (auto action = exportNode->assign.get()) {
                        ...
                        switch (exportNode->target->getId()) {
                            ...
                            case id<ExpList_t>():
                                return static_cast<Exp_t *>(
                                        exportNode->target.to<ExpList_t>()->exprs.back());
                        }
                    }
                    return nullptr;
                }
                case id<Local_t>(): {
                    if (auto localValues = static_cast<Local_t *>(stmt->content.get())
                                                   ->item.as<local_values_t>()) {
                ...
                case id<Global_t>(): {
                    if (auto globalValues = static_cast<Global_t *>(stmt->content.get())
                                                    ->item.as<global_values_t>()) {
        ...
        ChainType specialChainValue(ChainValue_t *chainValue) const {
            if (isMacroChain(chainValue)) { return ChainType::Macro; }
            ...
            if (auto dot = ast_cast<DotChainItem_t>(chainValue->items.back())) {
                if (dot->name.is<Metatable_t>()) { return ChainType::Metatable; }
            }
            ...
            for (auto item: chainValue->items.objects()) {
                if (auto colonChain = ast_cast<ColonChainItem_t>(item)) {
                    ...
                    } else if (auto meta = colonChain->name.as<Metamethod_t>();
                               meta && !meta->item.is<Name_t>()) {
        ...
        std::string singleVariableFrom(ChainValue_t *chainValue) {
            ...
            if (!var) {
                if (auto self = callable->item.as<SelfName_t>()) { var = self->name.as<self_t>(); }
            }
            BREAK_IF(!var);
            str_list tmp;
        ...
        std::string singleVariableFrom(ast_node *expList, bool accessing) {
            ...
            BREAK_IF(!callable || !(callable->item.is<Variable_t>() ||
                                    callable->getByPath<SelfName_t, self_t>()));
            str_list tmp;
            if (accessing) {
                transformCallable(callable, tmp);
            } else {
            ...
            if (chainItems.size() == 1) {
                ...
                if (auto callable = ast_cast<Callable_t>(firstItem)) {
                    switch (callable->item->getId()) {
                        case id<Variable_t>():
                            checkConst(_parser.toString(callable->item.get()),
                                       callable->item.get());
                            return true;
                        case id<SelfName_t>():
                            return true;
            ...
            if (auto value = singleValueFrom(exp)) {
                ...
                switch (item->getId()) {
                    ...
                    case id<SimpleValue_t>(): {
                        ...
                        if (simpleValue->value.is<TableLit_t>()) { return true; }
                        return false;
                    }
        ...
        void checkAssignable(ExpList_t *expList) {
            for (auto exp_: expList->exprs.objects()) {
                ...
                if (!isAssignable(exp)) {
                    throw std::logic_error(
                            _info.errorMessage("left hand expression is not assignable"sv, exp));
                }
            }
        }
        ...
        bool isMacroChain(ChainValue_t *chainValue) const {
            ...
            if (callable && callable->item.is<MacroName_t>()) { return true; }
            return false;
        }
        ...
        std::string anonFuncStart() const {
            return !_varArgs.empty() && _varArgs.top().hasVar && _varArgs.top().usedVar
                           ? "(function(...)"s
                           : "(function()"s;
        }
        ...
        void transformStatement(Statement_t *statement, str_list &out) {
                ...
                if (auto assignment = assignmentFrom(statement)) {
                    ...
                    switch (appendix->item->getId()) {
                        ...
                        case id<while_line_t>(): {
                            throw std::logic_error(_info.errorMessage(
                                    "while-loop line decorator is not supported here"sv,
                                    appendix->item.get()));
                            break;
                        }
                        case id<CompInner_t>(): {
                            throw std::logic_error(_info.errorMessage(
                                    "for-loop line decorator is not supported here"sv,
                                    appendix->item.get()));
                            break;
                        }
                        default:
            ...
            switch (content->getId()) {
                ...
                case id<PipeBody_t>():
                    throw std::logic_error(
                            _info.errorMessage("pipe chain must be following a value"sv, x));
                    break;
                case id<ExpListAssign_t>(): {
                    ...
                    if (expListAssign->action) {
                        ...
                        if (auto singleValue = singleValueFrom(expList)) {
                            if (auto simpleValue = singleValue->item.as<SimpleValue_t>()) {
                                ...
                                switch (value->getId()) {
                                    case id<If_t>():
                                        transformIf(static_cast<If_t *>(value), out,
                                                    ExpUsage::Common);
                                        break;
                                    case id<ClassDecl_t>():
                                        transformClassDecl(static_cast<ClassDecl_t *>(value), out,
                                                           ExpUsage::Common);
                                        break;
                                    case id<Switch_t>():
                                        transformSwitch(static_cast<Switch_t *>(value), out,
                                                        ExpUsage::Common);
                                        break;
                                    case id<With_t>():
                                    ...
                                    case id<While_t>():
                                        ...
                                        break;
                                    case id<Do_t>():
                                        transformDo(static_cast<Do_t *>(value), out,
                                                    ExpUsage::Common);
                                        break;
                                    case id<Try_t>():
                                        transformTry(static_cast<Try_t *>(value), out,
                                                     ExpUsage::Common);
                                        break;
                                    case id<Comprehension_t>():
                                        transformCompCommon(static_cast<Comprehension_t *>(value),
                                                            out);
                                        break;
                                    default:
                        ...
                        throw std::logic_error(_info.errorMessage(
                                "expression list is not supported here"sv, expList));
                    }
                    break;
                }
            ...
            if (statement->needSep && !out.empty() && !out.back().empty()) {
                ...
                if (index != std::string::npos) {
                    ...
                    if (ind != std::string::npos) { ending = ending.substr(ind + 1); }
        ...
        std::string getDestrucureDefine(ExpListAssign_t *assignment) {
            ...
            if (info.assignment) { _buf << getPreDefineLine(info.assignment); }
                ...
                for (const auto &destruct: info.destructures) {
                    ...
                    for (const auto &item: destruct.items) {
                        if (!item.targetVar.empty()) {
                            if (addToScope(item.targetVar)) { defs.push_back(item.targetVar); }
                        }
                    }
        ...
        ExpList_t *expListFrom(Statement_t *statement) {
            if (auto expListAssign = statement->content.as<ExpListAssign_t>()) {
                if (!expListAssign->action) { return expListAssign->expList.get(); }
            }
            return nullptr;
        }
        ...
        ExpListAssign_t *assignmentFrom(Statement_t *statement) {
            if (auto expListAssign = statement->content.as<ExpListAssign_t>()) {
                if (expListAssign->action) { return expListAssign; }
            }
            return nullptr;
        }

        ast_ptr<false, ExpListAssign_t> assignmentFrom(Exp_t *target, ast_node *value,
                                                       ast_node *x) {
        ...
        void markDestructureConst(ExpListAssign_t *assignment) {
            ...
            for (auto &destruct: info.destructures) {
                for (auto &item: destruct.items) {
                    if (item.targetVar.empty()) {
                        throw std::logic_error(_info.errorMessage(
                                "can only declare variable as const"sv, item.target));
                    }
        ...
        void transformAssignment(ExpListAssign_t *assignment, str_list &out,
            ...
            if (exprs.size() < values.size()) {
                auto num = exprs.size();
                if (num > 1) {
                    _buf << "no more than "sv << num << " right values expected, got "sv
                         << values.size();
                } else {
                ...
                for (auto it = exprs.begin(); it != exprs.end(); ++it) {
                    ...
                    if (auto dot = ast_cast<DotChainItem_t>(chainValue->items.back())) {
                        ...
                        if (tmpChain->items.empty()) {
                            if (_withVars.empty()) {
                                throw std::logic_error(_info.errorMessage(
                                        "short dot/colon syntax must be called within a with block"sv,
                                        x));
                            } else {
                        ...
                        if (vit == values.end()) {
                            throw std::logic_error(
                                    _info.errorMessage("right value missing"sv, values.front()));
                        }
                        transformAssignItem(*vit, args);
                        _buf << indent() << globalVar("setmetatable"sv, x) << '('
                             << join(args, ", "sv) << ')' << nll(x);
                        ...
                        if (tmpChain->items.empty()) {
                            if (_withVars.empty()) {
                                throw std::logic_error(_info.errorMessage(
                                        "short table appending must be called within a with block"sv,
                                        x));
                            } else {
                                tmpChain->items.push_back(
                                        toAst<Callable_t>(_withVars.top(), chainValue));
                            }
                        }
                        ...
                        auto newAssignment = x->new_ptr<ExpListAssign_t>();
                        newAssignment->expList.set(
                                toAst<ExpList_t>(varName + "[#"s + varName + "+1]"s, x));
                        auto assign = x->new_ptr<Assign_t>();
                        if (vit == values.end()) {
                            throw std::logic_error(
                                    _info.errorMessage("right value missing"sv, values.front()));
            ...
            if (ast_is<Exp_t>(value)) {
                if (auto val = simpleSingleValueFrom(value)) { value = val->value.get(); }
            }
            switch (value->getId()) {
                case id<If_t>(): {
                ...
                case id<Comprehension_t>(): {
                    auto expList = assignment->expList.get();
                    std::string preDefine = getPreDefineLine(assignment);
                    transformComprehension(static_cast<Comprehension_t *>(value), out,
                                           ExpUsage::Assignment, expList);
                    out.back().insert(0, preDefine);
                    return;
                }
                case id<TblComprehension_t>(): {
                    auto expList = assignment->expList.get();
                    std::string preDefine = getPreDefineLine(assignment);
                    transformTblComprehension(static_cast<TblComprehension_t *>(value), out,
                                              ExpUsage::Assignment, expList);
                    out.back().insert(0, preDefine);
                    return;
                }
                ...
                case id<ClassDecl_t>(): {
                    auto expList = assignment->expList.get();
                    std::string preDefine = getPreDefineLine(assignment);
                    transformClassDecl(static_cast<ClassDecl_t *>(value), out, ExpUsage::Assignment,
                                       expList);
                    out.back().insert(0, preDefine);
                    return;
                }
                ...
                case id<TableLit_t>(): {
                    ...
                    if (hasSpreadExp(tableLit->values.objects())) {
                        auto expList = assignment->expList.get();
                        std::string preDefine = getPreDefineLine(assignment);
                        transformSpreadTable(tableLit->values.objects(), out, ExpUsage::Assignment,
                                             expList);
                        out.back().insert(0, preDefine);
                        return;
                    }
                ...
                case id<TableBlock_t>(): {
                    ...
                    if (hasSpreadExp(tableBlock->values.objects())) {
                        auto expList = assignment->expList.get();
                        std::string preDefine = getPreDefineLine(assignment);
                        transformSpreadTable(tableBlock->values.objects(), out,
                                             ExpUsage::Assignment, expList);
                        out.back().insert(0, preDefine);
                        return;
                    }
            ...
            if (auto chainValue = singleVal->item.as<ChainValue_t>()) {
                ...
                switch (type) {
                    ...
                    case ChainType::MetaFieldInvocation: {
                        std::string preDefine = getPreDefineLine(assignment);
                        transformChainValue(chainValue, out, ExpUsage::Assignment, expList, false,
                                            optionalDestruct);
                        out.back().insert(0, preDefine);
                        return;
                    }
            ...
            if (info.destructures.empty()) {
                ...
                str_list temp;
                if (info.assignment) { transformAssignmentCommon(info.assignment, temp); }
                auto x = assignment;
                for (auto &destruct: info.destructures) {
                    ...
                    if (!destruct.inlineAssignment && destruct.items.size() == 1) {
                        ...
                        if (pair.targetVar.empty() && pair.defVal) {
                        ...
                        } else if (auto val = singleValueFrom(destruct.value);
                                   val->item.is<ChainValue_t>()) {
                            ...
                            if (pair.structure) { newChain->items.dup(pair.structure->items); }
                            auto newAssignment =
                                    assignmentFrom(pair.target, newExp(newChain, val->item), x);
                            ...
                            if (pair.defVal) {
                                ...
                                if (!isNil) {
                                    auto stmt = toAst<Statement_t>(pair.targetVar + "=nil if "s +
                                                                           pair.targetVar + "==nil",
                                                                   pair.defVal);
                        ...
                        if (isLocalValue) {
                            ...
                            auto newAssignment =
                                    assignmentFrom(toAst<Exp_t>(objVar, x), destruct.value, x);
                            transformAssignment(newAssignment, temp);
                        }
                        ...
                        for (auto &item: destruct.items) {
                            if (!item.targetVar.empty()) {
                                if (!isDefined(item.targetVar)) { defs.push_back(item.targetVar); }
                            } else if (item.defVal) {
                                extraScope = true;
                                auto objVar = getUnusedName("_tmp_"sv);
                                addToScope(objVar);
                        ...
                        popScope();
                        if (_parser.match<Name_t>(destruct.valueVar) &&
                            isLocal(destruct.valueVar)) {
                            auto callable = toAst<Callable_t>(destruct.valueVar, destruct.value);
                            for (auto &v: values) { v->items.push_front(callable); }
                            if (extraScope) {
                                if (!defs.empty()) {
                                    ...
                                    temp.push_back(indent() + "local "s + join(defs, ", "sv) +
                                                   nll(x));
                                }
                                temp.push_back(indent() + "do"s + nll(x));
                                pushScope();
                            ...
                            auto newAssignment =
                                    assignmentFrom(targetVar, destruct.value, destruct.value);
                            transformAssignment(newAssignment, temp);
                            auto callable = singleValueFrom(targetVar)
                                                    ->item.to<ChainValue_t>()
                                                    ->items.front();
                            for (auto &v: values) { v->items.push_front(callable); }
                        }
                        ...
                        if (optionalDestruct) {
                            ...
                            auto newAssignment = x->new_ptr<ExpListAssign_t>();
                            auto assignList = x->new_ptr<ExpList_t>();
                            for (auto name: names) { assignList->exprs.push_back(name); }
                            newAssignment->expList.set(assignList);
                            auto assign = x->new_ptr<Assign_t>();
                            assign->values.dup(valueList->exprs);
                            newAssignment->action.set(assign);
                    ...
                    for (const auto &item: destruct.items) {
                        if (item.defVal) {
                            ...
                            if (!isNil) {
                                auto stmt = toAst<Statement_t>(item.targetVar + "=nil if "s +
                                                                       item.targetVar + "==nil",
                                                               item.defVal);
                                auto defAssign = stmt->content.as<ExpListAssign_t>();
                                auto assign = defAssign->action.as<Assign_t>();
        ...
        std::list<DestructItem> destructFromExp(ast_node *node, bool optional) {
            ...
            ast_ptr<false, existential_op_t> sep =
                    optional ? node->new_ptr<existential_op_t>() : nullptr;
            switch (node->getId()) {
                case id<Exp_t>(): {
                    auto item = singleValueFrom(node)->item.get();
                    if (!item)
                        throw std::logic_error(
                                _info.errorMessage("invalid destructure value"sv, node));
            ...
            if (!tableItems)
                throw std::logic_error(_info.errorMessage("invalid destructure value"sv, node));
                ...
                switch (pair->getId()) {
                    ...
                    case id<normal_def_t>(): {
                        ...
                        if (!isAssignable(static_cast<Exp_t *>(pair))) {
                            throw std::logic_error(
                                    _info.errorMessage("can't destructure value"sv, pair));
                        }
                        ...
                        if (ast_is<simple_table_t>(item) || item->getByPath<TableLit_t>()) {
                            ...
                            if (!subPairs.empty()) {
                                if (defVal) {
                                    throw std::logic_error(_info.errorMessage(
                                            "default value is not supported here"sv, defVal));
                                }
                            }
                            ...
                            pairs.push_back({exp, varName, chain, defVal});
                        }
                        break;
                    }
                    ...
                    case id<normal_pair_def_t>(): {
                        ...
                        if (np->key) {
                            if (auto key = np->key->getByPath<Name_t>()) {
                                ...
                                throw std::logic_error(_info.errorMessage(
                                        "unsupported key for destructuring"sv, np));
                            }
                        }
                        if (auto exp = np->value.as<Exp_t>()) {
                            if (!isAssignable(exp))
                                throw std::logic_error(
                                        _info.errorMessage("can't do destructure value"sv, exp));
        ...
        DestructureInfo extractDestructureInfo(ExpListAssign_t *assignment, bool varDefOnly,
                                               bool optional) {
            auto x = assignment;
            ...
            for (auto i = exprs.begin(), j = values.begin(); i != exprs.end(); ++i, ++j) {
                ...
                if (destructNode || (destructNode = value->item.as<simple_table_t>())) {
                        ...
                        if (auto ssVal = simpleSingleValueFrom(*j)) {
                            switch (ssVal->value->getId()) {
                                case id<const_value_t>():
                                    throw std::logic_error(_info.errorMessage(
                                            "can not destructure a constant"sv, ssVal->value));
                                    break;
                                case id<Num_t>():
                                    throw std::logic_error(_info.errorMessage(
                                            "can not destructure a number"sv, ssVal->value));
                                    break;
                                case id<FunLit_t>():
                                    throw std::logic_error(_info.errorMessage(
                                            "can not destructure a function"sv, ssVal->value));
                                    break;
                            }
                        }
                    }
                    ...
                    if (!varDefOnly && !subDestruct->values.empty() &&
                        !subMetaDestruct->values.empty()) {
                        auto objVar = getUnusedName("_obj_"sv);
                        addToScope(objVar);
                        valueItems.pop_back();
                    ...
                    for (auto tab: tabs) {
                        if (!tab->values.empty()) {
                            ...
                            if (pairs.empty()) {
                                throw std::logic_error(_info.errorMessage(
                                        "expect items to be destructured"sv, tab));
                            }
                            destruct.items = std::move(pairs);
                            if (!varDefOnly) {
                                if (*j == nil) {
                                    for (auto &item: destruct.items) { item.structure = nullptr; }
                                } else if (tab == subMetaDestruct.get()) {
                                    auto p = destruct.value.get();
                                    auto chainValue = toAst<ChainValue_t>("getmetatable()", p);
                                    static_cast<Invoke_t *>(chainValue->items.back())
                                            ->args.push_back(destruct.value);
                        ...
                        for (auto node: item.structure->items.objects()) {
                            if (auto exp = ast_cast<Exp_t>(node)) {
                                if (auto value = simpleSingleValueFrom(node)) {
                                    if (ast_is<Num_t, const_value_t>(value->value)) { continue; }
                                }
                                if (auto value = singleValueFrom(exp);
                                    !value || !value->item.is<String_t>()) {
            ...
            switch (action->getId()) {
                case id<Update_t>(): {
                    ...
                    BLOCK_START {
                        ...
                        for (auto item: chain->items.objects()) {
                            if (item != ptr) { tmpChain->items.push_back(item); }
                        }
                    ...
                    if (op == "??"sv) {
                        ...
                        transformNilCoalesedExp(rightExp, temp, ExpUsage::Assignment,
                                                assignment->expList, true);
                        out.push_back(join(temp));
                        return;
                    }
                    ...
                    temp.pop_back();
                    if (!singleValueFrom(update->value)) { right = '(' + right + ')'; }
                    _buf << join(temp);
                ...
                case id<Assign_t>(): {
                    ...
                    if (oneLined) {
                        ...
                        for (const auto &def: defs) { addToScope(def); }
                        if (preDefine.empty()) {
                            transformExpList(expList, temp);
                            std::string left = std::move(temp.back());
                            temp.pop_back();
                            out.push_back(indent() + left + " = "s + join(temp, ", "sv) +
                                          nll(assignment));
                        } else {
                            out.push_back(preDefine + " = "s + join(temp, ", "sv) +
                                          nll(assignment));
                        }
                    } else {
                        std::string preDefine = toLocalDecl(defs);
                        ...
                        out.push_back((preDefine.empty() ? Empty : preDefine + nll(assignment)) +
                                      indent() + left + " = "s + join(temp, ", "sv) +
                                      nll(assignment));
                    }
                    break;
                }
        ...
        void transformCond(const node_container &nodes, str_list &out, ExpUsage usage, bool unless,
                           ExpList_t *assignList) {
            ...
            if (nodes.size() != ns.size()) {
                ...
                auto newIf = x->new_ptr<If_t>();
                newIf->type.set(toAst<IfType_t>("if"sv, x));
                for (auto j = ns.rbegin(); j != ns.rend(); ++j) { newIf->nodes.push_back(*j); }
            ...
            for (const auto &pair: ifCondPairs) {
                if (pair.first) {
                    ...
                    _buf << indent();
                    if (pair != ifCondPairs.front()) { _buf << "else"sv; }
                    _buf << "if "sv << condStr << " then"sv << nll(condition);
                    temp.push_back(clearBuf());
                }
                if (pair.second) {
                    if (!pair.first) { temp.push_back(indent() + "else"s + nll(pair.second)); }
                    pushScope();
        ...
        void transform_pipe_exp(const node_container &values, str_list &out, ExpUsage usage,
                                ExpList_t *assignList = nullptr) {
            if (values.size() == 1 && usage == ExpUsage::Closure) {
                ...
                for (auto it = begin; it != values.end(); ++it) {
                    ...
                    auto value = static_cast<Value_t *>(
                            singleUnaryExpFrom(unary) ? unary->expos.back() : nullptr);
                    if (values.back() == *it && !unary->ops.empty() && usage == ExpUsage::Common) {
                        throw std::logic_error(
                                _info.errorMessage("expression list is not supported here"sv, x));
                    }
                    if (!value)
                        throw std::logic_error(_info.errorMessage(
                                "pipe operator must be followed by chain value"sv, *it));
                    if (auto chainValue = value->item.as<ChainValue_t>()) {
                            ...
                            for (auto a: args->objects()) {
                                ...
                                if (name == "_"sv) {
                                    if (!findPlaceHolder) {
                                        ...
                                        throw std::logic_error(_info.errorMessage(
                                                "pipe placeholder can be used only in one place"sv,
                                                a));
                                    }
                                }
                            }
                            if (!findPlaceHolder) { args->push_front(arg); }
                        } else {
                        ...
                        throw std::logic_error(_info.errorMessage(
                                "pipe operator must be followed by chain value"sv, value));
                    }
                }
                switch (usage) {
                    ...
                    case ExpUsage::Common: {
                        ...
                        if (value && value->item.is<ChainValue_t>()) {
                            transformChainValue(value->item.to<ChainValue_t>(), out,
                                                ExpUsage::Common);
                        } else {
        ...
        void transformExp(Exp_t *exp, str_list &out, ExpUsage usage,
                          ExpList_t *assignList = nullptr) {
            if (exp->opValues.empty() && !exp->nilCoalesed) {
                transform_pipe_exp(exp->pipeExprs.objects(), out, usage, assignList);
                return;
            }
            if (usage != ExpUsage::Closure) { SERROR("invalid expression usage", exp); }
            if (exp->nilCoalesed) {
        ...
        void transformNilCoalesedExp(Exp_t *exp, str_list &out, ExpUsage usage,
                                     ExpList_t *assignList = nullptr, bool nilBranchOnly = false) {
            auto x = exp;
            str_list temp;
            std::string prefix;
        ...
        void transformCallable(Callable_t *callable, str_list &out,
                               const ast_sel<false, Invoke_t, InvokeArgs_t> &invoke = {}) {
            ...
            switch (item->getId()) {
                ...
                case id<VarArg_t>():
                    if (_varArgs.empty() || !_varArgs.top().hasVar) {
                        throw std::logic_error(_info.errorMessage(
                                "cannot use '...' outside a vararg function near '...'"sv, item));
                    }
        ...
        void transformFunLit(FunLit_t *funLit, str_list &out) {
            ...
            pushScope();
            if (isFatArrow) { forceAddToScope("self"s); }
            str_list temp;
        ...
        void transformBody(Body_t *body, str_list &out, ExpUsage usage,
                           ExpList_t *assignList = nullptr) {
            auto x = body;
            if (auto stmt = body->content.as<Statement_t>()) {
        ...
        void transformBlock(Block_t *block, str_list &out, ExpUsage usage,
                            ExpList_t *assignList = nullptr, bool isRoot = false) {
            if (!block) {
                out.push_back(Empty);
                return;
            }
            ...
            for (auto it = nodes.begin(); it != nodes.end(); ++it) {
                ...
                if (auto pipeBody = stmt->content.as<PipeBody_t>()) {
                    ...
                    if (lst->appendix) {
                        throw std::logic_error(_info.errorMessage(
                                "statement decorator must be placed at the end of pipe chain"sv,
                                lst->appendix.get()));
                    }
                    ...
                    BREAK_IF(!exp);
                    for (auto val: pipeBody->values.objects()) { exp->pipeExprs.push_back(val); }
                    cond = true;
                    BLOCK_END
                    if (!cond)
                        throw std::logic_error(
                    ...
                    throw std::logic_error(
                            _info.errorMessage("indent mismatch in pipe chain"sv, *next));
                    BLOCK_END
                        ...
                        for (auto a: args->objects()) {
                            ...
                            if (name == "_"sv) {
                                if (!findPlaceHolder) {
                                    ...
                                    throw std::logic_error(_info.errorMessage(
                                            "backcall placeholder can be used only in one place"sv,
                                            a));
                                }
                            }
                        }
                        if (!findPlaceHolder) { args->push_back(arg); }
                    } else {
                        auto invoke = x->new_ptr<Invoke_t>();
                    ...
                    if (!local->collected) {
                        ...
                        switch (local->item->getId()) {
                            case id<local_flag_t>(): {
                                ...
                                LocalMode newMode = _parser.toString(flag) == "*"sv
                                                            ? LocalMode::Any
                                                            : LocalMode::Capital;
                                if (int(newMode) > int(mode)) { mode = newMode; }
            ...
            if (isRoot && !_info.moduleName.empty() && !_info.exportMacro) {
                block->statements.push_front(toAst<Statement_t>(
                        _info.moduleName + (_info.exportDefault ? "=nil"s : "={}"s), block));
            }
            switch (usage) {
                case ExpUsage::Closure:
                case ExpUsage::Return: {
                    ...
                    if (bLast != nodes.rend()) {
                        ...
                        BLOCK_END
                        if (!isMacro) { ast_to<Statement_t>(*bLast)->needSep.set(nullptr); }
                    }
                    BLOCK_END
                    break;
                }
                case ExpUsage::Assignment: {
                    ...
                    if (!last) return;
                    bool lastAssignable =
                            expListFrom(last) || ast_is<For_t, ForEach_t, While_t>(last->content);
                    if (lastAssignable) {
                        auto x = last;
                        auto newAssignment = x->new_ptr<ExpListAssign_t>();
                        newAssignment->expList.set(assignList);
        ...
        std::optional<std::string> getOption(std::string_view key) {
            ...
            if (it != _config.options.end()) { return it->second; }
            return std::nullopt;
        }
        ...
        int getLuaTarget(ast_node *x) {
            if (auto target = getOption("target")) {
                if (target.value() == "5.1"sv) {
                    ...
                    throw std::logic_error(
                            _info.errorMessage("get invalid Lua target \""s + target.value() +
                                                       "\", should be 5.1, 5.2, 5.3 or 5.4"s,
                                               x));
                }
            }
#ifndef MU_NO_MACRO
        ...
        void pushCurrentModule() {
            ...
            if (!L) {
                L = luaL_newstate();
                if (_luaOpen) { _luaOpen(static_cast<void *>(L)); }
                passOptions();
                _stateOwner = true;
            }
            lua_pushliteral(L, MU_MODULE);   // MU_MODULE
        ...
        bool isModuleLoaded(std::string_view name) {
            ...
            if (lua_isnil(L, -1) != 0) { return false; }
            return true;
        }
        ...
        void transformMacro(Macro_t *macro, str_list &out, bool exporting) {
            if (_scopes.size() > 1) {
                throw std::logic_error(
                        _info.errorMessage("can not define macro outside the root block"sv, macro));
            }
            ...
            if (argsDef) {
                for (auto def_: argsDef->definitions.objects()) {
                    ...
                    if (def->name.is<SelfName_t>()) {
                        throw std::logic_error(_info.errorMessage(
                                "self name is not supported for macro function argument"sv,
                                def->name));
                    } else {
                        std::string defVal;
                        if (def->defaultValue) {
                ...
                if (argsDef->varArg) { newArgs.emplace_back(_parser.toString(argsDef->varArg)); }
            }
            ...
            if (lua_pcall(L, 3, 2, 0) != 0) {      // loadstring(codes,chunk,options), cur f err
                ...
                throw std::logic_error(
                        _info.errorMessage("failed to load macro codes\n"s + err, macro->macroLit));
            }                           // cur f err
            if (lua_isnil(L, -2) != 0) {// f == nil, cur f err
                std::string err = lua_tostring(L, -1);
                throw std::logic_error(_info.errorMessage(
                        "failed to load macro codes, at (macro "s + macroName + "): "s + err,
                        macro->macroLit));
            }
            lua_pop(L, 1);                   // cur f
            ...
            if (lua_pcall(L, 1, 2, 0) != 0) {// f(), cur success macro
                std::string err = lua_tostring(L, -1);
                throw std::logic_error(_info.errorMessage(
                        "failed to generate macro function\n"s + err, macro->macroLit));
            }// cur success res
        ...
        void transformReturn(Return_t *returnNode, str_list &out) {
            if (!_enableReturn.top()) {
                ...
                throw std::logic_error(_info.errorMessage(
                        "can not mix use of return and export statements in module scope"sv,
                        target));
            }
                    ...
                    if (auto simpleValue = singleValue->item.as<SimpleValue_t>()) {
                        ...
                        switch (value->getId()) {
                            case id<Comprehension_t>():
                                ...
                                return;
                            case id<TblComprehension_t>():
                                transformTblComprehension(static_cast<TblComprehension_t *>(value),
                                                          out, ExpUsage::Return);
                                return;
                            case id<With_t>():
                            ...
                            case id<TableLit_t>(): {
                                auto tableLit = static_cast<TableLit_t *>(value);
                                if (hasSpreadExp(tableLit->values.objects())) {
                                    transformSpreadTable(tableLit->values.objects(), out,
                                                         ExpUsage::Return);
                                    return;
                                }
                            }
                        ...
                        if (specialChainValue(chainValue) != ChainType::Common) {
                            transformChainValue(chainValue, out, ExpUsage::Return, nullptr,
                                                returnNode->allowBlockMacroReturn);
                            return;
                        }
                    }
        ...
        void transformFnArgsDef(FnArgsDef_t *argsDef, str_list &out) {
            ...
            if (argsDef->shadowOption) { transform_outer_var_shadow(argsDef->shadowOption); }
        }
                ...
                switch (def->name->getId()) {
                    ...
                    case id<SelfName_t>(): {
                        ...
                        if (def->op) {
                            if (def->defaultValue) {
                                throw std::logic_error(_info.errorMessage(
                                        "argument with default value should not check for existence"sv,
                                        def->op));
                            }
                        ...
                        switch (selfName->name->getId()) {
                            case id<self_class_name_t>(): {
                                auto clsName =
                                        static_cast<self_class_name_t *>(selfName->name.get());
                            ...
                            case id<self_t>():
                                ...
                                if (def->op)
                                    throw std::logic_error(_info.errorMessage(
                                            "can only check existence for assigning self field"sv,
                                            selfName->name));
                                break;
                            default:
                                throw std::logic_error(_info.errorMessage(
                                        "invald self expression here"sv, selfName->name));
                                break;
                        }
                        break;
                    }
            ...
            if (assignSelf) {
                for (const auto &item: argItems) {
                    ...
                    if (item.checkExistence) {
                        auto stmt = toAst<Statement_t>(item.assignSelf + " = "s + item.name +
                                                               " if "s + item.name + '?',
                                                       x);
                        transformStatement(stmt, temp);
                    } else {
                        auto assignment =
                                toAst<ExpListAssign_t>(item.assignSelf + " = "s + item.name, x);
                        transformAssignment(assignment, temp);
                    }
                }
            }
        ...
        void transformSelfName(SelfName_t *selfName, str_list &out,
                               const ast_sel<false, Invoke_t, InvokeArgs_t> &invoke = {}) {
            auto x = selfName;
        ...
        bool transformChainEndWithEOP(const node_container &chainList, str_list &out,
                                      ExpUsage usage, ExpList_t *assignList) {
            ...
            if (ast_is<existential_op_t>(chainList.back())) {
                auto parens = x->new_ptr<Parens_t>();
                {
                    ...
                    for (auto item: chainList) { chainValue->items.push_back(item); }
                    chainValue->items.pop_back();
                    auto value = x->new_ptr<Value_t>();
                    value->item.set(chainValue);
                    auto exp = newExp(value, toAst<BinaryOperator_t>("!="sv, x),
                                      toAst<Value_t>("nil"sv, x), x);
                    parens->expr.set(exp);
                }
                switch (usage) {
                    case ExpUsage::Assignment: {
        ...
        bool transformChainWithEOP(const node_container &chainList, str_list &out, ExpUsage usage,
                                   ExpList_t *assignList, bool optionalDestruct) {
            auto opIt = std::find_if(chainList.begin(), chainList.end(),
                                     [](ast_node *node) { return ast_is<existential_op_t>(node); });
            if (opIt != chainList.end()) {
                ...
                if (optionalDestruct) {
                    ...
                    _buf << indent() << "if \"table\" == " << typeVar << " or \"userdata\" == "sv
                         << typeVar << " then"sv << nll(x);
                } else {
                    _buf << indent() << "if "sv << objVar << " ~= nil then"sv << nll(x);
                }
                temp.push_back(clearBuf());
        ...
        bool transformChainEndWithColonItem(const node_container &chainList, str_list &out,
                                            ExpUsage usage, ExpList_t *assignList) {
            if (ast_is<ColonChainItem_t>(chainList.back())) {
                ...
                auto funLit = toAst<Exp_t>(
                        fnVar + " and (...)-> "s + fnVar + ' ' + baseVar + ", ..."s, x);
                switch (usage) {
                    case ExpUsage::Closure:
                    case ExpUsage::Return: {
                        auto returnNode = x->new_ptr<Return_t>();
        ...
        bool transformChainWithMetatable(const node_container &chainList, str_list &out,
                                         ExpUsage usage, ExpList_t *assignList) {
            auto opIt = std::find_if(chainList.begin(), chainList.end(), [](ast_node *node) {
                if (auto colonChain = ast_cast<ColonChainItem_t>(node)) {
                    if (ast_is<Metamethod_t>(colonChain->name)) { return true; }
                } else if (auto dotChain = ast_cast<DotChainItem_t>(node)) {
                    if (ast_is<Metatable_t, Metamethod_t>(dotChain->name)) { return true; }
                }
                return false;
            });
            ...
            if (opIt == chainList.begin() && ast_is<ColonChainItem_t, DotChainItem_t>(x)) {
                if (_withVars.empty()) {
                    throw std::logic_error(_info.errorMessage(
                            "short dot/colon syntax must be called within a with block"sv, x));
                } else {
                    chain->items.push_back(toAst<Callable_t>(_withVars.top(), x));
                }
            }
            for (auto it = chainList.begin(); it != opIt; ++it) { chain->items.push_back(*it); }
            {
            ...
            switch ((*opIt)->getId()) {
                case id<ColonChainItem_t>(): {
                    ...
                    switch (meta->item->getId()) {
                        case id<Name_t>(): {
                            auto newColon = toAst<ColonChainItem_t>(
                                    "\\__"s + _parser.toString(meta->item), x);
                ...
                case id<DotChainItem_t>(): {
                    ...
                    switch (meta->item->getId()) {
                        case id<Name_t>(): {
                            auto newDot =
                                    toAst<DotChainItem_t>(".__"s + _parser.toString(meta->item), x);
            ...
            for (auto it = ++opIt; it != chainList.end(); ++it) { chain->items.push_back(*it); }
            transformChainValue(chain, out, usage, assignList);
            return true;
        }

        void transformChainList(const node_container &chainList, str_list &out, ExpUsage usage,
                                ExpList_t *assignList = nullptr) {
            auto x = chainList.front();
            if (chainList.size() > 1) {
                ...
                if (callable && callable->item.is<VarArg_t>()) {
                    throw std::logic_error(
                            _info.errorMessage("can not access variadic arguments directly"sv, x));
                }
            }
            str_list temp;
            switch (x->getId()) {
                ...
                case id<Exp_t>():
                    if (_withVars.empty()) {
                        throw std::logic_error(_info.errorMessage(
                                "short dot/colon and indexing syntax must be called within a with block"sv,
                                x));
                    } else {
                ...
                switch (item->getId()) {
                    ...
                    case id<ColonChainItem_t>(): {
                        ...
                        if (current != chainList.begin()) {
                            --current;
                            if (!ast_is<existential_op_t>(*current)) { ++current; }
                        }
                        ...
                        if (!ast_is<Invoke_t, InvokeArgs_t>(followItem)) {
                            throw std::logic_error(_info.errorMessage(
                                    "colon chain item must be followed by invoke arguments"sv,
                                    colonItem));
                        }
                        if (colonItem->name.is<LuaKeyword_t>()) {
                            ...
                            auto block = x->new_ptr<Block_t>();
                            {
                                ...
                                switch (chainList.front()->getId()) {
                                    case id<DotChainItem_t>():
                                    case id<ColonChainItem_t>():
                                        chainValue->items.push_back(
                                                toAst<Callable_t>(_withVars.top(), x));
                                        break;
                                }
                    ...
                    case id<Callable_t>(): {
                        ...
                        if (ast_is<Invoke_t, InvokeArgs_t>(followItem)) { invoke.set(followItem); }
                    ...
                    case id<Exp_t>():
                        ...
                        temp.back() =
                                (temp.back().front() == '[' ? "[ "s : "["s) + temp.back() + ']';
                        break;
                    case id<InvokeArgs_t>():
                        transformInvokeArgs(static_cast<InvokeArgs_t *>(item), temp);
                        break;
                    case id<table_appending_op_t>():
                        transform_table_appending_op(static_cast<table_appending_op_t *>(item),
                                                     temp);
                        break;
                    default:
                        SERROR("AST node mismatch", item);
        ...
        void transformMacroInPlace(MacroInPlace_t *macroInPlace) {
#ifdef MU_NO_MACRO
            throw std::logic_error(
                    _info.errorMessage("macro feature not supported"sv, macroInPlace));
#else // MU_NO_MACRO
            auto x = macroInPlace;
            pushCurrentModule();// cur
            int top = lua_gettop(L) - 1;
            ...
            if (lua_isnil(L, -2) != 0) {// f == nil, f err
                std::string err = lua_tostring(L, -1);
                throw std::logic_error(_info.errorMessage(
                        "failed to load macro codes, at (macro in-place): "s + err, x));
            }
            lua_pop(L, 1);                   // f
            ...
            if (lua_pcall(L, 1, 2, 0) != 0) {// f(), success macroFunc
                std::string err = lua_tostring(L, -1);
                throw std::logic_error(
                        _info.errorMessage("failed to generate macro function\n"s + err, x));
            }// success res
            if (lua_toboolean(L, -2) == 0) {
                std::string err = lua_tostring(L, -1);
                throw std::logic_error(
        ...
        std::string expandBuiltinMacro(const std::string &name, ast_node *x) {
            if (name == "LINE"sv) { return std::to_string(x->m_begin.m_line + _config.lineOffset); }
            if (name == "FILE"sv) {
        ...
        std::tuple<std::string, std::string, str_list> expandMacroStr(ChainValue_t *chainValue) {
            ...
            if (args) {
                for (auto arg: *args) {
                    ...
                    if (!rawString && str.empty()) { str = _parser.toString(arg); }
                    Utils::trim(str);
                    Utils::replace(str, "\r\n"sv, "\n"sv);
            ...
            if (lua_isstring(L, -1) == 0 && lua_istable(L, -1) == 0) {
                throw std::logic_error(
                        _info.errorMessage("macro function must return string or table"sv, x));
            }// cur res
            std::string codes;
            std::string type;
            str_list localVars;
            if (lua_istable(L, -1) != 0) {
                ...
                if (lua_isstring(L, -1) != 0) {
                    ...
                    throw std::logic_error(_info.errorMessage(
                            "macro table must contain field \"code\" of string"sv, x));
                }
                lua_pop(L, 1);              // cur res
                lua_getfield(L, -1, "type");// cur res type
                if (lua_isstring(L, -1) != 0) { type = lua_tostring(L, -1); }
                if (type != "lua"sv && type != "text"sv) {
                    throw std::logic_error(_info.errorMessage(
                            "macro table must contain field \"type\" of value \"lua\" or \"text\""sv,
                            x));
                }
                lua_pop(L, 1);                // cur res
                ...
                if (lua_istable(L, -1) != 0) {
                    for (int i = 0; i < static_cast<int>(lua_objlen(L, -1)); i++) {
                        ...
                        if (lua_isstring(L, -1) == 0) {
                            throw std::logic_error(_info.errorMessage(
                                    "macro table field \"locals\" must be a table of strings"sv,
                                    x));
                        }
                        ...
                        if (_parser.match<Variable_t>({name, len})) {
                            ...
                            throw std::logic_error(_info.errorMessage(
                                    "macro table field \"locals\" must contain names for local variables, got \""s +
                                            std::string(name, len) + '"',
                                    x));
                        }
                        lua_pop(L, 1);
        ...
        std::tuple<ast_ptr<false, ast_node>, std::unique_ptr<input>, std::string, str_list>
        expandMacro(ChainValue_t *chainValue, ExpUsage usage, bool allowBlockMacroReturn) {
            ...
            bool isBlock =
                    (usage == ExpUsage::Common) &&
                    (chainList.size() < 2 ||
                     (chainList.size() == 2 && ast_is<Invoke_t, InvokeArgs_t>(chainList.back())));
            ParseInfo info;
            if (type == "lua"sv) {
                if (!isBlock) {
                    throw std::logic_error(_info.errorMessage(
                            "lua macro can only be placed where block macro is allowed"sv, x));
                }
                ...
                if (!codes.empty()) {
                    if (_config.reserveLineNumber) { codes.insert(0, nll(chainValue).substr(1)); }
                ...
                if (!isBlock) {
                    throw std::logic_error(_info.errorMessage(
                            "text macro can only be placed where block macro is allowed"sv, x));
                }
                if (!codes.empty()) { codes.append(_newLine); }
                return {nullptr, nullptr, std::move(codes), std::move(localVars)};
            } else {
                if (!codes.empty()) {
                    if (isBlock) {
                        ...
                        if (!info.node) {
                            ...
                            throw std::logic_error(_info.errorMessage(
                                    "failed to expand macro as block: "s + info.error, x));
                        }
                    } else {
                        ...
                        if (!info.node && allowBlockMacroReturn) {
                            ...
                            if (!info.node) {
                                info.error = info.error.substr(info.error.find(':') + 2);
                                throw std::logic_error(_info.errorMessage(
                                        "failed to expand macro as expr or block: "s + info.error,
                                        x));
                            }
                            isBlock = true;
                        } else if (!info.node) {
                            info.error = info.error.substr(info.error.find(':') + 2);
                            throw std::logic_error(_info.errorMessage(
                                    "failed to expand macro as expr: "s + info.error, x));
                        }
                    }
                    ...
                    if (!isBlock) {
                        ...
                        if (!exp->opValues.empty() ||
                            (chainList.size() > 2 ||
                             (chainList.size() == 2 &&
                              !ast_is<Invoke_t, InvokeArgs_t>(chainList.back())))) {
                            auto paren = x->new_ptr<Parens_t>();
                            paren->expr.set(exp);
                            auto callable = x->new_ptr<Callable_t>();
                            callable->item.set(paren);
                            ...
                            for (; it != chainList.end(); ++it) { newChain->items.push_back(*it); }
                        ...
                        for (auto stmt_: block->statements.objects()) {
                            ...
                            if (auto global = stmt->content.as<Global_t>()) {
                                if (global->item.is<global_op_t>()) {
                                    throw std::logic_error(_info.errorMessage(
                                            "can not insert global statement with wildcard operator from macro"sv,
                                            x));
                                }
                            } else if (auto local = stmt->content.as<Local_t>()) {
                                if (local->item.is<local_flag_t>()) {
                                    throw std::logic_error(_info.errorMessage(
                                            "can not insert local statement with wildcard operator from macro"sv,
                                            x));
                                }
                            }
                        }
                    ...
                    if (!isBlock)
                        throw std::logic_error(
                                _info.errorMessage("failed to expand empty macro as expr"sv, x));
        ...
        void transformChainValue(ChainValue_t *chainValue, str_list &out, ExpUsage usage,
                                 ExpList_t *assignList = nullptr,
            ...
            if (isMacroChain(chainValue)) {
                ...
                std::string luaCodes;
                str_list localVars;
                std::tie(node, codes, luaCodes, localVars) =
                        expandMacro(chainValue, usage, allowBlockMacroReturn);
                if (!node) {
                    out.push_back(luaCodes);
                    if (!localVars.empty()) {
                        for (const auto &var: localVars) { addToScope(var); }
                    }
                    return;
                }
                if (usage == ExpUsage::Common ||
                    (usage == ExpUsage::Return && node.is<Block_t>())) {
                ...
                throw std::logic_error(
                        _info.errorMessage("macro feature not supported"sv, chainValue));
#endif// MU_NO_MACRO
            }
            const auto &chainList = chainValue->items.objects();
            if (transformChainEndWithEOP(chainList, out, usage, assignList)) { return; }
            if (transformChainWithEOP(chainList, out, usage, assignList, optionalDestruct)) {
                return;
            }
            if (transformChainWithMetatable(chainList, out, usage, assignList)) { return; }
            if (transformChainEndWithColonItem(chainList, out, usage, assignList)) { return; }
        ...
        void transform_table_appending_op(table_appending_op_t *op, str_list &) {
            throw std::logic_error(
                    _info.errorMessage("table appending syntax not supported here"sv, op));
        }
        ...
        void transform_unary_value(unary_value_t *unary_value, str_list &out) {
            ...
            for (auto _op: unary_value->ops.objects()) {
                ...
                if (op == "~"sv && getLuaTarget(_op) < 503) {
                    throw std::logic_error(_info.errorMessage(
                            "bitwise operator is not available when not targeting Lua version 5.3 or higher"sv,
                            _op));
                }
        ...
        void transformSpreadTable(const node_container &values, str_list &out, ExpUsage usage,
                                  ExpList_t *assignList = nullptr) {
            auto x = values.front();
            switch (usage) {
                case ExpUsage::Closure:
                ...
                switch (item->getId()) {
                    ...
                    case id<variable_pair_def_t>(): {
                        if (auto pair = ast_cast<variable_pair_def_t>(item)) {
                            if (pair->defVal) {
                                throw std::logic_error(_info.errorMessage(
                                        "invalid default value here"sv, pair->defVal));
                            }
                            item = pair->pair.get();
                        }
                        auto variablePair = static_cast<variable_pair_t *>(item);
                        auto nameStr = _parser.toString(variablePair->name);
                        auto assignment = toAst<ExpListAssign_t>(
                                tableVar + '.' + nameStr + '=' + nameStr, item);
                        transformAssignment(assignment, temp);
                        break;
                    }
                    case id<normal_pair_t>():
                    case id<normal_pair_def_t>(): {
                        ...
                        auto chainValue =
                                singleValueFrom(ast_to<Exp_t>(assignment->expList->exprs.front()))
                                        ->item.to<ChainValue_t>();
                    ...
                    case id<meta_variable_pair_def_t>(): {
                        ...
                        auto assignment = toAst<ExpListAssign_t>(
                                tableVar + ".<"s + nameStr + ">="s + nameStr, item);
                        transformAssignment(assignment, temp);
                        break;
                    }
                    case id<meta_normal_pair_t>():
            ...
            switch (usage) {
                ...
                case ExpUsage::Assignment: {
                    ...
                    out.push_back(join(temp));
                    out.back() =
                            indent() + "do"s + nll(x) + out.back() + indent() + "end"s + nlr(x);
                    break;
                }
                case ExpUsage::Return:
                    out.push_back(join(temp));
            ...
            for (auto value: values) {
                ...
                switch (item->getId()) {
                    case id<variable_pair_def_t>(): {
                        ...
                        if (pair->defVal) {
                            throw std::logic_error(
                                    _info.errorMessage("invalid default value"sv, pair->defVal));
                        }
                        item = pair->pair.get();
                        break;
                    }
                ...
                switch (item->getId()) {
                    ...
                    case id<meta_variable_pair_t>(): {
                        ...
                        if (metatableItem) {
                            throw std::logic_error(_info.errorMessage(
                                    "too many metatable declarations"sv, mp->name));
                        }
                        auto name = _parser.toString(mp->name);
                        _buf << "__"sv << name << ':' << name;
                    ...
                    case id<meta_normal_pair_t>(): {
                        ...
                        if (mp->key) {
                            if (metatableItem) {
                                throw std::logic_error(_info.errorMessage(
                                        "too many metatable declarations"sv, mp->key));
                            }
                            switch (mp->key->getId()) {
                                case id<Name_t>(): {
                                    auto key = _parser.toString(mp->key);
                            ...
                            if (!metatable->pairs.empty()) {
                                throw std::logic_error(_info.errorMessage(
                                        "too many metatable declarations"sv, mp->value));
                            }
                ...
                if (!isMetamethod) {
                    temp.back() = indent() +
                                  (value == values.back() ? temp.back() : temp.back() + ',') +
                                  nll(value);
                }
            }
            if (metatable->pairs.empty() && !metatableItem) {
                ...
                if (!metatable->pairs.empty()) {
                    ...
                    switch (metatableItem->getId()) {
                        case id<Exp_t>():
                            transformExp(static_cast<Exp_t *>(metatableItem.get()), tmp,
                                         ExpUsage::Closure);
                            break;
                        case id<TableBlock_t>():
                            transformTableBlock(static_cast<TableBlock_t *>(metatableItem.get()),
                                                tmp);
                            break;
                    }
                tabStr += tmp.back();
        ...
        void transformComprehension(Comprehension_t *comp, str_list &out, ExpUsage usage,
                                    ExpList_t *assignList = nullptr) {
            auto x = comp;
            switch (usage) {
                case ExpUsage::Closure:
            ...
            temp.pop_back();
            for (size_t i = 0; i < compInner->items.objects().size(); ++i) { popScope(); }
            _buf << indent() << "local "sv << accumVar << " = { }"sv << nll(comp);
            _buf << indent() << "local "sv << lenVar << " = 1"sv << nll(comp);
            _buf << join(temp);
            _buf << assignStr;
            _buf << indent(int(temp.size())) << lenVar << " = "sv << lenVar << " + 1"sv
                 << nll(comp);
            ...
            switch (usage) {
                ...
                case ExpUsage::Assignment: {
                    ...
                    transformAssignment(assignment, temp);
                    popScope();
                    out.back() = indent() + "do"s + nll(comp) + out.back() + temp.back() +
        ...
        bool transformForEachHead(AssignableNameList_t *nameList, ast_node *loopTarget,
                                  str_list &out, bool isStatement = false) {
            auto x = nameList;
            str_list temp;
            str_list vars;
            ...
            switch (loopTarget->getId()) {
                case id<star_exp_t>(): {
                    ...
                    if (!value)
                        throw std::logic_error(
                                _info.errorMessage("invalid star syntax"sv, star_exp));
                    ...
                    chainList.pop_back();
                    auto chain = x->new_ptr<ChainValue_t>();
                    for (auto item: chainList) { chain->items.push_back(item); }
                    ...
                    if (listVar.empty()) {
                        ...
                        _buf << prefix << indent() << "local "sv << listVar << " = "sv
                             << temp.back() << nll(nameList);
                    }
                    std::string maxVar;
                    if (!stopValue.empty()) {
                        std::string prefix;
                        ...
                        _buf << prefix << indent() << "local "sv << maxVar << " = "sv << stopValue
                             << nll(nameList);
                    }
                    ...
                    if (stopValue.empty()) {
                        ...
                        _buf << maxVar << " < 0 and #"sv << listVar << " + "sv << maxVar << " or "sv
                             << maxVar;
                    }
                    if (!stepValue.empty()) { _buf << ", "sv << stepValue; }
                    _buf << " do"sv << nlr(loopTarget);
                    _buf << indent(1) << "local "sv << join(vars, ", "sv) << " = "sv << listVar
                         << "["sv << indexVar << "]"sv << nll(nameList);
                    out.push_back(clearBuf());
                    BLOCK_END
                    ...
                    if (!endWithSlice) {
                        ...
                        if (newListVal)
                            _buf << indent() << "local "sv << listVar << " = "sv << temp.back()
                                 << nll(nameList);
                        _buf << indent() << "for "sv << indexVar << " = 1, #"sv << listVar
                             << " do"sv << nlr(loopTarget);
                        _buf << indent(1) << "local "sv << join(vars, ", "sv) << " = "sv << listVar
                             << "["sv << indexVar << "]"sv << nll(nameList);
                        out.push_back(clearBuf());
                    }
                ...
                case id<Exp_t>():
                    ...
                    _buf << indent() << "for "sv << join(vars, ", "sv) << " in "sv << temp.back()
                         << " do"sv << nlr(loopTarget);
                    out.push_back(clearBuf());
                    break;
                case id<ExpList_t>():
        ...
        void transformForHead(Variable_t *var, Exp_t *startVal, Exp_t *stopVal,
                              for_step_value_t *stepVal, str_list &out) {
            str_list temp;
            ...
            const auto &stop = *(++it);
            const auto &step = *(++it);
            _buf << indent() << "for "sv << varName << " = "sv << start << ", "sv << stop
                 << (step.empty() ? Empty : ", "s + step) << " do"sv << nll(var);
            pushScope();
        ...
        void transformForHead(For_t *forNode, str_list &out) {
            transformForHead(forNode->varName, forNode->startValue, forNode->stopValue,
                             forNode->stepValue, out);
        }

        void transform_plain_body(ast_node *body, str_list &out, ExpUsage usage,
                                  ExpList_t *assignList = nullptr) {
            switch (body->getId()) {
                case id<Block_t>():
            ...
            return traversal::Stop == body->traverse([&](ast_node *node) {
                if (auto stmt = ast_cast<Statement_t>(node)) {
                    if (stmt->content.is<BreakLoop_t>()) {
                        return _parser.toString(stmt->content) == "continue"sv ? traversal::Stop
                                                                               : traversal::Return;
        ...
        void transformLoopBody(ast_node *body, str_list &out, const std::string &appendContent,
                               ExpUsage usage, ExpList_t *assignList = nullptr) {
            str_list temp;
            bool extraDo = false;
            ...
            if (withContinue) {
                if (target < 502) {
                    ...
                    transformLabel(toAst<Label_t>("::"s + _continueVars.top().var + "::"s, body),
                                   temp);
            ...
            if (withContinue) {
                if (target < 502) {
                    ...
                    if (!appendContent.empty()) { _buf << indent() << appendContent; }
                    _buf << indent() << _continueVars.top().var << " = true"sv << nll(body);
                    popScope();
                    _buf << indent() << "until true"sv << nlr(body);
                    _buf << indent() << "if not "sv << _continueVars.top().var << " then"sv
                         << nlr(body);
                    ...
                    if (!appendContent.empty()) { temp.push_back(indent() + appendContent); }
        ...
        std::string transformRepeatBody(Repeat_t *repeatNode, str_list &out) {
            ...
            if (withContinue) {
                if (target < 502) {
                    ...
                    {
                        auto assignment = toAst<ExpListAssign_t>(conditionVar + "=nil"s,
        ...
        void transformForInPlace(For_t *forNode, str_list &out,
                                 ExpList_t *assignExpList = nullptr) {
            auto x = forNode;
            str_list temp;
            if (assignExpList) {
        ...
        void checkOperatorAvailable(const std::string &op, ast_node *node) {
            if (op == "&"sv || op == "~"sv || op == "|"sv || op == ">>"sv || op == "<<"sv) {
                ...
                if (getLuaTarget(node) < 503) {
                    throw std::logic_error(_info.errorMessage(
                            "floor division is not available when not targeting Lua version 5.3 or higher"sv,
                            node));
                }
            }
        }
        ...
        void transformForEach(ForEach_t *forEach, str_list &out) {
            str_list temp;
            bool extraScope =
                    transformForEachHead(forEach->nameList, forEach->loopValue, temp, true);
        ...
        void transformForEachInPlace(ForEach_t *forEach, str_list &out,
                                     ExpList_t *assignExpList = nullptr) {
            auto x = forEach;
            str_list temp;
            if (assignExpList) {
        ...
        void transformLuaString(LuaString_t *luaString, str_list &out) {
            ...
            out.push_back(_parser.toString(luaString->open) + content +
                          _parser.toString(luaString->close));
        }
        ...
        void transformClassDecl(ClassDecl_t *classDecl, str_list &out, ExpUsage usage,
                                ExpList_t *expList = nullptr) {
            str_list temp;
            auto x = classDecl;
            ...
            if (assignable) {
                if (!isAssignable(assignable)) {
                    throw std::logic_error(_info.errorMessage(
                            "left hand expression is not assignable"sv, assignable));
                }
            ...
            if (body) {
                ...
                for (auto item: body->contents.objects()) {
                    if (auto statement = ast_cast<Statement_t>(item)) {
                        ...
                        if (auto assignment = assignmentFrom(statement)) {
                            auto names =
                                    transformAssignDefs(assignment->expList.get(), DefOp::Mark);
            ...
            if (body) {
                ...
                for (auto content: classDecl->body->contents.objects()) {
                    switch (content->getId()) {
                        case id<class_member_list_t>(): {
                            size_t inc = transform_class_member_list(
                                    static_cast<class_member_list_t *>(content), members, classVar);
                            auto it = members.end();
                ...
                for (auto &member: members) {
                    switch (member.type) {
                        case MemType::Common:
                            commons.push_back((commons.empty() ? Empty : ',' + nll(member.node)) +
                                              member.item);
                            break;
                        case MemType::Builtin:
                            builtins.push_back((builtins.empty() ? Empty : ',' + nll(member.node)) +
                                               member.item);
                            break;
                        default:
                            break;
            ...
            if (classDecl->mixes) {
                ...
                _buf << '\t' << cls << ',' << mixin << '=' << item << ".__base?,"sv << item
                     << ".__base or "sv << item << '\n';
                _buf << "\tfor "sv << key << ',' << val << " in pairs "sv << mixin << '\n';
                _buf << "\t\t"sv << baseVar << '[' << key << "]="sv << val << " if "sv << baseVar
                     << '[' << key << "]==nil and (not "sv << cls << " or not "sv << key
                     << "\\match \"^__\")"sv;
            ...
            if (!parentVar.empty()) {
                ...
                _buf << "for "sv << key << ',' << val << " in pairs "sv << parentVar << ".__base"sv
                     ...
                     << key << "]==nil and "sv << key << "\\match(\"^__\") and not ("sv << key
                     << "==\"__index\" and "sv << val << "=="sv << parentVar << ".__base)"sv;
                transformBlock(toAst<Block_t>(clearBuf(), x), temp, ExpUsage::Common);
            }
            transformAssignment(
                    toAst<ExpListAssign_t>(baseVar + ".__index ?\?= "s + baseVar, classDecl), temp);
            str_list tmp;
            ...
            if (extend) {
                _buf << indent() << "setmetatable("sv << baseVar << ", "sv << parentVar
                     << ".__base)"sv << nll(classDecl);
            }
            _buf << indent() << classVar << " = "sv << globalVar("setmetatable"sv, classDecl)
                 << "({"sv << nll(classDecl);
            if (!builtins.empty()) {
                ...
                if (extend) {
                    ...
                    _buf << indent(2) << "return "sv << classVar << ".__parent.__init(self, ...)"sv
                         << nll(classDecl);
            ...
            if (extend) {
                ...
                _buf << indent(2) << "local val = rawget("sv << baseVar << ", name)"sv
                     << nll(classDecl);
            ...
            addToScope(selfVar);
            _buf << indent(1) << "local "sv << selfVar << " = setmetatable({ }, "sv << baseVar
                 << ")"sv << nll(classDecl);
            ...
            if (extend) {
                ...
                _buf << indent(1) << parentVar << ".__inherited("sv << parentVar << ", "sv
                     << classVar << ")"sv << nll(classDecl);
        ...
        size_t transform_class_member_list(class_member_list_t *class_member_list,
                                           std::list<ClassMember> &out,
                                           const std::string &classVar) {
            str_list temp;
            size_t count = 0;
            for (auto keyValue: class_member_list->values.objects()) {
                ...
                if (selfName) {
                    ...
                    if (!name)
                        throw std::logic_error(
                                _info.errorMessage("invalid class poperty name"sv, selfName->name));
                            ...
                            if (_parser.toString(var) == "super"sv) {
                                ...
                                if (chainList.size() >= 2) {
                                    ...
                                    if (!insertSelfToArguments(secondItem)) {
                                        if (auto colonChainItem =
                                                    ast_cast<ColonChainItem_t>(secondItem)) {
                                            if (chainList.size() > 2 &&
                                                insertSelfToArguments(*(++it))) {
                ...
                BLOCK_END
                pushScope();
                if (type == MemType::Property) { decIndentOffset(); }
                switch (keyValue->getId()) {
                    case id<variable_pair_t>():
                ...
                if (type == MemType::Property) { incIndentOffset(); }
                popScope();
            ...
            if (with->assigns) {
                ...
                if (vars.front().empty() || isGlobal(vars.front())) {
                    if (with->assigns->values.objects().size() == 1) {
                        auto var =
                                singleVariableFrom(with->assigns->values.objects().front(), true);
                        if (!var.empty() && isLocal(var)) { withVar = var; }
                    }
                    if (withVar.empty()) {
                ...
                scoped = traversal::Stop == with->body->traverse([&](ast_node *node) {
                    if (auto statement = ast_cast<Statement_t>(node)) {
                        ...
                        if (auto assignment = assignmentFrom(statement)) {
                            ...
                            if (!names.empty()) { return traversal::Stop; }
                            auto info = extractDestructureInfo(assignment, true, false);
                            if (!info.destructures.empty()) {
                                for (const auto &destruct: info.destructures)
                                    for (const auto &item: destruct.items)
        ...
        void transformExport(Export_t *exportNode, str_list &out) {
            ...
            if (_scopes.size() > 1) {
                throw std::logic_error(_info.errorMessage(
                        "can not do module export outside the root block"sv, exportNode));
            }
            if (exportNode->assign) {
                auto expList = exportNode->target.to<ExpList_t>();
                if (expList->exprs.size() != exportNode->assign->values.size()) {
                    throw std::logic_error(_info.errorMessage(
                            "left and right expressions must be matched in export statement"sv, x));
                }
                for (auto _exp: expList->exprs.objects()) {
                    ...
                    if (!variableFrom(exp) &&
                        ...
                        throw std::logic_error(_info.errorMessage(
                                "left hand expressions must be variables in export statement"sv,
                                x));
                    }
                }
                auto assignment = x->new_ptr<ExpListAssign_t>();
                ...
                if (_info.exportDefault) {
                    out.back().append(indent() + _info.moduleName + " = "s + names.back() +
                                      nlr(exportNode));
                } else {
                    ...
                    out.back().append(indent() + join(lefts, ", "sv) + " = "s +
                                      join(rights, ", "sv) + nlr(exportNode));
                }
            } else {
                if (auto macro = exportNode->target.as<Macro_t>()) {
                    ...
                    auto expList = exportNode->target.to<ExpList_t>();
                    auto assignment = x->new_ptr<ExpListAssign_t>();
                    auto assignList = toAst<ExpList_t>(
                            _info.moduleName + "[#"s + _info.moduleName + "+1]"s, x);
                    assignment->expList.set(assignList);
                    for (auto exp: expList->exprs.objects()) {
                        if (auto classDecl = exp->getByPath<unary_exp_t, Value_t, SimpleValue_t,
                                                            ClassDecl_t>()) {
                            if (classDecl->name &&
                                classDecl->name->item->getId() == id<Variable_t>()) {
                                transformClassDecl(classDecl, temp, ExpUsage::Common);
                                auto name = _parser.toString(classDecl->name->item);
                                assignment->expList.set(toAst<ExpList_t>(
                                        _info.moduleName + "[\""s + name + "\"]"s, x));
                                auto assign = x->new_ptr<Assign_t>();
        ...
        void transformTblComprehension(TblComprehension_t *comp, str_list &out, ExpUsage usage,
                                       ExpList_t *assignList = nullptr) {
            switch (usage) {
                case ExpUsage::Closure:
                    pushFunctionScope();
            ...
            if (comp->value) { transformExp(comp->value->value, kv, ExpUsage::Closure); }
            for (size_t i = 0; i < compInner->items.objects().size(); ++i) { popScope(); }
            ...
            if (!comp->value) {
                ...
                _buf << indent(int(temp.size()) - 1) << "local "sv << keyVar << ", "sv << valVar
                     << " = "sv << kv.front() << nll(comp);
                kv.front() = keyVar;
                kv.push_back(valVar);
            }
            _buf << indent(int(temp.size()) - 1) << tbl << "["sv << kv.front() << "] = "sv
                 << kv.back() << nll(comp);
        ...
        void transformCompFor(CompFor_t *comp, str_list &out) {
            transformForHead(comp->varName, comp->startValue, comp->stopValue, comp->stepValue,
                             out);
        }
        ...
        void transformDo(Do_t *doNode, str_list &out, ExpUsage usage,
                         ExpList_t *assignList = nullptr) {
            str_list temp;
            std::string *funcStart = nullptr;
            if (usage == ExpUsage::Closure) {
        ...
        void transformTry(Try_t *tryNode, str_list &out, ExpUsage usage) {
            ...
            if (auto tryBlock = tryNode->func.as<Block_t>()) {
                ...
                if (usage == ExpUsage::Common) { out.back().append(nlr(x)); }
                return;
                ...
                if (errHandler) {
                    ...
                    invoke->args.push_back(errHandler);
                    for (auto arg: args->objects()) { invoke->args.push_back(arg); }
                    transformChainValue(xpcall, out, ExpUsage::Closure);
                } else {
                    auto pcall = toAst<ChainValue_t>("pcall()", x);
                    auto invoke = ast_to<Invoke_t>(pcall->items.back());
        ...
        void transformImportFrom(ImportFrom_t *import, str_list &out) {
            ...
            if (objAssign) {
                ...
                if (!preDef.empty()) { temp.push_back(preDef + nll(import)); }
            ...
            out.push_back(join(temp));
            auto vars = getAssignVars(assignment);
            for (const auto &var: vars) { markVarConst(var); }
        }
        ...
        void transformImportAs(ImportAs_t *import, str_list &out) {
            ...
            if (ast_is<import_all_macro_t, ImportTabLit_t>(import->target)) {
                    ...
                    for (auto item: tabLit->items.objects()) {
                        switch (item->getId()) {
                            ...
                            case id<import_all_macro_t>(): {
                                throw std::logic_error(
                                        _info.errorMessage("macro feature not supported"sv, item));
                                break;
                            }
#else // MU_NO_MACRO
                            case id<MacroName_t>(): {
                            ...
                            case id<macro_name_pair_t>(): {
                                ...
                                macroPairs.emplace_back(_parser.toString(pair->key->name),
                                                        _parser.toString(pair->value->name));
                                break;
                            }
                            case id<import_all_macro_t>():
                                if (importAllMacro)
                                    throw std::logic_error(_info.errorMessage(
                                            "import all macro symbol duplicated"sv, item));
                                importAllMacro = true;
                                break;
#endif// MU_NO_MACRO
                            case id<variable_pair_t>():
                ...
                if (importAllMacro || !macroPairs.empty()) {
                    ...
                    if (lua_pcall(L, 1, 2, 0) != 0) {
                        std::string err = lua_tostring(L, -1);
                        throw std::logic_error(
                                _info.errorMessage("failed to resolve module path\n"s + err, x));
                    }
                    if (lua_isnil(L, -2) != 0) {
                        str_list files;
                        if (lua_istable(L, -1) != 0) {
                        ...
                        throw std::logic_error(_info.errorMessage("module '"s + moduleName +
                                                                          "\' not found:\n\t"s +
                                                                          join(files, "\n\t"sv),
                                                                  x));
                    }
                    ...
                    if (!isModuleLoaded(moduleFullName)) {
                        ...
                        if (lua_pcall(L, 1, 1, 0) != 0) {
                            std::string err = lua_tostring(L, -1);
                            throw std::logic_error(
                                    _info.errorMessage("failed to read module file\n"s + err, x));
                        }// cur text
                        if (lua_isnil(L, -1) != 0) {
                            throw std::logic_error(
                                    _info.errorMessage("failed to get module text"sv, x));
                        }// cur text
                        ...
                        if (result.codes.empty() && !result.error.empty()) {
                            throw std::logic_error(
                                    _info.errorMessage("failed to compile module '"s + moduleName +
                                                               "\': "s + result.error,
                                                       x));
                        }
                        lua_pop(L, 1);// cur
                ...
                if (importAllMacro) {
                    throw std::logic_error(
                            _info.errorMessage("macro feature not supported"sv, import->target));
                }
#endif// MU_NO_MACRO
            ...
            auto assign = x->new_ptr<Assign_t>();
            assign->values.push_back(
                    toAst<Exp_t>("require "s + _parser.toString(import->literal), x));
            auto assignment = x->new_ptr<ExpListAssign_t>();
            assignment->expList.set(assignList);
            assignment->action.set(assign);
            transformAssignment(assignment, out);
        ...
        void transformWhileInPlace(While_t *whileNode, str_list &out,
                                   ExpList_t *expList = nullptr) {
            auto x = whileNode;
            str_list temp;
            if (expList) {
            ...
            if (expList) { temp.push_back(indent() + "end"s + nlr(whileNode)); }
            out.push_back(join(temp));
        }
        ...
        void transformSwitch(Switch_t *switchNode, str_list &out, ExpUsage usage,
                             ExpList_t *assignList = nullptr) {
            auto x = switchNode;
            str_list temp;
            std::string *funcStart = nullptr;
            ...
            for (auto branch_: branches) {
                ...
                if (auto value = singleValueFrom(branch->valueList);
                    ...
                    if (tabCheckVar.empty()) {
                        ...
                        temp.push_back(indent() + "local "s + tabCheckVar + " = "s +
                                       globalVar("type", branch) + '(' + objVar + ')' +
                                       nll(branch));
                        temp.push_back(indent() + tabCheckVar + " = \"table\" == "s + tabCheckVar +
                                       " or \"userdata\" == "s + tabCheckVar + nll(branch));
                    }
                    std::string matchVar;
                    ...
                    pushScope();
                    auto assignment =
                            assignmentFrom(static_cast<Exp_t *>(branch->valueList->exprs.front()),
                                           toAst<Exp_t>(objVar, branch), branch);
                    auto info = extractDestructureInfo(assignment, true, false);
                    ...
                    if (!conds.empty()) {
                        temp.push_back(indent() + "if "s + join(conds, " and "sv) + " then"s +
                                       nll(branch));
                        pushScope();
                    }
                    if (!lastBranch) {
                    ...
                    for (auto exp_: exprs) {
                        ...
                        if (!singleValueFrom(exp)) { tmp.back() = '(' + tmp.back() + ')'; }
        ...
        void transformLocalDef(Local_t *local, str_list &out) {
            if (!local->forceDecls.empty() || !local->decls.empty()) {
                ...
                for (const auto &decl: local->decls) {
                    if (addToScope(decl)) { defs.push_back(decl); }
                }
                auto preDefine = toLocalDecl(defs);
                if (!preDefine.empty()) { out.push_back(preDefine + nll(local)); }
            }
        }
            ...
            if (!listA->names.empty()) {
                ...
                if (getLuaTarget(x) >= 504) {
                    ...
                    if (localAttrib->attrib.is<close_attrib_t>()) {
                        throw std::logic_error(_info.errorMessage(
                                "close attribute is not available when not targeting Lua version 5.4 or higher"sv,
                                x));
                    }
                    for (auto &var: vars) { markVarConst(var); }
                }
                str_list temp;
                for (auto item: assignA->values.objects()) { transformAssignItem(item, temp); }
                out.push_back(indent() + "local "s + join(vars, ", "sv) + " = "s +
                              join(temp, ", "sv) + nll(x));
            }
        ...
        void transformBreakLoop(BreakLoop_t *breakLoop, str_list &out) {
            ...
            if (_continueVars.empty())
                throw std::logic_error(
                        _info.errorMessage("continue is not inside a loop"sv, breakLoop));
            str_list temp;
            auto &item = _continueVars.top();
            if (item.condAssign) { transformAssignment(item.condAssign, temp); }
            if (getLuaTarget(breakLoop) < 502) {
                if (!temp.empty()) { _buf << temp.back(); }
        ...
        void transformLabel(Label_t *label, str_list &out) {
            if (getLuaTarget(label) < 502) {
                throw std::logic_error(_info.errorMessage(
                        "label statement is not available when not targeting Lua version 5.2 or higher"sv,
                        label));
            }
            ...
            if (auto it = scope.find(labelStr); it != scope.end()) {
                throw std::logic_error(_info.errorMessage("label '"s + labelStr +
                                                                  "' already defined at line "s +
        ...
        void transformGoto(Goto_t *gotoNode, str_list &out) {
            if (getLuaTarget(gotoNode) < 502) {
                throw std::logic_error(_info.errorMessage(
                        "goto statement is not available when not targeting Lua version 5.2 or higher"sv,
                        gotoNode));
            }
            auto labelStr = _parser.toString(gotoNode->label);
            gotos.push_back(
                    {gotoNode, labelStr, _gotoScopes.top(), static_cast<int>(_scopes.size())});
        ...
        void transformShortTabAppending(ShortTabAppending_t *tab, str_list &out) {
            if (_withVars.empty()) {
                throw std::logic_error(_info.errorMessage(
                        "short table appending syntax must be called within a with block"sv, tab));
            }
        ...
        void transformChainAssign(ChainAssign_t *chainAssign, str_list &out) {
            ...
            if (chainAssign->assign->values.size() != 1) {
                throw std::logic_error(
                        _info.errorMessage("only one right value expected"sv, value));
            }
            str_list temp;
            bool constVal = false;
            ...
            bool localVal = false;
            if (auto var = singleVariableFrom(value, false); isLocal(var)) { localVal = true; }
            if (!constVal && !localVal) {
                for (auto exp: chainAssign->exprs.objects()) {
                    std::string var = singleVariableFrom(exp, false);
                    if (!var.empty()) {
                        str_list temp;
                        transformAssignment(assignmentFrom(static_cast<Exp_t *>(exp), value, exp),
                                            temp);
                        ...
                        for (auto e: chainAssign->exprs.objects()) {
                            if (e != exp) { newChainAssign->exprs.push_back(e); }
                        }
    ...
    MuCompiler::MuCompiler(void *sharedState, const std::function<void(void *)> &luaOpen,
                           bool sameModule)
        :
#ifndef MU_NO_MACRO
          _compiler(std::make_unique<MuCompilerImpl>(static_cast<lua_State *>(sharedState), luaOpen,
                                                     sameModule)) {
    }
#else
    ...
    CompileInfo MuCompiler::compile(std::string_view codes, const MuConfig &config) {
