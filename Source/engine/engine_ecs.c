...
void FreeECS() {
    ...
    for (int e = 0; e <= ECS.maxEntities; e++) {
        if (IsValidEntity(e)) FreeList(&ECS.Entities[e].childs);
    }
    free(ECS.Entities);
...
ComponentID GetComponentID(char componentName[25]) {
    ...
    METADOT_WARN("GetComponentID: Component named %s not found", componentName);
    return -1;
}
...
ComponentMask CreateComponentMaskByName(int numComp, ...) {
    ...
    for (i = 0; i < numComp; i++) {
        ...
        if (index < 0 || index > 31) {
            METADOT_WARN("CreateComponentMaskByName: Component index out of range! (%d)", index);
            continue;
        }
...
ComponentMask CreateComponentMaskByID(int numComp, ...) {
    ...
    for (i = 0; i < numComp; i++) {
        ...
        if (index < 0 || index > 31) {
            METADOT_WARN("CreateComponentMaskByID: Component index out of range! (%d)", index);
            continue;
        }
...
EntityID CreateEntity() {
    ...
    METADOT_ERROR("CreateEntity: No entity avaliable to spawn! (Max %ud)", ECS.maxEntities);
    return -1;
}

void DestroyEntity(EntityID entity) {
    if (!IsValidEntity(entity)) {
        METADOT_WARN("DestroyEntity: Entity is not spawned or out of range!(%d)", entity);
        return;
    }
...
char *GetPrefabPath(EntityID entity) {
    if (!EntityIsPrefab(entity)) {
        METADOT_WARN("GetPrefabPath: Entity is not a prefab! (%d)", entity);
        return NULL;
    }
...
char *GetPrefabName(EntityID entity) {
    if (!EntityIsPrefab(entity)) {
        METADOT_WARN("GetPrefabName: Entity is not a prefab! (%d)", entity);
        return NULL;
    }
...
void AddComponentToEntity(ComponentID component, EntityID entity) {
    if (!IsValidEntity(entity)) {
        METADOT_WARN("AddComponentToEntity: Entity is not spawned or out of range!(%d)", entity);
        return;
    }
    if (component < 0 || component >= ECS.numberOfComponents) {
        METADOT_WARN("AddComponentToEntity: Component index out of range!(%d)", component);
        return;
    }
    if (EntityContainsMask(entity, CreateComponentMaskByID(1, component))) {
        METADOT_WARN(
                "AddComponentToEntity: An entity can only have one of each component type! (E:%d "
                "C:%d)",
                entity, component);
        return;
    }
...
void RemoveComponentFromEntity(ComponentID component, EntityID entity) {
    if (!IsValidEntity(entity)) {
        METADOT_WARN("RemoveComponentToEntity: Entity is not spawned or out of range!(%d)", entity);
        return;
    }
    if (component < 0 || component >= ECS.numberOfComponents) {
        METADOT_WARN("RemoveComponentToEntity: Component index out of range!(%d)", component);
        return;
    }
    if (!EntityContainsMask(entity, CreateComponentMaskByID(1, component))) {
...
EntityID DuplicateEntity(EntityID entity) {
    if (!IsValidEntity(entity)) {
        METADOT_WARN("DuplicateEntity: Entity is not spawned or out of range!(%d)", entity);
        return -1;
    }
...
ComponentMask GetEntityComponents(EntityID entity) {
    if (!IsValidEntity(entity)) {
        METADOT_WARN("GetEntityComponents: Entity is not spawned or out of range!(%d)", entity);
        return (ComponentMask){0};
    }
    return ECS.Entities[entity].mask;
}

int IsEmptyComponentMask(ComponentMask mask) { return mask.mask ? 0 : 1; }
...
int EntityContainsComponent(EntityID entity, ComponentID component) {
    if (!IsValidEntity(entity)) {
        METADOT_WARN("EntityContainsComponent: Entity is not spawned or out of range!(%d)", entity);
        return 0;
    }
    if (component < 0 || component > ECS.numberOfComponents) {
        METADOT_WARN("EntityContainsComponent: Component index out of range!(%d)", component);
        return 0;
    }
...
int MaskContainsComponent(ComponentMask mask, ComponentID component) {
    if (component < 0 || component > ECS.numberOfComponents) {
        METADOT_WARN("MaskContainsComponent: Component index out of range!(%d)", component);
        return 0;
    }
...
int EntityIsParent(EntityID entity) {
    if (!IsValidEntity(entity)) {
        METADOT_WARN("EntityIsParent: Entity is not spawned or out of range!(%d)", entity);
        return 0;
    }
...
int EntityIsChild(EntityID entity) {
    if (!IsValidEntity(entity)) {
        METADOT_WARN("EntityIsChild: Entity is not spawned or out of range!(%d)", entity);
        return 0;
    }
...
void SetEntityParent(EntityID child, EntityID parent) {
    ...
    if (!IsValidEntity(child)) {
        METADOT_WARN("EntityIsParent: Child is not spawned or out of range!(%d)", child);
        return;
    }
    if (!IsValidEntity(parent)) {
        METADOT_WARN("EntityIsParent: Parent is not spawned or out of range!(%d)", parent);
        return;
    }
...
EntityID GetEntityParent(EntityID entity) {
    if (!IsValidEntity(entity)) {
        METADOT_WARN("GetEntityParent: Entity is not spawned or out of range!(%d)", entity);
        return 0;
    }
...
List *GetChildsList(EntityID parent) {
    if (!IsValidEntity(parent)) {
        METADOT_WARN("GetChildsList: Parent is not spawned or out of range!(%d)", parent);
        return NULL;
    }
...
int UnsetParent(EntityID child) {
    if (!IsValidEntity(child)) {
        METADOT_WARN("UnsetParent: Child is not spawned or out of range!(%d)", child);
        return 0;
    }
    if (!EntityIsChild(child)) {
        METADOT_WARN("UnsetParent: Entity is not anyone's child. (%d)", child);
        return 0;
    }
...
//             if (currentData)
//                 currentCompJSON =
//                         cJSON_GetObjectItemCaseSensitive(currentData, ECS.ComponentTypes[c].name);

//             cJSON *compEncoded =
//                     ECS.ComponentTypes[c].encode(&ECS.Components[c][entity].data, currentCompJSON);
//             if (compEncoded) {
...
//     if (currentData) cJSON_Delete(currentData);

//     if (EntityIsParent(entity) &&
//         ((EntityIsPrefab(entity) && encodingToPrefab) || !EntityIsPrefab(entity))) {

//         cJSON *childsArray = cJSON_AddArrayToObject(entityObj, "childs");

//         ListCellPointer childCell;
//         ListForEach(childCell, *GetChildsList(entity)) {
//             cJSON_AddItemToArray(childsArray,
//                                  EncodeEntity(GetElementAsType(childCell, EntityID), 0));
//         }
//     }
...
//             if (newChild >= 0) SetEntityParent(newChild, newEntity);
//         }
//     }
//     return newEntity;
// }
...
//     if (!StringCompareEqual(&name[strlen(name) - 7], ".prefab")) { strcat(fullPath, ".prefab"); }
