...
#include "engine/imgui/css/select.h"
...
public:
    ...
    ElementIterator(ContainerElement* target, int i = 0) : index(i) { setActiveElement(target); }
...
css_error resolve_font(void* pw, lwc_string* name, css_system_font* system_font) {
    UNUSED(pw);
    UNUSED(name);
    UNUSED(system_font);
    return CSS_INVALID;
...
css_error named_generic_sibling_node(void* pw, void* n, const css_qname* qname, void** sibling) {
    ...
    *sibling = NULL;
    ...
    for (iter.prev(); iter.current(); iter.prev()) {
        ...
        bool match = false;
        lwc_error status = lwc_string_caseless_isequal(qname->name, lwc_string_from_char(previous->getType()), &match);
        UNUSED(status);
...
css_error named_sibling_node(void* pw, void* n, const css_qname* qname, void** sibling) {
    ...
    *sibling = NULL;
    ...
    if (previous) {
        ...
        UNUSED(status);
        IM_ASSERT(status == lwc_error_ok);
        if (match == true) *sibling = (void*)previous;
    }
...
css_error node_has_attribute(void* pw, void* n, const css_qname* qname, bool* match) {
    UNUSED(pw);
    *match = ((Element*)n)->hasAttribute(lwc_string_data(qname->name));
    return CSS_OK;
}

css_error node_has_attribute_equal(void* pw, void* n, const css_qname* qname, lwc_string* expected, bool* match) {
    UNUSED(pw);
    char* actual = NULL;
...
css_error node_has_attribute_dashmatch(void* pw, void* n, const css_qname* qname, lwc_string* value, bool* match) {
    UNUSED(pw);
    UNUSED(n);
    UNUSED(qname);
    UNUSED(value);
...
css_error node_has_attribute_includes(void* pw, void* n, const css_qname* qname, lwc_string* value, bool* match) {
    UNUSED(pw);
    UNUSED(n);
    UNUSED(qname);
    UNUSED(value);
...
css_error node_has_attribute_prefix(void* pw, void* n, const css_qname* qname, lwc_string* value, bool* match) {
    UNUSED(pw);
    UNUSED(n);
    UNUSED(qname);
    UNUSED(value);
...
css_error node_has_attribute_suffix(void* pw, void* n, const css_qname* qname, lwc_string* value, bool* match) {
    UNUSED(pw);
    UNUSED(n);
    UNUSED(qname);
    UNUSED(value);
...
css_error node_has_attribute_substring(void* pw, void* n, const css_qname* qname, lwc_string* value, bool* match) {
    UNUSED(pw);
    UNUSED(n);
    UNUSED(qname);
    UNUSED(value);
...
css_error node_count_siblings(void* pw, void* n, bool same_name, bool after, int32_t* count) {
    ...
    *count = 0;
    ...
    while (true) {
        ...
        if (!same_name || ImStricmp(name, el->getType()) == 0) cnt++;
    }
...
css_error node_is_lang(void* pw, void* n, lwc_string* lang, bool* match) {
    UNUSED(pw);
    char* value = NULL;
...
css_error compute_font_size(void* pw, const css_hint* parent, css_hint* size) {
    static css_hint_length sizes[] = {{FLTTOFIX(6.75), CSS_UNIT_PX}, {FLTTOFIX(7.50), CSS_UNIT_PX}, {FLTTOFIX(9.75), CSS_UNIT_PX}, {FLTTOFIX(12.0), CSS_UNIT_PX},
                                      {FLTTOFIX(15.0), CSS_UNIT_PX}, {FLTTOFIX(18.0), CSS_UNIT_PX}, {FLTTOFIX(24.0), CSS_UNIT_PX}};
...
css_error set_libcss_node_data(void* pw, void* n, void* libcss_node_data) {
    UNUSED(pw);

    Element* element = (Element*)n;
    if (element->style()->libcssData) {
        css_libcss_node_data_handler((css_select_handler*)pw, CSS_NODE_DELETED, NULL, element, NULL, element->style()->libcssData);
...
css_error get_libcss_node_data(void* pw, void* n, void** libcss_node_data) {
    UNUSED(pw);
