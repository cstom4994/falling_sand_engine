...
inline ImVec2 quadBezierCalc(ImVec2 p1, ImVec2 p2, ImVec2 p3, float t) {
    return ImVec2((1 - t) * (1 - t) * p1.x + 2 * (1 - t) * t * p2.x + t * t * p3.x, (1 - t) * (1 - t) * p1.y + 2 * (1 - t) * t * p2.y + t * t * p3.y);
}
...
void ComputedStyle::Decoration::drawJoint(const ImVec2& p1, const ImVec2& p2, const ImVec2& p3, const ImVec2& p1i, const ImVec2& p2i, const ImVec2& p3i, ImU32 startColor, ImU32 endColor,
                                          int numSegments) {
    ...
    for (int i = 0; i < pointsCount; ++i) {
        ...
        // Add vertices
        drawList->_VtxWritePtr[0].pos = nextOuter + ImVec2(dmx, dmy) / 2;
        drawList->_VtxWritePtr[0].uv = uv;
        drawList->_VtxWritePtr[0].col = col_trans;
        drawList->_VtxWritePtr[1].pos = nextOuter - ImVec2(dmx, dmy) / 2;
        drawList->_VtxWritePtr[1].uv = uv;
        drawList->_VtxWritePtr[1].col = col;
        drawList->_VtxWritePtr[2].pos = nextInner + ImVec2(dmx, dmy) / 2;
        drawList->_VtxWritePtr[2].uv = uv;
        drawList->_VtxWritePtr[2].col = col;
        drawList->_VtxWritePtr[3].pos = nextInner - ImVec2(dmx, dmy) / 2;
        drawList->_VtxWritePtr[3].uv = uv;
        drawList->_VtxWritePtr[3].col = col_trans;
        ...
        // Add indices
        drawList->_IdxWritePtr[0] = (ImDrawIdx)(idx2 + 1);
        drawList->_IdxWritePtr[1] = (ImDrawIdx)(idx1 + 1);
        drawList->_IdxWritePtr[2] = (ImDrawIdx)(idx1 + 2);
        drawList->_IdxWritePtr[3] = (ImDrawIdx)(idx1 + 2);
        drawList->_IdxWritePtr[4] = (ImDrawIdx)(idx2 + 2);
        drawList->_IdxWritePtr[5] = (ImDrawIdx)(idx2 + 1);
        drawList->_IdxWritePtr[6] = (ImDrawIdx)(idx2 + 1);
        drawList->_IdxWritePtr[7] = (ImDrawIdx)(idx1 + 1);
        drawList->_IdxWritePtr[8] = (ImDrawIdx)(idx1 + 0);
        drawList->_IdxWritePtr[9] = (ImDrawIdx)(idx1 + 0);
        drawList->_IdxWritePtr[10] = (ImDrawIdx)(idx2 + 0);
        drawList->_IdxWritePtr[11] = (ImDrawIdx)(idx2 + 1);
        drawList->_IdxWritePtr[12] = (ImDrawIdx)(idx2 + 2);
        drawList->_IdxWritePtr[13] = (ImDrawIdx)(idx1 + 2);
        drawList->_IdxWritePtr[14] = (ImDrawIdx)(idx1 + 3);
        drawList->_IdxWritePtr[15] = (ImDrawIdx)(idx1 + 3);
        drawList->_IdxWritePtr[16] = (ImDrawIdx)(idx2 + 3);
        drawList->_IdxWritePtr[17] = (ImDrawIdx)(idx2 + 2);
...
void ComputedStyle::Decoration::drawJoint(const ImVec2& p1, const ImVec2& p2, const ImVec2& p3, const ImVec2& center, ImU32 startColor, ImU32 endColor, const ImVec2& startPoint,
                                          const ImVec2& endPoint, int numSegments) {
    ImDrawList* drawList = ImGui::GetWindowDrawList();
    bool half = false;
...
void ComputedStyle::Decoration::renderBackground(ImRect rect) {
    ...
    if (tlrx > 0 || tlry > 0) {
        drawList->PathBezierCurveTo(rect.Min + ImVec2(0, tlry), rect.Min, rect.Min + ImVec2(tlrx, 0), IM_SEGMENTS(rounding[0]));
    }
    drawList->PathLineTo(ImVec2(rect.Max.x - trrx, rect.Min.y));
    if (trrx > 0 || trry > 0) {
        drawList->PathBezierCurveTo(ImVec2(rect.Max.x - trrx, rect.Min.y), ImVec2(rect.Max.x, rect.Min.y), ImVec2(rect.Max.x, rect.Min.y + trry), IM_SEGMENTS(rounding[1]));
    }
    drawList->PathLineTo(ImVec2(rect.Max.x, rect.Max.y - tbry));

    if (tbrx > 0 || tbry > 0) {
        drawList->PathBezierCurveTo(ImVec2(rect.Max.x, rect.Max.y - tbry), rect.Max, ImVec2(rect.Max.x - tbrx, rect.Max.y), IM_SEGMENTS(rounding[2]));
    }
    drawList->PathLineTo(ImVec2(rect.Min.x + tblx, rect.Max.y));

    if (tblx > 0 || tbly > 0) {
        drawList->PathBezierCurveTo(ImVec2(rect.Min.x + tblx, rect.Max.y), ImVec2(rect.Min.x, rect.Max.y), ImVec2(rect.Min.x, rect.Max.y - tbly), IM_SEGMENTS(rounding[3]));
    }
...
void ComputedStyle::Decoration::render(ImRect rect) {
    ...
    ImVec2 corners[4] = {rect.Min, ImVec2(rect.Max.x, rect.Min.y), rect.Max, ImVec2(rect.Min.x, rect.Max.y)};
...
static css_select_handler selectHandler = {CSS_SELECT_HANDLER_VERSION_1,
...
float parseUnits(css_fixed value, css_unit unit, ComputedStyle& style, ParseUnitsAxis axis) { return parseUnits(value, unit, style, style.contentRegion, axis); }
...
bool setDimensions(ComputedStyle& cs) {
    ...
    uint8_t type = css_computed_width(cs.style->styles[CSS_PSEUDO_ELEMENT_NONE], &width, &unit);
    ...
    type = css_computed_height(cs.style->styles[CSS_PSEUDO_ELEMENT_NONE], &height, &unit);
...
bool setPosition(ComputedStyle& cs) {
    ...
    css_fixed y = 0;
    css_unit unit = CSS_UNIT_PX;
    uint16_t leftType = css_computed_left(cs.style->styles[CSS_PSEUDO_ELEMENT_NONE], &x, &unit);
    ...
    uint16_t topType = css_computed_top(cs.style->styles[CSS_PSEUDO_ELEMENT_NONE], &y, &unit);
    ...
    if (css_computed_right(cs.style->styles[CSS_PSEUDO_ELEMENT_NONE], &right, &unit) == CSS_RIGHT_SET) {
    ...
    if (css_computed_bottom(cs.style->styles[CSS_PSEUDO_ELEMENT_NONE], &bottom, &unit) == CSS_BOTTOM_SET) {
...
bool setColor(ComputedStyle& cs) {
    css_color color;
    if (css_computed_color(cs.style->styles[CSS_PSEUDO_ELEMENT_NONE], &color) != CSS_COLOR_COLOR) {
        return false;
    }
...
bool setBackgroundColor(ComputedStyle& cs) {
    css_color color;
    uint16_t type = css_computed_background_color(cs.style->styles[CSS_PSEUDO_ELEMENT_NONE], &color);
...
bool setPadding(ComputedStyle& cs) {
    ...
    SpacingFunc funcs[4] = {SpacingFunc{css_computed_padding_left, ParseUnitsAxis::X}, SpacingFunc{css_computed_padding_top, ParseUnitsAxis::Y},
                            SpacingFunc{css_computed_padding_right, ParseUnitsAxis::X}, SpacingFunc{css_computed_padding_bottom, ParseUnitsAxis::Y}};
...
bool setMargins(ComputedStyle& cs) {
    ...
    SpacingFunc funcs[4] = {SpacingFunc{css_computed_margin_top, ParseUnitsAxis::Y}, SpacingFunc{css_computed_margin_left, ParseUnitsAxis::X},
                            SpacingFunc{css_computed_margin_right, ParseUnitsAxis::X}, SpacingFunc{css_computed_margin_bottom, ParseUnitsAxis::Y}};
...
bool setBorderRadius(ComputedStyle& cs) {
    ...
    css_unit unit;
    bool changed = false;
    BorderRadiusFunc funcs[4] = {css_computed_border_radius_top_left, css_computed_border_radius_top_right, css_computed_border_radius_bottom_right, css_computed_border_radius_bottom_left};
...
bool setBorder(ComputedStyle& cs) {
    bool changed = false;
    BorderStyleFunc styles[4] = {css_computed_border_left_style, css_computed_border_top_style, css_computed_border_right_style, css_computed_border_bottom_style};

    BorderWidthFunc widths[4] = {css_computed_border_left_width, css_computed_border_top_width, css_computed_border_right_width, css_computed_border_bottom_width};

    BorderColorFunc colors[4] = {css_computed_border_left_color, css_computed_border_top_color, css_computed_border_right_color, css_computed_border_bottom_color};
...
bool ComputedStyle::compute(Element* e) {
    ...
    code = css_select_style(mSelectCtx, element, &media, mInlineStyle, &selectHandler, &selectHandler, &newStyle);

    if (style && memcmp(newStyle, style, sizeof(css_select_results)) == 0) {
        if (css_select_results_destroy(newStyle) != CSS_OK) {
            IMGUICSS_EXCEPTION(ImGui::StyleError, "failed to destroy tmp style %s", css_error_to_string(code));
        }
        return false;
    }
    ...
    if (style) {
        if (css_select_results_destroy(style) != CSS_OK) {
            IMGUICSS_EXCEPTION(ImGui::StyleError, "failed to destroy style %s", css_error_to_string(code));
        }
    }
    ...
    if (code != CSS_OK) {
        IMGUICSS_EXCEPTION(ImGui::StyleError, "failed to select style %s", css_error_to_string(code));
        return false;
    }
    ...
    // dimensions
    {
        ...
        css_fixed height = 0;
        css_unit unit = CSS_UNIT_PX;
        mWidthMode = css_computed_width(style->styles[CSS_PSEUDO_ELEMENT_NONE], &width, &unit);

        mHeightMode = css_computed_height(style->styles[CSS_PSEUDO_ELEMENT_NONE], &height, &unit);

        bool shouldSetDimensions = mWidthMode == CSS_WIDTH_SET || mHeightMode == CSS_HEIGHT_SET;

        if (shouldSetDimensions && display != CSS_DISPLAY_INLINE) mStyleCallbacks.push_back(setDimensions);
...
void ComputedStyle::destroy() {
    ...
    if (libcssData != 0) {
        css_libcss_node_data_handler(&selectHandler, CSS_NODE_DELETED, NULL, element, NULL, libcssData);
        libcssData = 0;
    }
...
ImVector<lwc_string*>& ComputedStyle::getClasses() { return mClasses; }
...
void ComputedStyle::initFonts(css_media* media) {
    ...
    if (fontNames) {
        for (int i = 0;; i++) {
            ...
            if (fontName && strcmp(fontName, data) == 0) break;

            css_select_font_faces_results* fontFaces = NULL;
            css_select_font_faces(mSelectCtx, media, fontNames[i], &fontFaces);
            ...
            if (fontFaces && fontFaces->n_font_faces != 0) {
                for (uint32_t j = 0; j < fontFaces->n_font_faces; j++) {
                    ...
                    for (uint32_t k = 0; k < count; k++) {
                        ...
                        if (code != CSS_OK) {
                            IMGUICSS_EXCEPTION(ImGui::StyleError, "failed to get font src %s", css_error_to_string(code));
                            continue;
                        }
                        ...
                        if (fmt != CSS_FONT_FACE_FORMAT_OPENTYPE && fmt != CSS_FONT_FACE_FORMAT_UNSPECIFIED) {
                            IMGUICSS_EXCEPTION(ImGui::StyleError, "unsupported font format %d", fmt);
                            continue;
                        }

                        // only local location is supported
                        if (location == CSS_FONT_FACE_LOCATION_TYPE_URI) {
                            IMGUICSS_EXCEPTION(ImGui::StyleError, "uri location is not supported");
                            continue;
                        }
                        ...
                        if (code != CSS_OK) {
                            IMGUICSS_EXCEPTION(ImGui::StyleError, "failed to get font src location %s", css_error_to_string(code));
                            continue;
                        }
                        ...
                        if (element->context()->fontManager->loadFontFromFile(data, lwc_string_data(path), glyphRanges)) {
                            ...
                            IMGUICSS_EXCEPTION(ImGui::StyleError, "failed to load font %s", data);
                        }
                    }
                }
...
Style::Style(Style* parent) : mBase(0), mParent(parent) { parse(cssBase, &mBase); }
...
css_select_ctx* Style::select() {
    ...
    if (code != CSS_OK) {
        IMGUICSS_EXCEPTION(ImGui::StyleError, "failed to create select context %s", css_error_to_string(code));
        return 0;
    }

    code = css_select_ctx_append_sheet(ctx, mBase, CSS_ORIGIN_AUTHOR, NULL);
    if (code != CSS_OK) IMGUICSS_EXCEPTION(ImGui::StyleError, "failed to append base stylesheet: %s", css_error_to_string(code));
...
void Style::load(const char* data, bool scoped) {
    ...
    parse(data, &sheet);
    if (sheet) {
        mSheets.push_back(Sheet{sheet, scoped});
    }
}
...
void Style::appendSheets(css_select_ctx* ctx, bool scoped) {
    ...
    for (int i = 0; i < mSheets.size(); ++i) {
        ...
        if (code != CSS_OK) {
            IMGUICSS_EXCEPTION(ImGui::StyleError, "failed to append stylesheet: %s", css_error_to_string(code));
        }
    }
}
...
void Style::parse(const char* data, css_stylesheet** dest, bool isInline) {
    ...
    if (sheet) {
        ...
        if (code != CSS_OK) {
            IMGUICSS_EXCEPTION(ImGui::StyleError, "failed to destroy stylesheet: %s", css_error_to_string(code));
        }
    }
    ...
    if (code != CSS_OK) IMGUICSS_EXCEPTION(ImGui::StyleError, "failed to create stylesheet: %s", css_error_to_string(code));
    code = css_stylesheet_append_data(sheet, (const uint8_t*)data, strlen(data));
    if (code != CSS_OK && code != CSS_NEEDDATA) IMGUICSS_EXCEPTION(ImGui::StyleError, "css_stylesheet_append_data failed: %s", css_error_to_string(code));
    code = css_stylesheet_data_done(sheet);
    if (code != CSS_OK) IMGUICSS_EXCEPTION(ImGui::StyleError, "css_stylesheet_data_done failed: %s", css_error_to_string(code));
