...
#ifndef _METADOT_IMGUICSS_ELEMENT_H__
#define _METADOT_IMGUICSS_ELEMENT_H__
...
inline void str_to_floating(const char* value, float* dest) { *dest = std::stof(value); }

inline void str_to_floating(const char* value, double* dest) { *dest = std::stod(value); }

inline void str_to_floating(const char* value, long double* dest) { *dest = std::stold(value); }
...
template <class C>
typename std::enable_if<std::is_floating_point<C>::value, bool>::type str_to_number(const char* value, C* dest) {
    try {
...
inline void str_to_int(const char* value, long long* dest) { *dest = std::stoll(value); }

inline void str_to_int(const char* value, unsigned long long* dest) { *dest = std::stoull(value); }

template <class C>
typename std::enable_if<std::is_signed<C>::value>::type str_to_int(const char* value, C* dest) {
...
template <class C>
typename std::enable_if<std::is_integral<C>::value, bool>::type str_to_number(const char* value, C* dest) {
    try {
...
template <class C>
typename std::enable_if<std::is_arithmetic<C>::value, bool>::type read(const char* value, C** dest) {
    if (*dest) {
        ImGui::MemFree(*dest);
        *dest = NULL;
    }
...
bool parse_array(const char* value, ImVector<C>& values, char separator) {
    ...
    while (start < end + 1) {
        ...
        if (len > bufferSize) {
            IMGUICSS_EXCEPTION(ImGui::ElementError, "failed to parse array element. Max supported length is %d, got %d", bufferSize, len);
#if IMGUICSS_NO_EXCEPTIONS
            start = next + 1;
            return false;
#endif
        }
...
template <class C>
typename std::enable_if<std::is_arithmetic<C>::value, bool>::type read(Object& value, C** dest) {
    if (*dest) {
        ImGui::MemFree(*dest);
        *dest = NULL;
    }
...
typename std::enable_if<std::is_arithmetic<C>::value, bool>::type read(Object& value, C* dest) {
    try {
        *dest = value.as<C>();
    } catch (const ImGui::ScriptError& e) {
        (void)e;
        return false;
    }
    return true;
...
public:
    ...
    enum InvalidationFlag { BUILD = 1 << 0, STYLE = 1 << 1, MODEL = 1 << 2 };

    // mutually excluding element states
    enum ElementState { HIDDEN = 1 << 0, ACTIVE = 1 << 1, HOVERED = 1 << 2, DISABLED = 1 << 3, CHECKED = 1 << 4, LINK = 1 << 5, VISITED = 1 << 6, FOCUSED = 1 << 7 };

    enum Flag { CONTAINER = 1 << 0, PSEUDO_ELEMENT = 1 << 1, COMPONENT = 1 << 2, WINDOW = 1 << 3, BUTTON = 1 << 4 };
     ...
     */
    inline void enable() {
        if (!mConfigured) invalidateFlags(Element::BUILD);
     ...
     * Get element ctx
     */
    inline Context* context() { return mCtx; }
     ...
     */
    inline bool isContainer() const { return (mFlags & Element::CONTAINER) != 0; }

    /**
     * Check if Element is pseudo element
     */
    inline bool isPseudoElement() const { return (mFlags & Element::PSEUDO_ELEMENT) != 0; }
     ...
     */
    inline bool hasAttribute(const char* name) const { return mNode ? mNode->first_attribute(name) != 0 : false; }
    ...
    inline bool hasClass(const char* cls) const { return classes.find(const_cast<char*>(cls)) != classes.end(); }

    inline ComputedStyle* style() { return &mStyle; }

    inline bool visible() const { return enabled && display != CSS_DISPLAY_NONE; }

    inline bool hasState(ElementState s) { return (mState & s) != 0; }

    inline ImRect getMarginsRect() const {
        return ImRect(ImVec2(margins[1] == FLT_MIN ? 0 : margins[1], margins[0] == FLT_MIN ? 0 : margins[0]), ImVec2(margins[2] == FLT_MIN ? 0 : margins[2], margins[3] == FLT_MIN ? 0 : margins[3]));
    }
...
protected:
     ...
     */
    virtual void renderBody() { IM_ASSERT(false && "tried to render abstract element"); }
...
public:
    ...
    Attribute(bool r = false) : required(r) {}
     ...
     */
    virtual void read(const char* attr, const char* str, Element* element, ScriptState* scriptState, int flags = 0, ScriptState::Fields* fields = 0) const = 0;
...
bool evaluateString(const char* str, Element* element, ScriptState* scriptState, int flags, ScriptState::Fields* fields, Type* value) {
    ...
    if (scriptState) {
        if (flags & Attribute::SCRIPT) {
            ...
            if (!object.valid()) {
                IMGUICSS_EXCEPTION(ImGui::ScriptError, "failed to evaluate data %s", str);
                return false;
            }
...
public:
    AttributeMemPtr(D C::*memPtr, bool required = false) : Attribute(required), mMemPtr(memPtr) {}
    ...
    void read(const char* attribute, const char* str, Element* element, ScriptState* scriptState, int flags = 0, ScriptState::Fields* fields = 0) const {
        ...
        if (!success && required) {
            IMGUICSS_EXCEPTION(ImGui::ElementError, "failed to read required attribute %s", attribute);
        }
    }
...
public:
    ...
    AttributeSetter(SetterFunc func, bool required = false) : Attribute(required), mSetter(func) {
        static_assert(std::is_trivially_constructible<Type>::value, "setter can only be used with trivially constructible types");
    }
...
public:
    ...
    EventHandler* create(Element* element, const char* handlerName, const char* script) const { return new C(element, handlerName, script); }
};
...
public:
    ...
    virtual ~ElementBuilder() {
        ...
        for (Handlers::iterator iter = mHandlers.begin(); iter != mHandlers.end(); ++iter) {
            if (iter->second->owner == this) delete iter->second;
        }
        mHandlers.clear();
    }
    ...
    const ElementBuilder::RequiredAttrs& getRequiredAttrs() const { return mRequiredAttrs; }
...
public:
    enum Modifiers { Ctrl = 1 << 0, Alt = 1 << 1, Shift = 1 << 2, Super = 1 << 3 };
...
public:
    enum Type { Click, DoubleClick, MouseDown, MouseUp, MouseOver, MouseOut };
...
protected:
    bool check() { return ImGui::IsItemEdited(); }
};
...
public:
    ...
    inline const Elements& getChildren() const { return mChildren; }
...
public:
    PseudoElement() { mFlags |= Element::PSEUDO_ELEMENT; }
...
public:
    ...
    void renderBody() {
        if (!mParent || !text) {
            IMGUICSS_EXCEPTION(ImGui::ElementError, "text node must always be attached to some parent");
            return;
        }
        ...
        if (!mEstimatedSize) {
            ...
            if (popWrap) ImGui::PopTextWrapPos();
        }
    }
