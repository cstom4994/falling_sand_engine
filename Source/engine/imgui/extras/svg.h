#ifndef _METADOT_IMGUICSS_SVG__
#define _METADOT_IMGUICSS_SVG__
...
public:
    ...
    SvgImage() : src(NULL), size(0, 0), tint_col(ImVec4(1, 1, 1, 1)), border_col(ImVec4(0, 0, 0, 0)), c(0), mTextureID(NULL), mData(NULL), mImage(NULL), mRasterizer(NULL) {
    ...
    bool build() {
        if (!mTextureManager) {
            IMGUICSS_EXCEPTION(ImGui::ElementError, "Image requires texture manager for rendering");
            return false;
        }
        ...
        if (!mImage) {
            IMGUICSS_EXCEPTION(ImGui::ElementError, "Failed to read image");
            return false;
        }
        mShapes.reserve(0);
