...
public:
    enum InputType { BUTTON, CHECKBOX, COLOR, DATE, DATETIME_LOCAL, EMAIL, TFILE, HIDDEN, IMAGE, MONTH, NUMBER, PASSWORD, RADIO, RANGE, RESET, SEARCH, SUBMIT, TEL, TEXT, TIME, URL, WEEK };

    Input() : placeholder(0), format(0), min(0.0f), max(100.0f), step(1.0f), name(0), speed(1.0f), mValue(0), mModel(0), mValueUpdated(false), mActivate(false), mType(TEXT) {
        mBuffer.resize(48);
    ...
    void renderBody() {
        ...
        switch (mType) {
            ...
            case TEXT:
                ...
                break;
            case BUTTON:
                if (mValue) ImGui::TextUnformatted(mValue);
                break;
            case COLOR:
                if (mValueUpdated) {
                    char* p = mValue;
    ...
    void setChecked(bool value) { value ? setState(CHECKED) : resetState(CHECKED); }
...
private:
    void syncModel(bool write = false) {
        if (!mModel || (mFlags & BUTTON)) return;
