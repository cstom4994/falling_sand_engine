...
#include "engine/imgui/imgui_element.h"
...
#define IMGUICSS_LUA_VERSION 500
#else
#define IMGUICSS_LUA_VERSION 502
...
#if IMGUICSS_LUA_VERSION < 502
    return lua_objlen(L, index);
#else
    return lua_rawlen(L, index);
#endif
}
#if defined(IMGUICSS_LUA_VERSION) && IMGUICSS_LUA_VERSION <= 501
...
struct StackGuard {
    ...
    StackGuard(lua_State* state) : L(state), refs(0) { top = lua_gettop(L); }

    StackGuard(StackGuard& other) { (*this) = other; }
...
public:
    LuaObject(lua_State* L) : mLuaState(L), mType(ObjectType::NIL) {}

    virtual ~LuaObject(){};

    virtual ObjectType type() const { return mType; }

    virtual Object get(const Object& key) {
        if (type() != ObjectType::OBJECT && type() != ObjectType::USERDATA && type() != ObjectType::ARRAY) {
            IMGUICSS_EXCEPTION(ImGui::ScriptError, "failed to get key: not an object");
            return Object();
        }
        ...
        if (!impl) {
            IMGUICSS_EXCEPTION(ImGui::ScriptError, "null pointer access");
            return Object();
        }
    ...
    virtual bool call(Object* args, Object* rets, int nargs, int nrets) {
        ...
        if (lua_pcall(mLuaState, nargs, nrets, 0)) {
            IMGUICSS_EXCEPTION(ImGui::ScriptError, lua_tostring(mLuaState, -1));
            return false;
        }
    ...
    void initObject() {
        StackGuard g(mLuaState);
        unwrap(mLuaState);
        if (lua_type(mLuaState, -1) == LUA_TTABLE) return;
...
public:
    LuaStackReference(lua_State* L, int ref, StackGuard guard) : LuaObject(L), mIndex(lua_absindex(L, ref)), mStackGuard(guard) { initType(); }

    virtual ~LuaStackReference() {}

    void unwrap(lua_State* L = 0) const { lua_pushvalue(L ? L : mLuaState, mIndex); }

    void assign(lua_State* L = 0) { lua_replace(L ? L : mLuaState, mIndex); }
...
public:
    LuaReference(lua_State* L, int ref) : LuaObject(L), mRef(ref) { initType(); }

    virtual ~LuaReference() {
        if (mRef != LUA_NOREF) luaL_unref(mLuaState, LUA_REGISTRYINDEX, mRef);
    }
    ...
    void assign(lua_State* L = 0) { lua_rawseti(L ? L : mLuaState, LUA_REGISTRYINDEX, mRef); }
...
public:
    LuaFieldReference(lua_State* L, int ref, const char* key) : LuaObject(L), mRef(ref) {
        lua_pushstring(L, key);
        mKeyReference = luaL_ref(L, LUA_REGISTRYINDEX);
        initType();
    }

    LuaFieldReference(lua_State* L, int ref, int index) : LuaObject(L), mRef(ref) {
        lua_pushinteger(L, index);
        mKeyReference = luaL_ref(L, LUA_REGISTRYINDEX);
        initType();
    }

    LuaFieldReference(lua_State* L, int index) : LuaObject(L) {
        lua_pushinteger(L, index);
        mKeyReference = luaL_ref(L, LUA_REGISTRYINDEX);
    }

    virtual ~LuaFieldReference() {
        if (mKeyReference != LUA_NOREF) luaL_unref(mLuaState, LUA_REGISTRYINDEX, mKeyReference);
    }
...
Object createObject(lua_State* L, int ref) { return Object(new LuaReference(L, ref)); }

Object createObject(lua_State* L, int ref, const char* key) { return Object(new LuaFieldReference(L, ref, key)); }

Object createObject(lua_State* L, int ref, int index) { return Object(new LuaFieldReference(L, ref, index)); }

#define IMGUICSS "ImGuiCSS"
#define IMGUICSS_REFMAPPER "ImGuiCSSRefMapper"
#define IMGUICSS_CONTEXT "ImGuiCSSContext"
#define IMGUICSS_REACTIVE_TABLE "ImGuiCSSReactiveTable"
...
class ReactiveTable {
public:
    ReactiveTable(lua_State* L, int ref, LuaScriptState* scriptState, const char* field) : mRef(ref), mScriptState(scriptState), mField(ImStrdup(field)), mLuaState(L) {}
    ...
    inline void invalidate() { mScriptState->pushChange(mField); }
...
static ReactiveTable* lua_GetReactiveTable(lua_State* L, int index = -1) { return *reinterpret_cast<ReactiveTable**>(luaL_checkudata(L, index, IMGUICSS_REACTIVE_TABLE)); }

static int lua_ImGuiCSSReactiveTableIndex(lua_State* L) { return lua_GetReactiveTable(L, 1)->index(L); }

static int lua_ImGuiCSSReactiveTableNewIndex(lua_State* L) { return lua_GetReactiveTable(L, 1)->newIndex(L); }

static int lua_ImGuiCSSReactiveTableLen(lua_State* L) {
...
static int lua_ImGuiCSSReactiveTableDelete(lua_State* L) {
...
static int lua_searchImv(lua_State* L) {
    ...
    lua_pushvalue(L, 1);
    lua_getfield(L, package, "imcss_path");
    lua_call(L, 2, 2);
...
inline void wrapTable(lua_State* L, int index, int keyIdx, LuaScriptState* scriptState) {
    if (lua_type(L, index) == LUA_TTABLE) {
        ...
        *reinterpret_cast<ReactiveTable**>(lua_newuserdata(L, sizeof(ReactiveTable*))) = new ReactiveTable(L, ref, scriptState, field);
        luaL_getmetatable(L, IMGUICSS_REACTIVE_TABLE);
...
class RefMapper {
public:
    RefMapper(ScriptState::RefMap* refs) : mRefs(refs) {}
...
/**
 * ImGuiCSS state class to be used from lua
 */
class ImGuiCSS {

public:
    ImGuiCSS(lua_State* L) : mLuaState(L), mScriptState(NULL), mRef(LUA_NOREF) {
        luaL_checktype(L, -1, LUA_TTABLE);
        mRef = luaL_ref(mLuaState, LUA_REGISTRYINDEX);
    }

    ~ImGuiCSS() { luaL_unref(mLuaState, LUA_REGISTRYINDEX, mRef); }
    ...
    int index(lua_State* L) {
        ...
        if (ImStricmp(lua_tostring(L, 2), "_refs") == 0) {
            ...
            luaL_getmetatable(L, IMGUICSS_REFMAPPER);
            lua_setmetatable(L, -2);
            return 1;
        }
    ...
    void initEnvironment(int ref) { mSelfRef = ref; }
    ...
    void registerData(int tableIndex, int src = -1) {
        ...
        if (callFunction("data", false, src)) {
            ...
            if (lua_type(mLuaState, -1) != LUA_TTABLE) {
                IMGUICSS_EXCEPTION(ImGui::ScriptError, "data() method must return a table, got %d", lua_type(mLuaState, -1));
                return;
            }
    ...
    bool callFunction(const char* functionName, bool resetState = true, int tableIndex = -1) {
        StackGuard* g = 0;
        if (resetState) g = new StackGuard(mLuaState);
    ...
    inline void injectScriptState(LuaScriptState* state) { mScriptState = state; }

    inline void injectRefMapper(RefMapper* mapper) { mRefMapper = mapper; }

    inline ScriptState* getState() { return mScriptState; }

    inline void unwrap() { lua_rawgeti(mLuaState, LUA_REGISTRYINDEX, mRef); }
...
static ImGuiCSS* lua_GetImVue(lua_State* L, int index = -1) { return *reinterpret_cast<ImGuiCSS**>(luaL_checkudata(L, index, IMGUICSS)); }

static int lua_CreateImVue(lua_State* L) {
    *reinterpret_cast<ImGuiCSS**>(lua_newuserdata(L, sizeof(ImGuiCSS*))) = new ImGuiCSS(L);
    luaL_getmetatable(L, IMGUICSS);
    lua_setmetatable(L, -2);
    return 1;
}
...
static int lua_DeleteImVue(lua_State* L) {
    delete *reinterpret_cast<ImGuiCSS**>(lua_touserdata(L, 1));
    return 0;
}

static int lua_ImVueIndex(lua_State* L) { return lua_GetImVue(L, 1)->index(L); }

static int lua_ImVueNewIndex(lua_State* L) { return lua_GetImVue(L, 1)->newIndex(L); }

static int lua_ImVueMapperIndex(lua_State* L) {
    RefMapper* mapper = *reinterpret_cast<RefMapper**>(luaL_checkudata(L, 1, IMGUICSS_REFMAPPER));
    return mapper->index(L);
}

static int lua_ImGuiCSSContextIndex(lua_State* L) {
...
LuaScriptState::LuaScriptState(lua_State* L) : mLuaState(L), mRefMapper(new RefMapper(&mRefMap)), mImVue(NULL), mRef(LUA_NOREF), mLogAccess(false) {}
...
void LuaScriptState::initialize(Object data) {
    ...
    luaL_checkudata(mLuaState, copyIndex, IMGUICSS);
...
void LuaScriptState::initialize(const char* scriptData) {
    ...
    luaL_checkudata(mLuaState, -1, IMGUICSS);
    setupState(luaL_ref(mLuaState, LUA_REGISTRYINDEX));
}

ScriptState* LuaScriptState::clone() const { return new LuaScriptState(mLuaState); }

MutableObject LuaScriptState::operator[](const char* key) { return MutableObject(new LuaFieldReference(mLuaState, mRef, key)); }
            ...
            *retval = lua_toboolean(mLuaState, -1) ? "1" : "0";
            ...
            if (c) {
                *retval = std::string(c);
                ...
                IMGUICSS_EXCEPTION(ImGui::ScriptError, ss.str().c_str());
            }
        }
    }
...
void LuaScriptState::requested(const char* field) { requested(hash(field)); }
...
void LuaScriptState::setupState(int ref) {
    ...
    ImGuiCSS* imvue = lua_GetImVue(mLuaState);
...
void LuaScriptState::setObject(char* key, Object& value, int tableIndex) {
    ...
    if (!v) {
        IMGUICSS_EXCEPTION(ImGui::ScriptError, "tried to set null value in context");
        return;
    }
...
void LuaScriptState::activateContext(ScriptState::Context* ctx) {
    ...
    lua_createtable(mLuaState, 0, 0);
    luaL_getmetatable(mLuaState, IMGUICSS_CONTEXT);
...
void registerBindings(lua_State* L) {
    ...
    lua_setglobal(L, "imgui_Array");
    lua_pushnumber(L, ObjectType::OBJECT);
    lua_setglobal(L, "imgui_Object");
    lua_pushnumber(L, ObjectType::NUMBER);
    lua_setglobal(L, "imgui_Number");
    lua_pushnumber(L, ObjectType::STRING);
    lua_setglobal(L, "imgui_String");
    lua_pushnumber(L, ObjectType::BOOLEAN);
    lua_setglobal(L, "imgui_Boolean");
    lua_pushnumber(L, ObjectType::NIL);
    lua_setglobal(L, "imgui_Nil");
    lua_pushnumber(L, ObjectType::USERDATA);
    lua_setglobal(L, "imgui_Userdata");
    lua_pushnumber(L, ObjectType::FUNCTION);
    lua_setglobal(L, "imgui_Function");

    if (luaL_newmetatable(L, IMGUICSS)) {
        // register main userdata object
        static const luaL_Reg imvueFuncs[] = {
                {"new", lua_CreateImVue}, {"component", lua_ImVueCreateComponent}, {"__newindex", lua_ImVueNewIndex}, {"__index", lua_ImVueIndex}, {"__gc", lua_DeleteImVue}, {NULL, NULL}};
        luaL_setfuncs(L, imvueFuncs, 0);
        lua_setglobal(L, IMGUICSS);
    }

    if (luaL_newmetatable(L, IMGUICSS_REFMAPPER)) {
        static const luaL_Reg mapperFuncs[] = {{"__index", lua_ImVueMapperIndex}, {NULL, NULL}};
        luaL_setfuncs(L, mapperFuncs, 0);
        lua_setglobal(L, IMGUICSS_REFMAPPER);
    }

    if (luaL_newmetatable(L, IMGUICSS_CONTEXT)) {
        static const luaL_Reg contextFuncs[] = {{"__index", lua_ImGuiCSSContextIndex}, {NULL, NULL}};
        luaL_setfuncs(L, contextFuncs, 0);
        lua_setglobal(L, IMGUICSS_CONTEXT);
    }

    if (luaL_newmetatable(L, IMGUICSS_REACTIVE_TABLE)) {
        static const luaL_Reg rtFuncs[] = {{"__index", lua_ImGuiCSSReactiveTableIndex},
                                           {"__newindex", lua_ImGuiCSSReactiveTableNewIndex},
                                           {"__len", lua_ImGuiCSSReactiveTableLen},
                                           {"__gc", lua_ImGuiCSSReactiveTableDelete},
                                           {NULL, NULL}};
        luaL_setfuncs(L, rtFuncs, 0);
        lua_setglobal(L, IMGUICSS_REACTIVE_TABLE);
    }
