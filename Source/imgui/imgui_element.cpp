...
EventHandler::EventHandler(Element* element, const char* handlerName, const char* script) : mHandlerName(handlerName), mScript(script), mElement(element) {}
...
InputHandler::InputHandler(Element* element, const char* handlerName, const char* script) : EventHandler(element, handlerName, script), mModifiers(0), mExact(false) {
    parseProperties();
    if (checkProp("ctrl")) {
...
MouseEventHandler::MouseEventHandler(Element* element, const char* handlerName, const char* script) : InputHandler(element, handlerName, script), mType(Click), mMouseButton(0), mHovered(false) {
    if (checkProp("right")) {
        mMouseButton = 1;
...
KeyboardEventHandler::KeyboardEventHandler(Element* element, const char* handlerName, const char* script) : InputHandler(element, handlerName, script), mKeyIndex(-1), mType(Down) {
    parseProperties();
    if (checkProp("keyup")) {
        mType = Up;
    ...
    for (size_t i = end - 1; i > 0; --i) {
        if (handlerName[i] == '.') {
            ...
            if (keyCode > 0 && keyCode < ImGuiKey_COUNT) {
                mKeyIndex = ImGui::GetKeyIndex((ImGuiKey)keyCode);
            }
            break;
        }
    }
...
bool KeyboardEventHandler::check() {
    ...
    switch (mType) {
        case Down:
            trigger = ImGui::IsKeyDown((ImGuiKey)mKeyIndex);
            break;
        case Up:
            trigger = ImGui::IsKeyReleased((ImGuiKey)mKeyIndex);
            break;
        case Press:
            trigger = ImGui::IsKeyPressed((ImGuiKey)mKeyIndex);
            break;
    }
    return trigger && checkModifiers();
}
...
Element::~Element() {
    if (mClickHandler) ImGui::MemFree(mClickHandler);
...
void Element::invalidate(const char* attribute) { mDirtyProperties[attribute] = true; }

const Attribute* Element::getAttribute(const char* attrID) const { return mBuilder->get(attrID); }
...
bool Element::isHovered(ImGuiHoveredFlags flags) const {
    ...
    if (g.NavDisableMouseHover && !g.NavDisableHighlight) return ImGui::IsItemFocused();

    // Test for bounding box overlap, as updated as ItemAdd()
    if (!(ImGui::GetItemStatusFlags() & ImGuiItemStatusFlags_HoveredRect) && !ImRect(ImGui::GetItemRectMin(), ImGui::GetItemRectMin() + getSize()).Contains(ImGui::GetIO().MousePos)) return false;
    ...
    // itself. Until a solution is found I believe reverting to the test from 2017/09/27 is safe since this was the test that has been running for a long while.
    ...
    if (!(flags & ImGuiHoveredFlags_AllowWhenBlockedByActiveItem))
        if (g.ActiveId != 0 && g.ActiveId != ImGui::GetItemID() && !g.ActiveIdAllowOverlap && g.ActiveId != window->MoveId) return false;
    ...
    if (g.NavWindow)
        if (ImGuiWindow* focused_root_window = g.NavWindow->RootWindow)
            if (focused_root_window->WasActive && focused_root_window != window->RootWindow) {
                ...
                if ((focused_root_window->Flags & ImGuiWindowFlags_Popup) && !(flags & ImGuiHoveredFlags_AllowWhenBlockedByPopup)) return false;
            }

    // Test if the item is disabled
    if ((ImGui::GetItemFlags() & ImGuiItemFlags_Disabled) && !(flags & ImGuiHoveredFlags_AllowWhenDisabled)) return false;

    // Special handling for the dummy item after Begin() which represent the title bar or tab.
    // When the window is collapsed (SkipItems==true) that last item will never be overwritten so we need to detect the case.
    if (ImGui::GetItemID() == window->MoveId && window->WriteAccessed) return false;
    return true;
}

void Element::invalidateFlags(unsigned int flags) { mInvalidFlags |= flags; }
...
bool Element::build() {
    ...
    if (mRequiredAttrsCount != requiredAttrs.size()) {
        ...
        IMGUICSS_EXCEPTION(ImGui::ElementError, "failed to build element %s, missing required properties %s", getType(), missed.str().c_str());
        return false;
    }
...
void Element::render() {
    ...
    if (layout) layout->beginElement(this);

    pos = ImGui::GetCursorPos();
    ImGuiWindow* window = GetCurrentWindowNoDefault();
    if (window) {
        // render decoration
        mStyle.decoration.render(ImRect(window->Pos - window->Scroll + pos, window->Pos - window->Scroll + pos + getSize()));
    }
    ...
    if (layout) layout->endElement(this);
...
ContainerElement* Element::getParent() { return mParent ? static_cast<ContainerElement*>(mParent) : NULL; }
...
void Element::addHandler(const char* name, const char* value, const ElementBuilder* builder) {
    ...
    if (handlerName[0] == 0) {
        IMGUICSS_EXCEPTION(ImGui::ElementError, "malformed handler name %s", name);
        return;
    }

    EventHandler* handler = builder->createHandler(this, handlerName, fullName, value);
    if (!handler) {
        IMGUICSS_EXCEPTION(ImGui::ElementError, "failed to create handler of type %s", handlerName);
        return;
    }
...
Element* Element::createElement(rapidxml::xml_node<>* node, ScriptState::Context* sctx, Element* parent) { return mCtx->root->createElement(node, sctx, parent); }

ContainerElement::ContainerElement() { mFlags |= Element::CONTAINER; }

ContainerElement::~ContainerElement() { removeChildren(); }
...
void ContainerElement::createChildren(rapidxml::xml_node<>* doc) {
    ...
    for (rapidxml::xml_node<>* node = root->first_node(); node; node = node->next_sibling()) {
        ...
        if (vfor) {
            ...
            if (!e) {
                IMGUICSS_EXCEPTION(ImGui::ElementError, "failed to create element '%s'", node->name());
                continue;
            }
            ...
            try {
                if (e->enabledAttr != NULL) {
                    ...
                    if (!chain) {
                        IMGUICSS_EXCEPTION(ImGui::ElementError, "using v-else without v-if is not allowed");
                        return;
                    }
        ...
        if (e) mChildren.push_back(e);
    }
}

void ContainerElement::renderBody() { renderChildren(); }
...
bool ElementGroup::build() {
    ...
    if (!mScriptState->parseIterator(vfor->value(), values)) {
        IMGUICSS_EXCEPTION(ImGui::ScriptError, "failed to parse vfor %s", vfor->value());
        return false;
    }
    ...
    if (!list || !valueVar) {
        ...
        IMGUICSS_EXCEPTION(ImGui::ScriptError, "malformed vfor definition %s", vfor->value());
        return false;
    }
    ...
    if (!object) {
        cleanupValues(values);
        IMGUICSS_EXCEPTION(ImGui::ScriptError, "object evaluation failed %s", list);
        return false;
    }
    ...
    for (Object::iterator iter = object.begin(); iter != object.end(); ++iter, ++index) {
        ...
        if (mElementsByKey.count(hash) != 0) {
            ...
            if (!e) {
                IMGUICSS_EXCEPTION(ImGui::ElementError, "failed to create element %s", mNode->name());
                return false;
            }
...
ConditionChain::ConditionChain() : mEnabledElement(NULL), mDefault(NULL) {}

void ConditionChain::pushChild(Element* element, rapidxml::xml_node<>* node) {
    if (!mScriptState) {
        IMGUICSS_EXCEPTION(ImGui::ElementError, "conditionals are supported only with script enabled");
        return;
    }

    IM_ASSERT(element->enabledAttr != NULL && "element does not have enabledAttr defined, why is it in the ConditionChain?");
    if (mDefault) {
        IMGUICSS_EXCEPTION(ImGui::ElementError, "malformed condition chain: v-else/v-else-if/v-if after the last v-else");
        return;
    }
...
void Slot::configure(rapidxml::xml_node<>* node, Context* ctx, ScriptState::Context* sctx, Element* parent) {
    ...
    if (!ctx->root || (ctx->root->getFlags() & Element::COMPONENT) == 0) {
        IMGUICSS_EXCEPTION(ImGui::ElementError, "slot must be a child of a component");
        return;
    }
...
bool skipLayout(Element* element) {
    return element->getFlags() & Element::WINDOW || !GetCurrentWindowNoDefault() || element->style()->position == CSS_POSITION_FIXED || element->style()->position == CSS_POSITION_ABSOLUTE;
}
...
void Layout::beginElement(Element* element) {
    ...
    if (element->display == CSS_DISPLAY_BLOCK || (contentRegion.x > 0 && lineEnd.x + element->getSize().x > contentRegion.x)) {
        newLine();
    }
...
void Layout::endElement(Element* element) {
    if (skipLayout(element)) {
        if (GetCurrentWindowNoDefault()) ImGui::SetCursorPos(lineEnd);
        return;
    }
