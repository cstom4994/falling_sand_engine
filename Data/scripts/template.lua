...
-- util:
local function tree_dump_helper(t, p, strlimit, ignorekeys)
...
local function tree_dump(t, strlimit, ignorekeys)
    print(tree_dump_helper(t, "", strlimit and strlimit or 80, ignorekeys or {}))
end

local function qesc(str)
    local len = str:len()
...
local function printlines(s, from, to)
    from = from or 0
    to = to or 100000000
    local l = 1
    for w in s:gmatch("(.-)\r?\n") do
