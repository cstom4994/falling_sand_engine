...
local persistence_methods = cecs._persistence_methods()
cecs.writer = persistence_methods.writer
cecs.reader = persistence_methods.reader
...
local typeid = {
    int = assert(cecs._TYPEINT),
    float = assert(cecs._TYPEFLOAT),
    bool = assert(cecs._TYPEBOOL),
    int64 = assert(cecs._TYPEINT64),
    dword = assert(cecs._TYPEDWORD),
    word = assert(cecs._TYPEWORD),
    byte = assert(cecs._TYPEBYTE),
    double = assert(cecs._TYPEDOUBLE),
    userdata = assert(cecs._TYPEUSERDATA),
}
...
do -- newtype
    ...
    function M:register(typeclass)
        ...
        local id = ctx.id + 1
        assert(typenames[name] == nil and id <= cecs._MAXTYPE)
        ctx.id = id
        local c = {
            id = id,
            name = name,
...
local template_methods = cecs._template_methods()
...
function M:type(name)
    ...
    elseif t.tag then
        return "tag"
    elseif t.size == cecs._LUAOBJECT then
        return "lua"
    else
        return "c"
    end
